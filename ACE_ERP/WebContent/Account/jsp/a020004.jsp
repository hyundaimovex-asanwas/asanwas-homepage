...
+ 서 블 릿 명	:
-----------------------------------------------------------------------------
+ 수 정 내 용 :	 결재완료 및 결재취소시점에 상위집계현황 상태 업데이트 추가 
+ 수   정  자 :  정  영  식
+ 수 정 일 자 :  2010.08.27
+ 서 블 릿 명	:
-----------------------------------------------------------------------------
+ 수 정 내 용 :	결재자 변경 ( 이정식 >  봉균일 )
+ 수   정  자 :  정  영  식
+ 수 정 일 자 :  2011.12. 09
+ 서 블 릿 명	:
-----------------------------------------------------------------------------
+ 수 정 내 용 :	결재취소 체크 ( 대금지급생성 전표는 취소 안됨 ) 
+ 수   정  자 :  정  영  식
+ 수 정 일 자 :  2012.12.21
+ 서 블 릿 명	:

-----------------------------------------------------------------------------
+ 수 정 내 용 :	결재취소 체크 ( 대금지급생성 전표 예외 처리 ) 
+ 수   정  자 :  정  영  식
+ 수 정 일 자 :  2013.03.04
+ 서 블 릿 명	:

-----------------------------------------------------------------------------
+ 수 정 내 용 :	전표 잠금/풀림 상태 ( 반제처리 안되도록 ) 
+ 수   정  자 :  이 동 훈
+ 수 정 일 자 :  2013.10.22
+ 서 블 릿 명	:
-----------------------------------------------------------------------------
...
function fnOnLoad(tree_idx){
  ...
  if(gs_userid=="6070001"||gs_userid=="6060002"||gs_userid=="2060006"){
		sp0.style.display = "";
		chkbox_banjae.checked = false;
	}else{
  ...
  //회계개발자 편의상 만듬---------------------
	if(gs_userid=="6060002"||gs_userid=="6070001"){
			//gs_userid="2020009";
			gs_deptcd= 'C260';
			gs_deptnm='회계정산팀';
            gs_fdcode='02';
...
function ln_Before(){
   ...
   if( gcds_login.countrow>0){
	    ...
	    //2011.12.09 임시 사용 
	    if(gs_userid=="2040080"){
	    	txt_gubun.value="회계";
	    	hid_gubun.value="01";
	    	
	     //2013.05.06 임시 사용 
	    }else if(gs_userid=="2060006"){
	        txt_gubun.value="회계";
	    	hid_gubun.value="01";
	    	
	     //2013.10.22 이동훈 수정 
	    }else if(gs_userid=="6060002"){
	        txt_gubun.value="";
	    	hid_gubun.value="";	    	
	    }else{
...
function ln_Query(){
  ...
  if(gclx_fdcode.bindcolval=="02"){ //서울
	...
	gcds_data01.DataID = "<%=dirPath%><%=HDConstant.PATH_ACCOUNT%>a020004_s1?v_str1="+gcem_fsdat_fr.text
																		 ...
																		 +"&v_str8="+strGubun //결재 구분 01 -회계, 02-예산, 06-자금		
																		 +"&v_str9="+gclx_lockstat.bindcolval; //잠금 구분 A - 전체, T-잠금, F-풀림																										 																									
...
function ln_Chk(p){
    ...
    if (p=='02'){
		...
		//2012.12.21 JYS
		//대금지급생성한 데이터는 결재취소 체크함.
		
		if(chkbox_custpay.checked==true){
			return true;
	    }else{
			for(a=1;a<=gcds_data01.countrow;a++){
				if(gcds_data01.namevalue(a,"CHK")=="T" && gcds_data01.namevalue(a,"ACCSGNEND")=="Y"){
				    ...
				    gcds_paychk.DataID = "<%=dirPath%><%=HDConstant.PATH_ACCOUNT%>a020004_s5?v_str1="+gcds_data01.namevalue(a,"FDCODE")
																										                                             +"&v_str2="+gcds_data01.namevalue(a,"FSDAT")
																										                                             +"&v_str3="+gcds_data01.namevalue(a,"FSNBR");
																										                                             
	                gcds_paychk.Reset();		
	                
	                if( gcds_paychk.namevalue(gcds_paychk.rowposition,"CNT") >0 ){
	                	alert("대금지급건이 존재합니다. 결재취소 불가합니다. 대금지급 담당자에게 확인하세요.");
	                	return false;
	                }																						                                             
				}
			}	
				  ...
				  strAtcode = gcds_data03.namevalue(s,"ATCODE");
					...
					if(strAtcode == "1"|| strAtcode =="2" ){ 
					  gcds_data03.namevalue(s,"EXRATE")= 0;   //월말환율
						...
						//예외처리(상품/저장품)--평균환율적용
						if(gcds_data03.namevalue(s,"ATCODE")=="1120100"||gcds_data03.namevalue(s,"ATCODE")=="1120700"){
							gcds_data03.namevalue(s,"EXRATEDT")="";    
...
/******************************************************************************
	Description : 잠금/풀림
	parameter   : 
	참       고 : 전표건에 대해 잠금/풀림 설정
	              2013.10.22 이동훈 수정 : 체크건에 대해서 전표 Lock 설정
******************************************************************************/
function ln_Lock_Save(){
	
		if (confirm("잠금상태를 업데이트 하시겠습니까?")){		
			gctr_data04.Action = "<%=dirPath%><%=HDConstant.PATH_ACCOUNT%>a020004_t4?";
...
function ln_Excel(){
	...
	gcgd_disp01.GridToExcel("전표결재","",2);
...
<comment id="__NSID__">
   <!-- 반제정산전표갯수 -->
   <object  id=gcds_banjaerel classid=clsid:3267EA0D-B5D8-11D2-A4F9-00608CEBEE49 VIEWASTEXT>
	<PARAM NAME="SyncLoad" VALUE="true">
   </object>
   
   <!-- 대금지급체크 -->
   <object  id=gcds_paychk classid=clsid:3267EA0D-B5D8-11D2-A4F9-00608CEBEE49 VIEWASTEXT>
	<PARAM NAME="SyncLoad" VALUE="true">
</object>
...
<!-- 잠금/풀림 -->
<comment id="__NSID__"><object  id=gctr_data04 classid=clsid:0a2233ad-e771-11d2-973d-00104b15e56f viewastext>
	<param name="keyname"			value="toinb_dataid4">
	<param name="keyvalue"    value="a020004_t4(I:USER=gcds_data01)">   
</object></comment><script>__ws__(__NSID__);</script> 
...
<script language="javascript" for="gcgd_disp01" event="OnClick(row,colid)">
  gs_save_gubun="";
	...
	//2013.10.22 이동훈 수정
	if((row!=0) && (colid=="FSLOCK")){	

		
			if(gcds_data01.namevalue(row,"FSLOCK")=="T"){
				gcds_data01.namevalue(row,"FSLOCK")="F";
			}else{		
				gcds_data01.namevalue(row,"FSLOCK")="T";
			}
...
<script language=JavaScript for=gcgd_disp01 event=OnDblClick(row,colid)>
	...
	if(row>0){
		if(colid!="CHK" && colid!="FSLOCK" && colid!="ACTDAT" && colid!="BIGO"){
...
<table width="876" cellpadding="0" cellspacing="0" border="0" style="position:relative;left:3px">
	<tr> 
		<td><img src="../img/a020004_head.gif"></td>
		<td width="685" align="right" background="../../Common/img/com_t_bg.gif" style="padding-top:4px;"> 
		   <!--<img src="../../Common/img/btn/com_b_total.gif"	style="cursor:hand" onClick="ln_Batch()" >--> 	
		   <!--<img src="../../Common/img/btn/com_b_unlock.gif"	style="cursor:hand" onClick="ln_Unlock()" >	-->	   
		   <img src="../../Common/img/btn/com_b_excel.gif"	style="cursor:hand" onClick="ln_Excel()" >
		   <img src="../../Common/img/btn/com_b_save.gif"	style="cursor:hand" onClick="ln_Lock_Save()" >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;		   
...
<table>
	<tr>
		<td>
			<table  cellpadding="0" cellspacing="0" style="width:876px;height:30px;font-size:9pt;background-color:#ffffff;">
				<tr width="876px"> 
					...
					<IMG border="0" src="com_b_lock2.gif" width="40" height="21">
        ...
        <tr>
          ...
          <td width="70px" style="" bgcolor="#eeeeee" class="tab19" align=center><nobr>&nbsp;결재구분</nobr></td>
					<td class="tab19" style="width:80px;" align=left colspan=1><nobr>&nbsp;
					...
					<td width="50px" style="" bgcolor="#eeeeee" class="tab19" align=center><nobr>잠금</nobr></td>
          			<td class="tab19" style="width:70px;" align=left><nobr>&nbsp;<comment id="__NSID__">
						 <object  id=gclx_lockstat classid=clsid:60109D65-70C0-425C-B3A4-4CB001513C69 
										style="position:relative;left:0px;top:2px;font-size:12px;width:60px;">
											<param name=CBData			   		value="A^전체,T^잠금,F^풀림">
											<param name=CBDataColumns	 value="CODE,NAME">
											<param name=SearchColumn	 	value=NAME>
											<param name=Sort			     		value=false>
											<param name=ListExprFormat 		value="NAME">								
	...
	<tr>
		<td width="876px"  class="tab15" align="right">
	      ...
	      <font style="position:relative;top:-4px;left:0px">대금지급체크안함</font>&nbsp;
          <input type=checkbox  id=chkbox_custpay  style="width:15px;position:relative;left:0px;top:-3px;color=#CCFFCC";>&nbsp;&nbsp;
		...
		<td style="width:280" ALIGN=LEFT VALIGN=TOP>
			<comment id="__NSID__"><object  classid=clsid:1F57AEAD-DB12-11D2-A4F9-00608CEBEE49 id=gcgd_disp01  
				style="position:relative;left:3px;width:875px; height:400px; border:1 solid #708090;display:block;" viewastext>
					...
					<C> Name='지점'		    ID=FDNAME	   HeadAlign=Center HeadBgColor=#B9D4DC Width=40	  SumBgColor=#C3D0DB edit=none  align=center sort=false BgColor={IF(FSLOCK='T','#FFCC66','FFFFFF')}  </C> 
					<C> Name='잠금'		    ID=FSLOCK 	   HeadAlign=Center HeadBgColor=#B9D4DC Width=50	  SumBgColor=#C3D0DB align=center sort=true	EditStyle=CheckBox  BgColor={IF(FSLOCK='T','#FFCC66','FFFFFF')} </C> 
					<C> Name='구분'	 	    ID=TOGB 	   HeadAlign=Center HeadBgColor=#B9D4DC Width=50	  SumBgColor=#C3D0DB edit=none  align=center sort=true  EditStyle=Combo Data='01:,02:예산,03:회계,05:완료,06:자금' BgColor={IF(FSLOCK='T','#FFCC66','FFFFFF')}</C> 
					<C> Name='전표번호'		ID=SSDATNBR	   HeadAlign=Center HeadBgColor=#B9D4DC Width=102	  SumBgColor=#C3D0DB edit=none  align=center sort=true  BgColor={IF(FSLOCK='T','#FFCC66','#CCFFCC')}</C> 
					<C> Name='접수번호'		ID=FSDATNBR	   HeadAlign=Center HeadBgColor=#B9D4DC Width=102	  SumBgColor=#C3D0DB edit=none  align=center sort=true  BgColor={IF(FSLOCK='T','#FFCC66','FFFFFF')}</C> 
					<C> Name='담당자'		    ID=EMPNO 	   HeadAlign=Center HeadBgColor=#B9D4DC Width=0  	  SumBgColor=#C3D0DB edit=none  align=center sort=true show=false BgColor={IF(FSLOCK='T','#FFCC66','FFFFFF')}</C> 
					<C> Name='담당' 		    ID=EMPNMK 	   HeadAlign=Center HeadBgColor=#B9D4DC Width=50	  SumBgColor=#C3D0DB edit=none  align=center sort=true BgColor={IF(FSLOCK='T','#FFCC66','FFFFFF')}</C> 
					<C> Name='적요'			ID=REMARK      HeadAlign=Center HeadBgColor=#B9D4DC Width=200	  SumBgColor=#C3D0DB edit=none  align=left	 sort=true  SumText='합   계' BgColor={IF(FSLOCK='T','#FFCC66','FFFFFF')}</C> 
					<C> Name='금액'		  	ID=FSAMT 	   HeadAlign=Center HeadBgColor=#B9D4DC Width=90	  SumBgColor=#C3D0DB edit=none  align=right  sort=true  SumText=@sum BgColor={IF(FSLOCK='T','#FFCC66','FFFFFF')}</C> 
					<C> Name='상태'		    ID=ACCSGNEND   HeadAlign=Center HeadBgColor=#B9D4DC Width=55	  SumBgColor=#C3D0DB edit=none  align=center sort=true  EditStyle=Combo Data='Y:결재완료,R:결재요청,N:결재대기,C:결재취소,B:반송' BgColor={IF(FSLOCK='T','#FFCC66','FFFFFF')}</C> 
					<C> Name='회계일자'		ID=ACTDAT 	   HeadAlign=Center HeadBgColor=#B9D4DC Width=75	  SumBgColor=#C3D0DB align=center sort=true  Mask='XXXX/XX/XX' BgColor={IF(FSLOCK='T','#FFCC66','#ffffcc')} </C> 
					<!-- <C> Name='반송사유'		ID=BIGO 	   HeadAlign=Center HeadBgColor=#B9D4DC Width=40	  SumBgColor=#C3D0DB align=center sort=true   BgColor=#ffffcc </C>  -->           
					">
			</object></comment><script>__ws__(__NSID__);</script> 
