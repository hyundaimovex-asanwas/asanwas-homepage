...
<% /*
	* [ 수정일자 ][수정자] 내용
	* [2015-06-15][심동현] '회계'외 업무에서 호출할 때 전표 보기(EX 공사ERP 등)
	* 	파라미터1	FDCODE=02
	*				FSDAT1=A
	*				FSDAT2=20150602
	*				FSNBR=000026
	...
	*						향후 버튼 불필요한 건 삭제하는게 좋을 수도~
	*/

	String strParam0 = request.getParameter("FDCODE");
	String strParam1 = request.getParameter("FSDAT1");
	String strParam2 = request.getParameter("FSDAT2");
	String strParam3 = request.getParameter("FSNBR");
%>
<%@ page import="Account.common.*" %>
<% String dirPath = request.getContextPath(); //HDASan // ContextPath 변수 설정. %>
<html>
<head>
<title>전표조회 - 외부호출</title>
...
//2015-06-15 심동현 : 독립실행 창 스탈
	strParam[0] = "<%=strParam0%>";
	strParam[1] = "<%=strParam1%>";
	strParam[2] = "<%=strParam2%>";
	strParam[3] = "<%=strParam3%>";
	//2015-06-15 심동현 :끝
...
function ln_Start(){
	...
	if(strParam[4]=="A020013"){ //전표복사
		sp1.style.display = "";  //보여줌
	}else{
...
function ln_PrintSubQuery2(){
  ...
  //alert("출력2");
	...
	if(gcds_data01_2.namevalue(gcds_data01_2.rowposition,"FSSTAT")=="N"){
  		...
  		if(gs_userid=="2020013"||gs_userid=="2040046"||gs_userid=="2080031"||gs_userid=="2110014"||gs_userid=="2130002"||gs_userid=="2190020"||gs_userid=="2080012"||gs_userid=="6060002"){
  ...
  var p2 = gcem_fs02.text+gcem_fsdat02.text;
    ...
    if(hid_docuval.value==""){
		  ...
		  //2009.09.10 전자세금계산서 적용 
			...
			if(arrParam[7].substring(0,1)=="A"||arrParam[7].substring(0,1)=="C"){
				strURL = "./a040110_view_popup.jsp";
			}else if(arrParam[7].substring(0,1)=="B"||arrParam[7].substring(0,1)=="D"){
...
function ln_Chk_Save(){
  ...
  //로그인한 사람과 담당자 사번이 같이 않으면 수정 및 저장 못함.
	...
	//^^
	//전표종류 FSKND = "A" 이면 수정 삭제 안됨 .
	if(gcds_data01_2.namevalue(gcds_data01_2.rowposition,"FSKND")=="A"){
		alert("자동생성 전표입니다. 수정불가합니다.");
		return false;
	}
    ...
    ///////////////////////////////////////////////// 
		...
		if(gcds_dtl.namevalue(i,"BANJAEYN")=="Y"){ //반제
			 ...
			 if(gcds_dtl.namevalue(i,"ATDECR")=="1"){ //차변
				...
				if(gs_userid!="2050026"&&gs_userid!="2040080"&&gs_userid!="2140004"){
...
function ln_banjae_atcode_find(stratcode){
		gcds_banjae_atcode.DataID = "<%=dirPath%><%=HDConstant.PATH_ACCOUNT%>nstant.PATH_ACCOUNT%>a020001_s21?v_str1="+stratcode;																						
...
/******************************************************************************
	...
	parameter   : 적요, 거래처, 부서
******************************************************************************/
function ln_Popup_Batch_Modify(){
	var result="";
	var arrParam	= new Array();
	var arrResult	= new Array();
	var strURL;	
	...
	strURL = "./batch_modify_popup.jsp";
	strPos = "dialogWidth:460px;dialogHeight:117px;dialogTop:350px;dialogLeft:320px;status:no;scroll:no;resizable:no";
	result = showModalDialog(strURL,window,strPos);
...
<B>id=Footer ,left=0 ,top=1500 ,right=2871 ,bottom=2000 ,face='Arial' ,size=10 ,penwidth=1
	...
	<L> left=2569 ,top=90 ,right=2569 ,bottom=386 </L>
	<L> left=2080 ,top=90 ,right=2080 ,bottom=386 </L>
	...
	<T>id='대표이사' ,left=2601 ,top=98 ,right=2791 ,bottom=143 ,face='굴림' ,size=10 ,bold=false ,underline=false ,italic=false ,forecolor=#000000 ,backcolor=#FFFFFF</T>
	...
	<C>id='C3', left=2355, top=98, right=2535, bottom=143, face='굴림', size=10, bold=false, underline=false, italic=false, forecolor=#000000, backcolor=#FFFFFF</C>
	...
	<C>id='C2', left=2111, top=98, right=2291, bottom=143, face='굴림', size=10, bold=false, underline=false, italic=false, forecolor=#000000, backcolor=#FFFFFF</C>
	...
	<C>id='C7', left=1865, top=183, right=2056, bottom=228, face='굴림', size=10, bold=false, underline=false, italic=false, forecolor=#000000, backcolor=#FFFFFF</C>
	<L> left=1320 ,top=267 ,right=2569 ,bottom=267 </L>
	<L> left=2334 ,top=93 ,right=2334 ,bottom=389 </L>
</B>
