		...
		for(j=1;j<=gcds_dtl.countrow;j++){
		   //반제 계정이면서 정산일 경우 체크
			 if(gcds_dtl.namevalue(j,"ATDEBTYN")=="Y"){
					if(gcds_dtl.namevalue(j,"ATDECR")=="1"&&gcds_dtl.namevalue(j,"DAEAMT")!=0){ //차변 일때 대변에 금액이 존재하면 정산
             alert("반제정산 계정이 존재합니다. 복사할 수 없는 전표입니다.");
						 return;
			 ...
			 if(gcds_dtl.namevalue(j,"FSWRKDIV")!="01"&&gcds_dtl.namevalue(j,"FSWRKDIV")!=""){
					alert("예산반영된 계정이 존재합니다. 복사할 수 없는 전표입니다.");
					return;
			 }
		}
