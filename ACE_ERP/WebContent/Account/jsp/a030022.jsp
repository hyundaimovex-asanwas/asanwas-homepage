...
<!---------------------------------------------------------------------------
+ 시 스 템 명	:  경리회계 - 계정원장(일별)	
+ 프로그램 ID	:  A030022.html
+ 기 능 정 의	:  일별계정 조회 및 출력 화면이다.
+ 작   성  자 :  이 순 미
+ 서 블 릿 명	:	 a030022_s1
-----------------------------------------------------------------------------
+ 수 정 내 용  :	개선사항 수정 및 출력물수정  
+ 수   정  자  :  이민정
+ 수 정 일 자  :  2006.10.
------------------------------------------------------------------------------->
...
function ln_PrtQuery(){
	...
	//for (var i=1;i<=gcds_temp.countrow;i++) {
		...
		var str4 = gcem_atcode_fr01.text;     //계정코드fr
		var str5 = gcem_atcode_to01.text;     //계정코드to
...
function ln_Data_Reset(){
  ...
  for(i=1;i<=gcds_data1.countrow;i++){
       ...
       // alert("GUBUN::"+gcds_data1.namevalue(i,"GUBUN"));
		...
		if(gcds_data1.namevalue(i,"GUBUN")=="A"){ //전기이월
             dblJanSum=0; 
			 dblDEATSum=0;
			 dblCRATSum=0;
		...
		}else if(gcds_data1.namevalue(i,"GUBUN")=="M"){       //월계
             ...
             dblDEMMSum=0;
             dblCRMMSum=0;
		}else if(gcds_data1.namevalue(i,"GUBUN")=="T"){ //누계
		    gcds_data1.namevalue(i,"ACTDAT") = "";
            gcds_data1.namevalue(i,"ATCODE")="";
		...
		}else if(gcds_data1.namevalue(i,"GUBUN")=="B"){//실데이타
		   if(gcds_data1.namevalue(i,"ATDECR")=="1"){      //차변계정
  ...
  ft_cnt01.innerText = "조회건수 : " + dblrealrow + " 건";
...
function ln_Data_Reset2(){
  ...
  for(i=1;i<=gcds_data2.countrow;i++){
		...
		if(gcds_data2.namevalue(i,"GUBUN")=="A"){ //전기이월
       dblJanSum=0; 
			 dblDEATSum=0;
			 dblCRATSum=0;
       ...
       gcds_data2.namevalue(i,"SSDAT")="";
		}else if(gcds_data2.namevalue(i,"GUBUN")=="M"){       //월계
       ...
       dblDEMMSum=0;
       dblCRMMSum=0;
		}else if(gcds_data2.namevalue(i,"GUBUN")=="T"){ //누계
		   gcds_data2.namevalue(i,"ACTDAT") = "";
       gcds_data2.namevalue(i,"ATCODE")="";
		...
		}else if(gcds_data2.namevalue(i,"GUBUN")=="B"){//실데이타
		   if(gcds_data2.namevalue(i,"ATDECR")=="1"){      //차변계정
		...
		<table cellpadding="0" cellspacing="0" border="0" style="position:relative;left:8px;width:866px;border:1 solid #708090;">
			...
			<tr> 				
				...
				<td class="tab23" colspan="8" width="690px">&nbsp;<comment id="__NSID__">
					...
					<img SRC="../../Common/img/btn/com_b_find.gif" BORDER="0" ALT="계정코드를 검색합니다" style="cursor:hand;position:relative;left:-8px;top:3px" align=center onclick="ln_Popup('fr','01');">
					...
					<img SRC="../../Common/img/btn/com_b_find.gif" BORDER="0" ALT="계정코드를 검색합니다" style="cursor:hand;position:relative;left:-3px;top:3px" align=center onclick="ln_Popup('to','01');">
			...
			<tr> 			
				<td style="width:280" ALIGN=LEFT VALIGN=TOP>
					<comment id="__NSID__"><object  id=gcgd_disp01 classid=clsid:1F57AEAD-DB12-11D2-A4F9-00608CEBEE49   
						...
						<PARAM NAME="Format"			VALUE="  
						<C> Name='계정'			ID=ATCODE    HeadAlign=Center HeadBgColor=#B9D4DC Width=60	align=center SumBgColor=#C3D0DB show=false</C> 
						<C> Name='계정명'		ID=ATKORNAM  HeadAlign=Center HeadBgColor=#B9D4DC Width=95	align=left SumBgColor=#C3D0DB </C> 
						<C> Name='일자'			ID=ACTDAT  HeadAlign=Center HeadBgColor=#B9D4DC Width=70	align=center SumBgColor=#C3D0DB Mask='XXXX/XX/XX'</C> 
						<C> Name='전표번호'	ID=SSNBR   HeadAlign=Center HeadBgColor=#B9D4DC Width=50	align=center SumBgColor=#C3D0DB </C> 
						<C> Name='순번'	    ID=FSNUM   HeadAlign=Center HeadBgColor=#B9D4DC Width=25	align=center show=false SumBgColor=#C3D0DB </C> 
						<C> Name='적요'			ID=REMARK  HeadAlign=Center HeadBgColor=#B9D4DC Width=235	align=left	 SumBgColor=#C3D0DB </C> 
						<C> Name='거래처코드'	ID=VEND_CD  HeadAlign=Center HeadBgColor=#B9D4DC Width=80	align=center	 SumBgColor=#C3D0DB SumText='' show=false</C>
						<C> Name='거래처명'	ID=VENDER  HeadAlign=Center HeadBgColor=#B9D4DC Width=115	align=left	 SumBgColor=#C3D0DB </C>
						<C> Name='차변'			ID=DETOT 	 HeadAlign=Center HeadBgColor=#B9D4DC Width=90	align=right	 SumBgColor=#C3D0DB color =#0000FF</C> 
						<C> Name='대변'			ID=CRTOT 	 HeadAlign=Center HeadBgColor=#B9D4DC Width=90	align=right	 SumBgColor=#C3D0DB color =#FF0000</C> 
						<C> Name='잔액'			ID=SLPVLA  HeadAlign=Center HeadBgColor=#B9D4DC Width=100	align=right  SumBgColor=#C3D0DB </C> 
						">
					</object></comment><script>__ws__(__NSID__);</script> 
...
<B>id=DHeader ,left=0,top=0 ,right=2871 ,bottom=375 ,face='Arial' ,size=10 ,penwidth=1
	...
	<C>id='CURDT', left=934, top=119, right=2077, bottom=167 ,mask='(  회계일자 : XXXX년 XX월 XX일  ~  XXXX년 XX월 XX일  )', face='굴림', size=12, bold=true, underline=false, italic=false, forecolor=#000000, backcolor=#FFFFFF</C>
	<T>id='계  정  원  장' ,left=1095 ,top=26 ,right=1921 ,bottom=87 ,face='HY중고딕' ,size=14 ,bold=true ,underline=false ,italic=false ,forecolor=#000000 ,backcolor=#C0C0C0</T>
	...
	<T>id='거래처명' ,left=1315 ,top=291 ,right=1699 ,bottom=365 ,face='굴림' ,size=10 ,bold=true ,underline=false ,italic=false ,forecolor=#000000 ,backcolor=#FFFBF0</T>
	<C>id='ATKORNAM', left=241, top=228, right=1087, bottom=280, align='left' ,mask='계정과목: XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX', face='굴림', size=10, bold=false, underline=false, italic=false, forecolor=#000000, backcolor=#FFFFFF</C>
	<T>id='일  자' ,left=246 ,top=291 ,right=439 ,bottom=365 ,face='굴림' ,size=10 ,bold=true ,underline=false ,italic=false ,forecolor=#000000 ,backcolor=#FFFBF0</T>
	<L> left=241 ,top=373 ,right=2810 ,bottom=373 ,penstyle=solid ,penwidth=2 ,pencolor=#000000 </L>
	<T>id='전표번호' ,left=452 ,top=291 ,right=622 ,bottom=365 ,face='굴림' ,size=10 ,bold=true ,underline=false ,italic=false ,forecolor=#000000 ,backcolor=#FFFBF0</T>
	<T>id='적  요' ,left=643 ,top=291 ,right=1296 ,bottom=365 ,face='굴림' ,size=10 ,bold=true ,underline=false ,italic=false ,forecolor=#000000 ,backcolor=#FFFBF0</T>
...
<B>id=default ,left=0,top=0 ,right=2871 ,bottom=53 ,face='Arial' ,size=10 ,penwidth=1
	...
	<C>id='SLPVLA', left=2447, top=0, right=2802, bottom=45, align='right', face='굴림', size=10, bold=false, underline=false, italic=false, forecolor=#000000, backcolor=#FFFFFF</C>
	...
	<C>id='VENDER', left=1315, top=0, right=1699, bottom=45, align='left', face='굴림', size=10, bold=false, underline=false, italic=false, forecolor=#000000, backcolor=#FFFFFF</C>
	<L> left=1310 ,top=0 ,right=1310 ,bottom=50 </L>
	<L> left=447 ,top=0 ,right=447 ,bottom=50 </L>
	<C>id='ACTDAT', left=246, top=0, right=439, bottom=45 ,mask='XXXX/XX/XX', face='굴림', size=10, bold=false, underline=false, italic=false, forecolor=#000000, backcolor=#FFFFFF</C>
	<L> left=241 ,top=0 ,right=241 ,bottom=50 </L>
	<L> left=241 ,top=50 ,right=2810 ,bottom=50 ,penstyle=solid ,penwidth=1 ,pencolor=#A0A0A4 </L>
	<C>id='SSNBR', left=458, top=0, right=619, bottom=45, face='굴림', size=10, bold=false, underline=false, italic=false, forecolor=#000000, backcolor=#FFFFFF</C>
	<C>id='REMARK', left=643, top=0, right=1296, bottom=45, align='left', face='굴림', size=10, bold=false, underline=false, italic=false, forecolor=#000000, backcolor=#FFFFFF</C>
