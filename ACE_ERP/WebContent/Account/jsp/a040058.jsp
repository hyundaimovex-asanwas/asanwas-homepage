...
+ 서 블 릿 명	:
------------------------------------------------------------------------------
+ 수 정 내 용     : 결재상태 추가  / 반송사유 등록 / 첨부파일 
+ 수   정  자      : JYS
+ 수 정 일 자     : 2017.06.23 
+ 서 블 릿 명	:
------------------------------------------------------------------------------
+ 수 정 내 용     : 입금예정일에 대한 확인 추가 
+ 수   정  자      : JYS
+ 수 정 일 자     : 2017.08.24 
+ 서 블 릿 명	:
------------------------------------------------------------------------------>
...
function ln_Grid_Fromat(){
	...
	if(gclx_gridfix.bindcolval=="1"){
	  ...
	  gcgd_data.format = ""+
	                    ...
	                    "<C>ID=DUECHK,	    Name='입금확인'	    ,width=75, HeadBgColor=#B9D4DC,HeadAlign=center, align=center  EditStyle=Combo, Data='Y:입금,N:미입금'  sort=true SumBgColor=#C3D0DB</C>";
...
function ln_Save(){
    if (gcds_data.IsUpdated) {
		if(confirm("저장 하시겠습니까?")){
		    ...
		    gstrRowpos = gcds_data.rowposition;
			gctr_data.KeyValue   = "Account.a040058_t1(I:USER=gcds_data)";
			gctr_data.Action     = "<%=dirPath%><%=HDConstant.PATH_ACCOUNT%>a040058_t1";
			gctr_data.post();
		}
	}else{
...
function ln_Delete(){
	  ...
	  alert("선택 후 삭제하십시요.");
		...
		for (var i=1;i<=gcds_data.countrow;i++){
			if(gcds_data.namevalue(i,"CHK")=="T"){
				...
				//결재상태가 승인, 요청 일 경우 삭제불가
				if(gcds_data.namevalue(i,"TAXSTS")=="R"||gcds_data.namevalue(i,"TAXSTS")=="Y"){
					
					if(gcds_data.namevalue(i,"DTI_STATUS")=="R"){
					
					}else if (gcds_data.namevalue(i,"DTI_STATUS")=="O"){
						
					}else{
						alert("결재상태가 삭제불가합니다. 확인하십시요.");
						return false;
					}
				}
			}
	...
	if (gcds_data.IsUpdated) {
		if(confirm("삭제 하시겠습니까?")){
			...
			gctr_data.Action     = "<%=dirPath%><%=HDConstant.PATH_ACCOUNT%>a040058_t3";
...
function ln_Apply(){
	...
	if(!ln_Chk_Status()){
		...
		if (confirm("매출 정보를 승인 하시겠습니까?")){	
			gctr_data.KeyValue   = "Account.a040058_t4(I:USER=gcds_data)";
			gctr_data.Action = "<%=dirPath%><%=HDConstant.PATH_ACCOUNT%>a040058_t4";
...
function ln_Confirm(){
	...
	//매출만 확인 가능  
	//매입일경우 확인 안됨.
	if(gclx_staxiodiv.bindcolval!="2"){
		alert("매출만 입금예정일 확인이 가능합니다.");
		return false;
	}
	
	if(gcds_data.namevalue(1,"TAXIODIV")!="B"){
		alert("매출만 입금예정일 확인이 가능합니다2.");
		return false; 
	}
	
	if (gcds_data.IsUpdated) {
		if(confirm("입금예정일 확인하시겠습니까? ")){	
		    gstrRowpos = gcds_data.rowposition;
			gctr_data.KeyValue   = "Account.a040058_t7(I:USER=gcds_data)";
			gctr_data.Action     = "<%=dirPath%><%=HDConstant.PATH_ACCOUNT%>a040058_t7";
			//prompt('',gcds_data.text);
			gctr_data.post();
		}
	}else{
...
<table width="875" cellpadding="0" cellspacing="0" border="0" style="position:relative;left:3px">
  <tr> 
        ...
        <td width="685" align="right" background="../../Common/img/com_t_bg.gif">
			...
			<img src="<%=HDConstant.PATH_ACC_COMMON%>img/btn/com_btn_send.gif"	style="cursor:hand" onclick="ln_Confirm()">
