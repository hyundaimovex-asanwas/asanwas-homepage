...
<!-------------------------------------------------------------------------------
+ 시 스 템 명	: 회계관리/매입관리(전자세금계산서)
+ 프로그램 ID	: A040075.html
+ 기 능 정 의	: 정발행매입 및 매입관리(전자세금계산서)
+ 최 초 이 력	: 정영식
+ 변 경 이 력	:  
-----------------------------------------------------------------------------
+ 수 정 내 용 :	
...
function ln_Query(){
  ...
  gcds_data.DataID = "<%=dirPath%><%=HDConstant.PATH_ACCOUNT%>a040075_s2?v_str1="+gclx_fdcode.bindcolval   //지점                
	                                                                  +"&v_str2="+strregno                 //사업소              
...
//	gs_interface_batch_id="";
    ...
    //2011.01.10 jys
	//사업자등록번호 1018510695로 된 건은 수신승인 안됨.
	...
	if(strChk=="N"){
		alert("사업자등로번호가 1018510695 입니다. 전산팀에 확인 부탁합니다.");	
    }
...
function ln_Status(strStatus,strSignal,strBtnname){
  ...
  if(strSignal=="REJECT"){               // 수신거부
		strTitle="수신거부 사유";
  }else if(strSignal=="CANCELRREQUEST"){ //역발행요청취소
		strTitle="역발행요청취소 사유";
  }else if(strSignal=="CANCELREQ"){      //발행취소요청
    strTitle="발행취소요청 사유";
  }else if(strSignal=="REQREJECTP"){     //발행취소거부
    strTitle="발행취소거부 사유";
  }
	...
	if (gcds_data.IsUpdated) {
		if (confirm(strBtnname+" 하시겠습니까?")){	
			...
			//BATCH_ID 생성하여 XXSB_DTI_MAIN에 UPDATE하고  UPDATE 된 BATCH_ID를 XXSB_DTI_STATUS_CHANGE.asp에 넘김(데이터셋으로 리턴 ) 
			ln_Batch_id();
		}
	}//IsUpdated
...
function ln_Email(){
	...
	if (gcds_data.IsUpdated){
		...
		//true false 적용
		if(!ln_Mail_Popup('메일입력', strSupEmail)) return;
...
function ln_SM_Call(strBatchId, strId, strPass){
		...
		if(gs_sm_call_status=="S"){          //상태변경
				...
				window.open(url,"", "status=1,  scrollbars=no, width=300, height=280, left="+v_left+", top="+v_top  );
		}else if (gs_sm_call_status=="I"){   //역발행매입요청
...
<script language=JavaScript  for=gcds_data event=OnLoadCompleted(rowcount)>
	...
	lbl_search.innerText = "총 ( " + rowcount + " ) 건     ***** 주의사항 : 정발행 매입의 경우 수신승인 후 세금계산서 출력하여 매입매출등록에서 등록해주십시요. *****";
...
<script language=JavaScript for=gclx_supbuy event=OnSelChange()>
    ...
    gclx_status.CBData ="''^전체,A^매입저장,V^역발행요청,I^수신미승인,C^수신승인,M^발행취소요청(공급받는자),N^발행취소요청(공급자),R^수신거부,O^취소완료,T^역발행거부,W^역발행요청 취소";
	}else if(gclx_supbuy.bindcolval=="2"){ //정발행
    gclx_status.CBData ="''^전체,I^수신미승인,C^수신승인,M^발행취소요청(공급받는자),N^발행취소요청(공급자),R^수신거부,O^취소완료";
	}
...
<script language=JavaScript for= gclx_status event=OnSelChange()>
   //매입
	if(gclx_status.bindcolval =="I"){ // 수신미승인 => 수신승인, 수신거부
	...
	}else if(gclx_status.bindcolval =="C"){ //수신승인 => 발행취소요청, 세금계산서보기
	...
	}else if(gclx_status.bindcolval =="N"){ //발행취소요청(공급자) => 발행취소승인, 발행취소거부
		 sp2.style.display = "";
 	     sp3.style.display = "";
 	...
 	}else if(gclx_status.bindcolval =="V"){ //역발행요청 => 역발행요청취소, 이메일재발송
	...
	}else if(gclx_status.bindcolval =="A"){ //매입저장
	     ...
	     sp0.style.display = "none"; //발행취소          X  ==> 역발행요청취소
		 sp1.style.display = "none"; //발행취소요청    O
		 sp2.style.display = "none"; //발행취소승인    O
		 sp3.style.display = "none"; //발행취소거부    O
		 sp4.style.display = "none"; //역발행거부       X  ==> 수신승인
		 sp5.style.display = "none"; //역매출발행       X  ==> 수신거부
		 sp6.style.display = "none"; //이메일재발송    0  
		 sp7.style.display = "none"; //세금계산서보기 0
		 sp8.style.display = "none";
	}
...
<table width="876" cellpadding="0" cellspacing="0" border="0" style="position:relative;left:3px">
  <tr> 
    ...
    <!--   <img src="../../common/img/btn/com_b_sale.gif"      style="cursor:hand" onclick="ln_Save()">-->
      		...
      		<span id=sp0 style="display:none;"> <!-- 역발행요청취소 -->
				<img src="../../Common/img/btn/com_b_op_order_cancel.gif"   style="cursor:hand" onclick="ln_Status('W','CANCELRREQUEST','역발행요청취소')">
			</span>

			<span id=sp1 style="display:none;"> <!-- 발행취소요청 -->
				<img src="../../Common/img/btn/com_b_p_cancel_re.gif"   style="cursor:hand" onclick="ln_Status('M','CANCELREQ','발행취소요청')">
			</span>

			<span id=sp2 style="display:none;"> <!-- 발행취소승인 -->
				<img src="../../Common/img/btn/com_b_p_cancel_ap.gif"   style="cursor:hand" onclick="ln_Status('O','SRADAPPRP','발행취소승인')">
			</span>

			<span id=sp3 style="display:none;"> <!-- 발행취소거부 -->
				<img src="../../Common/img/btn/com_b_p_cancel_rj.gif"   style="cursor:hand" onclick="ln_Status('C','REQREJECTP','발행취소거부')">
			</span>

			<!-- <span id=sp4 style="display:none;"> <!-- 수신승인
				<img src="../../common/img/btn/com_b_reveive_ap.gif"    style="cursor:hand" onclick="ln_Status('C','APPROVE','수신승인')">
			</span> -->

			<span id=sp4 style="display:none;"> <!-- 수신승인 -->
				<img src="../../Common/img/btn/com_b_reveive_ap.gif"    style="cursor:hand" onclick="ln_SaleIssue('C','APPROVE','수신승인')">
			</span>

			<span id=sp5 style="display:none;"> <!-- 수신거부 -->
				<img src="../../Common/img/btn/com_b_receive_rj.gif"    style="cursor:hand" onclick="ln_Status('R','REJECT','수신거부')">
			</span>

     <!--  <span id=sp5 style="display:none;"> 
				<img src="../../common/img/btn/com_b_op_sale.gif"       style="cursor:hand" onclick="ln_SaleIssue('R','REJECT','수신거부')">
			</span> -->
			...
			<span id=sp8 style="display:none;"> <!-- 매입저장 -->
				<img src="../../Common/img/btn/com_b_op_order.gif"     style="cursor:hand" onclick="ln_Issue('I','','역발행매입요청')">
			</span>
      ...
      <td width="100px" align=center  bgcolor="#eeeeee" class="tab17" ><nobr>발행구분</nobr></td>
			...
			<td width="150px" class="tab19" colspan=3><nobr>&nbsp;<comment id="__NSID__">
				<object id=gclx_status      classid=clsid:60109D65-70C0-425C-B3A4-4CB001513C69 style="position:relative;left:0px;top:2px;width:180px;color:#FF0000;background:#FFFFCC">
					<param name=CBData			  value="''^전체,I^수신미승인,C^수신승인,R^수신거부,O^취소완료,M^발행취소요청(공급받는자),N^발행취소요청(공급자),V^역발행요청,T^역발행거부,W^역발행요청 취소">
					<param name=CBDataColumns	value="CODE,NAME">
					<param name=SearchColumn	value=NAME>
					<param name=Sort			      value=false>
				...
				</comment><script>__ws__(__NSID__);</script>&nbsp;&nbsp;&nbsp; <FONT SIZE="2" COLOR="BLUE"><B>☜ 상태 선택에 따라 버튼이 변경됩니다.</B></FONT>
			  </nobr> 
			</td>
		</tr>
		</table>
			...
			<td>
				<comment id="__NSID__">
					<object classid=clsid:1F57AEAD-DB12-11D2-A4F9-00608CEBEE49 id=gcgd_data style="WIDTH:874px;HEIGHT:377px;border:1 solid #777777;display:block;">
						...
						<FC> Name='거래처명'			 ID=CONVERSATION_ID   	  HeadAlign=Center HeadBgColor=#B9D4DC Width=250	align=left		edit=none  show = false </FC>
						<FC> Name='거래처명'			 ID=SUP_COM_NAME   	      HeadAlign=Center HeadBgColor=#B9D4DC Width=120	align=left		edit=none </FC>
						<FC> Name='사업자등록번호'	     ID=SUP_COM_REGNO         HeadAlign=Center HeadBgColor=#B9D4DC Width=90	    align=center    edit=none  Mask='XXX-XX-XXXXX'</FC>
						...
						<C>  Name='상태'              ID=DTI_STATUS    	      HeadAlign=Center HeadBgColor=#B9D4DC Width=150	align=left  	edit=none  EditStyle=Combo Data='A:매입저장,V:역발행요청,I:수신미승인,C:수신승인,M:발행취소요청(공급받는자),N:발행취소요청(공급자),R:수신거부,O:취소완료,T:역발행거부,W:역발행요청 취소'</C>			  					
						...
						<C>  Name='공급받는담당자'       ID=BYR_EMP_NAME	      HeadAlign=Center HeadBgColor=#B9D4DC Width=100	align=left		edit=none </C>
						<C>  Name='공급받는사업자등록번호' ID=BYR_COM_REGNO	      HeadAlign=Center HeadBgColor=#B9D4DC Width=100	align=left		edit=none  Mask='XXX-XX-XXXXX'</C>
	...
	<tr><td colspan="2" height="30" align="left">조회건수 : <label id="lbl_search"></label></td> </tr>
