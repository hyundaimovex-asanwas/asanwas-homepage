...
------------------------------------------------------------------------------
+ 수 정 일 자     :  2016.03.09
+ 수 정 내 용     :  종사업장 코드, 종사업자 주소 축가
------------------------------------------------------------------------------
+ 수 정 일 자     :  2016.09.06
+ 수 정 내 용     :  종사업장 대표자 추가 
------------------------------------------------------------------------------
+ 수 정 일 자     :  2017.04.14
+ 수 정 내 용     :  eBill구분 추가 ( 이건 2월에 처리했음 ) 
+ 수 정 내 용     :  입금예정일 필수항목으로 추가  && 매출발행막음
------------------------------------------------------------------------------
+ 수 정 일 자     :  2017.05.24
+ 수   정   자     :  정영식  
+ 수 정 내 용     :  매출등록 저장 후 결재요청 추가 
------------------------------------------------------------------------------
+ 수 정 일 자     :  2017.06.19
+ 수   정   자     :  정영식  
+ 수 정 내 용     :  파일 첨부기능 추가  // 사업자등록증 //발행승인요청서
------------------------------------------------------------------------------
+ 수 정 일 자     :  2018.11.05
+ 수   정   자     :  정영식  
+ 수 정 내 용     :  파일 첨부 찾아보기 시점에 계산서 번호 체크 
------------------------------------------------------------------------------
+ 수 정 일 자     :  2019.03.26
+ 수   정   자     :  정영식  
+ 수 정 내 용     :  ZACE에서 넘어 오는 건의 처리 문제 ==> 결재요청시 담당자 및 이메일 필수항목
------------------------------------------------------------------------------>
...
function ln_Initial(){
    ...
    gclx_ebillgb.bindcolval="1"; //매출은 스마트빌....
    gclx_taxsts.bindcolval="N";  //요청상태.....대기
...
function ln_Query(){
	...
	//매출 체크 
	if(str2.substring(0,1)=="A"||str2.substring(0,1)=="C"||str2.substring(0,1)=="E"||str2.substring(0,1)=="J"){
	...
	if(gcds_taxmst.countrow>0){
	
		ln_minorcd_call(gcem_vendcd.text);
	}
...
function ln_Save(){
	...
	if (gcds_taxmst.IsUpdated) {
		if (confirm("매출[세금]정보를 저장 하시겠습니까?")){	
			...
			//2110002
			//김인호DR 요청 건 
			if(gs_userid=="6070001"||gs_userid=="2110002"){
		        alert("gs_date::"+gs_date+"::");
		        if(gs_date=="20171002"||gs_date=="20171003"){
		        	gclx_taxsts.bindcolval="Y" ;
		        }
			}
			///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			
			if(gclx_minor_cd.bindcolval!=""){
				gcds_taxmst.namevalue(gcds_taxmst.countrow,"B_BIZPLACE") = gclx_minor_cd.bindcolval;
			}
...
function ln_Chk(){
	...
	//외부시스템 발행일 경우 스마트빌은 이외....2017.02.21 jys
	if(chk_ext.checked==true){ 
		if(gclx_ebillgb.bindcolval =="1"){
			alert("외부시스템 매출발행은 eBill구분이 스마트빌이 될 수 없습니다.");
			return false;
		}	
	}
...
function ln_Delete(){
	...
	//첨부파일이 존재할 경우 첨부파일 삭제 후 삭제가능
	if(gcds_taxmst.namevalue(1,"REAL_FILE")!=""){
		alert("첨부 파일이 존재 합니다. 첨부파일 선 삭제 후 삭제 가능합니다.");
		return false;
	}
	
	if(document.fileForm.fileList.options[0].text!=""){
		alert("첨부 파일이 존재 합니다. 첨부파일 선 삭제 후 삭제 가능합니다2.");
		return false;
	}
...
function ln_Taxmst_SetDataHeader(){
	...
	var s_tmp1 = "TAXNBR:STRING,COCODE:STRING,DEPTCD:STRING,EMPNO:STRING,TAXIODIV:STRING,"
			   ...
			   + "REAL_FILE:STRING,SYS_FILE:STRING,REAL_FILE2:STRING,SYS_FILE2:STRING";
...
<%
/******************************************************************************
Description : 거래처 종사업장 정보 호출 
******************************************************************************/
%>
function ln_minorcd_call(strVendcd){
	if(strVendcd=="") return;  
	
	gcds_minor_cd.DataID = "<%=dirPath%><%=HDConstant.PATH_ACCOUNT%>a040120_s3?v_str1="+strVendcd; 
	gcds_minor_cd.Reset();
	
	
	//alert("B_BIZPLACE::"+gcds_taxmst.countrow+"::"+gcds_taxmst.namevalue(gcds_taxmst.countrow,"B_BIZPLACE")+"::");
	
	if(gcds_minor_cd.countrow>0 && gcds_taxmst.countrow>0 ){
		gclx_minor_cd.bindcolval = gcds_taxmst.namevalue(gcds_taxmst.countrow,"B_BIZPLACE");
		gcem_remark3.text = gcds_taxmst.namevalue(gcds_taxmst.countrow,"REMARK3");
	}
}
<%
/******************************************************************************
...
function ln_ChkBox2(){
	...
	if(chk_ext.checked==true){ 
		...
		gclx_ebillgb.Enable= true;
		
	}else{
		gcds_taxmst.namevalue(gcds_taxmst.rowposition,"EXTCHK")="0";
		
		gclx_ebillgb.Enable= false;
		gclx_ebillgb.bindcolval ="1";
	}	
}
<%
/******************************************************************************
...
function ln_B_Save(){
	...
	//체크항목 추가 
	//세금계산서 작성일자가 발행일자보다 크면 안됨...
	//alert("DTI_STATUS::"+gcds_data.namevalue(gcds_data.rowposition,"DTI_STATUS"));
...
function ln_User_Chk(){
	...
	if(gcds_chk.countrow<=0){
		alert("전자세금계산서 발행전 반드시 [계산서 사용자관리]에 등록하셔야 합니다. ");
		return false;
	}
...
function ln_Batch_id(strdataSet){      
	...
	gctr_data4.Action = "<%=dirPath%><%=HDConstant.PATH_ACCOUNT%>a040065_t1";
	gctr_data4.Parameters = "v_str1="+gusrid+",v_str2="+strregno;
	gctr_data4.post();
}
<%
/***********************************************************************************************
...
function ln_Request(){
	...
	//2019.03.26 ZACE에서 누락건을 입력항목 한번 더 체크 
	if(!ln_Chk()) return false;
...
function ln_Chk_Status(){
	...
	//alert("options.length L::::"+f.fileList.options.length);
...
function fnFileUpload(){
	...
	/*
	if(gcds_taxmst.namevalue(gcds_taxmst.rowpostion,"TjyAXNBR")==""||gcds_taxmst.namevalue(gcds_data.rowpostion,"TAXNBR")==undefined){
...
//파일 찾기 체크
%>
function ln_File_Find_Chk(obj){
	

		if (gcem_taxnbr_s.text==""){
			alert("매출정보 및 세부내역 데이터를 저장 후 파일첨부 하십시요.");
...
<object id=gcds_minor_cd classid=clsid:3267EA0D-B5D8-11D2-A4F9-00608CEBEE49 VIEWASTEXT>
	<PARAM NAME="SyncLoad" VALUE="true">
</object>
...
<script language="javascript" for="gcds_minor_cd" event="onloadCompleted(row,colid)">

	
	if (gcds_taxmst.countrow>0 && gcds_taxmst.namevalue(gcds_taxmst.countrow,"B_BIZPLACE")!="" ){
		
		gclx_minor_cd.bincolval = gcds_taxmst.namevalue(gcds_taxmst.countrow,"B_BIZPLACE");
	}else {
		gcds_minor_cd.addrow();
		gclx_minor_cd.index =0;
	}
...
<script language=JavaScript for= gclx_minor_cd event=OnSelChange()>

	if(gclx_minor_cd.bindcolval!=""){
		gcem_remark3.text = gcds_minor_cd.namevalue(gcds_minor_cd.rowposition,"ADDRESS");	
	}else{
		gcem_remark3.text = "";
	}
...
<script language=JavaScript for=gclx_minor_cd event=OnKillFocus()>
	if(gclx_minor_cd.bindcolval!=""&&gcds_minor_cd.namevalue(gcds_minor_cd.rowposition,"VD_DIRECT")!=""){
		gcem_vd_direct.text = gcds_minor_cd.namevalue(gcds_minor_cd.rowposition,"VD_DIRECT");
	}else{
		gcem_vd_direct.text = arrParam[14]; //대표자명 원상복구
	}
...
<table id="div_disp1" cellpadding="0" cellspacing="0" border="0">	
	   	...
	   	<td style="position:relative;left:6px" colspan=2>
			<table cellpadding="1" cellspacing="0" border="0" style='width:950px;height:30px;border:1 solid #708090'>
				...
				<tr>
					...
					<td class="tab11" width="670px;" colspan="5" ><nobr>
						<comment id="__NSID__">
						...
						<comment id="__NSID__">
							<object  id=gcem_vendid classid=CLSID:E6876E99-7C28-43AD-9088-315DC302C05F	style="position:relative;top:3px;left:5px;width:100px;">
								<param name=Text			value="">
								<param name=Alignment		value=0>
								<param name=Border			value=true>
								<param name=Format			value="###-##-#####">
							...
							<object  id=gcem_seq classid=CLSID:E6876E99-7C28-43AD-9088-315DC302C05F	style="position:relative;top:2px;left:5px;width:40px;">
								<param name=Text			value="">
								<param name=Alignment		value=0>
								<param name=Border			value=true>
								<param name=visible			value=false>
					...
					<td class="tab11" width="70px" bgcolor="#eeeeee"><nobr>종사업장</nobr></td>
					<td class="tab22" >
						<comment id="__NSID__">
							<object  id=gclx_minor_cd classid=clsid:60109D65-70C0-425C-B3A4-4CB001513C69 height=106 style="position:relative;left:5px;top:0px;font-size:12px;width:110px;height:200px;">
								<param name=ComboDataID			value="gcds_minor_cd">
								<param name=CBDataColumns		value="MINOR_CD">
								<param name=SearchColumn		value="MINOR_CD">
								<param name=Sort				value="true">
								<param name=ListExprFormat	    value="MINOR_CD^0^110">
					...
					<td class="tab22" >
						 <comment id="__NSID__">
							<object  id=gclx_ebillgb classid=clsid:60109D65-70C0-425C-B3A4-4CB001513C69  style="position:relative;left:5px;top:1px;font-size:12px;width:110px;height:100px;">
								<param name=CBData			    value="1^스마트빌,2^트러스빌,9^기타">
								<param name=CBDataColumns		value="CDCODE,CDNAM">
								<param name=SearchColumn		value="CDNAM">
								<param name=Sort				value="false">
								<param name=Enable				value="false">
				...
				<tr>
					<td class="tab11" width="90px" bgcolor="#eeeeee"><nobr>비고1</nobr></td>
					<td class="tab22" colspan=7><nobr>
						<comment id="__NSID__">
							<object  id=gcem_amend_remark classid=CLSID:E6876E99-7C28-43AD-9088-315DC302C05F  style="width:400px;height:20px;position:relative;left:5px;top:1px">		
								<param name=Text		 value="">
								<param name=Alignment     value=0>
								<param name=Border        value=false>
								<param name=GeneralEdit   value=true>
				...
				<tr>
					<td class="tab11" width="90px" bgcolor="#eeeeee"><nobr>비고2</nobr></td>
					...
					<td class="tab11" width="90px" bgcolor="#eeeeee"><nobr>입금예정일</nobr></td>
					<td class="tab21" width="100px"><nobr>
						<comment id="__NSID__">
					    ...
					    <img src="../img/calender.gif" value="calender.gif" id=img_calendar1  OnClick="__GetCallCalendar('gcem_duedate', 'Text')" style="position:relative;width:20px;left:3px;top:2px;cursor:hand;">
					    </nobr>
					</td>
					
					<td class="tab22" colspan=2>
				...
				<tr>
					<td class="tab11" width="90px" bgcolor="#eeeeee"><nobr>거래처종사업장</nobr></td>
					<td class="tab21" colspan=3><nobr>
						<comment id="__NSID__">
							<object  id=gcem_remark3 classid=CLSID:E6876E99-7C28-43AD-9088-315DC302C05F  style="width:400px;height:20px;position:relative;left:5px;top:1px">		
								<param name=Text		 value="">
								<param name=Alignment     value=0>
								<param name=Border        value=false>
								<param name=GeneralEdit   value=true>
					...
					<td class="tab11" width="90px" bgcolor="#eeeeee"><nobr>결재상태</nobr></td>
					<td class="tab22" colspan=3><nobr>
						<comment id="__NSID__">
							<object  id=gclx_taxsts classid=clsid:60109D65-70C0-425C-B3A4-4CB001513C69  style="position:relative;left:5px;top:1px;font-size:12px;width:110px;height:100px;">
				...
				<tr>
					...
					<td class="tab33" width="90px" bgcolor="#eeeeee"><nobr>합계</nobr></td>
					<td class="tab22"  colspan=3 ><nobr>
						<comment id="__NSID__">
		...
		<td style="cursor:hand;position:relative;left:6px;" width="845px" colspan=2><nobr>
		 	<comment id="__NSID__">
		  		<object id=gcgd_taxdtl classid="clsid:1F57AEAD-DB12-11D2-A4F9-00608CEBEE49" width='950px' height='41px' border='1'>
					...
					<C>  Name='규격'         ID=TAXSTD          	   Width=100	align=left     bgcolor=#CCFFCC </C>			  					
					<C>  Name='수량'		    ID=TAXQTY          	   Width=30 	align=right    bgcolor=#CCFFCC </C>
					<C>  Name='단가'         ID=TAXPRIC             Width=100	align=right	   bgcolor=#CCFFCC </C>					
     	...
     	<td style="position:relative;left:6px" colspan=2>
			<table cellpadding="1" cellspacing="0" border="0" style='width:950px;height:30px;border:1 solid #708090'>	
			     ...
			     <tr>
					...
					<td class="tab22">
						<input type="file"  name="filename2"   style="position:relative;left:4px;top:0px;width:536px;height:20px" onchange="ln_File_Find_Chk(this)" >&nbsp;
					</td>	
			     </tr>
			     ...
			     <tr>
					...
					<td class="tab22">
						<input type="file"  name="filename1"   style="position:relative;left:4px;top:0px;width:536px;height:20px" onchange="ln_File_Find_Chk(this)" >&nbsp;
					</td>	
			     </tr>
			     <tr>
			    	<td class="tab11" width="120px" bgcolor="#eeeeee"><nobr>첨부파일</nobr> </td>
					<td class="tab22" >
						<select multiple name="fileList" id="fileList" style="position:relative;left:4px;top:2px; width:450px;height:60px;border-color:#666666;background-color:#FFFOFO;font-weight:bold;">
							 <option></option>
						</select>
    ...
    <tr>
	 ...
	 <tr>
      ...
      -->	
	 <tr>
     	<td style="width:865;height:20px;border:0 solid #708090;border-bottom-width:1px"  colspan=2> &nbsp;* 예) [나라빌] 같은 외부시스템에서 매출 발행했을 경우 <font color="red">[외부시스템 매출발행]</font>을 반드시 체크 해 주십시요.
		</td>
     </tr>
     <tr>
     	<td style="width:865;height:20px;border:0 solid #708090;border-bottom-width:1px"  colspan=2> &nbsp;* 전자세금계산서 매출등록 후 결재요청 ==> 김인호 DR 승인 ==> 승인건에 한해서 담당자 전자 발행 	</td>
     </tr>
     <tr>
     	<td style="width:865;height:20px;border:0 solid #708090;border-bottom-width:1px"  colspan=2> &nbsp;* 입금예정일은 필수항목입니다. ( 문의 : 봉균일GJ 3838 )	</td>
     </tr>
</table> 
...
<comment id="__NSID__">
    ...
    <C>Col=REMARK3     	    Ctrl=gcem_remark3        	Param=text       </C>
    ...
    <C>Col=DUEDATE   		Ctrl=gcem_duedate       	Param=text       </C>
    <C>Col=TAXSTS   		Ctrl=gclx_taxsts          	Param=bindcolval </C>
	">
	</object>
