...
+ 프로그램 ID	:  A070001_repay_popup2
+ 기 능 정 의	:  
+ 작   성  자 :  정 영 식
+ 작 성 일 자 :  2007.06.05
-----------------------------------------------------------------------------
+ 수 정 내 용 :	 신청전표 작성자만 조회가능함.
+ 수   정  자 :  jys
+ 수 정 일 자 :  2017.03.07
-----------------------------------------------------------------------------
+ 서 블 릿 명	: 
...
function ln_Start(){
	...
	txt_empno.value = strParam.g_arrParam[11];
...
function ln_Query(){
  ...
  //가수금의 경우 입금일자 및 관리항목 코드 넘김
	...
	if(txt_empno.value==""){
		alert("작성자가 누락되었습니다. 다시 조회 하십시요.");
		return false;
	}

	gcds_data01.DataID = "<%=dirPath%><%=HDConstant.PATH_ACCOUNT%>a070001_repay_s1?v_str1="+gs_fdcode               //지점       
...
<table  cellpadding="0" cellspacing="0" border=0 style="position:relative;left:3px;width:860px;height:30px;font-size:9pt;background-color:#ffffff;">
	...
	<tr>
			...
			<td class="tab19" width="70px"  height="30px" style="" bgcolor="#eeeeee"  align=center ><nobr>&nbsp;&nbsp;작성자&nbsp;</nobr></td>
			<td class="tab19" style="height:30px;width:250px;"><nobr>&nbsp;
					<input id="txt_empno" type="text" class="txtbox"  style= "position:relative;top:-1px;left:-4px;width:60px; height:20px;" maxlength="10" >
				</nobr>
			</td>
	</tr>
