...
<!-------------------------------------------------------------------------------
+ 시 스 템 명	:  자금관리 - 일일자금신청마감
+ 프로그램 ID	:   A700002.html
+ 기 능 정 의	:   출력,조회 화면이다.
+ 작   성  자   :  
+ 서 블 릿 명	:	
------------------------------------------------------------------------------
...
function ln_Query(){
	...
	//	var str2 = gcem_sgetdatefr.text;			//신청일자
	//	var str3 = gclx_statcd.bindcolval;		//마감구분
	...
	if(gcds_data1.countrow>0){
		if(gclx_statcd.bindcolval =="Y") {
			if (confirm("마감처리를 취소하시겠습니까?")) {
				var str1  = gclx_sfdcode.BindColVal;				   //지점코드         
				var str2  = fn_trim(gcem_sgetdatefr.text);            //일자
...
//	gcem_sgetdatefr1.text = gs_date;            //신청일자
//	gcem_sgetdateto2.text = gs_date;           //신청일자 
					...
					<tr> 
						<td style="width:460" ALIGN=LEFT VALIGN=TOP><comment id="__NSID__">
							<OBJECT id=gcgd_disp01 classid=clsid:1F57AEAD-DB12-11D2-A4F9-00608CEBEE49 style="WIDTH:867px;HEIGHT:380px;border:1 solid #777777;display:block;position:relative;left:-1px" viewastext>
								...
								<C> Name='신청일자'		ID=ACDATE		HeadAlign=Center HeadBgColor=#B9D4DC Width=70		align=center  SumText=''		SumBgColor='#C3D0DB' Mask='XXXX/XX/XX' show=true</C>
								<C> Name='마감여부'		ID=STATDIV		HeadAlign=Center HeadBgColor=#B9D4DC Width=70		align=center  SumText=''		SumBgColor='#C3D0DB'  show=true </C>
								<C> Name='부서'				ID=DEPTCD		HeadAlign=Center HeadBgColor=#B9D4DC Width=220	align=left		SumText='계'	SumBgColor='#C3D0DB' EditStyle='lookup' Data='gcds_dept:DEPTCD:DEPTNM' SubSumText='               소계'</C>
								...
								<C> Name='지급기준'		ID=AMTDIV		HeadAlign=Center HeadBgColor=#B9D4DC Width=70		align=center	SumText=''		SumBgColor='#C3D0DB' EditStyle='combo' Data='C:현금,B:어음'</C>
								<C> Name='결재일자'		ID=EXDATE		HeadAlign=Center HeadBgColor=#B9D4DC Width=70		align=center  SumText=''		SumBgColor='#C3D0DB' Mask='XXXX/XX/XX'</C>
								<C> Name='마감일자'		ID=STDATE		HeadAlign=Center HeadBgColor=#B9D4DC Width=70		align=center  SumText=''		SumBgColor='#C3D0DB' Mask='XXXX/XX/XX'</C>
