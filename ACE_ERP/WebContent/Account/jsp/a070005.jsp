...
<!-------------------------------------------------------------------------------
+ 시 스 템 명	:  자금관리 - 사전입금관리
+ 프로그램 ID	:   A700005.html
+ 기 능 정 의	:  입력,출력,조회 화면이다.
+ 작   성  자   :   
+ 서 블 릿 명	:	
------------------------------------------------------------------------------>
...
******************************************************************************/
function ln_Query(){
		var str1  = gclx_sfdcode1.BindColVal;				//지점구분
		var str2  = gclx_cadeptcd.BindColVal;					//부서
		var str3  = fn_trim(gcem_sgetdatefr.text);			//입금일자fr
		var str4  = fn_trim(gcem_sgetdateto.text);			//입금일자to
...
function ln_Save(){
	if(gcds_data1.countrow>0){
		if(gcds_data1.IsUpdated) {
			if(ln_Chk()) {
				var str1 = gclx_sfdcode1.BindColVal;  //지점
				var str2 = gclx_cadeptcd.BindColVal;  //부서
				var str3 = fn_trim(gcem_sgetdatefr.text);			//입금일자fr
				var str4 = fn_trim(gcem_sgetdateto.text);			//입금일자to
...
function ln_Chk() {
	if (fn_trim(gcem_asq_fr.text)==""){
		alert("정확한 입금일자를 입력하십시요.");
		gcem_asq_fr.focus();
		return false; 
	}

	if (gcem_amt.text==0){
		alert("정확한 입금액을 입력하십시요.");
		gcem_amt.focus();
		return false; 
	}
	return true;
...
function ln_Excel(e){
	if(e=="01"){
		...
		else gcgd_disp01.RunExcel('사전입금관리');
	}
}
		...
		<td colspan=8 align=left style="position:relative;left:1px">
			<table cellpadding="1" cellspacing="0" border="0" style='width:876px;height:30px;border:1 solid #708090'>
				<tr>
					...
					<td class="tab12" style="width:70px;" bgcolor="#eeeeee"><nobr>입금일자</nobr>
					</td>
				...
				<tr> 
					<td style="width:460" ALIGN=LEFT VALIGN=TOP><comment id="__NSID__">
						<OBJECT id=gcgd_disp01 classid=clsid:1F57AEAD-DB12-11D2-A4F9-00608CEBEE49 style="WIDTH:520px;HEIGHT:380px;border:1 solid #777777;display:block;position:relative;left:-1px" viewastext>
							...
							<C> Name='입금일자'	ID=ACDATE		HeadAlign=Center HeadBgColor=#B9D4DC Width=80		align=center	SumText='계'		SumBgColor='#C3D0DB' Mask='XXXX/XX/XX'</C>
							<C> Name='입금액'		ID=PYAMT		HeadAlign=Center HeadBgColor=#B9D4DC Width=100	align=right		SumText=@sum		SumBgColor='#C3D0DB'</C>
							<C> Name='적요'			ID=REMARK		HeadAlign=Center HeadBgColor=#B9D4DC Width=285	align=left		SumText=''			SumBgColor='#C3D0DB'</C>
					...
					<td>
						<table border= "0" class=itemdisp cellpadding=0 cellspacing=0  border=0  style='width:350px;height:402px;border:1 solid #708090;position:relative;left:1px'>
							<tr>															
								<td class="tab11"  bgcolor="#eeeeee" Style="height:25px;width:80px;border:0 solid #708090;border-right-width:1px;border-top-width:0px;border-bottom-width:0px;">입금일자
								</td>
							...
							<tr>											
								<td class="tab11"  bgcolor="#eeeeee" Style="height:25px;width:80px;border:0 solid #708090;border-right-width:1px;border-top-width:1px;border-bottom-width:0px;">입금액
								</td>
							...
							<tr>											
								<td class="tab11"  bgcolor="#eeeeee" Style="height:25px;width:80px;border:0 solid #708090;border-right-width:1px;border-top-width:1px;border-bottom-width:1px;">적요
								</td>
