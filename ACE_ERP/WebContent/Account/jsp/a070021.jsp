...
/*=============================================================================
 ...
 - 작 성 자		 : 정영식
 - 작 성 일		 : 2012-10-09
 - 기능정의		 : 대금지급 등록건을 대상으로 전표 발행.
 - [ 수정일자 ][수정자] 내용
 - [2012-10-09][정영식] 신규
 
 - [ 수정일자 ][수정자] 
 - [2017-08-24][정영식] a070021_s3에서 쿼리문 수정 (미지급금) 상대계정 - 미지급금(지급어음) 요청자 - 손명희
 
 - [ 수정일자 ][수정자] 
 - [2017-12-05][정영식] 기타업체 조회 추가 
...
function fnGetbyymm(v_yyyymm) {
    ...
    function fnSelect() {
    	 ...
    	 if(gcrd_vend_gubun.codevalue==""){
    		 alert("업체구분이 누락되었습니다. ");
             return false;
    	 }
    	
    	 if(gcrd_vend_gubun.codevalue=="1"){ 
    
	         //조회 조건확인 
	         ds_default.DataID = "<%=dirPath%><%=HDConstant.PATH_ACCOUNT%>a070021_s1"
										     + "?v_str1="+gclx_fdcode.BindColVal     
										 	 + "&v_str2="+gcem_actyymm.text           
											 + "&v_str3="+gcem_paydat.text       
											 + "&v_str4="+gcrd_gubun.codevalue
	                                         + "&v_str5="+gcem_actyymm_to.text; 
	        
			 // prompt('', gcds_code01.DataID);				
			 ds_default.Reset();
    	 }else if(gcrd_vend_gubun.codevalue=="2"){
    		   //조회 조건확인 
	         ds_default.DataID = "<%=dirPath%><%=HDConstant.PATH_ACCOUNT%>a070021_s11"
										     + "?v_str1="+gclx_fdcode.BindColVal     
										 	 + "&v_str2="+gcem_actyymm.text           
											 + "&v_str3="+gcem_paydat.text       
											 + "&v_str4="+gcrd_gubun.codevalue
   ...
   <%//전표생성할 데이터 조회 %>
    function fnCreateSelect() {
        ...
        if(gcrd_vend_gubun.codevalue=="1"){
	        ...
	        if(gcrd_gubun.codevalue==1){ 
		        ds_slipdtl.DataID = "<%=dirPath%><%=HDConstant.PATH_ACCOUNT%>a070021_s2"
				  					       + "?v_str1="+gclx_fdcode.BindColVal     
										   + "&v_str2="+gcem_actyymm.text           
										   + "&v_str3="+gcem_paydat.text       
										   + "&v_str4="+gcrd_gubun.codevalue
				...
				ds_slipdtl.Reset();
				 ...
				 ds_slipdtl.DataID = "<%=dirPath%><%=HDConstant.PATH_ACCOUNT%>a070021_s3"
				  					       + "?v_str1="+gclx_fdcode.BindColVal     
										   + "&v_str2="+gcem_actyymm.text           
										   + "&v_str3="+gcem_paydat.text       
										   + "&v_str4="+gcrd_gubun.codevalue  
        	...
        	if(gcrd_gubun.codevalue==1){ 
		        ds_slipdtl.DataID = "<%=dirPath%><%=HDConstant.PATH_ACCOUNT%>a070021_s12"
				  					       + "?v_str1="+gclx_fdcode.BindColVal     
										   + "&v_str2="+gcem_actyymm.text           
										   + "&v_str3="+gcem_paydat.text       
										   + "&v_str4="+gcrd_gubun.codevalue
				...
				ds_slipdtl.Reset();
				 ...
				 ds_slipdtl.DataID = "<%=dirPath%><%=HDConstant.PATH_ACCOUNT%>a070021_s13"
				  					       + "?v_str1="+gclx_fdcode.BindColVal     
										   + "&v_str2="+gcem_actyymm.text           
										   + "&v_str3="+gcem_paydat.text       
										   + "&v_str4="+gcrd_gubun.codevalue  
    ...
    <%//전표등록 그리드 데이터 기본셋팅_협력업체   %>
    function fnSlipdtl_Display_1(row){
    	var strPo="";
    ...
    <%//전표등록 그리드 데이터 기본셋팅_기타업체   %>
    function fnSlipdtl_Display_2(row){
    	var strPo="";
  			 ...
  			 if (gcds_slipmst.IsUpdated) {					
				 if(confirm("전표를 저장하시겠습니까?")){
					if(gcrd_vend_gubun.codevalue=="1"){
						...
						tr2.Keyvalue="Account.a070021_t1(I:USER3=gcds_slipmst,I:USER4=gcds_slipdtl,I:USER6=gcds_banjae,I:USER14=gcds_banjaemst,I:USER7=gcds_acustpay,O:USER5=gcds_slipno )"; 
						tr2.Action = "<%=dirPath%><%=HDConstant.PATH_ACCOUNT%>a070021_t1?";
						...
						tr2.Keyvalue="Account.a070021_t1(I:USER3=gcds_slipmst,I:USER4=gcds_slipdtl,I:USER6=gcds_banjae,I:USER14=gcds_banjaemst,I:USER7=gcds_acustpay,O:USER5=gcds_slipno )";
						tr2.Action = "<%=dirPath%><%=HDConstant.PATH_ACCOUNT%>a070021_t11?";
						tr2.Parameters="v_str1="+gusrid+",v_str2="+gfdcode;
						tr2.post();
					}
			     }
...
%>
	...
	<script language=javascript for="ds_slipdtl" event="OnLoadCompleted(row)">
		...
		if(gcrd_vend_gubun.codevalue=="1"){
			fnSlipdtl_Display_1(row);
		}else if(gcrd_vend_gubun.codevalue=="2"){
	    	fnSlipdtl_Display_2(row);
		}
...
<!--------------------------------- 코딩 부분 시작 ------------------------------->		   
    <table border='0' cellpadding='0' cellspacing='0' width="845px"> 
                    	...
                    	<td>
                    		<table  border='0' cellpadding="0" cellspacing="1" bgcolor="#666666" width='845px'>
                    			<tr>
                                    ...
                                    <td align="center" class="text" width="70">지급구분</td>
                                    <td bgcolor="#FFFFFF">&nbsp;
                                       <%=HDConstant.COMMENT_START%>
			                            <object id=gcrd_gubun classid=CLSID:754F3DC4-0C79-4C92-AD64-A806D8FF2AB0 style="height:24; width:140">
											<param name=Cols	value="2">
											<param name=Format	value="1^현금 ,2^어음">
										</object>
			                            <%=HDConstant.COMMENT_END%>
                                    </td>
                                    <td align="center" class="text" width="70">업체구분</td>
                                    <td bgcolor="#FFFFFF">&nbsp;
                                       <%=HDConstant.COMMENT_START%>
			                            <object id=gcrd_vend_gubun classid=CLSID:754F3DC4-0C79-4C92-AD64-A806D8FF2AB0 style="height:24; width:165">
											<param name=Cols	value="2">
											<param name=Format	value="1^협력업체 ,2^기타(업체)">
										</object>
			                            <%=HDConstant.COMMENT_END%>
                                    </td>
                    			</tr>
                                <tr>
			                        <td align=left class="text"  width="70">회계년월</td>
			                        <td align=left bgcolor="#ffffff" >&nbsp;&nbsp;<%=HDConstant.COMMENT_START%>
			                            ...
			                            <%=HDConstant.COMMENT_END%>        
			                            <img src="../img/calender.gif" value="calender.gif" id=img_calendar1 OnClick="__GetCallMonth('gcem_actyymm_to', 'Text')" style="position:relative;width:20px;left:2px;cursor:hand;"> (2개월만가능)
			                        
			                        </td>   
			                        <td align="center" class="text" width="70">지급일자</td>
        ...
        <tr height='10px'>
            <td><b><font size="2" color="blue">[대금지급정보]</font></b></td>
        </tr>
        <tr>
            <td valign="top">
                        ...
                        <td valign='top' width="845px">
                            ...
                            <object id=gr_default classid=<%=HDConstant.CT_GRID_CLSID%> width='845px' height='170px' border='1'>
                                ...
                                <param name="Format"                 value="
                                     ...
                                     <C> name='구분'	            ID='GUBUN'    	         width=40 	align=center	 sumtext='총' BgColor={IF(FSLOCK='T','#FFCC66','FFFFFF')}</C>
                                     <C> name='계정코드'       ID='ATCODE'       	     width=60 	align=center      sumtext='@cnt' sumtextalign='center'       BgColor={IF(FSLOCK='T','#FFCC66','FFFFFF')}</C>
                                     <C> name='계정명'		    ID='ATKORNAM' 		 width=73 	align=left      	 sumtext='건'     sumtextalign='center'       BgColor={IF(FSLOCK='T','#FFCC66','FFFFFF')}</C>
									 <C> name='거래처' 	        ID='VENDCD'             width=60 	    align=left       	 BgColor={IF(FSLOCK='T','#FFCC66','FFFFFF')}</C>
									 <C> name='거래처명' 	    ID='VENDNM'    	     width=120 	align=left      	 BgColor={IF(FSLOCK='T','#FFCC66','FFFFFF')}</C>
                                     <C> name='지급금액'  	ID='JANAMT'              		width=80 	align=right 		 sumtext='@sum'  BgColor={IF(FSLOCK='T','#FFCC66','FFFFFF')}</C>
                                     <C> name='회계일자'      ID='ACTDAT'   	         width=65 	align=center      mask ='XXXX/XX/XX' BgColor={IF(FSLOCK='T','#FFCC66','FFFFFF')}</C>
                                     <C> name='지급일자'      ID='PAYDAT'   	         width=65 	align=center   	 mask ='XXXX/XX/XX' BgColor={IF(FSLOCK='T','#FFCC66','FFFFFF')}</C>
                                     <C> name='적요'            ID='REMARK'      	     width=170 	align=left  	    BgColor={IF(FSLOCK='T','#FFCC66','FFFFFF')}</C>
                                     <C> name='만기일자'      ID='FULDAT'      	     		width=65  	align=center  	 mask ='XXXX/XX/XX' BgColor={IF(FSLOCK='T','#FFCC66','FFFFFF')}</C>                                     
                                     <C> name='지점'	           ID='FDCODE'    	          width=80 	align=center 	BgColor={IF(FSLOCK='T','#FFCC66','FFFFFF')}</C>
                                     <C> name='접수일자'	   ID='FSDAT'    	          		width=80 	align=center 	BgColor={IF(FSLOCK='T','#FFCC66','FFFFFF')}</C>
                                     <C> name='접수번호'      ID='FSNBR'    	          width=80 	align=center 	BgColor={IF(FSLOCK='T','#FFCC66','FFFFFF')}</C>
                                     <C> name='잠김'      		ID='FSLOCK'    	          width=80 	align=center 	BgColor={IF(FSLOCK='T','#FFCC66','FFFFFF')}</C>    
                             ">
                            </object>   
                            <%=HDConstant.COMMENT_END%>
                        </td>                      
							...
							<%=HDConstant.COMMENT_START%>
								<object id=gr_slipdtl classid=<%=HDConstant.CT_GRID_CLSID%> width='843px' height='170px' border='1'>
									...
									<param name="Format"		  	    value=" 
										...
										<C> Name='적요'		   ID=REMARK              Width=250	    align=left	    SumText={sum(CHAAMT)-sum(DAEAMT)} SumTextAlign=right </C> 
										...
										<C> Name='거래처'	   ID=VEND_NM     	   Width=140 	align=left       edit=none   </C> 
