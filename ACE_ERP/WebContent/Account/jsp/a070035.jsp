...
<%
/*=============================================================================
 - 시스템명 	     : 법인명 법인카드 대금지급 전표등록
 - 프로그램ID 	 : a070035.jsp
 - J  S  P		 : a070035.jsp
 - 서 블 릿		 : a070035_s1 , a070035_s2  
 - 작 성 자		 : 정영식
 - 작 성 일		 : 2015-08-12
...
<script language="javascript">
    ...
    function fnInit() {
	    ...
	    txt_atcodenm_fr.value ="미지급금(법인카드)";
	    txt_atcode_fr.value ="2100520";
	    
	    gclx_bankcd.bindcolval ="004";
...
function fnGetbyymm(v_yyyymm) {
    ...
    <%//메인 조회  %>
     function fnSelect(strParam) {
         ...
         //조회 조건확인 
         ds_default.DataID = "<%=dirPath%><%=HDConstant.PATH_ACCOUNT%>a070035_s1"
   ...
   <%//전표생성할 데이터 조회 %>
    function fnCreateSelect() {
        ...
        var strAtcode ="2100520";
        
        ds_slipdtl.DataID = "<%=dirPath%><%=HDConstant.PATH_ACCOUNT%>a070035_s2"
		  					       + "?v_str1="+gclx_fdcode.BindColVal     
								   + "&v_str2="+strAtcode
								   + "&v_str3="+strfsdat
    ...
    function fnSlipdtl_Display(row){
		...
		for(var i=1;i<=row;i++){
			...
			if (ds_slipdtl.namevalue(i,"ATCODE")=="2100110"){ 
			...
			}else if(ds_slipdtl.namevalue(i,"ATCODE")=="2100520"){  //미지급금(법인명법인카드)
				
				strPo="";
				strPo = fnRef_position_find(i,"1134");  //카드종류
				ds_slipdtl.namevalue(i,"FSREFVAL"+strPo)=ds_slipdtl.namevalue(i,"CARDKD") ;  
				
				if(ds_slipdtl.namevalue(i,"CARDKD")=="003"){
					ds_slipdtl.namevalue(i,"FSRVALNM"+strPo)="농협카드" ; 
				}else if(ds_slipdtl.namevalue(i,"CARDKD")=="004"){
				...
				//법인카드 거래처 
				strPo="";
				strPo = fnRef_position_find(i,"0029");
				ds_slipdtl.namevalue(i,"FSREFVAL"+strPo)  = "";
				ds_slipdtl.namevalue(i,"FSRVALNM"+strPo) = ds_slipdtl.namevalue(i,"CARD_VENDER") ;
	...
	function fnSlipdtlSetDataHeader() {
	 	  var s_temp = "FDCODE:STRING,FSDAT:STRING,FSNBR:STRING,FSSEQ:STRING,ATCODE:STRING,"
					 ...
					 + "BGTDIV:STRING,FUNDDIV:STRING,BANJAEREF:STRING,LASTREF:STRING,BANJAEMOD:STRING,ATCURTYPE:STRING,CARD_VENDER:STRING";
...
%>
	...
	<script language=javascript for="ds_default" event="OnLoadCompleted(row)">
		...
		//alert("CurLevel:;"+gr_default.CurLevel);
		/*
		for(var i=1;i<=row;i++){
			alert("i::"+ds_default.namevalue(i,"Count"));
		}
		*/
		
		
		//gr_default.ColumnProp("CARD_NO", "SumText") = "Card Count : " +gr_default.ColumnProp("CARD_NO", "SumText") ds_default.RealCount;
...
<object id=ds_default classid=<%=HDConstant.CT_DATASET_CLSID%>>  <!--조회-->
    ...
    <param name=SubsumExpr	value="CARD_NO">
</object>
...
<!--------------------------------- 코딩 부분 시작 ------------------------------->		   
    <table border='0' cellpadding='0' cellspacing='0' width="845px"> 
                    	...
                    	<td>
                    		<table  border='0' cellpadding="0" cellspacing="1" bgcolor="#666666" width='845px'>
                    			<tr>
                                    ...
                                    <td bgcolor="#FFFFFF">&nbsp;
                                        <%=HDConstant.COMMENT_START%>
			                         		<input id="txt_atcodenm_fr"  type="text" class="txtbox"  style= "width:220px; height:20px;ime-mode:active;" maxlength="36" onkeydown="ln_Blur('txt_atcodenm_fr')">
                                    ...
                                    <td bgcolor="#FFFFFF" >
			                           		<object  id=gclx_bankcd classid=clsid:60109D65-70C0-425C-B3A4-4CB001513C69 style="position:relative;left:2px;top:0px;font-size:12px;width:80px;">
											<param name=CBData			    value="003^농협카드,004^우리카드">
        ...
        <tr height='10px'>
            <td><b><font size="2" color="blue">[법인명법인카드지급정보]</font></b></td>
        </tr>
        <tr>
            <td valign="top">
                        ...
                        <td valign='top' width="845px">
                            ...
                            <object id=gr_default classid=<%=HDConstant.CT_GRID_CLSID%> width='845px' height='175px' border='1'>
                                ...
                                <param name="Format"           value="
                                     ...
                                     <C> name='카드번호' 	    ID='CARD_NO'    	 width=120 	align=left    BgColor={IF(FSLOCK='T','#FFCC66','#FFFFFF')}  sumtext='카드수' </C>
                                     <C> name='전표번호'      ID='SSDAT_NO'    	 width=110 	align=center  BgColor={IF(FSLOCK='T','#FFCC66','#FFFFFF')} 	sumtext={Count-SubRealCount(curlevel=100)}</C>
                                     <C> name='지급금액'  	ID='CRAMT'           width=80 	align=right   BgColor={IF(FSLOCK='T','#FFCC66','#FFFFFF')} 	sumtext='@sum'  </C>
                                     <C> name='적요'         ID='REMARK'      	 width=250 	align=left    BgColor={IF(FSLOCK='T','#FFCC66','#FFFFFF')} 	</C>
                                     <C> name='카드사'        ID='GBCD'    	     width=55	align=center  BgColor={IF(FSLOCK='T','#FFCC66','#FFFFFF')} 	EditStyle=Combo		Data='003:농협카드,004:우리카드'</C>                                    
                                     <C> name='지점'	        ID='FDCODE'    	     width=30 	align=center  BgColor={IF(FSLOCK='T','#FFCC66','#FFFFFF')} 	show=false</C>
                                     <C> name='순번'         ID='FSNUM'    	     width=30 	align=center  BgColor={IF(FSLOCK='T','#FFCC66','#FFFFFF')} 	show=false</C>
                                     <C> name='작성자'       ID='EMPNMK'    	     width=50 	align=center  BgColor={IF(FSLOCK='T','#FFCC66','#FFFFFF')} 	</C>
                             ">
                            </object>   
                            <%=HDConstant.COMMENT_END%>
                        </td>                      
        	...
        	<td>
	        	<table cellpadding="0" cellspacing="0" border="0" style="width:845px;height:200px;font-size:9pt;background-color:#ffffff;" > 
							...
							<%=HDConstant.COMMENT_START%>
								<object id=gr_slipdtl classid=<%=HDConstant.CT_GRID_CLSID%> width='843px' height='175px' border='1'>
									...
									<param name="Format"		  	    value=" 
										...
										<C> Name='관리항목4' ID=FSRVALNM04 	   Width=130 	align=left	   edit=none   </C> 
					...
					<tr>
						...
						<td width="104px" class="tab35"     align=center  bgcolor="#B9D4DC" > 
						    <input type=text    id=txt_fsrefnm04   name=txt_fsrefnm04  value="관리항목4"  size="15"  class=txt41  style="position:relative;top:1px;width:120px;height:17px;background-color:#B9D4DC;text-align:center;" readonly tabindex=-1>
						...
						<td width="104px" class="tab35"     align=center  bgcolor="#B9D4DC" > 
						    <input type=text    id=txt_fsrefnm09   name=txt_fsrefnm09  value="관리항목9"  size="15"  class=txt41  style="position:relative;top:1px;width:80px;height:17px;background-color:#B9D4DC;text-align:center;" readonly tabindex=-1>
					...
					<tr>
						...
						<td width="124px" class="tab26"     align=center > 
						    <%=HDConstant.COMMENT_START%>
						 	<object id=gcem_fsrvalnm04  classid=CLSID:E6876E99-7C28-43AD-9088-315DC302C05F  style="color:#990000;width:120px;height:18px;position:relative;top:4px;" tabindex=8>				
							<param name=Text		    value="">
							<param name=Border          value=false>
							<param name=Format          value="">
							<param name=GeneralEdit     value="true">
						...
						<td width="104px" class="tab26"     align=center>
						    <%=HDConstant.COMMENT_START%>  
							<object id=gcem_fsrvalnm09  classid=CLSID:E6876E99-7C28-43AD-9088-315DC302C05F  style="color:#990000;width:80px;height:20px;position:relative;top:4px;" tabindex=8>				
							<param name=Text		        value="">
							<param name=Border          value=false>
							<param name=Format          value="">
							<param name=GeneralEdit   value="true">
