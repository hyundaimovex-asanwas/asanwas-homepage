...
function ln_Query(){
  ...
  //ln_Find_ym();
	...
	gcds_code01.DataID = "<%=dirPath%><%=HDConstant.PATH_ACCOUNT%>a090033_s1"
									    + "?v_str1="+gclx_fdcode.BindColVal               //지점
										+ "&v_str2="+gclx_asdivcod.BindColVal           //자산분류
										+ "&v_str3="+gcem_jasan.text                          //자산명코드									
										+ "&v_str4="+gcrd_gubun.codevalue               //구분 
										+ "&v_str5="+gclx_sys_fr.bindcolval                 //실사상태  
...
function ln_Save(){
	if(gcds_code01.IsUpdated){
		if(confirm("실사현황을  저장하시겠습니까?")){
				gctr_code01.KeyValue   =  "Account.a090033_t1(I:USER=gcds_code01)";
				gctr_code01.Action     = "<%=dirPath%><%=HDConstant.PATH_ACCOUNT%>a090033_t1?";
	...
	if((row==0) && (colid=="CHK")){
		...
		for(i=1;i<=gcds_code01.countrow;i++){
			if(gs_gubun=="1"||gs_gubun==""){
        		...
        		gcds_code01.namevalue(i,"DDSTS")="1";
			}else{
        		gcds_code01.namevalue(i,"CHK")="F";
        		gcds_code01.namevalue(i,"DDSTS")="";
			}
		}
	}
</script>
...
<table width="860px" cellpadding="0" cellspacing="0" border="0" style="position:relative;left:3px">
	<tr> 
		<td background="../../Common/img/com_t_bg.gif"  align=left ><img src="../img/a090033_head.gif">	</td>
...
<table cellpadding="0" cellspacing="0" border=0 style="position:relative;left:3px;width:860px;height:30px;font-size:9pt;background-color:#ffffff;">
	...
	<tr> 
		...
		<td width="80px"  class="tab28"  bgcolor="#eeeeee" align=center> 실사상태 </td>  
	  <td width="120px" class="tab28">&nbsp;
			<comment id="__NSID__">
				<OBJECT id=gclx_sys_fr classid=clsid:60109D65-70C0-425C-B3A4-4CB001513C69 style="position:relative;left:0px;top:2px;font-size:12px;width:100px;">
					<param name=CBData					value="^전체,1^보유,2^부족,3^불용">
	...
	<tr> 
		<td>
			...
			style="position:relative;left:0px;width:858px; height:350px; border:1 solid #708090;display:block;" viewastext>
					...
					<param name="Format"		  value=" 
					<F> Name='자산분류'			ID=AST2NDNM		HeadAlign=Center HeadBgColor=#B9D4DC Width=80 	align=left	     SumBgColor=#C3D0DB sumtext=''   sort=true   edit=none </F>
					<FC> Name='자산번호'			ID=ASTNBR			HeadAlign=Center HeadBgColor=#B9D4DC Width=65 	align=center SumBgColor=#C3D0DB sumtext=''   sort=true   edit=none</FC>
					<FC> Name='자산명'			    ID=ASTNM			HeadAlign=Center HeadBgColor=#B9D4DC Width=150	align=left	    SumBgColor=#C3D0DB sumtext='계' sort=true  edit=none SumTextAlign=center </FC>
					<C> Name='취득일자'			ID=ASAQSDAT		HeadAlign=Center HeadBgColor=#B9D4DC Width=70    align=center SumBgColor=#C3D0DB sumtext=''   sort=true   edit=none MASK='XXXX/XX/XX'   </C>
					<G> Name='실사현황 '		                                HeadAlign=Center HeadBgColor=#B9D4DC
					<C> Name='보유'			        ID=CHK    			HeadAlign=Center HeadBgColor=#B9D4DC Width=18 	align=center SumBgColor=#C3D0DB sumtext=''   BgColor=#CCFFCC  editstyle=checkbox   HeadCheck=false HeadCheckShow=true </C>
					<C> Name='상태'			        ID=DDSTS			HeadAlign=Center HeadBgColor=#B9D4DC Width=50 	align=center SumBgColor=#C3D0DB sumtext=''    EditStyle=Combo		  Data='1:보유,2:부족,3:불용' BgColor=#CCFFCC   HeadCheck=FALSE HeadCheckShow=true  </C>
					<C> Name='사유'	                ID=DDNOTE			HeadAlign=Center HeadBgColor=#B9D4DC Width=100	align=left       SumBgColor=#C3D0DB sumtext=''    BgColor=#CCFFCC </C>
					</G>
					<C> Name='취득금액'			ID=ASTAQAMT		HeadAlign=Center HeadBgColor=#B9D4DC Width=100  align=right	 SumBgColor=#C3D0DB sumtext='@sum'  sort=true  edit=none </C>
					<C> Name='전기누계액'	    ID=ABDEPRAMT	HeadAlign=Center HeadBgColor=#B9D4DC Width=100  align=right	 SumBgColor=#C3D0DB sumtext='@sum'  sort=true  edit=none </C>
					<C> Name='당기상각액'	    ID=ACDEPRAMT	HeadAlign=Center HeadBgColor=#B9D4DC Width=100  align=right	 SumBgColor=#C3D0DB sumtext='@sum'  sort=true  edit=none </C>
					<C> Name='잔존가액'			ID=ASTRMAMT	    HeadAlign=Center HeadBgColor=#B9D4DC Width=100  align=right	 Value={ASTAQAMT-(ABDEPRAMT+ACDEPRAMT)}   edit=none  SumBgColor=#C3D0DB sumtext={sum(ASTAQAMT-(ABDEPRAMT+ACDEPRAMT))} sort=true  </C>
					<C> Name='보관장소'			ID=USELOC			HeadAlign=Center HeadBgColor=#B9D4DC Width=150	align=left	     SumBgColor=#C3D0DB sumtext=''   sort=true  edit=none </C>
					<C> Name='원가명'				ID=COSTNM			HeadAlign=Center HeadBgColor=#B9D4DC Width=150	align=left	     SumBgColor=#C3D0DB sumtext=''   sort=true  edit=none </C>
					<C> Name='상태'		    	    ID=ASTSTATNM	HeadAlign=Center HeadBgColor=#B9D4DC Width=50 	align=center  SumBgColor=#C3D0DB sumtext=''   sort=true  edit=none</C>
					">
			</object></comment><script>__ws__(__NSID__);</script>
...
<B>id=default ,left=0,top=0 ,right=2000 ,bottom=330 ,face='굴림체' ,size=10 ,penwidth=1
	<L> left=693 ,top=13 ,right=693 ,bottom=312 </L>
	<L> left=93 ,top=312 ,right=693 ,bottom=312 </L>
	<L> left=93 ,top=13 ,right=93 ,bottom=312 </L>
	<T>id='자 산 명 :' ,left=103 ,top=127 ,right=315 ,bottom=177 ,face='굴림체' ,size=10 ,bold=true ,underline=false ,italic=false ,forecolor=#000000 ,backcolor=#FFFFFF</T>
	<T>id='취득일자 :' ,left=103 ,top=183 ,right=315 ,bottom=233 ,face='굴림체' ,size=10 ,bold=true ,underline=false ,italic=false ,forecolor=#000000 ,backcolor=#FFFFFF</T>
	<T>id='자산코드 :' ,left=103 ,top=238 ,right=315 ,bottom=288 ,face='굴림체' ,size=10 ,bold=true ,underline=false ,italic=false ,forecolor=#000000 ,backcolor=#FFFFFF</T>
	<C>id='ASTNBR', left=323, top=238, right=688, bottom=288, align='left', face='굴림체', size=10, bold=true, underline=false, italic=false, forecolor=#000000, backcolor=#FFFFFF, MargineX=3</C>
	<C>id='ASAQSDAT', left=323, top=183, right=688, bottom=233, align='left' ,mask='XXXX-XX-XX', face='굴림체', size=10, bold=true, underline=false, italic=false, forecolor=#000000, backcolor=#FFFFFF, MargineX=3</C>
	<C>id='ASTNM', left=323, top=127, right=688, bottom=177, align='left', face='굴림체', size=10, bold=true, underline=false, italic=false, forecolor=#000000, backcolor=#FFFFFF, MargineX=3</C>
	<C>id='BARCODE', left=323, top=29, right=643, bottom=106, face='WASP 39 M', size=20, bold=false, underline=false, italic=false, forecolor=#000000, backcolor=#FFFFFF</C>
	<L> left=93 ,top=111 ,right=693 ,bottom=111 </L>
	<L> left=93 ,top=13 ,right=693 ,bottom=13 </L>
	<I>id='../../Common/img/icon.jpg' ,left=103 ,top=45 ,right=304 ,bottom=87</I>
</B>
