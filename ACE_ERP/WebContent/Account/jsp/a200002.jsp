...
<!-------------------------------------------------------------------------------
+ 시 스 템 명	:  어음관리 - 받을어음관리
+ 프로그램 ID	:  A200002.html
+ 기 능 정 의	:  받을어음관리 등록,저장,삭제,조회 및 출력 화면이다.
+ 작   성  자 :  이 순 미
+ 서 블 릿 명	:	 a200001_s1
------------------------------------------------------------------------------>
...
function ln_Query(s){
	if(s=="01") {
		...
		var str2  = fn_trim(gcem_sgetdatefr1.text);	//발행일자fr
		var str3  = fn_trim(gcem_sgetdateto2.text);	//발행일자to 
		var str4  = gcem_svendnm1.text;							//거래처명   
		var str5  = gcem_svendcd1.text;							//거래처코드   
		var str6  = gclx_sstdiv1.BindColVal;				//처리구분   
		...
		var str2  = fn_trim(gcem_losdate1.text);		//배서일자fr
		var str3  = fn_trim(gcem_losdate2.text);		//배서일자to 
		var str4  = gcem_svendnm2.text;							//거래처명   
		var str5  = gcem_svendcd2.text;							//거래처코드   
		var str6  = gclx_sstdiv2.BindColVal;				//처리구분   
		...
		var str2  = fn_trim(gcem_stdatefr3.text);		//수탁일자fr
		var str3  = fn_trim(gcem_stdateto3.text);		//수탁일자to 
		var str4  = gcem_svendnm3.text;							//거래처명   
		var str5  = gcem_svendcd3.text;							//거래처코드   
		var str6  = gclx_sstdiv3.BindColVal;				//처리구분   
		...
		var str2  = fn_trim(gcem_stdatefr4.text);		//할인일자fr
		var str3  = fn_trim(gcem_stdateto4.text);		//할인일자to 
		var str4  = gcem_svendnm4.text;							//거래처명   
		var str5  = gcem_svendcd4.text;							//거래처코드   
		var str6  = gclx_sstdiv4.BindColVal;				//처리구분   
		...
		var str2  = fn_trim(gcem_stdatefr5.text);		//부도일자fr
		var str3  = fn_trim(gcem_stdateto5.text);		//부도일자to 
		var str4  = gcem_svendnm5.text;							//거래처명   
		var str5  = gcem_svendcd5.text;							//거래처코드   
		var str6  = gclx_sstdiv5.BindColVal;				//처리구분   
...
function ln_Before(){
	...
	gcem_getdate1.text    = gs_date;				//[입력]발행일자
	gcem_enddate1.text    = gs_date;				//[입력]만기일자
	...
	gcem_losdate1.text = gs_date2+"01";	//[검색]배서from
	gcem_losdate2.text = gs_date;				//[검색]배서to
	gclx_sstdiv2.index = 1;	//[검색]처리구분
	...
	gcem_stdatefr3.text = gs_date2+"01";	//[검색]수탁from
	gcem_stdateto3.text = gs_date;				//[검색]수탁to
	gclx_sstdiv3.index = 1;	//[검색]처리구분
	...
	gcem_stdatefr4.text = gs_date2+"01";	//[검색]할인from
	gcem_stdateto4.text = gs_date;				//[검색]할인to
	gclx_sstdiv4.index = 1;	//[검색]처리구분
	...
	gcem_stdatefr5.text = gs_date2+"01";	//[검색]할인from
	gcem_stdateto5.text = gs_date;				//[검색]할인to
	gclx_sstdiv5.index = 1;	//[검색]처리구분
...
function ln_Endorsement(){
	if(gcds_data1.countrow>0){
		if(gclx_sstdiv1.bindcolval=='0'){
			...
			arrParam[2]="50";									//자금구분
			arrParam[3]=gcem_refcode21.text;	//어음번호
			...
			arrParam[4]=gcem_getdate1.text;		//발행일자
			arrParam[5]=gcem_enddate1.text;		//만기일자
			arrParam[6]=gcem_amt.text;				//발행금액
			arrParam[7]=gcem_vendnm1.text;		//거래처명
			arrParam[8]=gcem_vendcd1.text;		//거래처코드
		...
		}else alert("이미 배서처리되었습니다.");
	}else alert("배서처리할 정보가 없습니다.");
}

/***********************************************************************************************
	Description : 수탁추심정보 팝업
	parameter   : arrParam[0] - 거래처코드, arrParam[1] - 거래처명
**********************************************************************************************/
function ln_TrustTake(){
	if(gcds_data1.countrow>0){
		...
		}else alert("이미 수탁추심처리되었습니다.");
	}else alert("수탁추심처리할 정보가 없습니다.");
}
...
function ln_Dishonor(){
	if(gcds_data1.countrow>0){
		...
		}else alert("이미 부도처리되었습니다.");
	}else alert("부도처리할 정보가 없습니다.");
}
					...
					<td width="0" align=left><nobr>
					  <comment id="__NSID__"><object  id=gcte_disp classid=clsid:ED382953-E907-11D3-B694-006097AD7252 VIEWASTEXT style="width:660px;height:20px;">
							<PARAM NAME="BackColor"					VALUE="#cccccc">
							  ...
							  <T>divid=div_1		title=원장관리</T>									
							  <T>divid=div_2		title=배서현황</T>
							  <T>divid=div_3		title=수탁추심현황</T>
							  <T>divid=div_4		title=할인현황</T>
							  <T>divid=div_5		title=부도현황</T>
						  ">
	...
	<tr>
		<td colspan=8 align=left style="position:relative;left:7px">
			<table cellpadding="1" cellspacing="0" border="0" style='width:869px;height:30px;border:1 solid #708090'>
				<tr>
					<td class="tab11" style="width:80px;" bgcolor="#eeeeee"><nobr>지점구분</nobr></td>
				...
				<table width="847" cellpadding="1" cellspacing="0" border="0">
					<tr> 
						...
						<C> Name='어음수표번호'	    ID=REFCODE2	HeadAlign=Center HeadBgColor=#B9D4DC Width=110	align=left  	SumText='계'	SumBgColor='#C3D0DB'</C> 
						<C> Name='발행금액'			ID=DRAMT		HeadAlign=Center HeadBgColor=#B9D4DC Width=100	align=right   SumText=@sum	SumBgColor='#C3D0DB' Mask='XXXX/XX/XX'</C>
						...
						<C> Name='거래처'				ID=VEND_CD	HeadAlign=Center HeadBgColor=#B9D4DC Width=180	align=left		SumText=''		SumBgColor='#C3D0DB' EditStyle='lookup' Data='gcds_data1:VEND_CD:VEND_NM'</C>
						<C> Name='은행'				ID=BANCOD		HeadAlign=Center HeadBgColor=#B9D4DC Width=180	align=left		SumText=''		SumBgColor='#C3D0DB' EditStyle='lookup' Data='gcds_data1:BANCOD:BANNAM'</C>
									">
							</OBJECT></comment><script>__ws__(__NSID__);</script>
						...
						<table border= "0" class=itemdisp cellpadding=0 cellspacing=0  border=0  style='width:300px;height:402px;border:1 solid #708090;position:relative;left:7px'>
							...
							<tr>
								<td class="tab11" style="width:100px;" bgcolor="#eeeeee">발행일자</td>
								<td class="tab22"><comment id="__NSID__">
							...
							<tr>
								<td class="tab11" style="width:100px;" bgcolor="#eeeeee">만기일자</td>
								<td class="tab22"><comment id="__NSID__">
							...
							<tr>
								<td class="tab11" style="width:100px;" bgcolor="#eeeeee">발행금액</td>
								<td class="tab22"><comment id="__NSID__">
							...
							<!-- <tr>
								<td class="tab11" style="width:100px;" bgcolor="#eeeeee">대변금액</td>
								<td class="tab22"><comment id="__NSID__">
									<object  id=gcem_amt classid=CLSID:E6876E99-7C28-43AD-9088-315DC302C05F	style="position:relative;left:5px;top:0px;width:100px;">
										<param name=Text				value="">
										<param name=Alignment		value=2>
							...
							<tr>
								<td class="tab11" style="width:100px;" bgcolor="#eeeeee">잔&nbsp;&nbsp;&nbsp;&nbsp;액</td>
								<td class="tab22"><comment id="__NSID__">
									<object  id=gcem_amt classid=CLSID:E6876E99-7C28-43AD-9088-315DC302C05F	style="position:relative;left:5px;top:0px;width:100px;">
										<param name=Text				value="">
										<param name=Alignment		value=2>
							...
							<tr>
								<td class="tab11" style="width:100px;" bgcolor="#eeeeee">적요</td>
								<td class="tab22"><comment id="__NSID__">
							...
							<!-- <tr>
								<td class="tab11" style="width:100px;" bgcolor="#eeeeee">발생전표</td>
								<td class="tab22"><comment id="__NSID__">
									<object  id=gcem_refcode21 classid=CLSID:E6876E99-7C28-43AD-9088-315DC302C05F  style="width:180px;height:20px;position:relative;left:5px;top:0px">		
										<param name=Text					value="">
										<param name=Border        value=true>
							...
							<tr>
								<td class="tab11" style="width:100px;" bgcolor="#eeeeee">회계일자</td>
								<td class="tab22"><comment id="__NSID__">
									<object id=gcem_getdate1 classid=CLSID:E6876E99-7C28-43AD-9088-315DC302C05F 
										style="position:relative; left:5px;top:2px; width:70px; height:20px;" class="txtbox">
										<param name=Alignment			value=0>
							...
							<tr>
								<td class="tab11" style="width:100px;" bgcolor="#eeeeee">처리전표</td>
								<td class="tab22"><comment id="__NSID__">
									<object  id=gcem_refcode21 classid=CLSID:E6876E99-7C28-43AD-9088-315DC302C05F  style="width:180px;height:20px;position:relative;left:5px;top:0px">		
										<param name=Text					value="">
										<param name=Border        value=true>
		...
		<td colspan=8 align=left style="position:relative;left:7px">
			<table cellpadding="1" cellspacing="0" border="0" style='width:869px;height:30px;border:1 solid #708090'>
				<tr>
					...
					<td class="tab11" style="width:70px;" bgcolor="#eeeeee"><nobr>배서일자</nobr></td>
					...
					<tr> 
						<td style="width:460" ALIGN=LEFT VALIGN=TOP><comment id="__NSID__">
							<OBJECT id=gcgd_disp02 classid=clsid:1F57AEAD-DB12-11D2-A4F9-00608CEBEE49 style="WIDTH:867px;HEIGHT:380px;border:1 solid #777777;display:block;position:relative;left:6px" viewastext>
								...
								<PARAM NAME="Format"        VALUE="  
									<C> Name='No.'					ID={CURROW}	HeadAlign=Center HeadBgColor=#B9D4DC Width=40		align=center	SumText=''		SumBgColor='#C3D0DB'</C>
									<C> Name='배서일자'			ID=STRDATE1	HeadAlign=Center HeadBgColor=#B9D4DC Width=70		align=center  SumText=''		SumBgColor='#C3D0DB' Mask='XXXX/XX/XX'</C>
									<C> Name='배서인'				ID=CHKMAN		HeadAlign=Center HeadBgColor=#B9D4DC Width=70		align=left		SumText=''		SumBgColor='#C3D0DB'</C>
									<C> Name='주소'					ID=ADDRESS	HeadAlign=Center HeadBgColor=#B9D4DC Width=200	align=left	  SumText=''		SumBgColor='#C3D0DB'</C>
									<C> Name='연락처'				ID=VD_TELNO	HeadAlign=Center HeadBgColor=#B9D4DC Width=100	align=center  SumText=''		SumBgColor='#C3D0DB'</C>
									<C> Name='어음수표번호'	ID=REFCODE2	HeadAlign=Center HeadBgColor=#B9D4DC Width=182	align=left  	SumText='계'	SumBgColor='#C3D0DB'</C>
									<C> Name='발행금액'			ID=DRAMT		HeadAlign=Center HeadBgColor=#B9D4DC Width=70		align=right   SumText=@sum	SumBgColor='#C3D0DB'</C>
									<C> Name='발행일자'			ID=STRDATE2	HeadAlign=Center HeadBgColor=#B9D4DC Width=70		align=center  SumText=''		SumBgColor='#C3D0DB' Mask='XXXX/XX/XX'</C>
									<C> Name='만기일자'			ID=ENDDATE	HeadAlign=Center HeadBgColor=#B9D4DC Width=70		align=center  SumText=''		SumBgColor='#C3D0DB' Mask='XXXX/XX/XX'</C>
									<C> Name='거래처'				ID=VEND_NM	HeadAlign=Center HeadBgColor=#B9D4DC Width=140	align=left		SumText=''		SumBgColor='#C3D0DB'</C>
								">
							</OBJECT></comment><script>__ws__(__NSID__);</script>
		...
		<td colspan=8 align=left style="position:relative;left:7px">
			<table cellpadding="1" cellspacing="0" border="0" style='width:869px;height:30px;border:1 solid #708090'>
				<tr>
					...
					<td class="tab11" style="width:70px;" bgcolor="#eeeeee"><nobr>수탁일자</nobr></td>
					<td class="tab22" style="width:210px">
					...
					<tr> 
						<td style="width:460" ALIGN=LEFT VALIGN=TOP><comment id="__NSID__">
							<OBJECT id=gcgd_disp03 classid=clsid:1F57AEAD-DB12-11D2-A4F9-00608CEBEE49 style="WIDTH:867px;HEIGHT:380px;border:1 solid #777777;display:block;position:relative;left:6px" viewastext>
								...
								<PARAM NAME="Format"        VALUE="  
									<C> Name='No.'					ID={CURROW}	HeadAlign=Center HeadBgColor=#B9D4DC Width=40		align=center	SumText=''		SumBgColor='#C3D0DB'</C> 
									<C> Name='수탁일자'			ID=STRDATE1	HeadAlign=Center HeadBgColor=#B9D4DC Width=70		align=center  SumText=''		SumBgColor='#C3D0DB' Mask='XXXX/XX/XX'</C>
									<C> Name='수탁은행'			ID=BANCOD 	HeadAlign=Center HeadBgColor=#B9D4DC Width=100	align=left    SumText=''		SumBgColor='#C3D0DB'</C>
									<C> Name='추심일자'			ID=ORDDATE	HeadAlign=Center HeadBgColor=#B9D4DC Width=70		align=center  SumText=''		SumBgColor='#C3D0DB' Mask='XXXX/XX/XX'</C>
									<C> Name='추심료'				ID=ORDAMT 	HeadAlign=Center HeadBgColor=#B9D4DC Width=100	align=right   SumText=''		SumBgColor='#C3D0DB'</C>
									<C> Name='어음수표번호'	ID=REFCODE2	HeadAlign=Center HeadBgColor=#B9D4DC Width=182	align=left  	SumText='계'	SumBgColor='#C3D0DB'</C> 									
									<C> Name='발행금액'			ID=DRAMT  	HeadAlign=Center HeadBgColor=#B9D4DC Width=70		align=right   SumText=@sum	SumBgColor='#C3D0DB'</C>
									<C> Name='발행일자'			ID=STRDATE2	HeadAlign=Center HeadBgColor=#B9D4DC Width=70		align=center  SumText=''		SumBgColor='#C3D0DB' Mask='XXXX/XX/XX'</C>
									<C> Name='만기일자'			ID=ENDDATE	HeadAlign=Center HeadBgColor=#B9D4DC Width=70		align=center  SumText=''		SumBgColor='#C3D0DB' Mask='XXXX/XX/XX'</C>
									<C> Name='거래처'				ID=VEND_CD	HeadAlign=Center HeadBgColor=#B9D4DC Width=110	align=left		SumText=''		SumBgColor='#C3D0DB'</C>
								">
							</OBJECT></comment><script>__ws__(__NSID__);</script>
						<fieldset style="position:relative;left:6px;width:869px;height:20px;border:1 solid #708090;border-top-width:0px;border-bottom-width:1px;text-align:left;">
		...
		<td colspan=8 align=left style="position:relative;left:7px">
			<table cellpadding="1" cellspacing="0" border="0" style='width:869px;height:30px;border:1 solid #708090'>
				<tr>
					...
					<td class="tab11" style="width:70px;" bgcolor="#eeeeee"><nobr>할인일자</nobr></td>
					<td class="tab22" style="width:210px">
					...
					<tr> 
						<td style="width:460" ALIGN=LEFT VALIGN=TOP><comment id="__NSID__">
							<OBJECT id=gcgd_disp04 classid=clsid:1F57AEAD-DB12-11D2-A4F9-00608CEBEE49 style="WIDTH:867px;HEIGHT:380px;border:1 solid #777777;display:block;position:relative;left:6px" viewastext>
								...
								<PARAM NAME="Format"        VALUE="  
									<C> Name='No.'					ID={CURROW}	HeadAlign=Center HeadBgColor=#B9D4DC Width=40		align=center	SumText=''	SumBgColor='#C3D0DB'</C> 
									<C> Name='할인일자'			ID=STRDATE1	HeadAlign=Center HeadBgColor=#B9D4DC Width=70		align=center  SumText=''	SumBgColor='#C3D0DB' Mask='XXXX/XX/XX'</C>
									<C> Name='할인은행'			ID=BANCOD 	HeadAlign=Center HeadBgColor=#B9D4DC Width=180	align=left    SumText=''	SumBgColor='#C3D0DB'</C>
									<C> Name='할인율'				ID=DCRATE		HeadAlign=Center HeadBgColor=#B9D4DC Width=60		align=right		SumText=''	SumBgColor='#C3D0DB'</C>
									<C> Name='할인일수'			ID=DCDAYS		HeadAlign=Center HeadBgColor=#B9D4DC Width=60		align=right		SumText=''	SumBgColor='#C3D0DB'</C>
									<C> Name='할인료'				ID=DCAMT		HeadAlign=Center HeadBgColor=#B9D4DC Width=100	align=right		SumText=''	SumBgColor='#C3D0DB'</C>
									<C> Name='어음수표번호'	ID=REFCODE2	HeadAlign=Center HeadBgColor=#B9D4DC Width=182	align=left  	SumText='계'	SumBgColor='#C3D0DB'</C> 
									<C> Name='발행금액'			ID=DRAMT		HeadAlign=Center HeadBgColor=#B9D4DC Width=100	align=right		SumText=@sum	SumBgColor='#C3D0DB'</C>
									<C> Name='발행일자'			ID=STRDATE2	HeadAlign=Center HeadBgColor=#B9D4DC Width=70		align=center	SumText=''	SumBgColor='#C3D0DB' Mask='XXXX/XX/XX'</C>
									<C> Name='만기일자'			ID=ENDDATE	HeadAlign=Center HeadBgColor=#B9D4DC Width=70		align=center	SumText=''	SumBgColor='#C3D0DB' Mask='XXXX/XX/XX'</C>
									<C> Name='거래처'				ID=VEND_NM	HeadAlign=Center HeadBgColor=#B9D4DC Width=150	align=left  	SumText=''	SumBgColor='#C3D0DB'</C>
								">
							</OBJECT></comment><script>__ws__(__NSID__);</script>
						<fieldset style="position:relative;left:6px;width:869px;height:20px;border:1 solid #708090;border-top-width:0px;border-bottom-width:1px;text-align:left;">
		...
		<td colspan=8 align=left style="position:relative;left:7px">
			<table cellpadding="1" cellspacing="0" border="0" style='width:869px;height:30px;border:1 solid #708090'>
				<tr>
					...
					<td class="tab11" style="width:70px;" bgcolor="#eeeeee"><nobr>부도일자</nobr></td>
					<td class="tab22" style="width:210px">
					...
					<tr> 
						<td style="width:460" ALIGN=LEFT VALIGN=TOP><comment id="__NSID__">
							<OBJECT id=gcgd_disp05 classid=clsid:1F57AEAD-DB12-11D2-A4F9-00608CEBEE49 style="WIDTH:867px;HEIGHT:380px;border:1 solid #777777;display:block;position:relative;left:6px" viewastext>
								...
								<PARAM NAME="Format"        VALUE="  
									<C> Name='No.'					ID={CURROW}	HeadAlign=Center HeadBgColor=#B9D4DC Width=40		align=center	SumText=''	SumBgColor='#C3D0DB'</C> 
									<C> Name='부도일자'			ID=STRDATE1	HeadAlign=Center HeadBgColor=#B9D4DC Width=70		align=center  SumText=''	SumBgColor='#C3D0DB' Mask='XXXX/XX/XX'</C>
									...
									<C> Name='회수금액'			ID=RTNAMT 	HeadAlign=Center HeadBgColor=#B9D4DC Width=100	align=right   SumText=''	SumBgColor='#C3D0DB'</C>
									<C> Name='어음수표번호'	ID=REFCODE2	HeadAlign=Center HeadBgColor=#B9D4DC Width=182	align=left  	SumText='계'	SumBgColor='#C3D0DB'</C> 
									<C> Name='발행금액'			ID=DRAMT		HeadAlign=Center HeadBgColor=#B9D4DC Width=100	align=right		SumText=@sum	SumBgColor='#C3D0DB'</C>
									<C> Name='발행일자'			ID=STRDATE2	HeadAlign=Center HeadBgColor=#B9D4DC Width=70		align=center	SumText=''	SumBgColor='#C3D0DB' Mask='XXXX/XX/XX'</C>
									<C> Name='만기일자'			ID=ENDDATE	HeadAlign=Center HeadBgColor=#B9D4DC Width=70		align=center	SumText=''	SumBgColor='#C3D0DB' Mask='XXXX/XX/XX'</C>
