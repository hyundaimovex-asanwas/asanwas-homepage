...
<!-------------------------------------------------------------------------------
+ 시 스 템 명	:  어음관리 - 거래처별현황
+ 프로그램 ID	:  A200005.html
+ 기 능 정 의	:  거래처별현황 조회 및 출력 화면이다.
+ 작   성  자 :  이 순 미
+ 서 블 릿 명	:	 a200005_s1
-------------------------------------------------------------------------------->
...
function ln_Query(s){
	if(s=="01") {
		...
		var str3  = fn_trim(gcem_sstrdatefr1.text);	//발생일자fr
		var str4  = fn_trim(gcem_sstrdateto1.text);	//발생일자to 
		var str5  = gcem_svendnm1.text;							//거래처명   
		var str6  = gcem_svendcd1.text;							//거래처코드   
		var str7  = gclx_sstatdiv1.BindColVal;			//처리구분   

		gcds_data1.DataID = "<%=dirPath%><%=HDConstant.PATH_ACCOUNT%>a200005_s1"
...
function ln_Excel(e){
  if(e=="01") {
		...
		else gcgd_disp01.RunExcel('거래처별현황');
	}
}
...
******************************************************************************/
function ln_Before(){
	gcem_sstrdatefr1.text = gs_date2+"01";	//[검색]발행from
	gcem_sstrdateto1.text = gs_date;				//[검색]발행to
...
<table width="876" cellpadding="0" cellspacing="0" border="0" style="position:relative;left:3px">
  <tr> 
    <td><img src="../img/a200005_head.gif"></td>
			...
			<table cellpadding="1" cellspacing="0" border="0" style='width:869px;height:30px;border:1 solid #708090'>
				...
				<tr>
					...
					<td class="tab23" width="90px">
						<comment id="__NSID__"><object  id=gclx_sstatdiv1 classid=clsid:60109D65-70C0-425C-B3A4-4CB001513C69  style="position:relative;left:5px;top:0px;width:80px;height:110px;">
					...
					<td width="250px" colspan="3"><nobr><comment id="__NSID__">
						<object  id=gcem_svendnm1 classid=CLSID:E6876E99-7C28-43AD-9088-315DC302C05F  style="width:155px;height:20px;position:relative;left:5px;top:0px" onkeyDown="if(event.keyCode==13) ln_Query('01')">		
						...
						<td style="width:460" ALIGN=LEFT VALIGN=TOP><comment id="__NSID__">
							<OBJECT id=gcgd_disp01 classid=clsid:1F57AEAD-DB12-11D2-A4F9-00608CEBEE49 style="WIDTH:867px;HEIGHT:380px;border:1 solid #777777;display:block;position:relative;left:-1px" viewastext>
								...
								<PARAM NAME="Format"        VALUE="  
									...
									<C> Name='거래처'				ID=VEND_NM	HeadAlign=Center HeadBgColor=#B9D4DC Width=200	align=left    SumText=''		SumBgColor='#C3D0DB'</C>
									...
									<C> Name='은행'					ID=BANNAM		HeadAlign=Center HeadBgColor=#B9D4DC Width=140	align=left		SumText=''		SumBgColor='#C3D0DB'</C>
									<C> Name='적요'					ID=REMARK		HeadAlign=Center HeadBgColor=#B9D4DC Width=140	align=left		SumText=''		SumBgColor='#C3D0DB'</C>
								">
							</OBJECT></comment><script>__ws__(__NSID__);</script>
