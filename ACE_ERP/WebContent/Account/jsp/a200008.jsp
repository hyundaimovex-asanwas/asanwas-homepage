...
<!-------------------------------------------------------------------------------
+ 시 스 템 명	:  어음관리 - 회계일자별현황
+ 프로그램 ID	:  A200008.html
+ 기 능 정 의	:  회계일자별현황 조회 및 출력 화면이다.
+ 작   성  자 :  이 순 미
+ 서 블 릿 명	:	 a200008_s1
-------------------------------------------------------------------------------->
...
function ln_Query(s){
	if(s=="01") {
		...
		var str3  = fn_trim(gcem_sgetdatefr1.text);	//회계일자fr
		var str4  = fn_trim(gcem_sgetdateto2.text);	//회계일자to 
...
function ln_Excel(e){
  if(e=="01") {
		...
		else gcgd_disp01.RunExcel('회계일자별현황');
	}
}
...
******************************************************************************/
function ln_Before(){
	gcem_sgetdatefr1.text = gs_date2+"01";	//[검색]발행from
	gcem_sgetdateto2.text = gs_date;				//[검색]발행to
		...
		<td colspan=8 align=left style="position:relative;left:0px">
			<table cellpadding="1" cellspacing="0" border="0" style='width:869px;height:30px;border:1 solid #708090'>
				<tr>
					...
					<td class="tab12" style="width:70px;" bgcolor="#eeeeee"><nobr>회계일자</nobr></td>
						...
						<td style="width:460" ALIGN=LEFT VALIGN=TOP><comment id="__NSID__">
							<OBJECT id=gcgd_disp01 classid=clsid:1F57AEAD-DB12-11D2-A4F9-00608CEBEE49 style="WIDTH:867px;HEIGHT:380px;border:1 solid #777777;display:block;position:relative;left:-1px" viewastext>
								...
								<PARAM NAME="Format"        VALUE="  
									...
									<C> Name='회계일자'			ID=ACTDAT		HeadAlign=Center HeadBgColor=#B9D4DC Width=70		align=center  SumText=''		SumBgColor='#C3D0DB' Mask='XXXX/XX/XX'</C>
