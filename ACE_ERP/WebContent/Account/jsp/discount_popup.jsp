...
<!-------------------------------------------------------------------------------
+ 시 스 템 명	: 받을어음 할인정보 팝업
+ 프로그램 ID	: discount_popup.html
+ 기 능 정 의	: 조회|등록|저장|삭제
+ 변 경 이 력	: 이순미
+ 서 블 릿 명	: discount_popup_s1, discount_popup_t1
------------------------------------------------------------------------------>
...
******************************************************************************/
function ln_Query(){
	gcds_data1.DataID = "<%=dirPath%><%=HDConstant.PATH_ACCOUNT%>discount_popup_s1?v_str1="+p_fdcode
...
function ln_Save(){
	...
	if (gcds_data1.IsUpdated) {
		gctr_data1.KeyValue = "discount_popup_t1(I:USER=gcds_data1)";
		gctr_data1.Action = "<%=dirPath%><%=HDConstant.PATH_ACCOUNT%>discount_popup_t1";
...
******************************************************************************/
function ln_Delete(){
	if(fn_trim(gcem_strdate2.text)!=""&&fn_trim(gcem_bancodnm1.text)!=""&&fn_trim(gcem_dcrate.text)!=0
		&&fn_trim(gcem_dcdays.text)!=0&&fn_trim(gcem_dcamt.text)!=0||gcds_data1.SysStatus(1)==3){
...
function ln_Valchk() {
	if (fn_trim(gcem_strdate2.text)==""||gcem_strdate2.text.length<=7) {			
		alert("정확한 할인일자를 입력해 주십시오.");
		gcem_strdate2.focus();
		return false;
	}

	if (fn_trim(gcem_bancodnm1.text)==""||fn_trim(gcem_bancod1.text)=="") {			
		alert("정확한 할인은행을 입력해 주십시오.");
		gcem_strdate2.focus();
		return false;
	}

	if (fn_trim(gcem_dcrate.text)==0) {			
		alert("정확한 할인율을 입력해 주십시오.");
		gcem_dcrate.focus();
		return false;
	}

	if (fn_trim(gcem_dcdays.text)==0) {			
		alert("정확한 할인일수를 입력해 주십시오.");
		gcem_dcdays.focus();
		return false;
	}

	if (fn_trim(gcem_dcamt.text)==0) {			
		alert("정확한 할인료를 입력해 주십시오.");
		gcem_dcamt.focus();
		return false;
	}
	return true;
...
function ln_SetDataHeader(){
	var s_data = "FDCODE:STRING,ATCODE:STRING,FUNDDIV:STRING,REFCODE2:STRING,STRDATE:STRING,"
						 + "BANCOD:STRING,DCRATE:DECIMAL,DCDAYS:DECIMAL,DCAMT:DECIMAL,SLIPYN:STRING,"
			...
			<table width="310" cellpadding="1" cellspacing="0" border="0">
					...
					<td colspan=8 align=left>
						<table width="310" cellpadding="1" cellspacing="0" border="0" style='position:relative;left:1px;width:310px;border:1 solid #708090'>
							<tr>
								...
								<!--<img src="../../Common/img/btn/com_b_insert.gif"	style="cursor:hand;position:relative;top:2px" onClick="ln_Add()">-->
				...
				<tr>
					<td ALIGN=CENTER>
						<table width="310" cellpadding="1" cellspacing="0" border="0">
							<tr> 
								<td>
							<table cellpadding=0 cellspacing=0  border=0  style='width:310px;height:270px;BORDER-LEFT-WIDTH:1px;border:1 solid #708090'>
							<tr>
                ...
                <td class="tab11" style="height:25px;width:100px;" bgcolor="#eeeeee"><nobr>만기일자</nobr></td>  
							...
							<tr>
								...
								<td class="tab22"><comment id="__NSID__">
									<object  id=gcem_amt classid=CLSID:E6876E99-7C28-43AD-9088-315DC302C05F	style="position:relative;left:5px;top:0px;width:120px;">
										<param name=Text				value="">
							...
							<tr>
								<td class="tab11" style="height:25px;width:100px;" bgcolor="#eeeeee">할인일자</td>
								<td class="tab22"><comment id="__NSID__">
							...
							<tr>
								<td class="tab11" style="width:100px;" bgcolor="#eeeeee">할인은행</td>  
								<td class="tab22" style="height:45px;"><nobr><comment id="__NSID__">
									...
									<img name="btn_find" src="../../Common/img/btn/com_b_find.gif"	style="position:relative;top:2px;left:8px;cursor:hand"	 onclick="ln_Popup2('01')"></nobr>
								</td>
							</tr>
							<tr>
								<td class="tab11" style="height:25px;width:100px;" bgcolor="#eeeeee">할&nbsp;인&nbsp;율</td>
								<td class="tab22"><comment id="__NSID__">
									<object  id=gcem_dcrate classid=CLSID:E6876E99-7C28-43AD-9088-315DC302C05F	style="position:relative;left:5px;top:0px;width:100px;">
										<param name=Text				value="">
										<param name=Alignment		value=2>
										<param name=Border			value=true>
							...
							<tr>
								<td class="tab11" style="height:25px;width:100px;" bgcolor="#eeeeee">할인일수</td>
								<td class="tab22"><comment id="__NSID__">
									<object  id=gcem_dcdays classid=CLSID:E6876E99-7C28-43AD-9088-315DC302C05F	style="position:relative;left:5px;top:0px;width:100px;">
										<param name=Text				value="">
										<param name=Alignment		value=2>
										<param name=Border			value=true>
							...
							<tr>
								<td class="tab12" style="height:25px;width:100px;" bgcolor="#eeeeee">할&nbsp;인&nbsp;료</td>
								<td><comment id="__NSID__">
									<object  id=gcem_dcamt classid=CLSID:E6876E99-7C28-43AD-9088-315DC302C05F	style="position:relative;left:5px;top:0px;width:120px;">
										<param name=Text				value="">
										<param name=Alignment		value=2>
										<param name=Border			value=true>
										<param name=MaxLength		value=15>
...
<comment id="__NSID__"><object  id=gcbn_data classid=CLSID:9C9AB433-EA85-11D2-A4F9-00608CEBEE49> 
	...
	<C>Col=DCRATE					Ctrl=gcem_dcrate				Param=text</C>
	<C>Col=DCDAYS					Ctrl=gcem_dcdays				Param=text</C>
	<C>Col=DCAMT					Ctrl=gcem_dcamt					Param=text</C>
	'>
</object></comment><script>__ws__(__NSID__);</script> 
