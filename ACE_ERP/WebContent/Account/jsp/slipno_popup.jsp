...
var strWinParam = window.dialogArguments;
...
function ln_Start(){
	...
	if(gs_userid=="asan001"||gs_userid=="asan002"||gs_userid=="asan003"||gs_userid=="asan004"||gs_userid=="asan005"){
		...
		sp5.style.display = "none"; //숨김
	}else{
		sp3.style.display = "";  //보여줌
		sp4.style.display = "";  //보여줌
		sp5.style.display = "";  //보여줌
  ...
  //ln_Add("02",''); 
	...
	gcem_fs02.text = strParam[1];
	gcem_fsdat02.text = strParam[2];
	gcem_fsnbr02.text = strParam[3];
  */
	/*
	if(gclx_fdcode02.bindcolval!=""&&gcem_fs02.text!=""&&gcem_fsdat02.text!=""&&gcem_fsnbr02.text!=""){
		ln_Query('03');
		ln_Query('01');
	}else{
  ...
  /*************************************************************************************************
	 파라미터 넘겨받기(반제,부가세,일반장부)
	************************************************************************************************/
  var strParam = new Array();
  strParam=window.dialogArguments;

 // alert("strWinParam"+strWinParam);
  if(strWinParam=="[object]"){ //반제화면에서 넘겨줌
		var strfsdat="";

		//전표MST
		gclx_fdcode02.bindcolval=strWinParam.gcds_mst.namevalue(strWinParam.gcds_mst.rowposition,"FDCODE");    
		gstr_fsdat=strWinParam.gcds_mst.namevalue(strWinParam.gcds_mst.rowposition,"FSDAT"); 
		gcem_fsnbr02.text=strWinParam.gcds_mst.namevalue(strWinParam.gcds_mst.rowposition,"FSNBR"); 
		gcem_fs02.text = gstr_fsdat.substring(0,1);
		gcem_fsdat02.text = gstr_fsdat.substring(1,9);
		gclx_fsstat02.bindcolval =strWinParam.gcds_mst.namevalue(strWinParam.gcds_mst.rowposition,"FSSTAT");
		txt_deptcd02.value=strWinParam.gcds_mst.namevalue(strWinParam.gcds_mst.rowposition,"DEPTCD");
    	txt_empno02.value = strWinParam.gcds_mst.namevalue(strWinParam.gcds_mst.rowposition,"EMPNO");  
		gcem_fswrtdat02.text =gs_date;
	 	gcem_remark02.text=strWinParam.gcds_dtl.namevalue(1,"REMARK");
		...
		//사업소코드
		//본부코드
    	gcds_data01_1.namevalue(gcds_data01_1.rowposition,"REMARK")=gcem_remark02.text;
		gcds_data01_1.namevalue(gcds_data01_1.rowposition,"COCODE")=strWinParam.gcds_mst.namevalue(strWinParam.gcds_mst.rowposition,"COCODE"); 
    	gcds_data01_1.namevalue(gcds_data01_1.rowposition,"DIVCD")=strWinParam.gcds_mst.namevalue(strWinParam.gcds_mst.rowposition,"DIVCD");
		strfsdat= gcem_fs02.text+gcem_fsdat02.text;
		gs_fsknd="6"; //대체전표 수동기표
     
		//계정코드로 관리항목 가져오기 및 계정추가  Start////////////////////////////////////////////////
		//parameter   :  p1 - 계정코드, p2- 계정명,    p3 -거래처코드   p4-거래처명   p6 - 부서코드,
	  //               p5 - 부서명,   p7 - 적요,     p8 - 차변금액,   p9-대변금액   p10 - 계산서번호(공백)
		//               p11 - 증빙코드(공백),         p12-본부코드     p13-계산서일자      
		////////////////////////////////////////////////////////////////////////////////////////////////
    
		//계정건수 만큼 추가 
		//alert("strWinParam.gcds_dtl.countrow::"+strWinParam.gcds_dtl.countrow);
		for(m=1;m<=strWinParam.gcds_dtl.countrow;m++){ 
			if(m==1){
				ln_Add('02','BANJAE');  //신규
			}else{ 
				ln_Fsseq_Temp_Banjaeadd(strWinParam.gcds_dtl.namevalue(m,"FDCODE"),strWinParam.gcds_dtl.namevalue(m,"FSDAT"),
			                          strWinParam.gcds_dtl.namevalue(m,"FSNBR"),strWinParam.gcds_dtl.namevalue(m,"FSSEQ"));
				ln_Add('99','BAN');  //추가
			}

			ln_Refcode_find(strWinParam.gcds_dtl.namevalue(m,"ATCODE"),
										  strWinParam.gcds_dtl.namevalue(m,"ATKORNAM"),
										  strWinParam.gcds_dtl.namevalue(m,"VENDCD"),
										  strWinParam.gcds_dtl.namevalue(m,"VENDNM"),
										  strWinParam.gcds_dtl.namevalue(m,"DEPTCD"),
										  strWinParam.gcds_dtl.namevalue(m,"DEPTNM"),
										  strWinParam.gcds_dtl.namevalue(m,"REMARK"),
										  strWinParam.gcds_dtl.namevalue(m,"CHAAMT"),
										  strWinParam.gcds_dtl.namevalue(m,"DAEAMT"),
										  '','',
										  strWinParam.gcds_dtl.namevalue(m,"DIVCD"),
										  '');
			//ifrm.ln_onblur('01','',m); //차변대변 금액합계
			//ifrm.ln_onblur('02','',m);
      		//alert("ln_Refcode_val");
			ln_Refcode_val(strWinParam.gcds_dtl.namevalue(m,"FSREFCD"),
										 strWinParam.gcds_dtl.namevalue(m,"FSREFVAL"),
										 strWinParam.gcds_dtl.namevalue(m,"FSREFNM")
										);
      //alert("ln_Refcode_val222");
			ln_Refcode_val(strWinParam.gcds_dtl.namevalue(m,"FSREFCD2"),
										 strWinParam.gcds_dtl.namevalue(m,"FSREFVAL2"),
										 strWinParam.gcds_dtl.namevalue(m,"FSREFNM2")
										 );
			
			//가수금
			if(strWinParam.gcds_dtl.namevalue(m,"ATCODE")=="2101300"){
				ln_Refcode_val(strWinParam.gcds_dtl.namevalue(m,"REFCD2"),
										   strWinParam.gcds_dtl.namevalue(m,"REFVAL2"),
										   strWinParam.gcds_dtl.namevalue(m,"REFVAL2")
										   );
			}

			//법인카드
			if(strWinParam.gcds_dtl.namevalue(m,"ATCODE")=="2100520"){
				ln_Refcode_val(strWinParam.gcds_dtl.namevalue(m,"REFCD2"),
										   strWinParam.gcds_dtl.namevalue(m,"REFVAL2"),
										   strWinParam.gcds_dtl.namevalue(m,"REFVALNM2")
										   );
			}
			...
			//외상매입금(지급어음)
			if(strWinParam.gcds_dtl.namevalue(m,"ATCODE")=="2100120"){
				
			var TSDAT = strWinParam.gcds_dtl.namevalue(m,"TSDAT");
			var TSNBR = strWinParam.gcds_dtl.namevalue(m,"TSNBR");			
			var TSSEQ = strWinParam.gcds_dtl.namevalue(m,"TSSEQ");			
			
				gcds_temp.DataID = "<%=dirPath%><%=HDConstant.PATH_ACCOUNT%>a020001_s25?v_str1="+TSDAT+"&v_str2="+TSNBR+"&v_str3="+TSSEQ;		
				gcds_temp.Reset();				
			
					for(i=1;i<=6;i++){ 					

						if( gcds_temp.namevalue(i,"FSREFCD") == "0967" ){

							gcds_dtl.namevalue(gcds_dtl.rowposition,"FSREFVAL03") = gcds_temp.namevalue(i,"FSREFVAL");
							gcds_dtl.namevalue(gcds_dtl.rowposition,"FSRVALNM03") = gcds_temp.namevalue(i,"FSRVALNM");
							
						}else if (gcds_temp.namevalue(i,"FSREFCD") == "0022"){
							gcds_dtl.namevalue(gcds_dtl.rowposition,"FSREFVAL04")= gcds_temp.namevalue(i,"FSREFVAL");							
							gcds_dtl.namevalue(gcds_dtl.rowposition,"FSRVALNM04")=gcds_temp.namevalue(i,"FSRVALNM");				
							
						}else if (gcds_temp.namevalue(i,"FSREFCD") == "0968"){
							gcds_dtl.namevalue(gcds_dtl.rowposition,"FSREFVAL05") = gcds_temp.namevalue(i,"FSREFVAL");							
							gcds_dtl.namevalue(gcds_dtl.rowposition,"FSRVALNM05")=gcds_temp.namevalue(i,"FSRVALNM");				
							
						}else if (gcds_temp.namevalue(i,"FSREFCD") == "0969"){
							gcds_dtl.namevalue(gcds_dtl.rowposition,"FSREFVAL06") = gcds_temp.namevalue(i,"FSREFVAL");							
							gcds_dtl.namevalue(gcds_dtl.rowposition,"FSRVALNM06")=gcds_temp.namevalue(i,"FSRVALNM");
      ...
      //2008.01.12 반제차에서 가져올 경우 반제관리항목 넣어주기
			hid_banjaeref.value = strWinParam.gcds_dtl.namevalue(m,"FSREFCD");

			//ln_lock(m,'6'); //자동기표 lock
      //alert("m::"+m+"GUBUN::"+strWinParam.gcds_dtl.namevalue(m,"GUBUN") );
		}

		//반제연결테이블에 저장////////////////////////////////////////////////////////////////////////
		// BTSSEQ는 eval("hid_fsseq"+i).value 이것으로 입력한다. 
    // FTSTAT	FDCODE	TSDAT	TSNBR	TSSEQ	
		// SSDAT, BTSDAT	BTSNBR	BTSSEQ	BANJAEYN	
		// SSNBR,FSNUM,BJDEAMT,BJCRAMT,BSDAT, BSNBR, BSNUM ==>추가된 부분
		// WRDT	WRID
		///////////////////////////////////////////////////////////////////////////////////////////////
		// alert("strWinParam.gcds_banjae.countrow"+strWinParam.gcds_banjae.countrow);
    	for (b=1;b<=strWinParam.gcds_banjae.countrow;b++){
			 ln_Banjae_Data_Save(strWinParam.gcds_banjae.namevalue(b,"FTSTAT"),
													 strWinParam.gcds_banjae.namevalue(b,"FDCODE"),
													 strWinParam.gcds_banjae.namevalue(b,"TSDAT"),
													 strWinParam.gcds_banjae.namevalue(b,"TSNBR"),
													 strWinParam.gcds_banjae.namevalue(b,"TSSEQ"),
													 strWinParam.gcds_banjae.namevalue(b,"BTSDAT"),
													 strWinParam.gcds_banjae.namevalue(b,"BTSNBR"),
													 strWinParam.gcds_banjae.namevalue(b,"BTSSEQ"),
													 strWinParam.gcds_banjae.namevalue(b,"BANJAEYN"),
													 strWinParam.gcds_banjae.namevalue(b,"SSDAT"),
													 strWinParam.gcds_banjae.namevalue(b,"SSNBR"),
													 strWinParam.gcds_banjae.namevalue(b,"FSNUM"),
													 strWinParam.gcds_banjae.namevalue(b,"BJDEAMT"),
													 strWinParam.gcds_banjae.namevalue(b,"BJCRAMT"),
													 strWinParam.gcds_banjae.namevalue(b,"BSDAT"),
													 strWinParam.gcds_banjae.namevalue(b,"BSNBR"),
													 strWinParam.gcds_banjae.namevalue(b,"BSNUM"),
													 strWinParam.gcds_banjae.namevalue(b,"WRDT"),
													 strWinParam.gcds_banjae.namevalue(b,"WRID"));
		}//for b


		//반제 MST에 저장////////////////////////////////////////////////////////////////////////
		// FDCODE,	TSDAT,	TSNBR,	  TSSEQ,	  ATCODE,	
    // DEPTCD,	VENDCD,	FSREFCD,	FSREFVAL,	DEAMT,	
    // CRAMT,	  STATUS,	SSDAT,	  SSNBR,  	FSNUM,	
		// REMARK, 
		// WRDT,    WRID,   UPDT,     UPID,     AMT
		// SEQ (추가함.)
		/////////////////////////////////////////////////////////////////////////////////////////
		  //alert("strWinParam.gcds_banjaemst"+strWinParam.gcds_banjaemst.countrow);
		for (c=1;c<=strWinParam.gcds_banjaemst.countrow;c++){
			 ln_Banjaemst_Data_Save(strWinParam.gcds_banjaemst.namevalue(c,"FDCODE"),
															strWinParam.gcds_banjaemst.namevalue(c,"TSDAT"),
															strWinParam.gcds_banjaemst.namevalue(c,"TSNBR"),
															strWinParam.gcds_banjaemst.namevalue(c,"TSSEQ"),
															strWinParam.gcds_banjaemst.namevalue(c,"ATCODE"),
															strWinParam.gcds_banjaemst.namevalue(c,"DEPTCD"),
															strWinParam.gcds_banjaemst.namevalue(c,"VENDCD"),
															strWinParam.gcds_banjaemst.namevalue(c,"FSREFCD"),
															strWinParam.gcds_banjaemst.namevalue(c,"FSREFVAL"),
															strWinParam.gcds_banjaemst.namevalue(c,"DEAMT"),
															strWinParam.gcds_banjaemst.namevalue(c,"CRAMT"),
															strWinParam.gcds_banjaemst.namevalue(c,"STATUS"),
															strWinParam.gcds_banjaemst.namevalue(c,"SSDAT"),
															strWinParam.gcds_banjaemst.namevalue(c,"SSNBR"),
															strWinParam.gcds_banjaemst.namevalue(c,"FSNUM"),
															strWinParam.gcds_banjaemst.namevalue(c,"REMARK"),
															strWinParam.gcds_banjaemst.namevalue(c,"WRDT"),
															strWinParam.gcds_banjaemst.namevalue(c,"WRID"),
															strWinParam.gcds_banjaemst.namevalue(c,"UPDT"),
															strWinParam.gcds_banjaemst.namevalue(c,"UPID"),
															strWinParam.gcds_banjaemst.namevalue(c,"AMT"),
															strWinParam.gcds_banjaemst.namevalue(c,"SEQ")
															);
    	}
		//prompt("strWinParam.gcds_dtl.text",strWinParam.gcds_dtl.text);
		//prompt("strWinParam.gcds_banjae.text",strWinParam.gcds_banjae.text);
		//prompt("strWinParam.gcds_banjaemst.text",strWinParam.gcds_banjaemst.text);

  }else if(strParam[0]=="T"){ //세금계산서에서 넘겨받음
	  
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    /* 넘겨받는값  Mst (13개)
       구분자 , fdcode, fsdat, fsnbr, fsstat, deptcd, deptnm, empno, empnmk, remark, cocode, divcd, 계산서금액합
       넘겨받는값 Dtl(11개씩 반복)
			 계정코드, 계정명, 거래처코드, 거래처명, 부서코드, 부서명, 적요 , 차변금액, 대변금액(반복), 계산서번호, 증빙코드
		*/
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	 /*
		alert("p0:"+strParam[0]+"p1:"+strParam[1]+"p2:"+strParam[2]+"p3:"+strParam[3]+"p4:"+strParam[4]+"p5:"+strParam[5]
		     +"p6:"+strParam[6]+"p7:"+strParam[7]+"p8:"+strParam[8]+"p9:"+strParam[9]
				 +"p10:"+strParam[10]+"p11:"+strParam[11]+"p12:"+strParam[12]);
		*/

		//20060616 - divcd : mst에서 사용하지 않은 필드이므로 부가세종류를 넣김.(과세,영세,면세,불공,,,,,)

	  	var strfsdat="";
		gclx_fdcode02.bindcolval=strParam[1];
		gs_fdcode = gclx_fdcode02.bindcolval; //나중에 삭제함.
		gstr_fsdat=strParam[2];
		gcem_fsnbr02.text=strParam[3];
		gcem_fs02.text = gstr_fsdat.substring(0,1);
		gcem_fsdat02.text = gstr_fsdat.substring(1,9);
		ln_Add('02');
		gclx_fsstat02.bindcolval =strParam[4];
		txt_deptcd02.value=strParam[5];
		txt_deptnm02.value=strParam[6];
    	txt_empno02.value = strParam[7];
		txt_empnmk02.value =strParam[8];
		//alert("txt_deptcd02:"+strParam[5]+":txt_deptnm02:"+strParam[6]+":txt_empno02:"+strParam[7]+":txt_empnmk02:"+strParam[8]);
		gcem_fswrtdat02.text =gs_date;
	 	gcem_remark02.text=strParam[9];
		...
		//사업소코드
		//본부코드
    	gcds_data01_1.namevalue(gcds_data01_1.rowposition,"REMARK")=strParam[9];
		gcds_data01_1.namevalue(gcds_data01_1.rowposition,"COCODE")=strParam[10];
    	gcds_data01_1.namevalue(gcds_data01_1.rowposition,"DIVCD")="";
    //alert("DIVCD"+strParam[11]); 
		gcem_fsamt02.text=strParam[12]; // 합계금액

		gs_fsknd="6"; //대체전표 수동기표
		
		strfsdat= gcem_fs02.text+gcem_fsdat02.text;

		var temp =strParam.length-13
  		var cnt = temp/13 //Dtl row 갯수
		//alert("cnt::"+cnt);
		...
		for(m=1;m<=cnt;m++){
		  ...
		  //Detail 항목 추가
			if(m==1){
			  strGubun="1";
			}else if(strParam[11]=="4"&&strParam[k+1]=="1112300"){ //불공이면서 선급부가가치세 
				k+=13;
				m+=1;
				if(strParam[k+1]==undefined){                        //
					strGubun="2";
				}else{
				  strGubun="1";
					//ln_Add('88','A');
			...
			if(strGubun=="1"){
				chasum+=parseInt(strParam[k+8]);
				daesum+=parseInt(strParam[k+9]);
				///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
				//계정코드로 관리항목 가져오기....
				//parameter   :  p1 - 계정코드, p2- 계정명,    p3 -거래처코드,   p4-거래처명,  p5 - 부서코드, 
				//               p6-  부서명,   p7 - 적요,     p8 - 차변,        p9-대변,      p10 - 계산서번호, 
				//               p11- 증빙코드 ,p12-본부       p13 - 계산서일자
				///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
				//alert("atcode::"+strParam[k+1]);
				ln_Refcode_find(strParam[k+1],strParam[k+2], strParam[k+3],strParam[k+4], strParam[k+5],
											  strParam[k+6],strParam[k+7], strParam[k+8],strParam[k+9], strParam[k+10], 
											  strParam[k+11],strParam[k+12],strParam[k+13]);
				 
				//동일계산서 번호는 한번만 넘김.
				if(stracnbr!=strParam[k+10]){
					//ifrm.ln_Tax_Data_Save(strParam[k+10],gs_date,gs_userid, gcem_ss02.text+gcem_ssdat02.text );
					ln_Tax_Data_Save(strParam[k+10],gs_date,gs_userid, gcem_fs02.text+gcem_fsdat02.text );
					/////////&&&ln_Tax_Sum(strParam[k+10],strParam[12]) //세금계산서 금액합계 체크를 위해넘김
				}
				stracnbr = strParam[k+10];
				ln_lock(m,'1'); //자동기표 lock
			}
			strGubun="";
		}//for
    ...
    //alert("chasum"+chasum);
		//alert("daesum"+daesum);
		//gcem_detot02.text = Number(chasum);
		//gcem_crtot02.text = Number(daesum);
	}else{
	  //alert("else" + strParam[0])
		gclx_fdcode02.bindcolval=strParam[0];
		...
		//반제전표구분을 위해 필요함.--> 반제전표는 삭제할 수 없음.
		gcds_data01_1.namevalue(gcds_data01_1.rowposition,"FSDAT")=gstr_fsdat;
		ln_Query('03');
		ln_Query('01');
...
function ln_Add(p,gubun){
  if(p=='02'){
	...
	if(gubun=="BANJAE"){
	  //FSSEQ 
	  ln_Fsseq_Temp_Banjaeadd(strWinParam.gcds_dtl.namevalue(1,"FDCODE"),strWinParam.gcds_dtl.namevalue(1,"FSDAT"),
		                        strWinParam.gcds_dtl.namevalue(1,"FSNBR"),strWinParam.gcds_dtl.namevalue(1,"FSSEQ"));
	}else{
		ln_MST_Data_Clear();
	}
    ...
    if(gcds_dtl.rowposition>0){
			if(gubun == "TAX"||gubun=="BAN"){
				...
				//순번설정
				gcds_dtl.namevalue(gcds_dtl.rowposition,"FSNUM")= ln_Fsnum_Find();
				 if(gubun=="BAN")ln_Lock_false(); //200711.14추가
			}else{
				if(ln_Chk_Addrow()){
					//첫번째에서 두번째 행추가 할때 부서,거래처,적요 저장
    ...
    if(gubun=="BANJAE"||gubun=="BAN"){
		ln_Data_Fdcode_Fsdat_Fsnbr("BANJAE");
	}else{
...
/************************************************************************************************************
	...
	parameter   : p - banjae : 반제건   
*************************************************************************************************************/
function ln_Data_Fdcode_Fsdat_Fsnbr(p){
    ...
    if(p=="BANJAE"){ //반제처리에서 가져올 경우는 그 값을 그대로 가져옴
		hid_fsseq.value= Number(gcds_fsseq.Max(4,0,0));
	}else{
...
/******************************************************************************
	...
	parameter   : 
******************************************************************************/
function ln_Fsseq_Temp_Banjaeadd(fdcode,fsdat,fsnbr,fsseq){
	ln_SetDataHeader("FSSEQ");
    gcds_fsseq.addrow();
	gcds_fsseq.namevalue(gcds_fsseq.rowposition,"FDCODE") = fdcode;
	gcds_fsseq.namevalue(gcds_fsseq.rowposition,"FSDAT") = fsdat;
	gcds_fsseq.namevalue(gcds_fsseq.rowposition,"FSNBR") = "000000";
	gcds_fsseq.namevalue(gcds_fsseq.rowposition,"FSSEQ") = fsseq;
}
  ...
  //예산관리항목
    ...
    if(gcds_dtl.namevalue(k,"FSWRKDIV")=="02"){ //예산적용
		...
		if((no1!=""&&no1!=undefined)&&(no2!=""&&no2!=undefined)){
			...
			//alert("gcds_dtl::BGTDIV"+gcds_dtl.namevalue(k,"BGTDIV"));
...
function ln_PrintQuery(p1,p2,p3){
	gcds_prttemp1.DataID = "<%=dirPath%><%=HDConstant.PATH_ACCOUNT%>a020001_s15?v_str1="+p1+"&v_str2="+p2+"&v_str3="+p3;
...
function ln_PrintSubQuery2(){
  ...
  //alert("출력0");
    ...
    if(gcds_prttemp2.namevalue(i,"ATCODE")=="1110110"||gcds_prttemp2.namevalue(i,"ATCODE")=="1110120"||gcds_prttemp2.namevalue(i,"ATCODE")=="1110130"){
		...
		gcds_print.namevalue(1,"DATA02") = "담당부서 : "+dept03;										//부서
  ...
  //alert("출력1");
	for(j=1;j<=gcds_print.countrow;j++){
		...
		if(s_leng1>0){
			//gcds_print.namevalue(j,"FOCHAAMT")= gcds_prttemp2.namevalue(j,"FOCHAAMT").substring(0,1)
...
function ln_PrtSetDataHeader(){
	if(gcds_print.countrow<1){
		var s_temp = "FDCODE:STRING,FSDAT:STRING,FSNBR:STRING,FSSEQ:STRING,ATCODE:STRING,"
							 ...
							 + "CHASUM:STRING,DAESUM:STRING,C1:STRING,C2:STRING,C3:STRING,C4:STRING,C5:STRING,C6:STRING";
  ...
  //팝업체크
    ...
    if(gf_Dept_Enable(gs_deptcd,gcds_name)){ //관리부서
      arrParam[2]="A";
		...
		**/
     //2010.01.21 JYS  관리부서 체크 안함.. 건 다수정 하고서 이것만 늦네..ㅋㅋ
		  	arrParam[2]="A";
			arrParam[3]="";
			arrParam[4]=gclx_fdcode02.bindcolval;
    ...
    strURL = "./bill_popup_ifrm.jsp";
	  ...
	  ///alert("hid_bgtdiv.value::"+hid_bgtdiv.value);
			...
			if(arrParam[0]=="0002"){          //부서
				...
				if(spo!=""&&spo!=undefined){
					if (eval("txt_fsrefcd"+spo).value=="0003"){
							...
							//alert("부서본부:예산부서:"+arrResult[6]);
					} 
				}
			}else if (arrParam[0]=="0003"){  ////예산부서
			  //alert("예산부서::"+arrResult[2]);
  ...
  var p2 = gcem_fs02.text+gcem_fsdat02.text;
	...
	if(strDocGB=="1"){
    	...
    	if(hid_docuval.value==""){
		  ...
		  //2009.09.10 전자세금계산서 적용
			//strURL = "./STaxBill_popup.jsp";
			//strURL = "./STaxBill_popup_e.jsp"; 
   ...
   // alert("step 11 k::"+k);
      ...
      gclx_fswrkdiv02.bindcolval="01";
		...
		******************/
		...
		for(b=3;b<=9;b++){
			if(eval("txt_fsrefcd0"+b).value=="1048"||eval("txt_fsrefcd0"+b).value=="0970"){
				eval("txt_fsrefval0"+b).value=p13;
				eval("gcem_fsrvalnm0"+b).text=p13;
				return;
			}
		}
	}//for
...
function ln_Chk_Save(){
  ...
  //DataSet 건별로 체크하기 
    ...
    /////////////////////////////////////// 
		...
		//계정이 선급부가가치세, 선수부가가치세일 경우 세금계산서가 없으면 저장안됨.
		//대변에 입력 될 경우는 체크 안함.
		if(gcds_dtl.namevalue(i,"ATCODE")=="1112300"||gcds_dtl.namevalue(i,"ATCODE")=="2101100"){
		  if(gcds_dtl.namevalue(i,"CHAAMT")!=0&&gcds_dtl.namevalue(i,"DAEAMT")==0){
				if((gcds_dtl.namevalue(i,"DOCUMCD")=="101"||gcds_dtl.namevalue(i,"DOCUMCD")=="102"||gcds_dtl.namevalue(i,"DOCUMCD")=="103"||gcds_dtl.namevalue(i,"DOCUMCD")=="104")&&gcds_dtl.namevalue(i,"DOCUVAL")!=""){
				}else{
					gcds_dtl.rowposition = i;
...
function ln_Chk_Addrow(){
  ...
  //금액체크
	...
	//계정이 선급부가가치세, 선수부가가치세일 경우 세금계산서가 없으면 저장안됨.
  //대변에 입력 될 경우는 체크 안함.
	if(gcem_atcode02.text=="1112300"||gcem_atcode02.text=="2101100"){
		if(gcem_detot02.text!=0&&gcem_crtot02.text==0){
			if((gclx_documcd02.bindcolval=="101"||gclx_documcd02.bindcolval=="102"||gclx_documcd02.bindcolval=="103"||gclx_documcd02.bindcolval=="104")&&gclx_documcd02.bindcolval!=""){
			}else{
				alert("해당계정은 세금계산서가 존재해야 합니다.");
				return false;
			}
...
function ln_Popup_Repay(){
  ...
  // alert("p1::"+p1+"p2::"+p2+"p3::"+p3+"::strseq::"+strseq);
	//alert("strbanjaeref::"+strbanjaeref+"strbanjaerefval::"+strbanjaerefval+"strbanjaerefnm::"+strbanjaerefnm);
	//alert("stratcode:"+stratcode+":stratkornam:"+stratkornam+":strbanjaeref:"+strbanjaeref+":strfsrefval:"+strfsrefval+":strfsrvalnm:"+strfsrvalnm);
  ...
  //2010.03.15 JYS
	...
	//2014.08.05 이동훈 추가 ////////////////////////////////////////////////////////////////////
	//관리항목 disabled
	//prompt(this, gcds_dtl.text );
	
	if(hid_banjaeref.value == gcds_dtl.namevalue(gcds_dtl.rowposition,"FSREFCD03")){ //계정코드과 반제단위가 같을 경우
...
/******************************************************************************
 ...
 ----------------------------------------------------------------------------------
******************************************************************************/
function ln_Banjaemst_Data_Save(p1,p2,p3,p4,p5,p6,p7,p8,p9,p10,p11,p12,p13,p14,p15,p16,p17,p18,p19,p20,p21,p22){
	...
	gcds_banjaemst.namevalue(gcds_banjaemst.rowposition,"SEQ")=p22;

	//alert("gcds_banjaemst.rowposition end:");
}
...
function ln_Banjae_Data_Save(p1, p2, p3, p4, p5, p6, p7,p8,p9,p10,p11,p12,p13,p14,p15,p16,p17,p18,p19){
	...
	//alert("ln_Banjae_Data_Save::"+"p1:"+p1+"p2:"+p2+"p3:"+p3+"p4:"+p4+"p5:"+p5+"p6:"+p6+"p7:"+p7+"p8:"+p8+"p9:"+p9+"p10:"+p10+"p11:"+p11+"p12:"+p12);
  if(gcds_banjae.countrow<=0){
	...
	//alert("ln_Banjae_Data_Save _end");
}
...
function ln_Refcode_val(p1,p2,p3){
  for( j=1;j<=9;j++){
		if(p1==gcds_dtl.namevalue(gcds_dtl.rowposition,"FSREFCD0"+j)){
			gcds_dtl.namevalue(gcds_dtl.rowposition,"FSREFVAL0"+j)=p2;
			gcds_dtl.namevalue(gcds_dtl.rowposition,"FSRVALNM0"+j)=p3;
			return;
		}
	}
}
...
<!-- 전표입력=========================================================================================================== -->
			...
			<table>
								...
								<td class="tab18" style="width:160px;" ><nobr>&nbsp;<comment id="__NSID__">
									<object  id=gclx_fdcode02 classid=clsid:60109D65-70C0-425C-B3A4-4CB001513C69 
										style="position:relative;left:0px;top:2px;font-size:12px;width:145px;">
										  <param name=CBData			   value="01^본사,02^서울,03^금강산,04^개성">
				...
				<tr>
             	...
             	<tr  width=876px  >			
							  <td color="#0000FF" width="476px"></td>
								 ...
								 <table width=876px cellpadding=0 cellspacing=0 border=0 style="position:relative;top:2px">
								  ...
								  <table width=876px cellpadding=0 cellspacing=0 border=0 style="position:relative;top:2px;left:-1px">
										...
										<td class="tab18" style="width:170px;"><nobr>&nbsp;<comment id="__NSID__">
										 <object  id=gcem_atkornam02 classid=CLSID:E6876E99-7C28-43AD-9088-315DC302C05F  style="width:165px;height:20px;position:relative;top:3px;font-size:12px" tabindex=2>				
											<param name=Text					value="">
											<param name=Alignment     value=0>
											<param name=Border        value=false>
											<param name=Format        value="">
...
<!-- <comment id="__NSID__"><object  id=gcbn_data01 classid=CLSID:9C9AB433-EA85-11D2-A4F9-00608CEBEE49> 
	...
	<C>Col=SSDAT1       Ctrl=gcem_fs02            Param=Text</C>
	<C>Col=SSDAT2       Ctrl=gcem_fsdat02         Param=Text</C>
	<C>Col=SSNBR	      Ctrl=gcem_fsnbr02		      Param=Text</C>
  ...
  <C>Col=DEPTNM	      Ctrl=txt_deptnm02	        Param=value</C>
	...
	<C>Col=FSDAT1       Ctrl=gcem_ss02            Param=Text</C>
	<C>Col=FSDAT2       Ctrl=gcem_ssdat02         Param=Text</C>
	<C>Col=FSNBR	      Ctrl=gcem_ssnbr02		      Param=Text</C>
	'>
</object></comment><script>__ws__(__NSID__);</script>  -->
