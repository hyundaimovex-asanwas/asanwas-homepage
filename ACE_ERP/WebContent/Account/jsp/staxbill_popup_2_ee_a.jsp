...
function ln_Query2(e){ 
	...
	if (e=="1") {
		...
		*/
		...
		gcds_data1_2.DataID = "<%=dirPath%><%=HDConstant.PATH_ACCOUNT%>a040001_s1_ee_A2?v_str2=B"+Number(str_e-1);
		gcds_data1_2.Reset();
	}
}
...
function ln_Save(e){
  ...
  //ln_Query4('A');
	...
	if(!gcds_data1.IsUpdated && !gcds_data2.IsUpdated) {
	  ...
	  if(hid_login.value=="M"){ //부가세 관리자
		//if (gcem_fsdat.text == "" || gusrid=="2050053" || gusrid=="6070001" ||gusrid=="2020009" ){ 
			if (!ln_Check('A'))	return false;
				...
				var str16 = gcem_canbr.text;	//신용카드번호  
				...
				var str28 = txt_costcd.value;	//2014.04.11 JYS 전자세금계산서 원가추가

				if(fn_trim(gcem_fsdat.text)=="") {	//매입MST
					var v_taxsum = 0;
					var v_taxvat = 0;
					var v_taxtot = 0;
					...
					gctr_data1.Parameters = "v_str1="+gs_date2+",v_str2="+gs_date+",v_str3="+gusrid
											...
											+ ",v_str25="+str25+",v_str26="+str26+",v_str27="+str27+",v_str28="+str28;
					gctr_data1.post();
					//alert("lnsave::"+gcds_data2.text+"::"+gcds_insert.text);
			}
	  ...
	  if(hid_login.value=="M"){ //부가세 관리자
		//if (gcem_fsdat.text==""|| gusrid=="2050053" || gusrid=="6070001"||gusrid=="2020009"){	//매입MST
			if (!ln_Check('A'))	return false;
				ln_TaxSum1();
...
//	alert("test");
   ...
   //&&2011.01.01 이후 데이터 입력불가
  /**
	if(gcem_taxdat2.text>="20110101"){
		alert("2011년 1월10일까지 2011년도 매출세금계산서 작성은 [회계팀] 확인 후 발행가능합니다. ");
		return false;
	}
	**/
    ...
    //2010.08.24 JYS 외부전자세금계산서 시스템으로 매출발행하여 전송한 경우 
		...
		if(hid_login.value=="M"){ //부가세 관리자
		//if (gcem_fsdat2.text == ""||gusrid=="2050053" || gusrid=="6070001"||gusrid=="2020009") {	//매출MST
	...
	if (gcds_data1_2.countrow<1) {
		...
		if(sknd=="1"||sknd=="2") {
			...
			if(gclx_cocode2.bindcolval=="02"){  //서울
				ln_RptFormat1();
			}else if(gclx_cocode2.bindcolval=="01"){ //고성
				ln_RptFormat3();
			}
...
ls_for += " 	<C>id='ADDNM2', left=1199, top=468, right=1820, bottom=508, align='left', face='굴림', size=10, bold=false, underline=false, italic=false, forecolor=#000000, backcolor=#FFFFFF, Multiline=true</C>                       ";
ls_for += " 	<C>id='ADDNM2', left=1199, top=1831, right=1820, bottom=1873, align='left', face='굴림', size=10, bold=false, underline=false, italic=false, forecolor=#000000, backcolor=#FFFFFF, Multiline=true</C>                     ";
...
ls_for += " 	<C>id='{decode(TAXVATAMT_4,0,,TAXVATAMT_4)}', left=1416, top=2405, right=1693, bottom=2466, align='right', face='굴림', size=10, bold=false, underline=false, italic=false, forecolor=#000000, backcolor=#FFFFFF</C>      ";
...
ls_for += " 	<C>id='{decode(TAXVATAMT_2,0,,TAXVATAMT_2)}', left=1413, top=2267, right=1691, bottom=2326, align='right', face='굴림', size=10, bold=false, underline=false, italic=false, forecolor=#000000, backcolor=#FFFFFF</C>      ";
...
ls_for += " 	<T>id='기타사업남북경제' ,left=669 ,top=1884 ,right=926 ,bottom=1924 ,align='left' ,face='굴림' ,size=9 ,bold=false ,underline=false ,italic=false ,forecolor=#000000 ,backcolor=#FFFFFF</T>                           "
ls_for += " 	<T>id='협력사업' ,left=669 ,top=1926 ,right=926 ,bottom=1966 ,align='left' ,face='굴림' ,size=9 ,bold=false ,underline=false ,italic=false ,forecolor=#000000 ,backcolor=#FFFFFF</T>                                   "
ls_for += "  <C>id='ADDNM2', left=1198, top=467, right=1820, bottom=509, align='left', face='굴림', size=10, bold=false, underline=false, italic=false, forecolor=#000000, backcolor=#FFFFFF, Multiline=true</C>           ";
ls_for += "   <C>id='ADDNM2', left=1198, top=1830, right=1820, bottom=1872, align='left', face='굴림', size=10, bold=false, underline=false, italic=false, forecolor=#000000, backcolor=#FFFFFF, Multiline=true</C>           ";
ls_for += " 	<T>id='"+g_ceoname+"' ,left=672 ,top=328 ,right=870 ,bottom=410 ,align='left' ,face='굴림' ,size=10 ,bold=false ,underline=false ,italic=false ,forecolor=#000000 ,backcolor=#FFFFFF</T>                                                      ";	
ls_for += "   <T>id='"+g_ceoname+"' ,left=672 ,top=1696 ,right=870 ,bottom=1778 ,align='left' ,face='굴림' ,size=10 ,bold=false ,underline=false ,italic=false ,forecolor=#000000 ,backcolor=#FFFFFF</T>                                                    ";
...
ls_for += "	<T>id='이 금액을  "+gclx_taxtype2.text+"  함'  ,left=1413 ,top=1106 ,right=1828 ,bottom=1212 ,face='굴림' ,size=10 ,bold=false ,underline=false ,italic=false ,forecolor=#0000FF ,backcolor=#FFFFFF</T>							";
ls_for += "	<T>id='이 금액을  "+gclx_taxtype2.text+"  함' ,left=1413 ,top=2475 ,right=1828 ,bottom=2581 ,face='굴림' ,size=10 ,bold=false ,underline=false ,italic=false ,forecolor=#FF0000 ,backcolor=#FFFFFF</T>							";
...
/******************************************************************************
	Description : 합계(공급가액+부가세액) - 매출 2009.07.16 jys
******************************************************************************/
function ln_TaxSum2(){
...
<!-- <script language="javascript" for="gcds_empno" event="onloadCompleted(row,colid)">
	...
	if(cuserid=="ACCOUNT"|| cuserid==null){
		...
		gcds_empno.NameValue(1,"EMPNMK")="박상미";
		gclx_empno.index=0;
		gclx_empno2.index=0;
	}else{
...
<script language="javascript" for="gcds_data1_2" event="onloadCompleted(row,colid)">
	...
	if (row<1) { 
      ...
      ln_Rdo_Chk('4');
		...
		//본부코드
		var str_tree = gcds_dept.namevalue(gcds_dept.rowposition,"TREECD");
		gcds_deptprt.DataID = "<%=dirPath%><%=HDConstant.PATH_ACCOUNT%>PATH_ACCOUNT%>PATH_ACCOUNT%>PATH_ACCOUNT%>PATH_ACCOUNT%>PATH_ACCOUNT%>PATH_ACCOUNT%>PATH_ACCOUNT%>hcdeptprt_s1?v_str3="+str_tree.substring(0,4)+"0000"; 
		gcds_deptprt.Reset();
...
<!-- 2008.06.12. 정영식 추가 START : 과세일 경우만 부가세 10% 자동 적용 [매입] -->
...
<!-- 매입 ===========================================================================================================-->
<fieldset id=field1 style="position:absolute;top:70px;left:4px;height:515px;width:875px;bacground-color:#708090"></fieldset>
  ...
  <table cellpadding=0 cellspacing=0 border=0>
		<tr>
			<td height="25px" align="right" valign="bottom">
				<img name="btn_taxselect"	src="../../Common/img/btn/com_b_taxselect.gif" style="position:relative;right:-2px;top:-290px;cursor:hand" onclick="ln_TaxQuery()">
				...
				<img name="btn_save_1"		src="../../Common/img/btn/com_b_save.gif"			 style="position:relative;right:-2px;top:-290px;cursor:hand" onclick="ln_Save('1')"> 
				<img name="btn_query_1"		src="../../Common/img/btn/com_b_query.gif"		 style="position:relative;right:-2px;top:-290px;cursor:hand" onclick="ln_Query('1')">
              ...
              <!-- Email -->
                ...
                <param name=Enable 	        value=false>
					...
					<tr> 
						...
						<td class="tab12" width="100px" bgcolor="#eeeeee" style="border:1 solid #708090; "><nobr>비고</nobr></td>
...
<div id="layer2" style="position:relative;top:5px;left:8px;width:867px;visibility:hidden;border:1 solid #708090" >
  <table cellpadding=0 cellspacing=0 border=0>
		<tr>
			<td height="25px" align="right" valign="bottom">
				<img name="btn_taxselect_2"	src="../../Common/img/btn/com_b_taxselect.gif" style="position:relative;right:-2px;top:-320px;cursor:hand" onclick="ln_TaxQuery2()">
				...
				<img name="btn_save_1_2"		src="../../Common/img/btn/com_b_save.gif"		   style="position:relative;right:-2px;top:-320px;cursor:hand" onclick="ln_Save2('1')"> 
				<img name="btn_query_1_2"		src="../../Common/img/btn/com_b_query.gif"		 style="position:relative;right:-2px;top:-320px;cursor:hand" onclick="ln_Query2('1')">
		  ...
		  <td style="width:865;border:0 solid #708090;border-top-width:1px" colspan="2">
				<comment id="__NSID__"><object  id=gcgd_data2_2 style="width:865px;height:120px;border:0 solid #777777" classid=clsid:1F57AEAD-DB12-11D2-A4F9-00608CEBEE49>
					...
					<PARAM NAME="Format"			VALUE="
						<C>ID=TAXMM			Name='월'				HeadAlign=Center HeadBgColor=#B9D4DC Width=20  align=center SumText=''    SumBgColor=#C3D0DB </C>
						<C>ID=TAXDT			Name='일'				HeadAlign=Center HeadBgColor=#B9D4DC Width=20  align=center SumText=''    SumBgColor=#C3D0DB </C>
						<C>ID=TAXPDTNAM Name='품목명'		HeadAlign=Center HeadBgColor=#B9D4DC Width=200 align=LEFT		SumText=''    SumBgColor=#C3D0DB </C>
						<C>ID=TAXSTD		Name='규격'			HeadAlign=Center HeadBgColor=#B9D4DC Width=60	 align=LEFT		SumText=''    SumBgColor=#C3D0DB </C>
						<C>ID=TAXQTY		Name='수량'			HeadAlign=Center HeadBgColor=#B9D4DC Width=60  align=right	SumText=''    SumBgColor=#C3D0DB </C> 
						<C>ID=TAXPRIC   Name='단가'			HeadAlign=Center HeadBgColor=#B9D4DC Width=60  align=right	SumText=''    SumBgColor=#C3D0DB </C> 
						<C>ID=TAXSUM		Name='공급가액'	HeadAlign=Center HeadBgColor=#B9D4DC Width=120 align=right	SumText=@sum  SumBgColor=#C3D0DB</C>
