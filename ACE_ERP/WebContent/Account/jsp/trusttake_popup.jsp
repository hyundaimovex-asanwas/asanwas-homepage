...
<!-------------------------------------------------------------------------------
+ 시 스 템 명	: 받을어음 수탁추심정보 팝업
+ 프로그램 ID	: trusttake_popup.html
+ 기 능 정 의	: 조회|등록|저장|삭제
+ 변 경 이 력	: 이순미
+ 서 블 릿 명	: trusttake_popup_s1, trusttake_popup_t1
------------------------------------------------------------------------------>
...
******************************************************************************/
function ln_Query(){
	gcds_data1.DataID = "<%=dirPath%><%=HDConstant.PATH_ACCOUNT%>trusttake_popup_s1?v_str1="+p_fdcode
...
function ln_Save(){
	...
	if (gcds_data1.IsUpdated) {
		gctr_data1.KeyValue = "trusttake_popup_t1(I:USER=gcds_data1)";
		gctr_data1.Action = "<%=dirPath%><%=HDConstant.PATH_ACCOUNT%>trusttake_popup_t1";
...
******************************************************************************/
function ln_Delete(){
	if(fn_trim(gcem_strdate2.text)!=""&&fn_trim(gcem_bancodnm1.text)!=""
	&&fn_trim(gcem_orddate.text)!=""&&fn_trim(gcem_ordamt.text)!=0||gcds_data1.SysStatus(1)==3){
...
function ln_Valchk() {
	if (fn_trim(gcem_strdate2.text)==""||gcem_strdate2.text.length<=7) {			
		alert("정확한 수탁일자를 입력해 주십시오.");
		gcem_strdate2.focus();
		return false;
	}

	if (fn_trim(gcem_bancodnm1.text)==""||fn_trim(gcem_bancod1.text)=="") {			
		alert("정확한 수탁은행를 입력해 주십시오.");
		gcem_bancodnm1.focus();
		return false;
	}

	if (fn_trim(gcem_orddate.text)==""||gcem_orddate.text.length<=7) {			
		alert("정확한 추심일자를 입력해 주십시오.");
		gcem_orddate.focus();
		return false;
	}

	if (fn_trim(gcem_ordamt.text)==0) {			
		alert("정확한 추심료를 입력해 주십시오.");
		gcem_ordamt.focus();
		return false;
	}
	return true;
...
function ln_SetDataHeader(){
	var s_data = "FDCODE:STRING,ATCODE:STRING,FUNDDIV:STRING,REFCODE2:STRING,STRDATE:STRING,"
						 + "BANCOD:STRING,ORDDATE:STRING,ORDAMT:DECIMAL,SLIPYN:STRING,"
		...
		<td colspan=2> 
			<table width="310" cellpadding="1" cellspacing="0" border="0">
                ...
                <td class="tab11" style="height:25px;width:100px;" bgcolor="#eeeeee"><nobr>만기일자</nobr></td>  
							...
							<tr>
								<td class="tab11" style="height:25px;width:100px;" bgcolor="#eeeeee">수탁일자</td>
								<td class="tab22"><comment id="__NSID__">
							...
							<tr>
								<td class="tab11" style="width:100px;" bgcolor="#eeeeee">수탁은행</td>  
								<td class="tab22" style="height:45px;"><nobr><comment id="__NSID__">
							...
							<tr>
								<td class="tab11" style="height:25px;width:100px;" bgcolor="#eeeeee">추심일자</td>
								<td class="tab22"><comment id="__NSID__">
									<object id=gcem_orddate classid=CLSID:E6876E99-7C28-43AD-9088-315DC302C05F 
										style="position:relative; left:5px;top:2px; width:70px; height:20px;" class="txtbox">
										<param name=Alignment			value=0>
										<param name=Border	      value=true>
										<param name=Format	      value="YYYY/MM/DD">
									...
									<img src="../img/calender.gif" value="calender.gif" id=img_calendar3 OnClick="__GetCallCalendar('gcem_orddate', 'Text')" style="position:relative;width:20px;left:9px;top:1px;cursor:hand;">
								</td>
							</tr>
							<tr>
								<td class="tab12" style="height:25px;width:100px;" bgcolor="#eeeeee">추&nbsp;심&nbsp;료</td>
								<td><comment id="__NSID__">
									<object  id=gcem_ordamt classid=CLSID:E6876E99-7C28-43AD-9088-315DC302C05F	style="position:relative;left:5px;top:0px;width:100px;">
										<param name=Text				value="">
										<param name=Alignment		value=2>
										<param name=Border			value=true>
										<param name=MaxLength		value=15>
...
<comment id="__NSID__"><object  id=gcbn_data classid=CLSID:9C9AB433-EA85-11D2-A4F9-00608CEBEE49> 
	...
	<C>Col=ORDDATE				Ctrl=gcem_orddate			Param=text</C>
	<C>Col=ORDAMT					Ctrl=gcem_ordamt			Param=text</C>
	'>
</object></comment><script>__ws__(__NSID__);</script> 
