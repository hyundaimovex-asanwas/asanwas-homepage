...
<!---------------------------------------------------------------------------
+ 시 스 템 명	:  자동전표 - POS 자동 전표 
+ 프로그램 ID	:  xat1006.jsp
+ 기 능 정 의	:  POS 매출 데이터를 이용하여 전표 자동 발행   
+ 작   성  자      :  정 영 식
+ 작성 일 자      :  2018.04.25
+ 서 블 릿 명	:  
+ 가우스 버젼    :  5.2 
...
function ln_Before(){
	...
	gcem_saledt.text = gs_date;
	//gcem_saledt.text = "20180101";
...
function ln_Query(e){
	...
	if(gcem_dfscost.text==""){
		alert("매장 정보를 입력하십시요.");
		return false;
	}
	
    
    gcds_data01.DataID = "<%=dirPath%><%=HDConstant.PATH_ACCOUNT%>xat1006_s1?v_str1="+gcem_saledt.text
    		                                                              +"&v_str2="+gcem_dfscost.text;
...
function fnCreateSelect() {
	  ...
	  **/
	  ...
	  //서블릿 조회해야지		
	  ds_slipdtl.DataID = "<%=dirPath%><%=HDConstant.PATH_ACCOUNT%>xat1006_s2?v_str1="+gcem_saledt.text
                                                                           +"&v_str2="+gcem_dfscost.text;
...
function fnSlipdtl_Display(row){
	...
	strSaledt=ds_slipdtl.namevalue(1,"SALEDT");
	
	//alert("strSaledt::")+strSaledt;
	
	strSalemm = strSaledt.substr(4,2);
	strSaledd = strSaledt.substr(6,2);
	
	strSaledt = strSalemm+"/"+strSaledd;
	...
	for(var i=1;i<=row;i++){
		...
		if(ds_slipdtl.namevalue(i,"FSREFCD"+strPo)=="0030"){
			ds_slipdtl.namevalue(i,"FSREFVAL"+strPo) = ds_slipdtl.namevalue(i,"COSTCD") ;
			ds_slipdtl.namevalue(i,"FSRVALNM"+strPo) = ds_slipdtl.namevalue(i,"COSTCDNM") ;
		}
		
		
		//적요
		ds_slipdtl.namevalue(i,"REMARK") = "[자동] "+ds_slipdtl.namevalue(i,"COSTCDNM")+" "+ds_slipdtl.namevalue(i,"REMARK")+""+strSaledt;
...
function fnSave() {
    ...
    gcds_posdata.clearall();
	fnPosDataSetDataHeader();
	fnPosdataSettting();
    ...
    if(fnSave_Chk()){
 		if (gcds_slipmst.IsUpdated) {
			if (confirm("저장 하시겠습니까?")){	
				tr2.Action = "<%=dirPath%><%=HDConstant.PATH_ACCOUNT%>xat1006_t1"
				tr2.Parameters = "v_str1="+gusrid+",v_str2="+gfdcode;
			    //<param name="KeyValue"    value="Si050I_t1(I:USER1=gcds_slipmst,I:USER2=gcds_slipdtl,I:USER3=gcds_banjaemst,I:USER4=gcds_banjae,I:USER5=gcds_posdata,O:USER9=gcds_slipno )">    
			    //tr2.keyvalue="Si050I_t1(I:USER3=gcds_slipmst,I:USER4=gcds_slipdtl,I:USER6=gcds_banjae,I:USER14=gcds_banjaemst,I:USER7=gcds_posdata,O:USER5=gcds_slipno )"; 
				//prompt('gcds_slipmst',gcds_slipmst.text);
				//prompt('gcds_slipdtl',gcds_slipdtl.text);
				//prompt('gcds_banjaemst',gcds_banjaemst.text);
				//prompt('gcds_banjae',gcds_banjae.text);
				//prompt('gcds_posdata',gcds_posdata.text);
...
function fnSlipdtlSetting() {
	...
	for (var k=1;k<=ds_slipdtl.countrow;k++){
		...
		gcds_slipdtl.addrow();
   		...
   		//**본부는 찾아야지여
   		gcds_slipdtl.namevalue(gcds_slipdtl.rowposition, "DIVCD" )       ="K000";
...
function fnSlipmstSetting() {
	...
	gcds_slipmst.namevalue(srow,"FSKND") = "S";  //자동 - POS    
...
<%//생성시 기본 데이터 조회  %>
function fnPosdataSettting() {
	for(var t=1;t<=gcds_data01.CountRow;t++){
	    //if(gcds_data01.namevalue(t,"CHKBOX")=="T"){
    		gcds_posdata.addrow();
    		gcds_posdata.namevalue(gcds_posdata.rowposition,"DFSCD")= gcds_data01.namevalue(t,"DFSCD");
    		gcds_posdata.namevalue(gcds_posdata.rowposition,"SALEDT")= gcds_data01.namevalue(t,"SALEDT");
    		gcds_posdata.namevalue(gcds_posdata.rowposition,"POSNO")= gcds_data01.namevalue(t,"POSNO");
    		gcds_posdata.namevalue(gcds_posdata.rowposition,"RECNO")= gcds_data01.namevalue(t,"RECNO");
    		gcds_posdata.namevalue(gcds_posdata.rowposition,"GOODCD")= gcds_data01.namevalue(t,"GOODCD");
    	//}
	}
}
...
function fnSlipdtlSetDataHeader() {
 ...
 <%//POSDATA head생성%>
function fnPosDataSetDataHeader() {
	 var s_temp = "DFSCD:STRING,SALEDT:STRING,POSNO:STRING,RECNO:STRING,GOODCD:STRING,FDCODE:STRING,FSDAT:STRING,FSNBR:STRING";
	 gcds_posdata.SetDataHeader(s_temp);
}
...
function ln_Print(){
	...
	if(gcds_data01.countrow<1) {
		...
		if(gcds_rptdtl.countrow>0){
			...
			gcds_rptmst.namevalue(gcds_rptmst.rowposition,"APPDT")= gcem_saledt.text;
			gcds_rptmst.namevalue(gcds_rptmst.rowposition,"EMPNMK")=gusrnm+"("+gusrid+")";
			gcds_rptmst.namevalue(gcds_rptmst.rowposition,"SLIPNO")=gcds_rptdtl.namevalue(1,"FSDAT")+gcds_rptdtl.namevalue(1,"FSNBR");
		}
		gcrp_print.preview();
	}
}
...
function ln_Chkbox_All(){
	if(gcgd_disp01.ColumnProp('CHKBOX','HeadCheck')=="FALSE"){
	   ...
	   //alert("true");
	   for(var i=1; i<=gcds_data01.countrow;i++){
	 		if(gcds_data01.namevalue(i,"FSDAT")!=""){
...
function ln_Popup4(){
	...
	if(arrResult != null) {
		arrParam = arrResult.split(";");       
		txt_deptcd.value = arrParam[0];
		txt_deptnm.value = arrParam[1]; 
	} else {
		txt_deptcd.value = "";
		txt_deptnm.value = "";
	}
...
<object  id=gcds_posdata classid=clsid:2506B38B-0FF7-4249-BA3E-8BC1DC399FBB VIEWASTEXT>
	<PARAM NAME="SyncLoad" VALUE="false">
</object> 
...
<object  id="tr2"     classid=clsid:78E24950-4295-43D8-9B1A-1F41CD7130E5>
    ...
    <param name="KeyValue"     value="xat1006_t1(I:USER3=gcds_slipmst,I:USER4=gcds_slipdtl,I:USER6=gcds_banjae,I:USER14=gcds_banjaemst,I:USER7=gcds_posdata,O:USER5=gcds_slipno)"> 
...
<script language="javascript"  for=gcgd_disp01  event=OnClick(row,colid)>
    ...
    if(row==0&&colid=="CHKBOX"){
		...
		if(colid =="CHKBOX"){
			//접수번호 있는지 확인
			if(gcds_data01.namevalue(row,"FSDAT")!=""){
				alert("전표접수일자가가  이미 존재합니다.");
		...
		<td align=left style="position:relative;left:6px;top:-2px">
			<table cellpadding="1" cellspacing="0" border="0" style='width:1000px;height:20px;border:0 solid #708090'>
				<tr>
					...
					<td class="tab25" style="width:100px;" bgcolor="#eeeeee" align=center><nobr>매출일자</nobr></td>
					<td class="tab24">
						<comment id="__NSID__">
							<object  id=gcem_saledt classid=CLSID:4AEAFD66-8D65-41AC-B1D1-57E7FF2A734F  style="position:relative;top:3px;left:5px;width:70px;height:20px;">
						...
						<img src="../img/calender.gif" value="calender.gif" id=img_dsudt OnClick="__GetCallCalendar('gcem_saledt', 'Text')" style="position:relative;width:20px;left:6px;top:2px;cursor:hand;">&nbsp;
					</td>
					<td class="tab24" style="width:100px;" bgcolor="#eeeeee" align=center><nobr>매장정보</nobr></td>
					<td class="tab24">
						<comment id="__NSID__">
						   <object  id=gcem_dfscost  classid=CLSID:E6876E99-7C28-43AD-9088-315DC302C05F	style="position:relative;top:2px;left:5px;width:150px">
						...
						<img name="btn_find2" src="../../Common/img/btn/com_b_find.gif"	style="position:relative;top:2px;left:-2px;cursor:hand"	 onclick="ln_Popup_DfsCost()">
					</td>
	...
	<tr>
		<td ALIGN=CENTER>
			<table width="1000" cellpadding="1" cellspacing="0" border="0">
			    <tr>
			        <td><font size="2" color="blue" ><strong>&nbsp;[POS매출내역]</strong></font></td>
				</tr>
				<tr>
					<td style="width:460;position:relative;top:3px" align=left valign=top >
						...
						<OBJECT id=gcgd_disp01 classid=clsid:EA8B6EE6-3DD8-4534-B4BB-27148CF0042B style="WIDTH:998px;HEIGHT:245px;border:1 solid #777777;display:block;position:relative;left:5px" viewastext>
							...
							<PARAM NAME="Format"        VALUE="  
								<FC> Name=''           ID=CHKBOX	    HeadAlign=Center HeadBgColor=#B9D4DC Width=30 	    align=center  	SumBgColor='#C3D0DB'  EditStyle=CheckBox   HeadCheckShow=true show=false </FC>
								...
								<FC> Name='매출일자'	    ID=SALEDT       HeadAlign=Center HeadBgColor=#B9D4DC Width=70	   	align=center	SumBgColor='#C3D0DB'  edit=none MASK ='XXXX/XX/XX' SumText='건수' </FC> 
								<FC> Name='포스'     	ID=POSNO        HeadAlign=Center HeadBgColor=#B9D4DC Width=30   	align=center	SumBgColor='#C3D0DB'  edit=none show =false</FC>
								...
								<C> Name='바코드'	        ID=BARCD        HeadAlign=Center HeadBgColor=#B9D4DC Width=90    	align=left  	SumBgColor='#C3D0DB'  edit=none  show=false</C>
								...
								<C> Name='할인구분'      ID=DISGB  	    HeadAlign=Center HeadBgColor=#B9D4DC Width=80   	align=left  	SumBgColor='#C3D0DB'  edit=none show =false</C>
								...
								<C> Name='원화합'        ID=WONAMT	    HeadAlign=Center HeadBgColor=#B9D4DC Width=80    	align=right 	SumBgColor='#C3D0DB'  edit=none SumText=@SUM</C>
								...
								<G> Name='발행전표정보'  HeadAlign=Center HeadBgColor=#B9D4DC
								<C> Name='접수일자'    ID=FSDAT 	   HeadAlign=Center HeadBgColor=#B9D4DC Width=80    	align=center	SumBgColor='#C3D0DB'  edit=none</C>
								<C> Name='접수번호'    ID=FSNBR	   HeadAlign=Center HeadBgColor=#B9D4DC Width=60    	align=center	SumBgColor='#C3D0DB'  edit=none</C>
							    </G>
							">
						</OBJECT></comment><script>__ws__(__NSID__);</script>
	    			...
	    			<tr>
	                   ...
	                   <td bgcolor='#ffffff'>&nbsp;&nbsp;<comment id="__NSID__">
                        ...
                        <td bgcolor="#FFFFFF"  colspan=5 >&nbsp;
                             <input id="txt_deptcd"   type=text    style= "position:relative;top:0px;left:4px;width:50px; height:20px;"  class="txtbox"  maxlength=7;  readOnly>
							 <img SRC="<%=dirPath%>/Sales/images/help.gif" BORDER="0" ALT="거래처를 검색합니다" style="cursor:hand;position:relative;left:3px;top:0px" align=center onclick="ln_Popup4();">
                             ...
                             &nbsp;&nbsp;
                             <img src="<%=dirPath%>/Common/img/btn/com_b_taxselect.gif"  style="cursor:hand;position:relative;top:0px;right:2px" onclick="ln_Slipno_Popup()">
                         </td>
	    			</tr>
	       		</table>
	       	</td>
			...
			<table width="1000" cellpadding="1" cellspacing="0" border="0">
				<tr>
					<td style="position:relative;top:3px" align=left valign=top colspan=9>
					    <comment id="__NSID__">
						<OBJECT id=gcgd_disp02 classid=clsid:EA8B6EE6-3DD8-4534-B4BB-27148CF0042B style="WIDTH:998px;HEIGHT:195px;border:1 solid #777777;display:block;position:relative;left:5px" viewastext>
							...
							<param name="Format"		  	 value=" 
								...
								<FC>Name='계정명'	   ID=ATKORNAM	    HeadBgColor=#B9D4DC   Width=110	    align=left	     edit=none   SumText='합   계'</FC> 
								...
								<C> Name='적요'	   ID=REMARK        HeadBgColor=#B9D4DC   Width=205	    align=left	     SumText={sum(DEAMT)-sum(CRAMT)} SumTextAlign=right </C> 
