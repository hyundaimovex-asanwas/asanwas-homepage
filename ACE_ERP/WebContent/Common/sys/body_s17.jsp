...
<%
	...
	//if(!ssID.substring(0,40).equals(session.getId().substring(0,40)) || !gusrip.equals(request.getHeader("Proxy-Client-IP")) || !gusrpos.equals("Tadaima") ){//서버	
	//경고와 함께 메인 화면으로 
		%>
		<script language="javascript">
			alert("시간이 오래되어 자동 로그아웃되었거나\n정상 접근이 아닙니다.\n\n새로 로그인 해주세요.");
			opener.location.href="<%=dirPath%>/Common/sys/logout.jsp";
		...
		function fnSetTitle(tree_idx){
			if(tree_idx==0){	//초기 인덱스화면인 경우
		        ...
		        ln_TRSetting(tr_title, 
		            "<%=dirPath%><%=HDConstant.PATH_ERP_COMMON%>Cm011I_17",
...
%>	
	<iframe id="loadingBar" 
			...
			src="<%=dirPath%>/Purch/common/include/loadingSales.jsp">
	</iframe>	

	<iframe id="LowerFrame" name="work" style="visibility:hidden; position:absolute; left:expression( (document.body.clientWidth - 400 ) / 2); top:expression(( document.body.clientHeight - 146 ) / 2); Z-INDEX: 9;" src="<%=dirPath%>/Purch/common/html/progress.htm" width="400px" height="146px"></iframe>	

<!-- HEADER START -->
<div id="haHeader">
	<div id="haHeaderTop">
	<table width="1030" height="100%">
	  <TR>
		...
		<td width="40px" class="r b" style="padding-bottom:1px;cursor:pointer;" onclick="mclose()">닫기</td>
	  </TR>
	</table>
	</div>
	<div id="haHeaderMenu">
		<table width="1030" height=100%>
		  <TR>
			<td class="c b bold" width="158px" style="padding-bottom:5px; color:#ffffff;"><%=gusrnm%>(<%=gusrid%>)님</td>
			<td class="l b bold" style="padding-bottom:5px;color:#ffffff;font-size:14px;padding-left:12px;" id="menu_title">&nbsp;</td>
			<td class="r b" style="color:#ffffff; padding-bottom:5px;padding-right:1px;" id="menu_full">&nbsp;</td>
		  </TR>
		</table>
