 ...
 * [2010-10-12][심동현] 세션정보 부르기 및 환경설정 메뉴로 수정
 *****************************************************************************/
%>
<%
///////////////////////////////////////////////////////////////////////////////
...
<%@ page import="sales.menu.my.My720Manager" %>
...
<%	// ContextPath 변수 설정.
	...
	My720Manager manager3 = new My720Manager();
...
<script language="javascript"> 	
   function view_send(brd_no,ref,ref_step){
		var readURL ="my710_read_pop.jsp?pageFrom=720&idx=400&brd_no="+brd_no+"&ref="+ref+"&ref_step="+ref_step;
   ...
   function write_ok(){
    var f = document.searchForm;
		f.action='my720_write.jsp?idx=404&work_flag=INSERT';
		f.target='';
		f.method = "POST";
		f.submit();
   }
   ...
   function sel_chk(){
    var f = document.searchForm;
      ...
      f.action='my720_list.jsp?idx=399';
      f.taget='';
      f.method = "POST";
      f.submit();
   }
   ...
   function goPage(requestedPage,position){
	...
	function fn_delete(){
		...
		document.listForm.action="my710_exec.jsp?work_flag=MULTI_DELETE2&brd_no_s="+brd_no_s;
...
<!-- News LIST-->
  ...
  <tr height="30" bgcolor="#F5F5E9">
	<td colspan=8 align=center>보낸 쪽지가 없습니다.</td>
  </tr>
  <%} else {
     	len3 = data3.size();
	   bean3 = null;
     ...
     <tr height="30" > 
		...
		<td><img src="http://www.hdasan.com/img/common/bbs_bar_top.gif"></td>
		<td><%=bean3.get("brd_recipient")%>(<%=bean3.get("brd_recipient_name")%>)</td>
		<td><img src="http://www.hdasan.com/img/common/bbs_bar_top.gif"></td>
