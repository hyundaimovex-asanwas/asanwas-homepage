<!--*************************************************************************
* @source      : acqa002.jsp												*
* @description : Action Plan > 등록관리 > 얼마나 잘 쓰는지 두고보자!!!! 	*
*****************************************************************************
...
<%
    String REQ_NO = request.getParameter("REQ_NO");         //사번
...
<meta http-equiv="Content-Type" content="text/html; charset=euc-kr">
    ...
    <script language="javascript" >
		...
		function fnc_SubmitApproval(kind) {
			...
			if (kind == "부결") {
				...
				window.showModalDialog("../../../Ehr/hr/acq/acqa092.jsp", obj, "dialogWidth:500px; dialogHeight:200px; help:No; resizable:No; status:No; scroll:No; center:Yes;");
            ...
            for(var i= 1; i <= dsT_ACT_APPROVAL.CountRow; i++){
                ...
                Aeno_no = dsT_ACT_APPROVAL.NameValue(i,"ENO_NO");   	  //결재자
                
				APP_STATUS = dsT_ACT_APPROVAL.NameValue(i,"APP_STATUS");   //결재여부  &&  (app_yn == null)

				if(Aeno_no == gusrid && ( APP_STATUS == "미결재")){
					
					//document.getElementById("btn_approval").style.display = "";
					
					dsT_ACT_APPROVAL.NameValue(i,"REMARK1") = "의견";
			...
			//dsT_ACT_APPROVAL.UserStatus(1) = 1;

			//dsT_ACT_APPROVAL.NameValue(dsT_ACT_APPROVAL.RowPosition,"REMARK1") = obj.app_cmt;   	//결재의견 Container

			trT_DI_APPROVAL.KeyValue = "tr01(I:dsT_ACT_APPROVAL=dsT_ACT_APPROVAL)";
		...
		function fnc_FileDelete() {
            ...
            //결재 상태 확인 : 결재 완료 or 결재 진행중이면 파일 삭제 불가,      "저장", "반려", "신규" 상태이면 파일 삭제 가능
    ...
    **************************************-->
    ...
    <script language=JavaScript for=dsT_ACT_PLANLIST event=OnRowPosChanged(row)>
        ...
        //결재 상태 확인 : 결재 완료 or 결재 진행중이면 수정 불가
  ...
  <script language="javascript"  for=grdT_DI_APPROVAL event=OnClick(Row,Colid)>
	...
	if(Colid == "REMARK1" && dsT_ACT_APPROVAL.NameValue(Row, "REMARK1") == "의견" ){
   ...
   <script language="javascript"  for=grdFILE event=OnDblClick(row,colid)>
       ...
       if(row > 0) {
           ...
           trT_ACT_PLANLIST.action = "../../../servlet/GauceChannelSVL?cmd=Ehr.acq.a.acqa002.cmd.ACQA002CMD&S_MODE=SHR&FILE_NM="+vFILE_NAME;
           trT_ACT_PLANLIST.post();   
					...
					<td align="left">
						...
						<object	id="grdT_DI_APPROVAL" classid="clsid:EA8B6EE6-3DD8-4534-B4BB-27148CF0042B" style="width:660px;height:127px;">
							...
							<param name="Format"				value="
								...
								<C> id='ENO_NM'			width=120	name='확인자'		align=center	</C>
								<C> id='JOB_NM'			width=120	name='직위'			align=center	</C>
                                <C> id='DPT_NM'			width=150	name='소속'			align=center	</C>
								<C> id='APP_STATUS'		width=200	name='확인상태'		align=center    </C>
							">
						</object>
		...
		<td align="left">
			...
			<object	id="grdT_ACT_PLANLIST" classid="clsid:EA8B6EE6-3DD8-4534-B4BB-27148CF0042B" style="width:1000px;height:0px;">
				...
				<param name="Format"				value="
					...
					<C> id='DOC_NM1'			width=100	name='대분류'			align=center	</C>
					<C> id='DOC_NM2'			width=100	name='중분류'			align=center	</C>
                    <C> id='DOC_NM3'			width=130	name='세부분류'			align=left		</C>
					<C> id='DOC_TITLE'			width=150	name='제목'				align=left	    </C>
					<C> id='DOC_REMARK'			width=150	name='설명'				align=left 		</C>
					<C> id='APP_STATUS'			width=90	name='결재상태'			align=center	value={decode(APP_STATUS_CD,'S','저장','G','결재진행중','R','반려','Y','결재완료','')}</C>
