...
<meta http-equiv="Content-Type" content="text/html; charset=euc-kr">
    ...
    *****************************-->
    <script language="javascript" >
        ...
        function fnc_SearchList() {
            ...
            var gbn1 = document.getElementById("cmbGBN1_SHR").value;
            var gbn2 = document.getElementById("cmbGBN2_SHR").value;
            ...
            //관리자는 모든 조회가 가능
			dsT_DI_BUSINESSTRIP_STATE.dataid = "../../../servlet/GauceChannelSVL?cmd=Ehr.but.a.buta010.cmd.BUTA010CMD&S_MODE=SHR&STR_YMD="+str_ymd+"&END_YMD="+end_ymd+"&GBN1="+gbn1+"&GBN2="+gbn2
												+"&ENO_NO="+eno_no+"&OUR_MAN="+our_man;
			dsT_DI_BUSINESSTRIP_STATE.reset();
			dsT_DI_BUSINESSTRIP_STATE.RowPosition = curRowPosition;
        }
        ...
        function fnc_Delete() {
            ...
            if(!(dsT_DI_BUSINESSTRIP_STATE.NameValue(dsT_DI_BUSINESSTRIP_STATE.RowPosition,'APP_STATUS') == 'SV')){
                alert("결재 이후의 데이터는 삭제할 수 없습니다!");
                return;
            }

			if(confirm("[품의번호:"+dsT_DI_BUSINESSTRIP_STATE.NameValue(dsT_DI_BUSINESSTRIP_STATE.RowPosition,'REQ_NO')+"]의 정보를 삭제하시겠습니까?")){
				dsT_DI_BUSINESSTRIP_STATE.DeleteRow(dsT_DI_BUSINESSTRIP_STATE.RowPosition);
				trT_DI_BUSINESSTRIP_STATE.KeyValue = "tr01(I:dsT_DI_BUSINESSTRIP_STATE=dsT_DI_BUSINESSTRIP_STATE)";
				trT_DI_BUSINESSTRIP_STATE.action = "../../../servlet/GauceChannelSVL?cmd=Ehr.but.a.buta010.cmd.BUTA010CMD&S_MODE=DEL";
        ...
        function fnc_ToExcel() {
            ...
            form1.grdT_DI_BUSINESSTRIP_STATE.GridToExcel("출장", '', 225);            
        ...
        function fnc_OnLoadProcess() {
			...
			//미결재 자동조회
			document.getElementById("cmbGBN1_SHR").value = "1";
		...
		function fnc_DeciPop() {
            ...
            var url = "/hr/but/buta020.jsp?ORD_NO=" + dsT_DI_BUSINESSTRIP_STATE.NameValue(dsT_DI_BUSINESSTRIP_STATE.RowPosition,"ORD_NO") + "&ENO_NO=" + dsT_DI_BUSINESSTRIP_STATE.NameValue(dsT_DI_BUSINESSTRIP_STATE.RowPosition,"ENO_NO") + "&APP_STATUS=" + dsT_DI_BUSINESSTRIP_STATE.NameValue(dsT_DI_BUSINESSTRIP_STATE.RowPosition,"APP_STATUS");

			window.showModalDialog(url, "Modal", "dialogWidth:870px; dialogHeight:600px; help:No; resizable:No; status:No; scroll:Yes; center:Yes;");
    ...
    ***************************************************************************************-->
    ...
    | 3. 사용되는 Table List(T_DI_BUSINESSTRIP_STATE) 			|
    +----------------------------------------------------------->
	<Object ID="dsT_DI_BUSINESSTRIP_STATE_CP" ClassID="CLSID:2506B38B-0FF7-4249-BA3E-8BC1DC399FBB">
		<Param Name="Syncload" Value="True">
		<Param Name="UseChangeInfo" Value="True">
		<Param Name="ViewDeletedRow" Value="False">
	</Object>
    ...
    ***************************************************************************************-->
	...
	<script language=JavaScript for=grdT_DI_BUSINESSTRIP_STATE event="OnDblClick(row,colid)">
		...
		curRowPosition = dsT_DI_BUSINESSTRIP_STATE.RowPosition;
		...
		if(row < 1) {
			...
			if(dsT_DI_BUSINESSTRIP_STATE.NameValue(row,"APP_STATUS") == "SV" && dsT_DI_BUSINESSTRIP_STATE.NameValue(row,"PENO_NO") != gusrid && gusrid != '6060002') {
				alert("저장상태입니다. 상신 후 결재 할 수있습니다.");
				return;
			}

			var url = "../../../Ehr/hr/but/buta025.jsp?ORD_NO=" + dsT_DI_BUSINESSTRIP_STATE.NameValue(row,"REQ_NO") + "&ENO_NO=" + dsT_DI_BUSINESSTRIP_STATE.NameValue(row,"PENO_NO") + "&APP_STATUS=" + dsT_DI_BUSINESSTRIP_STATE.NameValue(row,"APP_STATUS") + "&APP_CMT=" + dsT_DI_BUSINESSTRIP_STATE.NameValue(row,"APP_CMT");
	...
	<script language=JavaScript for=dsT_DI_BUSINESSTRIP_STATE event=OnRowPosChanged(row)>
		...
		<tr>
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				...
				<tr>
					...
					<td>
						<select id="cmbGBN2_SHR" style="width:100" onChange="fnc_SearchList();">
					...
					<td style="DISPLAY:none" class="searchState" align="right">결재구분&nbsp;</td>
					<td>
						<div  id="doNotUse1" style="DISPLAY:none">
						<select id="cmbGBN1_SHR" style="width:100" onChange="fnc_SearchList();">
							<option value="1" >전체</option>
							<option value="2" >승인</option>
                            <option value="3" >승인외</option>
						</select>
						</div>
					</td>
				...
				<tr align="center">
					...
					<object id="grdT_DI_BUSINESSTRIP_STATE" classid="clsid:EA8B6EE6-3DD8-4534-B4BB-27148CF0042B" style="width:1000px;height:500px;">
						...
						<param name="Format" value='
                            <C> id="REQ_NO"			width=120	name="품의번호"		align=center    </C>
                            <C> id="PENO_NM"		width=120	name="상신자"			align=center    </C>
							<C> id="APP_YMD"		width=120	name="상신일"			align=center    </C>
							<C> id="APP_TITLE"		width=500	name="출장내역"	    align=left	    </C>
                            <C> id="APP_CMT"		width=100	name="결재상태"		align=center    </C>
						'>
					</object>
