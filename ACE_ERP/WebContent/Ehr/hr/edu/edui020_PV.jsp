...
<comment id="__NSID__">
              ...
              <param name="hnwsrc"    VALUE="edui020.hnw">
              <param name="initvalue" VALUE="[:STR_YMD_SHR]=<%=STR_YMD_SHR%>[:END_YMD_SHR]=<%=END_YMD_SHR%>[:OCC_CD_SHR]=<%=OCC_CD_SHR%>[:DPT_CD_SHR]=<%=DPT_CD_SHR%>[:DPT_NM_SHR]=<%=DPT_NM_SHR%>[:EDU_CD_SHR]=<%=EDU_CD_SHR%>[:EDU_NM_SHR]=<%=EDU_NM_SHR%>[:JOB_CD_SHR]=<%=JOB_CD_SHR%>[:JOB_NM_SHR]=<%=JOB_NM_SHR%>[:GUBUN_VAL]=<%=GUBUN_VAL%>[:OCC_VAL]=<%=OCC_VAL%>[:EDU_VAL]=<%=EDU_VAL%>[:GBN_CD]=<%=GBN_CD%>">
