...
***********************************************************************
* @source      : gunc071.jsp
* @description : 휴일근무 수당 정산 결재  PAGE
***********************************************************************
...
<html>
<head>
<title>휴일근무 수당 정산 결재(gunc071)</title>
...
%>
		...
		function fnc_SubmitApproval(kind) {
            ...
            var app_yn = "";    //결재완료여부
            var	row = "";
            var leno_no = dsT_DI_APPROVAL.NameValue(1,"ENO_NO");//최상위가 최종결재자
            var our_man = '<%=box.get("SESSION_ENONO")%>';

			if(leno_no != our_man){
	            dsT_DI_APPROVAL.NameString(2, "APP_YN") = "Y";  // 'R' 상신, 'Y' 결재, 'N' 반려
	            dsT_DI_APPROVAL.NameString(1, "APP_YN") = "R";  // 'R' 상신, 'Y' 결재, 'N' 반려
	            row = "2"

            }else if(leno_no == our_man){
	            dsT_DI_APPROVAL.NameString(1, "APP_YN") = "Y";  // 'R' 상신, 'Y' 결재, 'N' 반려
	            row = "1"
            }
...
<body leftmargin="20" topmargin="14" rightmargin="20" bottommargin="15" marginwidth="0" marginheight="0" scroll=auto onkeydown="fnc_HotKey();" onload="fnc_OnLoadProcess();">
	...
	<table width="800" border="0" cellspacing="0" cellpadding="0">
		<tr>
		<!-- 	<td height="35"  class="searchState" align="left">해당년월&nbsp;<input id="txtPIS_YM_SHR" size="7" maxlength="7" class="input_ReadOnly" readOnly></td> -->
			<td height="35"  width="103" class="searchState" align="center">해당년월</td>
			<td  width="120">
					...
					<td>
						...
						<object	id="grdT_DI_APPROVAL" classid="clsid:EA8B6EE6-3DD8-4534-B4BB-27148CF0042B" style="width:500px;height:110px;">
							...
							<param name="Format"				value="
            					...
            					<C> id='SEQ_NO'		width=70	name='구분'		align=center	Value={DECODE(ENO_NO,'100019','협조','결재')}</C>
								<C> id='ENO_NM'		width=90	name='결재자'	align=center</C>
								<C> id='JOB_NM'		width=80	name='직위'		align=center	</C>
                                <C> id='DPT_NM'		width=100	name='부서'		align=left		show=false</C>
								<C> id='APP_YN'		width=160	name='결재상태'	align=left		Value={DECODE(APP_YN,'Y','승인','N','부결','미결')&' '&APP_YMD} </C>
								<C> id='REMARK1'	width=75	name='의견'		align=center 	Color='Blue'	</C>
				...
				<tr align="center">
					...
					<object id="grdT_DI_HOLIDAYWORK" classid="clsid:EA8B6EE6-3DD8-4534-B4BB-27148CF0042B" style="width:800px;height:340px;">
						...
						<param name="Format" value='
                            ...
                            <C> id="DPT_NM"		width=120	name="소속"		    align=center    Edit=none	Suppress="1" </C>
                            <C> id="JOB_NM"		width=60	name="직위"		    align=center    Edit=none	Suppress="2" </C>
                            <C> id="ENO_NO"	    width=70	name="사번"         align=center    Edit=none	</C>
                            <C> id="ENO_NM"	    width=70	name="성명"	        align=center    Edit=none	</C>
                            <C> id="HOL_AMT"	width=80	name="휴일근무수당"	align=right     Edit=none	</C>
                            <C> id="CNT"		width=100	name="인원"    	    align=center    Edit=none	value={Decode(CNT,0,"",CNT)}</C>
   					'>
					</object>
					</comment><script> __ShowEmbedObject(__NSID__); </script>
					</td>
