...
<%
String SHR_YY = request.getParameter("STR_MM").substring(0,4);


int tmp = Integer.parseInt(SHR_YY) - 1 ;

String TOT_YY = Integer.toString(tmp);

String STR_MM 	= request.getParameter("STR_MM").substring(5,7);
String END_MM 	= request.getParameter("END_MM").substring(5,7);
String GBN_SHR = request.getParameter("GBN_SHR");
String GBN_PRT_SHR = request.getParameter("GBN_PRT_SHR");
...
<comment id="__NSID__">
              ...
              <param name="hnwsrc"    VALUE="misb020.hnw">
              <param name="initvalue" VALUE="[:TOT_YY]=<%=TOT_YY%>[:SHR_YY]=<%=SHR_YY%>[:STR_MM]=<%=STR_MM%>[:END_MM]=<%=END_MM%>[:GBN_SHR]=<%=GBN_SHR%>[:GBN_PRT_SHR]=<%=GBN_PRT_SHR%>">
