...
%>
    ...
    ******************************************************
    ...
    <script language="javascript" >
        ...
        function fnc_SearchList() {
            ...
            dsT_CM_ARMY.DataId = "../../../servlet/GauceChannelSVL?cmd=Ehr.pir.a.pira017.cmd.PIRA017CMD&S_MODE=SHR&ENO_NO="+form1.ENO_NO.value;
            dsT_CM_ARMY.Reset();
        ...
        function fnc_Save() {
            ...
            if(fnc_SaveItemCheck()) {
                ...
                trT_CM_ARMY.action = "../../../servlet/GauceChannelSVL?cmd=Ehr.pir.a.pira017.cmd.PIRA017CMD&S_MODE=SAV";
                trT_CM_ARMY.post();
        ...
        function fnc_Delete() {
            ...
            // 삭제 할 자료가 있는지 체크하고
            if (dsT_CM_ARMY.CountRow < 1) {
                fnc_Message(document.getElementById("resultMessage"), "MSG_05");
                return;
            }

            // 특정 컬럼의 값으로 삭제 여부를 물어 본다.
            if (confirm("병무구분[" + dsT_CM_ARMY.NameValue(dsT_CM_ARMY.RowPosition,"GUB_NM") + "] 자료를 제거하시겠습니까?") == false) return;
            dsT_CM_ARMY.DeleteRow(dsT_CM_ARMY.RowPosition);

            trT_CM_ARMY.action = "../../../servlet/GauceChannelSVL?cmd=Ehr.pir.a.pira017.cmd.PIRA017CMD&S_MODE=DEL";
            trT_CM_ARMY.post();
        ...
        function fnc_SaveItemCheck() {
            ...
            //DataSet의 변경 여부 확인
            if (!dsT_CM_ARMY.IsUpdated ) {
                document.getElementById("resultMessage").innerText = "* 저장할 내용이 없습니다.";
                return false;
            }

            for ( i = 1; i <= dsT_CM_ARMY.CountRow; i++ ) {
                if ( dsT_CM_ARMY.RowStatus(i) == 1 ||
                     dsT_CM_ARMY.RowStatus(i) == 3 ) {

                    // 병무구분
                    if( dsT_CM_ARMY.NameValue(i, "GUB_CD") == '' ) {
                        alert("병무구분은 필수 입력사항입니다.");
                        dsT_CM_ARMY.RowPosition = i;
                        document.getElementById("txtGUB_CD").focus();
                        return false;
                    }

                    // 복무기간의 시작일자 CHECK
                    if (!fnc_CheckDate2(dsT_CM_ARMY.NameValue(i, "ENL_YMD"), "복무시작일자") && dsT_CM_ARMY.NameValue(i, "ENL_YMD") != "") {
                        dsT_CM_ARMY.RowPosition = i;
                    ...
                    // 복무기간의 종료일자 CHECK
                    if (!fnc_CheckDate2(dsT_CM_ARMY.NameValue(i, "MIL_YMD"), "복무종료일자") && dsT_CM_ARMY.NameValue(i, "MIL_YMD") != "") {
                        dsT_CM_ARMY.RowPosition = i;
                    ...
                    // 복무기간의 시작일자와 종료일자 비교
                    if( dsT_CM_ARMY.NameValue(i, "ENL_YMD") != '' && dsT_CM_ARMY.NameValue(i, "MIL_YMD") != '' ) {
                        if( cfDateDiff(dsT_CM_ARMY.NameValue(i, "ENL_YMD").replace(/\-/g,''),dsT_CM_ARMY.NameValue(i, "MIL_YMD").replace(/\-/g,'')) < 0 ) {
                    ...
                    // 병무구분
                    if (dsT_CM_ARMY.NameValue(i, "GUB_CD") != "" &&
                        dsT_CM_ARMY.NameValue(i, "GUB_NM") == "") {
                    ...
                    // 역종
                    if (dsT_CM_ARMY.NameValue(i, "MIC_CD") != "" &&
                        dsT_CM_ARMY.NameValue(i, "MIC_NM") == "") {
                    ...
                    // 군별
                    if (dsT_CM_ARMY.NameValue(i, "MIL_CD") != "" &&
                        dsT_CM_ARMY.NameValue(i, "MIL_NM") == "") {
                    ...
                    // 전역사유
                    if (dsT_CM_ARMY.NameValue(i, "YSE_CD") != "" &&
                        dsT_CM_ARMY.NameValue(i, "YSE_NM") == "") {
                    ...
                    // 계급
                    if (dsT_CM_ARMY.NameValue(i, "GRADE") != "" &&
                        dsT_CM_ARMY.NameValue(i, "GRADE_NM") == "") {
                    ...
                    // 병과
                    if (dsT_CM_ARMY.NameValue(i, "MBK_CD") != "" &&
                        dsT_CM_ARMY.NameValue(i, "MBK_NM") == "") {
    ...
    ***************************************************************************************-->
    ...
    | 1. 조회 및 저장용 DataSet                     |
    | 2. 이름 : dsT_CM_ARMY                         |
    | 3. Table List : T_CM_ARMY                     |
    +----------------------------------------------->
    <Object ID="dsT_CM_ARMY" ClassID="CLSID:2506B38B-0FF7-4249-BA3E-8BC1DC399FBB">
    ...
    | 3. Table List : T_CM_ARMY                     |
    +----------------------------------------------->
    <Object ID ="trT_CM_ARMY" ClassID="CLSID:78E24950-4295-43D8-9B1A-1F41CD7130E5">
        <Param Name=KeyName     Value="toinb_dataid4">
        <Param Name=KeyValue    Value="SVL(I:SAV=dsT_CM_ARMY)">
    </Object>
    ...
    **************************************-->
    ...
    +-------------------------------------------------->
    <Script For=dsT_CM_ARMY Event="OnLoadCompleted(iCount)">
        ...
        if (iCount == 0)    {
            ...
            fnc_Message(document.getElementById("resultMessage"), "MSG_03", dsT_CM_ARMY.CountRow);
    ...
    +---------------------------------------------------------->
    <Script For=dsT_CM_ARMY Event="OnLoadError()">
    ...
    +------------------------------------------------------------------>
    <Script For=dsT_CM_ARMY Event="OnDataError()">
    ...
    +------------------------------>
    <script for=trT_CM_ARMY event="OnSuccess()">
    ...
    +--------------------------->
    <script for=trT_CM_ARMY event="OnFail()">
    ...
    <tr>
        <td>
            <table width="100%" border="1" cellspacing="0" cellpadding="0" style="border-collapse: collapse" bordercolor="#999999" class="table_cream">
                ...
                <tr>
                    <td align="center" class="blueBold">병무구분</td>
                    ...
                    <td align="center" class="blueBold">역&nbsp;&nbsp;&nbsp;&nbsp;종</td>
                    <td class="padding2423">
                ...
                <tr>
                    <td align="center" class="blueBold">군&nbsp;&nbsp;&nbsp;&nbsp;별</td>
                    <td class="padding2423">
                    ...
                    <td align="center" class="blueBold">복무기간</td>
                    <td class="padding2423" >
                        <input id="txtENL_YMD" style="width:32%;ime-mode:disabled" maxlength="10" onChange="fnc_CheckDate(this, '복무시작일자');" onkeypress="cfDateHyphen(this);cfCheckNumber();">
                        ...
                        <input id="txtMIL_YMD" style="width:32%;ime-mode:disabled" maxlength="10" onChange="fnc_CheckDate(this, '복무종료일자');" onkeypress="cfDateHyphen(this);cfCheckNumber();">
                        <a href="#" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('ImgMilYmd','','../../images/button/btn_HelpOver.gif',1)">
                        <img src="../../images/button/btn_HelpOn.gif" id="ImgMilYmd" name="ImgMilYmd" width="21" height="20" border="0" align="absmiddle" onclick="calendarBtn('datetype1','txtMIL_YMD','','535','0');"></a>
                    </td>

                </tr>
                <tr>
                    <td align="center" class="blueBold">전역사유</td>
                    <td class="padding2423">
                    ...
                    <td align="center" class="blueBold">계&nbsp;&nbsp;&nbsp;&nbsp;급</td>
                    <td class="padding2423">
                ...
                <tr>                    
                    <td align="center" class="blueBold">군&nbsp;&nbsp;&nbsp;&nbsp;번</td>
                    <td class="padding2423" >
                        <input id="txtMIL_NO" maxlength="10" style="width:100%">
                    </td>
                    <td align="center" class="blueBold">병&nbsp;&nbsp;&nbsp;&nbsp;과</td>
                    <td class="padding2423">
                ...
                <tr>
                    <td align="center" class="blueBold">전역구분</td>
                    <td class="padding2423" >
...
<object id="bnd" classid="CLSID:4A35BB2C-B831-4199-A486-FEA332D085D9">
    <Param Name="DataID",   Value="dsT_CM_ARMY">
