...
<html>
<head>
<title>개인별 고정지불/공제(sagb040)</title>
...
<script language="javascript" src="/common/calendar/calendar.js"></script>
    ...
    *****************************-->
    <script language="javascript" >
        ...
        function fnc_Delete() {
			...
			if(confirm("[사번:"+dsT_CP_FIXED.NameValue(dsT_CP_FIXED.RowPosition,"ENO_NO")+"/성명:"+dsT_CP_FIXED.NameValue(dsT_CP_FIXED.RowPosition,"ENO_NM")+"/급여항목:"+SAL_NM+"]\n의 정보를 삭제하시겠습니까?")){
        ...
        function fnc_ToExcel() {
            ...
            form1.grdT_CP_FIXED.GridToExcel("개인별고정지불공제", '', 225);
        ...
        function fnc_OnLoadProcess() {
			...
			cfStyleGrid(form1.grdT_CP_FIXED,15,"true","true");      // Grid Style 적용
    ...
    ***************************************************************************************-->
    ...
    <!-----------------------------------------------+
    | 1. 조회용 DataSet								 |
    | 2. 이름은 ds_ + 주요 테이블명(dsT_CP_FIXED)   |
    | 3. 사용되는 Table List(T_CP_FIXED)		     |
    +------------------------------------------------>
    ...
    | 2. 이름은 tr_ + 주요 테이블명(trT_CP_FIXED)	    |
    | 3. 사용되는 Table List(T_CP_FIXED)	            |
    +--------------------------------------------------->
    ...
    **************************************-->
    ...
    <Script For=dsT_CP_FIXED Event="OnDataError()">
        ...
        if (this.ErrorCode == 50018) {
	        alert("해당 필수입력항목[사번/급여항목/시작일/급상여구분]\n입력 후 다음 작업이 가능합니다.");
        }
        else if (this.ErrorCode == 50019) {
	        alert("[사번/급여항목/시작일/급상여구분]에 대한 중복값이 존재합니다. 확인바랍니다.");
        }
        else {
		    cfErrorMsg(this);
	    }
		...
		<td height="25" background="/images/common/barBg.gif">
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					...
					<td valign="top" background="/images/common/barGreenBg.gif" class="barTitle">개인별고정지불/공제</td>
					<td align="right" class="navigator">HOME/보상관리/수당관리/<font color="#000000">개인별고정지불/공제</font></td>
				</tr>
			</table>
		</td>
	</tr>
				...
				<tr>
					<td align="center" class="greenTable">
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
                                    ...
                                    <table width="100%" height="100%" border="1" cellspacing="1" cellpadding="2">
                                        <tr>
                                            <td><input type="radio" id="radio1" name="rdoGBN_SHR" style="border:none" value="0" checked>사원번호&nbsp;&nbsp;
                                                ...
                                                근로구분 <input id=txtOCC_NM_SHR name=txtOCC_NM_SHR size="8" class="input_ReadOnly" readonly>
                                                퇴사일 <input id=txtRET_YMD_SHR name=txtRET_YMD_SHR size="12" class="input_ReadOnly" readonly>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td><input type="radio" id="radio2" name="rdoGBN_SHR" style="border:none" value="0">급여항목&nbsp;&nbsp;
                            ...
                            <tr>
                                <td align="right" class="searchState">종료여부&nbsp;</td>
								<td>
									<select id="cmbEND_YN_SHR" style="width:100" onChange="fnc_SearchList();">
                                        <option value="" >전 체</option>
										<option value="Y" >종 료</option>
	                                    <option value="N" >미종료</option>
									</select>
								</td>
                            </tr>
						</table>
					...
					<td>
						...
						<object	id="grdT_CP_FIXED" classid="clsid:EA8B6EE6-3DD8-4534-B4BB-27148CF0042B" style="width:800px;height:250px;">
							...
							<param name="Format"					value="
                                ...
                                <C> id='SAL_GBN'	width=80	name='급상여구분'	align=center    Value={Decode(SAL_GBN,'0','급여','1','상여')} </C>
                                <C> id='STR_YMD'	width=80	name='시작일'	    align=center </C>
                                <C> id='END_YMD'	width=80	name='종료일'	    align=center </C>
								<C> id='AMOUNT'		width=75	name='금액'			align=right </C>
								<C> id='AD_TAG'		width=50	name='구분'		    align=center    Value={Decode(AD_TAG,'A','지불','D','공제')} </C>
							">
						</object>
