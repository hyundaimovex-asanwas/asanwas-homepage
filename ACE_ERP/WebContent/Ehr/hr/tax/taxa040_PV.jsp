...
<%
    ...
    if ( SAL_GBN.equals("8") ) {
        SAL_GBN_VAL = "연말정산";
    } else if ( SAL_GBN.equals("9") ) {
        SAL_GBN_VAL = "퇴직자정산";
    } else {
        SAL_GBN_VAL = "전체";
    }

    SALOCC_VAL = "정산구분 : " + SAL_GBN_VAL + "         근로구분 : " + OCC_CD_VAL;
...
<comment id="__NSID__">
              ...
              <param name="hnwsrc"    VALUE="taxa040.hnw">
              <param name="initvalue" VALUE="[:PIS_YM]=<%=PIS_YM%>[:PIS_YY]=<%=PIS_YY%>[:PIS_MM]=<%=PIS_MM%>[:OCC_CD]=<%=OCC_CD%>[:SAL_GBN]=<%=SAL_GBN%>[:ENO_NO]=<%=ENO_NO%>[:DTP_CD]=<%=DTP_CD%>[:MID_HIR]=<%=MID_HIR%>[:SALOCC_VAL]=<%=SALOCC_VAL%>">     
