...
<%
    String PIS_YYMM = request.getParameter("PIS_YYMM_SHR");
    String GMK_TAG  = request.getParameter("GMK_TAG_SHR");
    String SAL_GBN  = request.getParameter("SAL_GBN_SHR");
    String SEQ      = request.getParameter("SEQ_SHR");
    String OCC_CD   = request.getParameter("OCC_CD_SHR");
    String DPT_CD   = request.getParameter("DPT_CD_SHR");

    String title_val = "";
    if ( GMK_TAG.equals("0") )
        title_val = "출자금 현황";
    else if ( GMK_TAG.equals("E") )
        title_val = "사내 대출금 현황";
    else
        title_val = "공조 대출금 현황";
%>
...
<comment id="__NSID__">
              ...
              <param name="hnwsrc"    VALUE="taxa140.hnw">
              <param name="initvalue" VALUE="[:PIS_YYMM]=<%=PIS_YYMM%>[:GMK_TAG]=<%=GMK_TAG%>[:SAL_GBN]=<%=SAL_GBN%>[:SEQ]=<%=SEQ%>[:OCC_CD]=<%=OCC_CD%>[:DPT_CD]=<%=DPT_CD%>[:title_val]=<%=title_val%>">
