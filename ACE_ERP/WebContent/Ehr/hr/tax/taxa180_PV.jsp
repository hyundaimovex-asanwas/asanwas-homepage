<!--
***********************************************************************
* @source      : taxa180_PV.jsp 				                                                                                     *
* @description : 갑종 원천징수 PAGE
***********************************************************************
...
<%
    ...
    if(SEND_PARM.equals("1")){
    	rpt_name = "taxa180.hnw";
    }
    else{
    	rpt_name = "taxa250_180.hnw";
    }
%>
...
<comment id="__NSID__">
              ...
              <param name="initvalue" VALUE="[:ENO_NO]=<%=ENO_NO%>[:PIS_YY]=<%=PIS_YY%>[:DPT_CD]=<%=DPT_CD%>[:DOC1]=<%=DOC1%>[:DOC2]=<%=DOC2%>[:DOC3]=<%=DOC3%>[:STR_YMD]=<%=STR_YMD%>[:IS_YEAR_AMT]=<%=IS_YEAR_AMT%>[:CEF_CNT]=<%=CEF_CNT%>">
