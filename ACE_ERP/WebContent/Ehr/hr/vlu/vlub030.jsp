<!--
    ************************************************************************************
    * @Source         : vlub030.jsp                                                    *
    * @Description    : 다면평가처리 PAGE                                              *
    * @Developer Desc :                                                                *
    ************************************************************************************
    * DATE        |  AUTHOR   | DESCRIPTION                                            *
    *-------------+-----------+--------------------------------------------------------+
    * 2006/09/26  |  박인이   | 최초작성                                               *
    * 2008/11/10  |  김우태   | 다면평가로 변경 개발.                                  *
    ************************************************************************************
-->
    ...
    *****************************-->
    <script language="javascript" >
        ...
        function fnc_OnLoadProcess() {
            ...
            //Grid Style 적용
            cfStyleGrid(form1.grdT_EV_EVLIST_DT,15,"false","false");
    ...
    **************************************-->
    ...
    <Script For=dsT_EV_ESTINFO Event="OnLoadCompleted(iCount)">
        if ( iCount == 0 )    {
            ...
            // 상단부 데이터 조회
            dsT_EV_SCHCHG.DataId = "/servlet/GauceChannelSVL?cmd=hr.vlu.b.vlub030.cmd.VLUB030CMD&S_MODE=SHR_01&GUBUN="+GUBUN+"&EVL_YY="+form1.txtEVL_YY_SHR.value+"&EVL_GBN="+EVL_GBN+"&EVL_FRQ="+form1.txtEVL_FRQ_SHR.value+"&ENO_NO="+form1.txtEVLENO_NO_SHR.value;
            dsT_EV_SCHCHG.Reset();
        }
    ...
    <Script For=dsT_EV_SCHCHG Event="OnLoadCompleted(iCount)">
        ...
        //다면평가입력 사용유무
        dsT_EV_TYPE.DataID = "/servlet/GauceChannelSVL?cmd=hr.vlu.a.vlua010.cmd.VLUA010CMD&S_MODE=SHR&GUBUN="+GUBUN+"&EVL_YY="+form1.txtEVL_YY_SHR.value;
        dsT_EV_TYPE.Reset();
    ...
    <Script For=dsT_EV_TYPE Event="OnLoadCompleted(iCount)">
        ...
        if ( iCount == 0 )    {
            ...
            if(dsT_EV_TYPE.NameValue(1, "UP_YN") == "0"){
                alert("해당년도는 다면평가를 실시하지 않습니다.");
                return false;
            }

	        var EVL_YY  = form1.txtEVL_YY_SHR.value;
	        var EVL_FRQ = form1.txtEVL_FRQ_SHR.value;
	        var ENO_NO  = form1.txtEVLENO_NO_SHR.value;

	        // 그리드 데이터 조회
	        dsT_EV_EVLIST_DT.DataId = "/servlet/GauceChannelSVL?cmd=hr.vlu.b.vlub030.cmd.VLUB030CMD&S_MODE=SHR_02&GUBUN="+GUBUN+"&EVL_YY="+EVL_YY+"&EVL_FRQ="+EVL_FRQ+"&ENO_NO="+ENO_NO;
    ...
    <Script For=For=dsT_EV_TYPE Event="OnDataError()">
    ...
    <script language="javascript" for=grdT_EV_EVLIST_DT event=OnDblClick(Row,Colid)>
        ...
        if (form1.txtFLAG.value == "1") {
           alert("지금은 다면평가 입력기간이 아닙니다.");
        }
        else {

            //modal폼으로 부터 받은 confirm의 평가자명을 받아 온다.
                url = "/hr/" + "vlu" + "/" + "vlub041.jsp?GUBUN="+GUBUN+"&EVL_YY="+EVL_YY+"&EVL_GBN="+EVL_GBN+"&EVL_FRQ="+EVL_FRQ+"&IPT_STDT="+IPT_STDT+
                                                        "&IPT_ENDT="+IPT_ENDT+"&EVLENO_NO="+EVLENO_NO+"&EVLENO_NM="+EVLENO_NM+"&GROUP_CD="+GROUP_CD+
                                                        "&DEGREE_GBN="+DEGREE_GBN+"&ENO_NO="+ENO_NO+"&ENO_NM="+ENO_NM+"&DPT_NM="+DPT_NM+"&UPDN_GBN="+UPDN_GBN;
        ...
        <td height="25" background="/images/common/barBg.gif">
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr>
                    ...
                    <td valign="top" background="/images/common/barGreenBg.gif" class="barTitle">다면평가입력</td>
                    <td align="right" class="navigator"><span id="loctitle">&nbsp;</span><font color="#000000">다면평가입력</font></td>
                </tr>
            </table>
        </td>
    </tr>
                    ...
                    <td align="center" class="greenTable">
                        <table width="100%" border="0" cellspacing="0" cellpadding="0">
                            ...
                            <tr>
                                ...
                                <td align="right" class="searchState">평가기간</td>
        ...
        <td>
            <table border="0" cellspacing="0" cellpadding="0">
                <tr align="center">
                    <td>
                        <comment id="__NSID__">
                        <object    id="grdT_EV_EVLIST_DT" classid="clsid:EA8B6EE6-3DD8-4534-B4BB-27148CF0042B" style="width:800px;height:320px;">
                            <param name="DataID"                  value="dsT_EV_EVLIST_DT">
                            <param name="EdiTABLE"                value="false">
                            <param name="ColSizing"               value="true">
                            <param name="Format"                  value="
                                <C> id={currow}       width=33    name='순번',        align=center   </C>
                                <C> id='UPDN_GBN'     width=95    name='구 분'        align=center suppress =1  Value={Decode(UPDN_GBN,'1','부서장평가','3','동료평가')} </C>
                                <C> id='HEAD_NM'      width=125   name='사업본부'     align=center   </C>
                                <C> id='DPT_NM'       width=120   name='소 속'        align=center   </C>
                                <G> name='평 가 대 상 자'  HeadBgColor='#F7DCBB'
                                  <C> id='DUTY_NM'    width=90    name='직 책'      align=center  </C>
                                  <C> id='JOB_NM'     width=75    name='직 위'      align=center </C>
                                  <C> id='ENO_NO'     width=75    name='사 번'      align=center </C>
                                  <C> id='ENO_NM'     width=75    name='성 명'      align=center </C>
                                </G>
