    ...
    <script language="javascript" >
        ...
        function fnc_SearchList() {
            ...
            if (UPDN_GBN == '2') {
                ...
                trT_EV_ABLTBL.KeyValue = "tr01(O:SHR=dsT_EV_ABLTBL,O:SHR2=dsT_EV_EVLIST_HD)";
                trT_EV_ABLTBL.action = "/servlet/GauceChannelSVL?cmd=hr.vlu.b.vlub041.cmd.VLUB041CMD&S_MODE=SHR_02&GUBUN="+GUBUN+"&EVL_YY="+EVL_YY+"&EVL_GBN="+EVL_GBN+"&EVL_FRQ="+EVL_FRQ+"&ENO_NO="+ENO_NO+"&UPDN_GBN="+UPDN_GBN+"&DEGREE_GBN="+DEGREE_GBN+"&EVLENO_NO="+EVLENO_NO;
                trT_EV_ABLTBL.post();
            }
        }
        ...
        function fnc_ToExcel() {
            ...
            form1.grdT_EV_ABLTBL.GridToExcel("역량평가표", '', 225);
        ...
        function fnc_SaveItemCheck() {
            ...
            // 값이 2보다 작거나 5보다 크면 저장이 되지 않도록 한다.
            for (var i = 1; i<= dsT_EV_ABLTBL.CountRow; i++) {

                if ( dsT_EV_ABLTBL.NameValue(i, "SUBPNT_01") < 2  ||
                     dsT_EV_ABLTBL.NameValue(i, "SUBPNT_01") > 5 ) {
                    alert("["+dsT_EV_ABLTBL.NameValue(i, "EST_NM")+"]\n["+dsT_EV_ABLTBL.NameValue(i, "ABL_NM_SUB")+"]\n행동특성 기준 점수가 배점기준의 값이 아닙니다.\n해당 점수를 입력하시기 바랍니다. (2점~5점 범위)");

                    form1.grdT_EV_ABLTBL.Draw = false;
                    form1.grdT_EV_ABLTBL.RowPosition    = i;
                    form1.grdT_EV_ABLTBL.ColumnPosition = "SUBPNT_01";
                    form1.grdT_EV_ABLTBL.Draw = true;

                    form1.grdT_EV_ABLTBL.Focus();
                    return false;
                }
        ...
        function fnc_OnLoadProcess() {
            cfStyleGrid3(form1.grdT_EV_ABLTBL);
            ...
            document.getElementById("resultMessage").innerText = '* 해당 행동특성항목을 더블클릭 하거나 점수를 직접 입력하시면 됩니다.';
    ...
    **************************************-->
    ...
    <script for=dsT_EV_ABLTBL Event=onColumnChanged(row,colid)>
        ...
        if ( colid == 'SUBPNT_01') {
            ...
            SUBPNT_AVG = dsT_EV_ABLTBL.NameValue(row, 'SUBPNT_01');
    ...
    <Script For=grdT_EV_ABLTBL Event="OnDblClick(row, col)">

        if(col == "SUBITEM_01"){
	        dsT_EV_ABLTBL.NameValue(row, 'SUBPNT_01') = 2;
        }

        if(col == "SUBITEM_02"){
	        dsT_EV_ABLTBL.NameValue(row, 'SUBPNT_01') = 3;
        }

        if(col == "SUBITEM_03"){
	        dsT_EV_ABLTBL.NameValue(row, 'SUBPNT_01') = 4;
        }

        if(col == "SUBITEM_04"){
	        dsT_EV_ABLTBL.NameValue(row, 'SUBPNT_01') = 5;
        }
    </Script>

    <script language="javascript"  for=grdT_EV_ABLTBL event=OnExit(row,colid,olddata)>


        if (colid == "SUBPNT_01") {

                if ( dsT_EV_ABLTBL.NameValue(row, "SUBPNT_01") < 2  ||
                     dsT_EV_ABLTBL.NameValue(row, "SUBPNT_01") > 5 ) {
                    alert("행동특성 배점기준 값이 아닙니다.\n\n해당 점수를 입력하시기 바랍니다. (2점~5점 범위)");

					dsT_EV_ABLTBL.NameValue(row, "SUBPNT_01") = olddata;
                    form1.grdT_EV_ABLTBL.Draw = false;
                    form1.grdT_EV_ABLTBL.RowPosition    = row;
                    form1.grdT_EV_ABLTBL.ColumnPosition = "SUBPNT_01";
                    form1.grdT_EV_ABLTBL.Draw = true;
    ...
    <Script For=dsT_EV_ABLTBL Event="OnLoadCompleted(iCount)">
        ...
        if (iCount == 0)    {
            ...
            form1.grdT_EV_ABLTBL.Draw = false;
            form1.grdT_EV_ABLTBL.RowPosition    = 1;
            form1.grdT_EV_ABLTBL.ColumnPosition = "SUBPNT_01";
            form1.grdT_EV_ABLTBL.Draw = true;
    ...
    <script language=JavaScript for=grdT_EV_ABLTBL event=OnKeyDown(keycode,keystate)>

        if (keycode == 13) {

            if (grdT_EV_ABLTBL.ColumnPosition == 'SUBPNT_01') {

                if (grdT_EV_ABLTBL.RowPosition==dsT_EV_ABLTBL.CountRow) return false;

                grdT_EV_ABLTBL.RowPosition = grdT_EV_ABLTBL.RowPosition + 1;
                grdT_EV_ABLTBL.ColumnPosition ='SUBPNT_01';
                grdT_EV_ABLTBL.focus();
            }
...
<body leftmargin="20" topmargin="14" rightmargin="20" bottommargin="5" marginwidth="0" marginheight="0" scroll=auto onkeydown="fnc_HotKey();" onload="fnc_OnLoadProcess();">
        ...
        <td class="paddingTop3">
            <table border="0" cellspacing="0" cellpadding="0">
                <tr align="center">
                    <td>
                        <comment id="__NSID__">
                        <object id="grdT_EV_ABLTBL" classid="clsid:EA8B6EE6-3DD8-4534-B4BB-27148CF0042B" width=1050 height=590>
                            <Param Name="DataID"         value="dsT_EV_ABLTBL">
                            <param name="Sort"           value="false">
                            <param name="Editable"		value='true'>
							<param name="TitleHeight"	value='30'>
							<param name="RowHeight" value='115'>
                            <param name="Format"         value="
         							<FG> name='평가요소(등급/점수)' HeadBgColor='#F7DCBB'
         							    <FC> id='EST_NM'       width=60     name='역  량'     align=center edit=none suppress=1</FC>
	                                    <FC> id='ABL_NM'       width=50     name='평  가'    align=center edit=none Multiline=true suppress= 2</FC>
	                                    <FC> id='ABL_NM_SUB'   width=60     name='구  분'    align=left   edit=none Multiline=true</FC>
                                    </FG>
                                    <G> name='행동특성 기준' HeadBgColor='#F7DCBB'
	                                    <C>  id='SUBITEM_01'   width=200     name='D(2점)'   align=left   edit=none Multiline=true</C>
	                                    <C>  id='SUBITEM_02'   width=200     name='C(3점)'   align=left   edit=none Multiline=true</C>
	                                    <C>  id='SUBITEM_03'   width=200     name='B(4점)'   align=left   edit=none Multiline=true</C>
	                                    <C>  id='SUBITEM_04'   width=200     name='A(5점)'   align=left   edit=none Multiline=true</C>
                                    </G>
                                    <C>  id='SUBPNT_01'    width=40     name='점 수'                        align=center BgColor=#AFEEEE</C>
                             ">
                                 </object>
