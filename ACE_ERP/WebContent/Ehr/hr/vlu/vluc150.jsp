...
%>
        ...
        function fnc_EmplPop() {
            ...
            document.getElementById("txtEVLENO_NO_SHR").value = obj.eno_no;
            document.getElementById("txtEVLENO_NM_SHR").value = obj.eno_nm;
            document.getElementById("txtEVLDPT_NM_SHR").value = obj.dpt_nm;
            ENO = obj.eno_no;

            fnc_Clear();

            dsT_EV_ACHTBL.DataId = "/servlet/GauceChannelSVL?cmd=hr.vlu.c.vluc150.cmd.VLUC150CMD&S_MODE=SHR_01&GUBUN="+GUBUN+"&EVL_YY="+EVL_YY+"&EVL_FRQ="+EVL_FRQ+"&GBN="+GBN+"&ENO="+ENO;
            dsT_EV_ACHTBL.Reset();

            if(<%=box.get("SESSION_ENONO")%>!=ENO) Manager_YN = "Y";
            else Manager_YN = "N";
    ...
    **************************************-->
    ...
    <Script For=dsT_EV_ESTINFO Event="OnLoadCompleted(iCount)">
        ...
        if ( iCount == 0 )    {
            ...
            form1.txtEVLDPT_NM_SHR.value = '<%=box.get("SESSION_DPTNM")%>';
             ...
             // 상단부 데이터 조회
            dsT_EV_SCHCHG.DataId = "/servlet/GauceChannelSVL?cmd=hr.vlu.c.vluc150.cmd.VLUC150CMD&S_MODE=SHR_00&GUBUN="+GUBUN+"&EVL_YY="+form1.txtEVL_YY_SHR.value+"&EVL_GBN="+EVL_GBN+"&EVL_FRQ="+form1.txtEVL_FRQ_SHR.value+"&ENO_NO="+form1.txtEVLENO_NO_SHR.value;
            dsT_EV_SCHCHG.Reset();
        }
    ...
    <Script For=dsT_EV_SCHCHG Event="OnLoadCompleted(iCount)">
        ...
        ENO     = form1.txtEVLENO_NO_SHR.value;
...
%>
        ...
        if (IPT_STDT <= getToday() && IPT_ENDT >= getToday()) {
            ...
            alert('평가자 평가기간이 아닙니다.');
            document.getElementById("resultMessage").innerText = '* 평가자평가기간이 아닙니다.';
...
%>
    ...
    <script language="javascript" for=grdT_EV_ACHTBL event=OnDblClick(Row,Colid)>
        ...
        var EVLENO_NO  = form1.txtEVLENO_NO_SHR.value;                //평가자사번
        var EVLENO_NM  = form1.txtEVLENO_NM_SHR.value;                //평가자명
        var REFENO_NO1 = dsT_EV_ACHTBL.NameValue(Row, "REFENO_NO1");  //참조평가자사번1
        var REFENO_NO2 = dsT_EV_ACHTBL.NameValue(Row, "REFENO_NO2");  //참조평가자사번2
        var REFENO_NO3 = dsT_EV_ACHTBL.NameValue(Row, "REFENO_NO3");  //참조평가자사번3
        ...
        if (form1.txtFLAG.value == "1") {
            ...
            url = "/hr/" + "vlu" + "/" + "vluc151.jsp?GUBUN="+GUBUN+"&EVL_YY="+EVL_YY+"&EVL_FRQ="+EVL_FRQ+"&ENO_NO="+ENO_NO+"&ENO_NM="+ENO_NM+"&DPT_NM="+DPT_NM+
                                                    "&EVLENO_NO="+EVLENO_NO+"&EVLENO_NM="+EVLENO_NM+"&REFENO_NO1="+REFENO_NO1+"&REFENO_NO2="+REFENO_NO2+"&REFENO_NO3="+REFENO_NO3+"&Manager_YN="+Manager_YN;
            ...
            if (revck == -1 || revck == null) {
                ...
                if (firstList[0] == 0) {
                    ...
                    dsT_EV_ACHTBL.NameValue(Row,"EVL_PNT") = '미평가';

                }
                else {

                    dsT_EV_ACHTBL.NameValue(Row,"EVL_PNT") = firstList[0];
        ...
        <td height="25" background="/images/common/barBg.gif">
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr>
                    ...
                    <td align="right" class="navigator"><span id="loctitle">&nbsp;</span><font color="#000000">평가자평가</font></td>
                </tr>
            </table>
        </td>
    </tr>
                    ...
                    <td align="center" class="greenTable">
                        <table width="100%" border="0" cellspacing="0" cellpadding="0">
                            ...
                            <tr>
                                ...
                                <td class="padding2423">
                                    <input id=txtEVLDPT_NM_SHR size="14" class="input_ReadOnly" readonly>
                                </td>
                                <td align="center" class="searchState">평가자</td>
                                <td class="padding2423">
                                    <input id=txtEVLENO_NO_SHR size="10" class="input_ReadOnly" readonly>
                                    <input id=txtEVLENO_NM_SHR size="10" class="input_ReadOnly" readonly>
                                    <a href="#" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('imgENO_NO_SHR','','/images/button/btn_HelpOver.gif',1)"><img src="/images/button/btn_HelpOn.gif" id="imgENO_NO_SHR" name="imgENO_NO_SHR" width="21" height="20" border="0" align="absmiddle" onclick="javascript:if(txtEVLENO_NO_SHR.disabled == true){return;} fnc_EmplPop();"></a>
                                </td>
                                <!-- Hidden Filed (구분 : '1'이면 평가자가 조건임)-->
                                <input type=hidden name=txtGBN value="1">
    ...
    <tr>
        <td>
            <table border="0" cellspacing="0" cellpadding="0">
                    ...
                    <td>
                        ...
                        <object    id="grdT_EV_ACHTBL" classid="clsid:EA8B6EE6-3DD8-4534-B4BB-27148CF0042B" style="width:800px;height:340px;">
                            ...
                            <param name="Format"                  value="
                                <C> id='GROUP_NM'     width=70    name='평가그룹'      align=left   </C>
                                <C> id='HEAD_NM'      width=89    name='본부'          align=left   </C>
                                <C> id='DPT_NM'       width=89    name='소속'          align=left   </C>
                                ...
                                <C> id='SELF_PNT'     width=80    name='본인평가점수'  align=right  </C>
                                <C> id='EVL_PNT'      width=80    name='평가점수'      align=right  </C>
                            ">
                        </object>
                ...
                <tr>
                    <td class="searchState"><br>&nbsp;※ <u>해당 평가대상자 항목을 더블클릭하면 평가입력을 하실 수 있습니다.<u></td>
                </tr>
...
<object id="bndT_EV_ESTINFO" classid="CLSID:4A35BB2C-B831-4199-A486-FEA332D085D9">
    ...
    <Param Name="BindInfo", Value='
        ...
        <C>Col=ENO_NO      Ctrl=txtEVLENO_NO_SHR  Param=value </C>
        <C>Col=ENO_NM      Ctrl=txtEVLENO_NM_SHR  Param=value </C>
        <C>Col=DPT_NM      Ctrl=txtEVLDPT_NM_SHR  Param=value </C>
    '>
</object>