    ...
    <script language="javascript" >
        ...
        function fnc_SearchList() {
            ...
            if (document.form1.rdoGBN_SHR[0].checked) {
	            //정보 조회 - 정규직,계약직 - 일반
	        	dsT_EV_ABLRST.dataid = "/servlet/GauceChannelSVL?cmd=hr.vlu.l.vlul040.cmd.VLUL040CMD&S_MODE=SHR_02&REG_YM_SHR="+REG_YM_SHR;
	        	dsT_EV_ABLRST.reset();
            }else{
	            //정보 조회 - 부서장
        ...
        function fnc_Append() {
          ...
          if(document.form1.rdoGBN_SHR[0].checked){
            ...
            if (document.form1.rdoGBN_SHR[0].checked) {
	            ...
	            //정보 조회 - 부서장
	            dsT_EV_ABLRST.dataid = "/servlet/GauceChannelSVL?cmd=hr.vlu.l.vlul040.cmd.VLUL040CMD&S_MODE=SHR_01&REG_YM_SHR="+REG_YM_SHR;
	            dsT_EV_ABLRST.reset();
			}
			...
			if (dsT_EV_ABLRST.countrow>0) {
				...
				if (document.form1.rdoGBN_SHR[0].checked) {
					if (confirm( "현재월에 기생성된 일반 피고과자가 존재합니다. 기생성 피고과자를 삭제 후 다시 생성하겠습니까?")) {
			            ...
			            trT_EV_ABLRST.action = "/servlet/GauceChannelSVL?cmd=hr.vlu.l.vlul040.cmd.VLUL040CMD&S_MODE=DEL_01&REG_YM_SHR="+REG_YM_SHR;
			            trT_EV_ABLRST.post();
						//피고과자 생성
			       		fnc_Create();
					}
					...
					if (confirm( "현재월에 기생성된 부서장 피고과자가 존재합니다. 기생성 피고과자를 삭제 후 다시 생성하겠습니까?")) {
						//기생성 피고과자 삭제
			      		dsT_EV_ABLRST.DeleteAll();
			            trT_EV_ABLRST.KeyValue = "tr(I:dsT_EV_ABLRST=dsT_EV_ABLRST)";
			            trT_EV_ABLRST.action = "/servlet/GauceChannelSVL?cmd=hr.vlu.l.vlul040.cmd.VLUL040CMD&S_MODE=DEL_02&REG_YM_SHR="+REG_YM_SHR;
			            trT_EV_ABLRST.post();
						//피고과자 생성
			       		fnc_Create();
			       	}
		...
		function fnc_Create(){
            ...
            if (document.form1.rdoGBN_SHR[0].checked) {
	            //정보 조회 - 정규직,계약직 - 일반
				dsT_EV_ABLRST.DataID = "/servlet/GauceChannelSVL?cmd=hr.vlu.l.vlul040.cmd.VLUL040CMD&S_MODE=SHR_12&REG_YM_SHR="+REG_YM_SHR;
				dsT_EV_ABLRST.Reset();
            }else{
	            //정보 조회 - 부서장
	            dsT_EV_ABLRST.dataid = "/servlet/GauceChannelSVL?cmd=hr.vlu.l.vlul040.cmd.VLUL040CMD&S_MODE=SHR_13&REG_YM_SHR="+REG_YM_SHR;
	            dsT_EV_ABLRST.reset();
			}
            ...
            if (document.form1.rdoGBN_SHR[0].checked) {
				...
				if (confirm("일반 대상자를 조회하였습니다. 저장하시겠습니까?")) {
					...
					for(i=1;i<=dsT_EV_ABLRST.countrow;i++){
			            ...
			            trT_EV_ABLRST.KeyValue = "tr(I:dsT_EV_ABLRST=dsT_EV_ABLRST)";
			            trT_EV_ABLRST.action = "/servlet/GauceChannelSVL?cmd=hr.vlu.l.vlul040.cmd.VLUL040CMD&S_MODE=SAV_01";
			            trT_EV_ABLRST.post();
				}
            }else{
				if (confirm("부서장 대상자를 조회하였습니다. 저장하시겠습니까?")) {
					...
					for(i=1;i<=dsT_EV_ABLRST.countrow;i++){
			            ...
			            trT_EV_ABLRST.KeyValue = "tr(I:dsT_EV_ABLRST=dsT_EV_ABLRST)";
			            trT_EV_ABLRST.action = "/servlet/GauceChannelSVL?cmd=hr.vlu.l.vlul040.cmd.VLUL040CMD&S_MODE=SAV_02";
			            trT_EV_ABLRST.post();
				}
			}
		}
