	<!--*************************************************************************
	* @source      : welm011.jsp												*
	* @description : 주차권신청목록 PAGE											*
	*****************************************************************************
	* DATE            AUTHOR        DESCRIPTION									*
	*----------------------------------------------------------------------------			*
	* 2010/04/09            권혁수                      주차권신청목록최초작성
	***********************************************************************--*-->
...
<head>
    ...
    ***************************************************************************************-->
	...
	<script language="javascript">
        ...
        function fnc_SearchList() {
			...
			trT_WL_PARKING.KeyValue = "tr05(O:dsT_WL_PARKING=dsT_WL_PARKING)";
			trT_WL_PARKING.action = "/servlet/GauceChannelSVL?cmd=hr.wel.m.welm010.cmd.WELM010CMD"
                                   + "&S_MODE=SHR_LST"
                                   + "&PIS_YYMM="+STR_YM_SHR;
			trT_WL_PARKING.post();
        }
        ...
        function fnc_Save() {
			...
			trT_WL_PARKING.action = "/servlet/GauceChannelSVL?cmd=hr.wel.m.welm010.cmd.WELM010CMD&S_MODE=SAV_02";
			trT_WL_PARKING.post();
        }
		/********************************************
         * 04. 삭제 함수								*
        ...
        function fnc_ToExcel() {
            ...
            form1.grdT_WL_PARKING.GridToExcel("명함신청목록", '', 227);
        ...
        function fnc_OnLoadProcess() {
            ...
            cfStyleGrid(form1.grdT_WL_PARKING,15,"true","false");      // Grid Style 적용
    ...
    ***************************************************************************************-->
	...
	<!-----------------------------------------------------+
    | 1. 조회 및 저장용 DataSet								   |
    | 2. 이름은 ds_ + 주요 테이블명(dsT_WL_PARKING)			 	   |
    | 3. 사용되는 Table List(dsT_WL_PARKING) 				 	   |
    +------------------------------------------------------>
    ...
    ***************************************************************************************-->
    ...
    +------------------------------------------------------>
	<script for=trsT_WL_PARKING event="OnSuccess()">
    ...
    +------------------------------------------------------>
	<script for=trsT_WL_PARKING event="OnFail()">
        cfErrorMsg(this);
    </script>
    ...
    +------------------------------------------------------>
    <script language="javascript"  for="grdT_WL_PARKING" event="OnHeadCheckClick(Col,Colid,bCheck)">

        if(Colid == "END_TAG" && dsT_WL_PARKING.CountRow > 0) {
            for(var i=1; i<=dsT_WL_PARKING.CountRow; i++) {
                if(dsT_WL_PARKING.NameString(i, "END_TAG") == "F") {
                    dsT_WL_PARKING.NameString(i, "END_TAG") = "T";
                } else if(dsT_WL_PARKING.NameString(i, "END_TAG") == "T") {
                    dsT_WL_PARKING.NameString(i, "END_TAG") = "F";
                }
            }
        }
		...
		<tr>
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					...
					<td valign="top" background="/images/common/barGreenBg.gif" class="barTitle">주말주차권 신청목록</td>
					<td align="right" class="navigator">HOME/복리후생/주말주차권/<font color="#000000">주말주차권 신청목록</font></td>
				</tr>
			</table>
			</td>
		</tr>
		...
		<tr>
			...
			<table border="0" cellspacing="0" cellpadding="0">
				<tr align="center">
					<td>
					<comment id="__NSID__">
					<object id="grdT_WL_PARKING" classid="clsid:EA8B6EE6-3DD8-4534-B4BB-27148CF0042B" style="width:800px;height:280px;">
						...
						<param name="Format" value='
                            ...
                            <C> id="REQ_YMD"    width=70    name="신청일"      align=center    Edit=none Mask="XXXX-XX-XX" </C>
							<C> id="DPT_NM"		width=90   name="소속"			align=center	Edit=none    </C>
							<C> id="JOB_NM"		width=60	name="직위"			align=center	Edit=none    </C>
                            <C> id="ENO_NO"     width=50    name="사번"         align=center    Edit=none    </C>
                            <C> id="ENO_NM"     width=70    name="성명"        align=center    Edit=none    </C>
                            <C> id="CAR_KIND"   width="100"    name="차종"      align=center   Edit=none  </C>
                            <C> id="CAR_NO"   width="100"    name="차량번호"      align=center   Edit=none  </C>
                            <C> id="REMARK"   width=150 name="사유"             align=center   Edit=none  </C>
                            <C> id="END_TAG"  width=40  name="확인"             align=center   Edit=True EditStyle=CheckBox,Pointer=Hand HeadCheckShow=true </C>
						'>
					</object>
