...
<html>
<head>
	<title>연금저축등소득공제내역입력(yaca060_2013년도)</title>
			...
			for(i=0;i<dsT_AC_PENSION.CountRow;i++){
			    ...
			    if(dsT_AC_PENSION.RowLevel(i+1) !="9999"){
	                ...
	                if(dsT_AC_PENSION.NameValue(i+1,"PEN_TYPE_CD") == '2') {
		                if(dsT_AC_PENSION.NameValue(i+1,"PEN_GUBUN_CD") == '31' || dsT_AC_PENSION.NameValue(i+1,"PEN_GUBUN_CD") == '32' || dsT_AC_PENSION.NameValue(i+1,"PEN_GUBUN_CD") == '34') {
	                ...
	                if(dsT_AC_PENSION.NameValue(i+1,"PEN_TYPE_CD") == '3') {
		                if(dsT_AC_PENSION.NameValue(i+1,"PEN_GUBUN_CD") == '21' || dsT_AC_PENSION.NameValue(i+1,"PEN_GUBUN_CD") == '22') {
...
%>
    ...
    ***************************************************************************************-->
    ...
    /*
	    //alert("oSelectT.value="+oSelectT.value+", oSelect.value="+oSelect.value);	    
	    
    	if(dsT_AC_PENSION.NameValue(row,"PEN_TYPE_CD")=="2"){

		    //첫번째 라인을 제외한 모든 option 삭제
 		    for(var i = 1; i <= dsT_CM_COMMON_U6.CountRow; i++) {
 		    	alert("dsT_CM_COMMON_U6.NameValue(i,CODE)="+dsT_CM_COMMON_U6.NameValue(i,"CODE"));
 		    	if(dsT_CM_COMMON_U6.NameValue(i,"CODE") == "31" || dsT_CM_COMMON_U6.NameValue(i,"CODE") == "32" || dsT_CM_COMMON_U6.NameValue(i,"CODE") == "34" ){
 	 		    	alert("i="+i+", oSelect.options(i).value="+oSelect.options(i).value);
 		    		oSelect.remove(i);
 		    	}
 		    	
 		    	//alert("dsT_CM_COMMON_U6.NameValue(i)"+dsT_CM_COMMON_U6.NameValue(i,"CODE")); 
	        	  //if(dsT_CM_COMMON_U6.NameValue(i,"CODE") == "31" || dsT_CM_COMMON_U6.NameValue(i,"CODE") == "32" || dsT_CM_COMMON_U6.NameValue(i,"CODE") == "34" ){
		        //		alert("remove="+oSelect.value);
	        	//	  	oSelect.remove(i);
	        	//  }
    	...
    	*/
		...
		/*
			// 구분 검색 콤보박스 생성
			oSelectT = document.getElementById("cmbPEN_TYPE_CD");		
		    oSelect = document.getElementById("cmbPEN_GUBUN_CD");	    	
	    	...
	    	if(dsT_AC_PENSION.NameValue(row,"PEN_TYPE_CD")=="2"){
		          for( var i = 1; i <= dsT_CM_COMMON_U6.CountRow; i++ ) {
		        	  if(dsT_CM_COMMON_U6.NameValue(i,"CODE") != "31" && dsT_CM_COMMON_U6.NameValue(i,"CODE") != "32"  
		        		  		&& dsT_CM_COMMON_U6.NameValue(i,"CODE") != "34" ){
			              oOption       = document.createElement("OPTION");
	    	...
	    	}else if(dsT_AC_PENSION.NameValue(row,"PEN_TYPE_CD")=="3"){
		          for( var i = 1; i <= dsT_CM_COMMON_U6.CountRow; i++ ) {
		        	  if(dsT_CM_COMMON_U6.NameValue(i,"CODE") != "11" && dsT_CM_COMMON_U6.NameValue(i,"CODE") != "21" 
		        			  && dsT_CM_COMMON_U6.NameValue(i,"CODE") != "22"){	        	  
			              oOption       = document.createElement("OPTION");
			              oOption.value = dsT_CM_COMMON_U6.NameValue(i,"CODE");
			              oOption.text  = dsT_CM_COMMON_U6.NameValue(i,"CODE_NAME");
			              document.getElementById("cmbPEN_GUBUN_CD").add(oOption);
                ...
                <tr align="center">
					...
					<object id="grdT_AC_PENSION" classid="clsid:EA8B6EE6-3DD8-4534-B4BB-27148CF0042B" style="width:800px;height:230px;">
						...
						<param name="Format" 						value='
                            ...
                            <C> id="PEN_TYPE_NM"			width=120	name="저축유형명"		    		align=center  SumBgColor=#C3D0DB </C>
                            <C> id="PEN_GUBUN_CD"		width=30	name="코드"		    				align=center  SumBgColor=#C3D0DB show=false</C>
                            <C> id="PEN_GUBUN_NM"		width=150	name="저축구분명"		   	 		align=center  Value={decode(PEN_GUBUN_CD, 11,"퇴직연금",21,"개인연금저축",22,"연금저축",31,"청약저축",32,"주택청약종합저축",34,"근로자주택마련저축")} SumBgColor=#C3D0DB </C>
							<C> id="NTS_YN"					width=80	name="자료출처"		    			align=center  Value={decode(NTS_YN, 1,"국세청자료",2,"그밖의 자료")} SumBgColor=#C3D0DB</C>
                            <C> id="ABA_NM"					width=130	name="금융회사명"		    		align=center  SumBgColor=#C3D0DB </C>
                            <C> id="ACC_NO"					width=130	name="계좌번호"		        		align=center  SumBgColor=#C3D0DB </C>
                          	<C> id="AMOUNT"				width=120	name="불입금액"	            		align=right	  RightMargin="10" SumBgColor=#C3D0DB SumText=@sum</C>
						'>
					</object>
