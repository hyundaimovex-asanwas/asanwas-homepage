...
<%
    ...
    String GBN_VAL = "";
    String TITLE_VAL = PIS_YM.substring(0,4) + "년 연월차수당 지급내역";
    if ( GBN.equals("DPT_CD") ) GBN_VAL = TITLE_VAL+"(부서별)";
    else                        GBN_VAL = TITLE_VAL+"(직위별)";
/*
    ...
    System.out.println("JOB_CD ="+JOB_CD );
*/
%>
<html>
<head>
...
<comment id="__NSID__">
              ...
              <param name="hnwsrc"    VALUE="ymaa030.hnw">
              <param name="initvalue" VALUE="[:PIS_YM]=<%=PIS_YM%>[:OCC_CD]=<%=OCC_CD%>[:GBN]=<%=GBN%>[:DPT_CD]=<%=DPT_CD%>[:JOB_CD]=<%=JOB_CD%>[:GBN_VAL]=<%=GBN_VAL%>[:TITLE_VAL]=<%=TITLE_VAL%>">
