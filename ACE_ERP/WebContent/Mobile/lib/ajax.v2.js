var AJAX = {
    ...
    FileUpload: function(provider, path, info, file, func, error) {
        //var img = file.substr(file.indexOf('base64') + 7);
        ...
        try {
            $.ajax({
                ...
                xhr: provider,
                data: '{"path": "' + path + '", "name": "' + info.name + '", "size": "' + info.size + '", "type": "' + info.type + '", "data": "' + file + '"}',
                contentType: 'application/json; charset=utf-8',
                dataType: 'json',
                success: function(result) {
                    func(result);
    ...
    FileUploadSet: function(frmName, func) {

        var frm = $("#" + frmName);
        frm.attr("action", "/mobile/model/Ajax_File_Upload.aspx");
        frm.ajaxForm({
            beforeSend: function() {
                MOBILE.StartLoading();
            },
