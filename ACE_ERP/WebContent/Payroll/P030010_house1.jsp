...
<html>
<head>
<title><%=eyy%> 년 근로소득공제 주택자금 상세 입력</title>
...
******************************************************************************/
	function fnSelect(pYear, pEmpno) {	 //동현스 : 파라미터 받아서 기본 조회
			...
			ln_TRSetting(tr1, 
			    "/services/servlet/Payroll.p030010i_09",	//		경로 주의할 것!
			    "JSP(O:DS1=ds1)",
			    param);
...
******************************************************************************/
    function fnApply() {// 현재 데이터셋 반영
        ...
        if (ds1.IsUpdated) {
            ...
            ln_TRSetting(tr1, 
                "/services/servlet/Payroll.p030010i_09",
            ...
            alert("주택마련저축 상세 정보의 변경 내역이 없습니다.");
        }
...
******************************************************************************/
	...
	if(ds1.namevalue(ds1.rowposition,"DUDGB") == '31' ){
		if(ds1.namevalue(ds1.rowposition,"PAYAMT") > '1200000'){
		alert("청약저축은 120만원이하로만 입력할 수 있습니다.");		
		ds1.namevalue(ds1.rowposition,"PAYAMT")="";
		return false;
		}
	}

	if(ds1.namevalue(ds1.rowposition,"DUDGB") == '32' ){
		if(ds1.namevalue(ds1.rowposition,"PAYAMT") > '1200000'){
		alert("주택청약종합저축은 120만원이하로만 입력할 수 있습니다.");		
		ds1.namevalue(ds1.rowposition,"PAYAMT")="";
		return false;
		}
	}

	if(ds1.namevalue(ds1.rowposition,"DUDGB") == '33' ){
		if(ds1.namevalue(ds1.rowposition,"PAYAMT") > '12000000'){
		alert("장기주택마련저축은 1200만원이하로만 입력할 수 있습니다.");		
		ds1.namevalue(ds1.rowposition,"PAYAMT")="";
		return false;
		}
	}

	if(ds1.namevalue(ds1.rowposition,"DUDGB") == '34' ){
		if(ds1.namevalue(ds1.rowposition,"PAYAMT") > '1800000'){
		alert("근로자주택마련저축은 180만원이하로만 입력할 수 있습니다.");		
		ds1.namevalue(ds1.rowposition,"PAYAMT")="";
		return false;
		}
	}
		...
		<tr bgcolor="#FFFFFF">
			<td colspan="12" height="85">
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
					...
					<tr>
						<td>&nbsp;</td>
						<td><font color="#FF6633">2. 근로소득이 있는 거주자로서 과세연도 중 주택을 소유하지 않은 세대의 세대주가 해당 과세연도에 주택마련저축에 납입한 금액의 <br>
						&nbsp;&nbsp;&nbsp;&nbsp;40%에 상당하는 금액을 해당 과세연도의 근로소득금액에서 공제합니다.</font></b>
						</td>
					</tr>
...
<OBJECT id=gr1  classid=clsid:1F57AEAD-DB12-11D2-A4F9-00608CEBEE49 style="WIDTH: 970px; HEIGHT: 300px;border:1 solid #777777;display:;" viewastext>
	...
	<G> name='구 분'
		<FC> Name='유형'		ID=DUDGB		Width=120	HeadAlign=Center align=center EditStyle=Combo 
			 Data='31:청약저축,32:주택청약종합저축,33:장기주택마련저축,34:근로자주택마련저축'</FC>
	</G>

	<C> Name='금융기관코드'		ID=FINACD     Width=270	HeadAlign=Center align=LEFT show=TRUE	EDIT=none show=false</C>
	<C> Name='금융기관'			ID=FINANM     Width=240	HeadAlign=Center align=center show=TRUE	</C>
	<C> Name='계좌번호'			ID=ACCNO     Width=170	HeadAlign=Center align=center show=TRUE	Edit=numeric editlimit=16</C> 	
	<C> Name='가입일'				ID=STATDT     Width=120	HeadAlign=Center align=center show=TRUE	mask='XXXX-XX-XX' editlimit=8</C> 	
