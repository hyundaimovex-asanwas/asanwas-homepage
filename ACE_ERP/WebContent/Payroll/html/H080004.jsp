...
+ 프로그램 ID	: H080004
+ 기 능 정 의	: 조회,엑셀
+ 변 경 자	:   정영식. 2005.10.12
+ 변 경 이 력	: 선택조건 1에 자격증 대신 입사구분 추가 // 쿼리 데이타 확인.
+ 서 블 릿 명	: h080004_s1,h080004_s2,h080004_s3
------------------------------------------------------------------------------>
...
function ln_Query(){
	...
	if (chk_p01.checked && chk_p02.checked)	{
		alert("화면조회 자료 중 하나만 선택해 주십시오.");
		return false;
	}

	ls_data += "/servlet/Contract.h080004_s1?";
	...
	if (txt_empdiv.checked)	ls_data += "&v_str5=" +	gclx_empdiv.bindcolval
	else ls_data += "&v_str5=";

	if (txt_grddiv.checked)	ls_data += "&v_str19=" +	gclx_grddiv.bindcolval
	else ls_data += "&v_str19=";	//직급구분

	if (txt_chksex.checked)	ls_data += "&v_str20=" +	gclx_chksex.bindcolval
	else ls_data += "&v_str20=";	//성별

	if (txt_chkpayseq.checked)	ls_data += "&v_str21=" +	gclx_chkpayseq.bindcolval
	else ls_data += "&v_str21=";	//호봉

	ls_data += "&v_str22=" + gclx_chkorder.BindColVal;	//호봉 order by
	...
	//출력자료 구분
	if (gbn=="1") ls_data += "&v_str16=B";	//재중/파견
	else ls_data += "&v_str16=A";						//계약직
...
function ln_PrintQuery(){
	...
	gcds_temp.DataID = "/servlet/Contract.h080004_s2";
	gcds_temp.Reset();
...
function ln_Print(){
	...
	if (chk_p05.checked && gcds_data1.countrow>0) {	//본부 소속별 현황
		gcds_temp.ClearAll();
		ls_data += "/servlet/Contract.h080004_s2?";
		
		//재직유무
		if (txt_wrksts.checked)	ls_data += "v_str1=" + gclx_wrksts.bindcolval;	
		else ls_data += "v_str1=" + gclx_wrksts.bindcolval;
		...
		//입사구분코드
		if (txt_empdiv.checked)	ls_data += "&v_str7=" +	gclx_empdiv.bindcolval
		else ls_data += "&v_str7=";
		...
		//출력자료 구분
		if (gbn=="1") ls_data += "&v_str18=B";	//재중/파견
		else ls_data += "&v_str18=A";						//계약직
	...
	gcds_temp.Reset();
		...
		var THeader = "EMPNO:STRING(7),EMPNMK:STRING(30),DEPTCD:STRING(4),DEPTNM:STRING(30),TREECD:STRING(8),"
								...
								+"PERAGE:STRING(4),LSTEDU:STRING(60),PGCNM:STRING(30),PGCCD:STRING(4),ETC:STRING(30),"
								+"SEQ:STRING(5),CURDT:STRING(8),ENDDT:STRING(8),EDCDNM:STRING(30),EDCCNM:STRING(30)";
		...
		for (var s=1;s<=gcds_temp.countrow;s++) {			
			...
			for (var i=1;i<=22;i++) {
				if (i==22) {
					...
					gcds_print.namevalue(s,"PAYGNM") = '계약직';
				} else {
		...
		if (gcds_temp.countrow<1) { alert("출력하실 자료가 없습니다"); 
		}else {	
			gcrp_print.preview();
		}
...
function ln_BasPrint(){
	...
	if (chk_p04.checked && gcds_data1.countrow>0) {	//인사기본정보
		gcds_temp2.ClearAll();
		ls_data += "/servlet/Contract.h080004_s3?";
	...
	var THeader = "EMPNMK:STRING(20),NME:STRING(40),NMC:STRING(20),EMPNO:STRING(7),RESINO:STRING(13),"
							...
							+"TODT:STRING(8),STRTDT:STRING(8),ENDDT:STRING(8),EDIVNM:STRING(30),SCHNM:STRING(60),"
							+"BLOOD:STRING(2),TELNO:STRING(15),EMPADD:STRING(70),PICFILE:STRING(100),RILIGNM:STRING(30),"
							+"GRDDIVNM:STRING(30),CURDT:STRING(8),ENDDT:STRING(8),EDCDNM:STRING(30),EDCCNM:STRING(30)";//COL 27
	...
	var row = gcds_temp2.rowposition;
		for (var s=1;s<=gcds_temp2.countrow;s++) {
			gcds_print2.addrow();
			for (var i=1;i<=27;i++) {	//COL
				if (i==27) {
					gcds_print2.namevalue(s,"CURDT") = gs_date;
				} else if (i==10) {
					gcds_print2.namevalue(s,"PAYGNM") = '계약직';
				}else {
		...
		if (gcds_temp2.countrow<1) { alert("출력하실 자료가 없습니다"); 
		}else {	
			gcrp_print2.preview();
		}
...
function ln_Format() {
	...
	var ls_for = "";
		...
		ls_for += "	<T>id='계약만료일' ,left=334 ,top=256 ,right=506 ,bottom=306 ,face='굴림' ,size=10 ,bold=false ,underline=false ,italic=false ,forecolor=#000000 ,backcolor=#C0C0C0</T>																																		";
		...
		ls_for += "	<C>id='GLDIVNM', left=1478, top=69, right=1631, bottom=119, align='right' ,mask='XXXX / ', face='굴림', size=10, bold=false, underline=false, italic=false, forecolor=#000000, backcolor=#FFFFFF</C>																					";
		...
		ls_for += "	<C>id='TODT', left=516, top=256, right=803, bottom=306, align='left' ,mask='XXXX/XX/XX', face='굴림', size=10, bold=false, underline=false, italic=false, forecolor=#000000, backcolor=#FFFFFF</C>																		";
		ls_for += "	<X>left=0 ,top=325 ,right=153 ,bottom=391 ,backcolor=#C0C0C0 ,border=true ,penstyle=solid ,penwidth=1</X>																																																																	";
		ls_for += "	<T>id='직급' ,left=6 ,top=331 ,right=147 ,bottom=381 ,face='굴림' ,size=10 ,bold=false ,underline=false ,italic=false ,forecolor=#000000 ,backcolor=#C0C0C0</T>																																				";
		...
		ls_for += "	<C>id='GRDDIVNM', left=159, top=331, right=319, bottom=381, align='left', face='굴림', size=10, bold=false, underline=false, italic=false, forecolor=#000000, backcolor=#FFFFFF</C>																													";
...
function ln_Excel(){
	...
	else eval("gcgd_disp"+e).runexcel("계약직인사현황");
...
function ln_Before(){
	...
	//재직구분
	gcds_srh.DataID = "/servlet/Person.hclcode_s1?v_str1=1121";
	gcds_srh.Reset();

	//소속
	gcds_dept.DataID = "/servlet/Person.hcdept_s2?v_str1=";
	gcds_dept.Reset();
	...
	//직군
	gcds_jobgrph.DataID = "/servlet/Person.hclcode_s1?v_str1=2000";
	gcds_jobgrph.Reset();
...
<script language="javascript" for="gcds_grddiv" event="onloadCompleted(row,colid)">
	...
	gcds_grddiv.namevalue(1,"MINORNM") = "전체";
	gclx_grddiv.index = 0;
</script>
...
<!-- <script language="javascript" for="gcds_empdiv" event="onloadCompleted(row,colid)">
	...
	gcds_empdiv.namevalue(1,"MINORNM") = "전체";
	gclx_empdiv.index = 0;
</script> -->
	...
	<tr>
		<td width=100% height="25px">
			<table width="876" cellpadding="0" cellspacing="0" border="0" style="position:relative;left:3px;">
				<tr> 
					<td width="197" background="../../Common/img/com_t_bg.gif"><IMG height=47 src="../img/h080004_head.gif" width=197></td>
	    ...
	    <table width="876" cellpadding=0 cellspacing=0>
			  <tr>
					<td width=290 valign=top><fieldset style="height:70px;"><legend>화면조회 자료</legend>
						...
						<input type="text"     id=chk_p02nm value="직원주소록" style="position:relative;top:5px;left:-10px;border-width:0;width:68px"><BR><BR>
						...
						<!-- <comment id="__NSID__"><OBJECT id=gcra_prngbn classid=CLSID:754F3DC4-0C79-4C92-AD64-A806D8FF2AB0
			         style="position:relative;left:0px;top:8px;height:40;width:270;cursor:hand;">
		             <param name=Cols     value="2">
			           <param name=Format   value="1^인사 명부,6^직원 주소록 명부,7^계약직 현황">
            </OBJECT></comment><script>__ws__(__NSID__);</script><BR><BR> -->
							<fieldset style="height:20px;"><legend>출력자료</legend>
								...
								<!-- <comment id="__NSID__"><OBJECT id=gcra_prngbn2 classid=CLSID:754F3DC4-0C79-4C92-AD64-A806D8FF2AB0
			         style="position:relative;left:0px;top:10px;height:20;width:270;cursor:hand;">
		             <param name=Cols     value="2">
			           <param name=Format   value="1^인사 기본정보,2^본부 소속별 현황">
								</OBJECT></comment><script>__ws__(__NSID__);</script> -->
							</fieldset>
			    ...
			    <td width=250 valign=top><fieldset style="height:140px;"><legend>조건선택2</legend>
					  <table cellpadding="0" cellspacing="0" border=0>
							<tr>
						  	<td>
								<input type="checkbox" id=txt_wrksts style="position:relative;top:-2.5px;"></td>
						  ...
						  <tr>
							...
							<tr>
								<td><input type="checkbox" id=txt_grddiv style="position:relative;top:-2.5px;"></td>
								<td height="19px;">직급구분</td>
								<td><font color="#9999ff">
							...
							<tr>
					  		<td><input type="checkbox" id=txt_empdiv></td>
					  		<td height="19px;">입사구분</td>
					  		<td><font color="#9999ff">
									<comment id="__NSID__"><OBJECT id=gclx_empdiv classid=clsid:60109D65-70C0-425C-B3A4-4CB001513C69 
										style="position:relative;left:7px;top:1px;font-size:12px;width:120px;">
											<param name=CBData					value="^전체,3^한국인계약직,4^파견근로자,5^재중동포기능직,6^파견">
											<param name=CBDataColumns		value="MINORCD, MINORNM">
											<param name=SearchColumn		value=MINORNM>
											<param name=Sort						value=false>
											<param name=ListExprFormat	value="MINORNM^0^120">
				    ...
				    <table cellpadding="0" cellspacing="0" border="0">
							<tr>
				  			<td width="30px;"><input type="checkbox" id=txt_chk1 style="position:relative;top:-2.5px;"></td>
								...
								<td width="100px;" height="30px"><nobr>&nbsp;
				  			...
				  			<td width="30px;"><input type="checkbox" id=txt_chk3 style="position:relative;top:-7.5px;"></td>
								...
								<td width="100px;" height="30px"><nobr>&nbsp;
									<comment id="__NSID__"><object id=gcem_fr3 classid=CLSID:E6876E99-7C28-43AD-9088-315DC302C05F  tabindex="1"
										style="position:relative; left:1px;top:-4.5px; width:70px; height:20px; font-family:굴림; font-size:9pt;z-index:2; " class=objclsEme1>
											<param name=Alignment			value=0>
											<param name=Border	      value=true>
											<param name=Format	      value="YYYY/MM/DD">
											<param name=PromptChar	  value="_">
				  			...
				  			<td width="30px;"><input type="checkbox" id=txt_chk4 style="position:relative;top:-7.5px;"></td>
								...
								<td width="100px;" height="30px"><nobr>&nbsp;
									<comment id="__NSID__"><object id=gcem_fr4 classid=CLSID:E6876E99-7C28-43AD-9088-315DC302C05F  tabindex="1"
										style="position:relative; left:1px;top:-5.5px; width:70px; height:20px; font-family:굴림; font-size:9pt;z-index:2; " class=objclsEme1>
											<param name=Alignment			value=0>
											<param name=Border	      value=true>
											<param name=Format	      value="YYYY/MM/DD">
											<param name=PromptChar	  value="_">
...
<!---------직원명부----------->
<div id=disp01 name=disp01 style="position:absolute;top:300px; left:180px; width:874px; height:300px; visibility:hidden">
		        ...
		        <param name=dataid					value=gcds_data1>
				    ...
				    <param name=indwidth        value=0>
		   			...
		   			<param name=format          value='
							...
							<F>ID=GRDDIVNM,	Name=직급,					width=50,		HeadBgColor=#B9D4DC, bgcolor=#f5f5f5, align=center, sort = true</F>
							...
							<C>ID=TODT,			Name=계약만료일,		width=85,		HeadBgColor=#B9D4DC, align=center,		Mask="XXXX/XX/XX" ,sort = true</C>
							...
							<C>ID=ENDDT,		Name=퇴사일,				width=80,		HeadBgColor=#B9D4DC, align=center,		Mask="XXXX/XX/XX" ,sort = true</C>
  	  ...
  	  <td height="300" width=97%  valign="top">
		        ...
		        <param name=dataid value=gcds_data4>
				    ...
				    <param name=indwidth        value=0>
						...
						<param name=sortview     value=left>
		   			<param name=format          value='
							<F>ID=DEPTNM,		Name=소속명,				width=100,	HeadBgColor=#B9D4DC, bgcolor=#f5f5f5, align=left, suppress=1</F>
							<F>ID=PAYGNM,		Name=직위명,				width=80,		HeadBgColor=#B9D4DC, bgcolor=#f5f5f5, align=left, suppress=2</F>
							<F>ID=PAYSEQ,		Name=호봉,					width=50,		HeadBgColor=#B9D4DC, bgcolor=#f5f5f5, align=center, Editstyle=lookup, Data="gcds_payseq:minorcd:minornm", suppress=3</F>
  	  ...
  	  <td height="300" width=97%  valign="top">
		        ...
		        <param name=dataid value=gcds_data6>
				    ...
				    <param name=indwidth        value=0>
						...
						<param name=sortview     value=left>
		   			<param name=format          value='
							<F>ID=DEPTNM,		Name=소속명,				width=100,	HeadBgColor=#B9D4DC, bgcolor=#f5f5f5, align=left, suppress=1 ,sort = true</F>
							<F>ID=PAYGNM,		Name=직위명,				width=80,		HeadBgColor=#B9D4DC, bgcolor=#f5f5f5, align=left, suppress=2 ,sort = true</F>
							<F>ID=PAYSEQ,		Name=호봉,					width=50,		HeadBgColor=#B9D4DC, bgcolor=#f5f5f5, align=center, Editstyle=lookup, Data="gcds_payseq:minorcd:minornm", suppress=3 ,sort = true</F>
							<F>ID=EMPNMK,		Name=성명,					width=60,		HeadBgColor=#B9D4DC, bgcolor=#f5f5f5, align=left ,sort = true</F>
							<C>ID=EMPNO,		Name=사번,					width=60,		HeadBgColor=#B9D4DC, align=center ,sort = true</C>
  	  ...
  	  <td height="300" width=97%  valign="top">
		        ...
		        <param name=dataid					value=gcds_data7>
				    ...
				    <param name=indwidth        value=0>
						...
						<param name=sortview     value=left>
		   			<param name=format          value='
							<F>ID=DEPTNM,		Name=소속명,			width=120,	HeadBgColor=#B9D4DC, bgcolor=#f5f5f5, align=left,		suppress=1 ,sort = true</F>
							<F>ID=PAYGNM,		Name=직위명,			width=80,		HeadBgColor=#B9D4DC, bgcolor=#f5f5f5, align=left,		suppress=2 ,sort = true</F>
							<F>ID=GRDDIVNM,	Name=직급,				width=50,		HeadBgColor=#B9D4DC, bgcolor=#f5f5f5, align=center, suppress=2 ,sort = true</F>
							<F>ID=PAYSEQ,		Name=급호,				width=50,		HeadBgColor=#B9D4DC, bgcolor=#f5f5f5, align=CENTER, MASK="XX호" ,sort = true</F>
							<F>ID=EMPNMK,		Name=성명,				width=60,		HeadBgColor=#B9D4DC, bgcolor=#f5f5f5, align=left ,sort = true</F>
							<C>ID=STRTDT,		Name=입사일,			width=80,		HeadBgColor=#B9D4DC, align=center, Mask="XXXX/XX/XX" ,sort = true</C>
							<C>ID=RESINO,		Name=주민등록번호,width=100,	HeadBgColor=#B9D4DC, align=center, Mask="XXXXXX-XXXXXXX" ,sort = true</C>
  	  ...
  	  <td height="300" width=97%  valign="top">
				<comment id="__NSID__"><OBJECT id="gcgd_print" classid="clsid:1F57AEAD-DB12-11D2-A4F9-00608CEBEE49" style="height:317; width:873; border:1 solid #777777" >
					...
					<param name=format          value='
						...
						<C>ID=PGCNM,		Name=전공학과,			width=40,		HeadBgColor=#B9D4DC, align=left</C>
						<C>ID=EDCCNM,		Name=학력,		      width=160,	HeadBgColor=#B9D4DC, align=left		,sort = true</C>
						<C>ID=EDCDNM,		Name=학위,		      width=160,	HeadBgColor=#B9D4DC, align=left		,sort = true</C>
				...
				<fieldset style="width:875px;height:20px;border:1 solid #708090;border-top-width:0px;text-align:left;">
					&nbsp;<font id=ft_cnt8 style="position:relative;top:3px;"></font>
				</fieldset>
			</td>
		</tr>
  </table>
...
<B>id=default ,left=0,top=0 ,right=2869 ,bottom=59 ,face='Arial' ,size=10 ,penwidth=1
	...
	<C>id='PAYGNM', left=575, top=0, right=756, bottom=56, align='center', face='굴림', size=10, bold=false, underline=false, italic=false, forecolor=#000000, backcolor=#FFFFFF</C>
