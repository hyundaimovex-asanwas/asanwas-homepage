...
**********************************************************************************************/
          ...
          "CORAVG_SUM","TOTAVG_SUM" ,"GOGAGB_SUM"
				}; 
          ...
          if (str[9].equals("1")){ //정규직
            ...
            //sql.append( "          A.DEPTCD, RTRIM(B.DEPTNM) DEPTNM, A.PAYGRD PAYGRD, RTRIM(C.MINORNM) PAYGRDNM,        \n");
						...
						sql.append( "          COALESCE(A.TOTAVG,0) TOTAVG, E.PAYSEQ PAYSEQ, A.STRTDT STRTDT,COALESCE(Y.CORAVG_SUM,0)CORAVG_SUM, COALESCE(Y.TOTAVG_SUM,0)TOTAVG_SUM, COALESCE( Y.GOGAGB_SUM,0) GOGAGB_SUM       \n");
            ...
            sql.append( " SELECT RTRIM(A.EVAYM) EVAYM,  RTRIM(D.MINORNM) GRPNM, A.EMPNO, RTRIM(E.EMPNMK) EMPNMK,                                                                                                                     \n");
						...
						sql.append( "              WHEN (RTRIM(F.EMPNOH)<>'' AND (RTRIM(G.EMPNOH)IS NULL OR RTRIM(G.EMPNOH)='')AND(RTRIM(H.EMPNOH)IS NULL OR RTRIM(H.EMPNOH)=''))  THEN (IFNULL(A.SCR1,0)*1+IFNULL(A.SCR2,0)*0+IFNULL(A.SCR3,0)*0) ELSE 0 END AS SCRAVG,    \n");
						...
						sql.append( "              WHEN (RTRIM(F.EMPNOH)<>'' AND (RTRIM(G.EMPNOH)IS NULL OR RTRIM(G.EMPNOH)='')AND(RTRIM(H.EMPNOH)IS NULL OR RTRIM(H.EMPNOH)='')) THEN (IFNULL(A.AVG1,0)*1+IFNULL(A.AVG2,0)*0+IFNULL(A.AVG3,0)*0)  ELSE 0 END AS CORAVG,    \n");
						sql.append( "         COALESCE(A.TOTAVG,0) TOTAVG, E.PAYSEQ PAYSEQ, A.STRTDT STRTDT,COALESCE(Y.CORAVG_SUM,0)CORAVG_SUM, COALESCE(Y.TOTAVG_SUM,0)TOTAVG_SUM, COALESCE( Y.GOGAGB_SUM,0) GOGAGB_SUM                         \n");
						sql.append( "  FROM PAYROLL.HVEMP A ,PAYROLL.cIPERSON E                                                                                                                                                                  \n");
						sql.append( "  LEFT JOIN PAYROLL.HCDEPT   B ON A.DEPTCD = B.DEPTCD                                                                                                                                                        \n");
						sql.append( "  LEFT JOIN PAYROLL.HCLCODE C ON A.PAYGRD = C.MINORCD AND C.MAJORCD='2002'                                                                                                                                   \n");
