...
**********************************************************************************************/
				...
				if (!GauceReq.isBuilderRequest()) {					
					...
					sql.append( "  B.P33001, ");	/* 월차일수  */
					sql.append( "  B.P33000,  ");	/* 월차수당*/
					sql.append( "  B.P34001,  ");	/* 연차일수*/
					sql.append( "  B.P34000,  ");	/* 연차수당*/
					sql.append( "  --B.P35000, \n ");	/* 상여금*/
					...
					sql.append( "  B.T23000,  ");	/* 정산농특세*/
					sql.append( "  B.S91000,  ");	/* 지급합계*/
					sql.append( "  B.S92000,  ");	/* 소득합계*/
					sql.append( "  B.T11000,  ");	/* 소득세*/
					...
					sql.append( "  --B.T31000,  ");	/* 추징소득세*/
					sql.append( "  --B.T33000,  ");	/* 추징주민세*/
					sql.append( "  --B.T92000, \n  ");	/* 퇴직전환금*/
					sql.append( "  B.S93000,  ");	/* 공제합계*/
					sql.append( "  B.S94000,  ");	/* 실지급액*/
					sql.append( "  RTRIM(B.STRTDT) AS STRTDT,  "); /* 입사일 */
					sql.append( "  RTRIM(A.GRSTRTDT) AS GRSTRTDT,   ");	/* 그룹입사일*/
					sql.append( "  A.TRAINYN,  "); /* 수습적용여부 */
					sql.append( "  B.CHAGDT AS CHAGDT,   ");	/* 최종변동일*/
					sql.append( "  '0'  AS SENUM,  ");	/* 배부자장경*/
					sql.append( "  B.P23002, ");	/* 배우자*/
					sql.append( "  B.P23003,  ");	/* 부모*/
					sql.append( "  B.P23004, ");	/* 자녀*/
					sql.append( "  B.P23005,  ");	/* 장애*/
					sql.append( "  B.P23006 ");	/* 경로*/		
					...
					sql.append( " LEFT JOIN PAYROLL.HCLCODE D ON B.PAYGRD = D.MINORCD AND D.MAJORCD = '2002' "); /*직위*/
					sql.append( " LEFT JOIN PAYROLL.HCLCODE F ON B.JOBGRPH = F.MINORCD AND F.MAJORCD = '2000' "); /*직군*/
					sql.append( " LEFT JOIN PAYROLL.HCLCODE G ON B.PAYSEQ = G.MINORCD AND G.MAJORCD = '1184' "); /*호봉*/
					sql.append( " LEFT JOIN PAYROLL.HCLCODE H ON B.GRDDIV = H.MINORCD AND H.MAJORCD = '2001' "); /*직급*/
					...
					if (!str[3].equals("")&&!str[3].equals("0")) sql.append( " AND B.DEPTCD = '" + str[3]+ "' ");   //부서
					if (!str[4].equals("")&&!str[4].equals("0")) sql.append( " AND B.JOBGRPH= '"  + str[3] + "' "); //직군
					if (!str[5].equals("")&&!str[5].equals("0") && (!str[6].equals("")&&!str[6].equals("0")) )
						sql.append( " AND B.EMPNO BETWEEN '"  + str[5] + "' AND '"  + str[6] + "' ");              //사원번호부터 까지
												  													   
					if (!str[7].equals("")&&!str[7].equals("0")) sql.append( " AND A.EMPNMK= '"  + str[7] + "' "); //성명
					...
					sql.append( "  ORDER BY C.TREECD,SUBSTR(B.PAYDT,1,6),B.EMPNO "); /*지급구분*/
