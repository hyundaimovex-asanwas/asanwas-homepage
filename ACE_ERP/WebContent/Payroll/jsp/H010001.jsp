...
function ln_Query(){
	...
	gcds_hcl.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h010001_s1?v_str1=" + str1; 
...
function ln_MajorQuery(){
	gcds_hch.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h010001_s2?v_str1="; 
	gcds_hch.Reset(); 
...
function ln_Save(){
	...
	if (gcds_hcl.IsUpdated) {	//하위코드 조회
		...
		if (confirm("입력하신 사항을 등록하시겠습니까?")){
			...
			gctr_hcl.Action = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h010001_t1";
...
function ln_Delete2(){
	...
	if (confirm("선택하신 사항을 삭제하시겠습니까?")){
		...
		gctr_hch.Action = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h010001_t2";
		gctr_hch.post();
		tb_majorcd.index = 0;
		return ln_Query();
	}
...
function ln_Valchk2() {
	...
	gcds_hch3.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h010001_s3?v_str1=" + str1; 
	gcds_hch3.Reset(); 
