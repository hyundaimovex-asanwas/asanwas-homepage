...
function fnOnLoad(tree_idx){
	...
	//전체학과
	gcds_career.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>hclcode_s1?v_str1=1129";
	gcds_career.Reset();
}
...
function ln_Query(){
	...
	gcds_data.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h020001_s1?"
		...
		if (gcds_data.IsUpdated ){
			if (ln_ValChk()) {
				if (confirm("해당 채용공고를 저장하겠습니까 ?")) {
					...
					gctr_data.Action = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h020001_t1";
			...
			if (confirm("해당 채용공고를 저장하겠습니까 ?")) {
				...
				gctr_data.Action = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h020001_t2";
...
function ln_Before(){
	//해당학과
	gcds_careerd.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h020001_s2?v_str1=" + gcds_data.namevalue(gcds_data.rowposition,"INFODIV");
	gcds_careerd.Reset();
}
