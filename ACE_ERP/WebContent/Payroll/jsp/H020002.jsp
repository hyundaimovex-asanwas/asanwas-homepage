...
function ln_Query(){
	if (fn_trim(gcem_resino.text).length<13) {
		...
		gcds_data.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h020002_s3?"
...
function ln_Before(){
	//응시분야
	gcds_before.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h020002_s8?v_str1=" + gs_date;
	gcds_before.Reset();

	//출신도
	gcds_bircity.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>hclcode_s1?v_str1=2011";
	gcds_bircity.Reset();
...
function ln_SerEdu(){
	...
	//학력
	gcds_hreduc.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>hclcode_s1?v_str1=1115";
	gcds_hreduc.Reset();

	//전공
	gcds_preduc.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>hclcode_s1?v_str1=1129";
	gcds_preduc.Reset();

	//졸업구분
	gcds_greduc.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>hclcode_s1?v_str1=1118";
	gcds_greduc.Reset();
}
...
function ln_SerFam(){
	...
	//학력/대졸/중졸
	gcds_fameduc.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>hclcode_s1?v_str1=1115";
	gcds_fameduc.Reset();

	//가족관계
	gcds_famre.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>hclcode_s1?v_str1=1132";
	gcds_famre.Reset();
	...
	ls_for += "<R>id='basis.sbt' ,left=0 ,top=0 ,right=2000 ,bottom=130 ,MasterDataID='gcds_data'                                                                                                                         ";
		ls_for +="<B>id=DHeader ,left=0,top=0 ,right=2000 ,bottom=569 ,face='새굴림' ,size=9 ,penwidth=1                                                                                                                    ";
			...
			if (gcds_data.namevalue(1,"photo")=="") {
				...
				ls_for +="	<I>id='"+rDOMAIN+"<%=dirPath%><%=HDConstant.PATH_PERSON%>h020002_s9?v_str1="+str1+"&v_str2="+str2+"' ,left=21 ,top=225 ,right=352 ,bottom=545</I> ";
...
<comment id="__NSID__"><OBJECT id=gctr_data classid=clsid:0A2233AD-E771-11D2-973D-00104B15E56F VIEWASTEXT>
	...
	<param name="Action"		value="<%=dirPath%><%=HDConstant.PATH_PERSON%>h020002_t1">
</OBJECT></comment><script>__ws__(__NSID__);</script>
...
<script language="javascript" for="gcds_data" event="onLoadCompleted(row,colid)">
	if (row>0){
		if (gcds_data.namevalue(1,"photo")=="") {
			...
			var str1 = gcds_data.namevalue(1,"infodiv");
			var str2 = gcds_data.namevalue(1,"resino");
			photo.src="<%=dirPath%><%=HDConstant.PATH_PERSON%>h020002_s9?v_str1="+str1+"&v_str2="+str2
		}

		gcds_educ.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h020002_s4?"
...
<script language="javascript" for="gcds_educ" event="onloadCompleted(row,colid)">
	gcds_career.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h020002_s5?"
									 + "v_str1=" + gcds_data.namevalue(1,"RESINO");
	gcds_career.Reset();
</script>

<script language="javascript" for="gcds_career" event="onloadCompleted(row,colid)">
	gcds_family.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h020002_s6?"
									 + "v_str1=" + gcds_data.namevalue(1,"RESINO");
	gcds_family.Reset();
</script>

<script language="javascript" for="gcds_family" event="onloadCompleted(row,colid)">
	gcds_license.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h020002_s7?"
									 + "v_str1=" + gcds_data.namevalue(1,"RESINO");
	gcds_license.Reset();
</script>

<script language="javascript" for="gcds_license" event="onloadCompleted(row,colid)">
	gcds_hrself.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h020002_s0?"
