...
function fnOnLoad(tree_idx){
	...
	gcds_hclcode.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>hclcode_s1?v_str1=2013";
	gcds_hclcode.Reset();
...
******************************************************************************/
function ln_Query(){
	gcds_data.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h020005_s1?"
...
function ln_Execute(){
	if (confirm("합격자를 제외한 전체 접수자에 대하여 불합격 처리 하시겠습니까 ?")){
		gcds_execute.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h020005_s2?v_str1=" + gclx_info.BindColVal;
		gcds_execute.Reset();
	} 
}
...
function ln_Print(){
	if(gcds_data.countrow > 0)
	{
		...
		gcds_basis.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h020002_s3?"
	...
	ls_for += "<R>id='basis"+Area_num+".sbt' ,left=0 ,top=0 ,right=2000 ,bottom=130 ,MasterDataID='gcds_basis'                                                                                                                         ";
		ls_for +="<B>id=DHeader ,left=0,top=0 ,right=2000 ,bottom=569 ,face='새굴림' ,size=9 ,penwidth=1                                                                                                                    ";
			...
			if (gcds_basis.namevalue(1,"photo")=="") {
				...
				ls_for +="	<I>id='"+rDOMAIN+"<%=dirPath%><%=HDConstant.PATH_PERSON%>h020002_s9?v_str1="+str1+"&v_str2="+str2+"' ,left=17 ,top=225 ,right=348 ,bottom=545</I> ";
			}
...
<comment id="__NSID__"><OBJECT id=gctr_data classid=clsid:0A2233AD-E771-11D2-973D-00104B15E56F VIEWASTEXT>
	...
	<param name="Action"		value="<%=dirPath%><%=HDConstant.PATH_PERSON%>h020005_t1">
</OBJECT></comment><script>__ws__(__NSID__);</script>
