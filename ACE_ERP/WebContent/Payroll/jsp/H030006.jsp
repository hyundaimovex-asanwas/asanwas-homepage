...
function ln_Query(){
	...
	gcds_data00.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h030006_s1?"
...
function ln_EduQuery() {
	...
	gcds_data05.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h030001_s5?"
								  	 + "v_str1=" + gcds_data00.namevalue(gcds_data00.rowposition,"empno");
	gcds_data05.Reset();
	...
	if (!gcds_data00.IsUpdated ){
		alert("저장하실 정보가 없습니다.");
			...
			if (confirm("해당 입력정보를 저장하겠습니까 ?")) {
					...
					gctr_data.Action = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h030006_t1";
					gctr_data.post();
			}
	}
}
...
<script language="javascript" for="gcgd_disp" event="onclick(row,colid)">
		...
		if (gcds_data00.namevalue(row,"picture")=="") {
			...
			photo.src="http://www.singiro.co.kr<%=dirPath%><%=HDConstant.PATH_PERSON%>h030001_s0?v_str1="+str1;
		}
</script>
