...
if (chk_p01.checked == false) {
	gcds_data.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h040001_s1?"
	...
	gcds_data.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h040001_s2?"
									 + "v_str1=" + gclx_dept.bindcolval
									 + "&v_str2=&v_str3=" + fn_trim(txt_empnmk.value)
									 + "&v_str4="+gs_level
									 + "&v_str5="+gs_treecd;
...
function ln_Add(){
	for (var i=1; i<=gcds_data.countrow;i++){
		...
		if (gcds_data.namevalue(i,"BCHK") == "T" &&  //비상근,상근,승진,경력공채,경력특채,신입공채,신입특채,전환채용
				(v_ordcd=="B5" ||v_ordcd=="B6" || v_ordcd=="B8" || v_ordcd=="A1" || v_ordcd=="A2" || v_ordcd=="A3" || v_ordcd=="A4" ||v_ordcd=="C8"||v_ordcd=="D6")) {
			gcds_data2.addrow();  
			...
			gcds_data2.namevalue(row,"PAYDT")		= "";	//소급급여년월
			gcds_data2.namevalue(row,"ETC")			= "";	//비고
			gcds_data2.namevalue(row,"EMPNMK")	= gcds_data.namevalue(i,"EMPNMK");
			gcds_data2.namevalue(row,"DEPTGB")  = gclx_deptgb.BindColVal;
		}else if (gcds_data.namevalue(i,"BCHK") == "T" && v_ordcd == "A7") {  //호봉승급
			gcds_data2.addrow();
			var row = gcds_data2.rowposition;
			gcds_data2.namevalue(row,"EMPNO")		= fn_trim(gcds_data.namevalue(i,"EMPNO"));
			gcds_data2.namevalue(row,"ORDDT")		= gcem_orddt.text;
			...
			(v_ordcd != "B5" || v_ordcd!="B6" ||v_ordcd != "B8" || v_ordcd != "A1" || v_ordcd != "A2" || v_ordcd != "A3" || v_ordcd != "A4" || v_ordcd != "A7")) {  //그 밖의 것들
			gcds_data2.addrow();
			var row = gcds_data2.rowposition;
			gcds_data2.namevalue(row,"EMPNO")		= fn_trim(gcds_data.namevalue(i,"EMPNO"));
			gcds_data2.namevalue(row,"ORDDT")		= gcem_orddt.text;
	...
	if (gcds_data2.countrow <1) {
		...
		if (gcds_data2.IsUpdated ){
			if (confirm("해당 직원에 대한 발령요청 정보를 저장하겠습니까 ?")) {
				gctr_data.KeyValue = "Person.h040001_t1(I:USER=gcds_data2)";
				gctr_data.Action = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h040001_t1";
				gctr_data.post();
			}
		}
	}
...
function ln_Before(){
	...
	//직책코드
	gcds_deptgb.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>hclcode_s1?v_str1=2038";
	gcds_deptgb.Reset();
...
function ln_SetHeader(){
	...
	ls_html += "PAYGRD:STRING(4),PAYSEQ:STRING(4),ENDDT:STRING(8),APPDT:STRING(8),PAYDT:STRING(6),ETC:STRING(30),EMPNMK:STRING(20),";
...
<script language="javascript" for="gcds_payseq" event="onloadCompleted(row,colid)">
	...
	gcds_payseq.namevalue(1,"MINORNM") = "";
	gclx_payseq.index = 0;
</script>
...
<!-- <script language="javascript" for=gclx_dept2	event="OnSelChange()">
	//gcds_temp.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>hcdept_s1?v_str1="+gclx_dept2.BindColVal;
	//gcds_temp.Reset();
...
<script language="javascript" for=gclx_ordcd event=onSelChange()>
	...
	if (chkOrd == "B5" ||chkOrd == "B6" || chkOrd == "B8" || chkOrd == "A1" || chkOrd == "A2" || chkOrd == "A3" || chkOrd == "A4" || chkOrd == "C8"||chkOrd == "D6") {
...
<table width="876" cellpadding="0" cellspacing="0" border="0" style="position:relative;left:3px">
  <tr> 
    <td><img src="../img/h040001_head.gif"></td>
  ...
  <tr> 
	...
	<C> Name='부서'				ID=GROUPNM		HeadAlign=Center HeadBgColor=#B9D4DC Width=100 align=left,	Edit=none, EditStyle=LookUp, sort=true</C>
	...
	<C> Name='직급'				ID=GRDDIV		HeadAlign=Center HeadBgColor=#B9D4DC Width=80 align=center, Edit=none, EditStyle=LookUp, Data='gcds_grddiv:minorcd:minornm' SHOW=FALSE, sort=true</C>
       					...
       					<td style="padding-left:3px;">
									<table border="0" cellpadding="0" cellspacing="0" style="border:0 solid #708090;border-left-width:1px;border-top-width:1px;">
										...
										<tr>
											...
											<td class="tab21" width="100px">
												<comment id="__NSID__"><object id=gclx_deptgb classid=clsid:60109D65-70C0-425C-B3A4-4CB001513C69 
