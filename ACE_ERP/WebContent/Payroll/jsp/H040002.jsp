...
function ln_Query(){
	...
	gcds_data.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h040002_s1?"
...
function ln_PrintQuery(){
	...
	gcds_data_p.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h040002_s3?"
								 + "v_str1=" + gclx_dept.bindcolval
								 + "&v_str2=" + gclx_ordcd.bindcolval
								 + "&v_str3=" + gcem_fr.text
								 + "&v_str4=" + gcem_to.text
...
//	} else {
		if (confirm("해당 인사 발령요청 사항을 저장하겠습니까 ?")) {
			gcds_tr.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h040002_s2?"
										 + "v_str1=" + gclx_dept.bindcolval
										 + "&v_str2=" + gclx_ordcd.bindcolval
										 + "&v_str3=" + gcem_fr.text
										 + "&v_str4=" + gcem_to.text
...
function ln_Delete(){
	...
	if(gcds_data.namevalue(row,"APPDT") == "") {			
		if (confirm("선택하신 사항을 삭제하시겠습니까?")) {
			...
			gctr_data.KeyValue = "h040002_t1(I:USER=gcds_data)";
			gctr_data.Action = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h040002_t1";
			gctr_data.post();
			return ln_Query();
		}
	} else {
...
function ln_Update(){
	...
	if (gcds_data.IsUpdated ) {
		if (confirm("입력하신 비고란을 저장하시겠습니까?")) {
				gctr_data.KeyValue = "h040002_t2(I:USER=gcds_data)";
				gctr_data.Action = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h040002_t2";
				//prompt('',gcds_data.text);
				gctr_data.post();			
				//return ln_Query();
		}
...
<script language="javascript" for="gcds_tr" event="onloadCompleted(row,colid)">
	window.status="조회가 완료 되었습니다.";
	//document.all.LowerFrame.style.visibility="hidden";
	if (row <1) alert("인사발령작업이 실패하였습니다.");
	else {
		if (gcds_tr.namevalue(1,"FLAG")=="Y") alert("인사발령작업에 성공하였습니다");
		else alert("인사발령작업이 실패하였습니다.");
	}
</script>
...
<script language=JavaScript for=gcds_tr event=OnFail()>

alert("Error Code : " + gcds_tr.ErrorCode + "\n" + "Error Message : " + gcds_tr.ErrorMsg + "\n");
...
<table width="876" cellpadding="0" cellspacing="0" border="0" style="position:relative;left:3px">
  <tr> 
    <td><img src="../img/h040002_head.gif"></td>
			...
			<comment id="__NSID__"><OBJECT classid=clsid:1F57AEAD-DB12-11D2-A4F9-00608CEBEE49 id=gcgd_disp  
			    ...
			    <PARAM NAME="BorderStyle" VALUE="0">
					...
					<PARAM NAME="Format"			VALUE="  
						...
						<C> Name='소속명'				ID=DEPTCD HeadAlign=Center HeadBgColor=#B9D4DC Width=140	align=left,   edit=none EditStyle=lookup, Data='gcds_dept:deptcd:deptnm' </C>
						<C> Name='직책'					ID=DEPTGB	HeadAlign=Center HeadBgColor=#B9D4DC Width=75		align=center, edit=none EditStyle=lookup, Data='gcds_deptgb:minorcd:minornm'</C>
						<C> Name='인사적용일'		ID=APPDT	HeadAlign=Center HeadBgColor=#B9D4DC Width=70		align=center, edit=none Mask='XXXX/XX/XX' </C>
						<C> Name='비고'		      ID=ETC  	HeadAlign=Center HeadBgColor=#B9D4DC Width=124	align=left    BgColor=#ffffcc </C>
					">
			</OBJECT></comment><script>__ws__(__NSID__);</script>
