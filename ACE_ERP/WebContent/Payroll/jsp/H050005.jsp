...
function ln_Query(){
  ...
  var strdeptcd="";
	if(gclx_deptcd_0.bindcolval==""){ //전체일 경우
		for(i=1;i<=gcds_deptcd_0.countrow;i++){
			if(i==1){
				strdeptcd = "'"+gcds_deptcd_0.namevalue(i,"DEPTCD")+"'";
			}else{
				strdeptcd = strdeptcd+",'"+gcds_deptcd_0.namevalue(i,"DEPTCD")+"'";
			}
		}
	}else{
		strdeptcd =  "'"+gclx_deptcd_0.bindcolval+"'"
	}
  strdeptcd = "("+strdeptcd+")";

	gcds_data.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h050009_s2?v_str1=" + gcem_fr.text  // from 일자
...
function ln_Before(){
	...
	//소속,직위,성명
	//gcds_data2.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h050005_s2?v_str1="+gusrid;
	//gcds_data2.Reset();
	...
	gcds_holfee_0.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>hclcode_s1?v_str1=1124&v_str2=C";
	gcds_holfee_0.Reset();

  /*
	//소속코드[조회조건]
	gcds_deptcd_0.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>hcdept_s1?v_str1=";
	gcds_deptcd_0.Reset();
  */

  	//소속코드[조회조건]
	if(g_managegb=="Y"){
		gcds_deptcd_0.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>hcdept_s4?v_str1="+gdeptcd;
		gcds_deptcd_0.Reset();
  }else{
		gcds_deptcd_0.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>hcdept_s1?v_str1=";
		gcds_deptcd_0.Reset();
  ...
  //직위코드[검색조건]
	gcds_paygrd_0.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>hclcode_s1?v_str1=2002";
	gcds_paygrd_0.Reset();
	...
	//년차수
	gcds_year.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h050005_s3?v_str1="+gusrid+"&v_str2="+gcurdate.substring(0,4)+gcurdate.substring(5,7);
  //prompt('',gcds_year.DataID);

	gcds_year.Reset();

	if(gcds_year.countrow>0){
    BEFYEAR.value = gcds_year.namevalue(gcds_year.rowposition,"BEFYEAR");
		CURYEAR.value = gcds_year.namevalue(gcds_year.rowposition,"CURYEAR");
		USEYEAR.value = gcds_year.namevalue(gcds_year.rowposition,"USEYEAR");
		PAYYEAR.value = gcds_year.namevalue(gcds_year.rowposition,"PAYYEAR");

    g_BEFYEAR=BEFYEAR.value;
		g_CURYEAR=CURYEAR.value;
		g_USEYEAR=USEYEAR.value;
		g_PAYYEAR=PAYYEAR.value;
	}
...
function ln_Add(){
  ...
  txt_deptnm.value	= gcds_data2.namevalue(gcds_data2.rowposition,"DEPTNM");		//소속명
	txt_paygrdnm.value = gcds_data2.namevalue(gcds_data2.rowposition,"MINORNM");	//직위
	txt_empnmk_1.value	= gcds_data2.namevalue(gcds_data2.rowposition,"EMPNMK");		//성명	
	gcem_empno_1.text = gcds_data2.namevalue(gcds_data2.rowposition,"EMPNO");
	txt_deptcd.value =gcds_data2.namevalue(gcds_data2.rowposition,"DEPTCD");	
	txt_paygrdcd.value =gcds_data2.namevalue(gcds_data2.rowposition,"PAYGRD");	

 	gclx_LAZCD.index=0;
	gclx_closeyn_1.bindcolval ="N"; //결재대기  


	BEFYEAR.value=g_BEFYEAR;
	CURYEAR.value=g_CURYEAR;
	USEYEAR.value=g_USEYEAR;
	PAYYEAR.value=g_PAYYEAR;
}
...
//	if (gcem_frdt.text != "") { gcem_todt.text = gcem_frdt.text; }
  ...
  gcds_data.namevalue(row,"BEFYEAR") = BEFYEAR.value;
	...
	if (gcds_data.IsUpdated) {  
		...
		gctr_pyo.Action = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h050005_t1?v_str1="+ v_str1+"&v_str2="+v_str2 + "&v_str3=" + v_str3;
		gctr_pyo.post();
		ln_Query();
		}	
	}
...
function ln_Excel(){
  ...
  var THeader = "EMPNO:String(8),USEDT:String(7)";
	...
	gcds_holday.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h050004_s2?v_str1=200507";
	gcds_holday.Reset();

	//기존근태 중 연차조회
	gcds_duty.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h050009_s1?v_str1=200507";
	gcds_duty.Reset();
...
function ln_Submit(){
	...
	for(i=1;i<=gcds_data.countrow;i++){
		if(gcds_data.namevalue(i,"CHK")=="T"){
			 ...
			 arrMst[7]=gcds_data.namevalue(i,"APPDT");    //성명
			 break;
		}
  }
...
function ln_User_Setting(gbn){
	...
	if(gbn=="Y"){ // 부서별 관리자
		...
		txt_empnmmk.readOnly=false;
		gclx_deptcd_0.enable=true;
		gclx_paygrd_0.enable=true;
    gcem_empno_fr.text = gusrid;
		gcem_empno_to.text = gusrid;
		txt_empnmmk.value=gusrnm;
		gclx_deptcd_0.bindcolval=gcds_data2.namevalue(gcds_data2.rowposition,"DEPTCD");
		gclx_paygrd_0.bindcolval=gcds_data2.namevalue(gcds_data2.rowposition,"PAYGRD");
    ...
    gcem_empno_fr.ReadOnly=true;
		...
		txt_empnmmk.readOnly=true;
		gclx_deptcd_0.enable=false;
		gclx_paygrd_0.enable=false;
    ...
    gcem_empno_fr.text = gusrid;
	  ...
	  /*
		...
		gcem_amt_1.ReadOnlyBackColor = "#C0C0C0";
		gcem_empno_fr.ReadOnly = true;
    gcem_empno_to.ReadOnly = true;
...
function ln_Chk_Submit(){
	...
	for(i=1;i<=gcds_data.countrow;i++){
      ...
      if(gcds_data.namevalue(i,"EMPNO")!=strempno){
				alert("동일인만 결재 가능합니다.");
				return false;
			}			
		}
...
<comment id="__NSID__"><OBJECT id=gcds_manager classid=clsid:3267EA0D-B5D8-11D2-A4F9-00608CEBEE49 VIEWASTEXT>
	<PARAM NAME="SyncLoad" VALUE="true"><!-- 관리자구분 -->
</OBJECT></comment><script>__ws__(__NSID__);</script>

<comment id="__NSID__"><OBJECT id=gcds_year classid=clsid:3267EA0D-B5D8-11D2-A4F9-00608CEBEE49 VIEWASTEXT>
	<PARAM NAME="SyncLoad" VALUE="true"><!-- 년차 -->
</OBJECT></comment><script>__ws__(__NSID__);</script>
...
<script language=JavaScript  for=gcds_data event=OnLoadCompleted(row)>//근태총 조회 건수
  ln_Loading_End(row);
...
<script language=JavaScript  for=gcds_duty event=OnLoadCompleted(Row)>
   var tempdt = ""; usedt="",useyymm="", usedd="" ,gubun="", tcnt=0;
	 ...
	 for (i=1;i<=Row;i++) { /*근태테이블의 모든 건수*/
		...
		gctr_useduty.action = "<%=dirPath%><%=HDConstant.PATH_PERSON%>useduty_t1? " ; 
...
<script language="javascript"  for="gcds_data2"  event=onLoadCompleted(rowcount)>
	...
	if (txt_deptnm.value != "" && txt_paygrdnm.value != "" || gcds_data2.namevalue(row,"DEPTNM") != null) {
		...
		txt_deptcd.value =gcds_data2.namevalue(row,"DEPTCD");	
		txt_paygrdcd.value =gcds_data2.namevalue(row,"PAYGRD");	
	} else {
		txt_deptnm.value	= "";
		txt_paygrdnm.value = "";
		txt_empnmk_1.value	= "";	
  ...
  <tr> 
	...
	<tr>
	 <td colspan=2>
		...
		<tr> 
			<td width="60px" align=center  bgcolor="#eeeeee" class="tab15" ><nobr>검색일자</nobr></td>
			...
			<td class="tab18" style="width:170px;" >&nbsp;
		...
		<tr> 
			...
			<td class="tab19" colspan=3>&nbsp;
				<comment id="__NSID__"><OBJECT id=gclx_paygrd_0 classid=clsid:60109D65-70C0-425C-B3A4-4CB001513C69 
						style="position:relative;left:-5px;top:2px;font-size:12px;width:130px;">
			...
			<td>
              ...
              <param name=SortView      value="Left">
							...
							<PARAM NAME="Format"			VALUE="  
								...
								<C> Name='결재자명'		ID=SGNM			HeadAlign=Center HeadBgColor=#B9D4DC Width=88		align=LEFT		Edit=None  sort= true  show=false</C>
								<C> Name='결재일'			ID=SGDT			HeadAlign=Center HeadBgColor=#B9D4DC Width=75		align=CENTER	Mask='XXXX/XX/XX' Edit=None show=false </C>
								<C> Name='결재자ID'		ID=SGNID		HeadAlign=Center HeadBgColor=#B9D4DC Width=73		align=CENTER	Edit=None show=false</C>
								<C> Name='승인자명'		ID=AGNM			HeadAlign=Center HeadBgColor=#B9D4DC Width=88		align=LEFT		Edit=None sort= true  show=false</C>
								<C> Name='승인일'			ID=AGRDT		HeadAlign=Center HeadBgColor=#B9D4DC Width=80		align=CENTER	Mask='XXXX/XX/XX' Edit=None show=false</C>
