...
function ln_Before(){
	//근태구분
	gcds_lazcd.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>hclcode_s1?v_str1=1124&v_str2=G";
	gcds_lazcd.Reset();

	gcds_lazcd_1.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>hclcode_s1?v_str1=1124&v_str2=G";
	gcds_lazcd_1.Reset();

	gcds_lazcd_2.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>hclcode_s1?v_str1=1124&v_str2=G";
	gcds_lazcd_2.Reset();
  ...
  //관리자구분
	...
	gcds_manager.Reset();	
	 ...
	 //교통편구분
	gcds_traffic.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>hclcode_s1?v_str1=2053";
	gcds_traffic.Reset();	

	 //숙박
	gcds_instamt.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>hclcode_s1?v_str1=2055";
	gcds_instamt.Reset();	

  //식비
	gcds_infdamt.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>hclcode_s1?v_str1=2056";
	gcds_infdamt.Reset();	

  //일비
	gcds_dayamt.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>hclcode_s1?v_str1=2054";
	gcds_dayamt.Reset();	

	 //환율
  gcds_exchage.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>hclcode_s1?v_str1=2057";
	gcds_exchage.Reset();	
}
...
function ln_Query(){
	...
	if(gclx_lazcd.bindcolval=="G02"){//국내출장
		gcds_data02.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h050010_s1?"
		                   + "v_str1=" + gcem_frdt.text
											 + "&v_str2=" + gcem_todt.text
											 + "&v_str3=" + gcem_empno_fr.text
											 + "&v_str4=" + gcem_empno_to.text
		...
		gcds_data01.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h050010_s1?"
		                   + "v_str1=" + gcem_frdt.text
											 + "&v_str2=" + gcem_todt.text
											 + "&v_str3=" + gcem_empno_fr.text
											 + "&v_str4=" + gcem_empno_to.text
...
function ln_Query_Dtl(row){
	//if(row<1)return;
	gcds_data03.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h050010_s2?"
...
function ln_Save(){
	 if(gclx_lazcd.bindcolval=="G02"){ //국내출장
          ...
          //국내출장DTL
					for(i=1;i<=gcds_data03.countrow;i++){
					  if(gcds_data03.SysStatus(i)==1||gcds_data03.SysStatus(i)==3){
							...
							gcds_data03.namevalue(i,"FRDT")=gcds_data02.namevalue(gcds_data02.rowposition,"FRDT");
							gcds_data03.namevalue(i,"HLNO")=gcds_data02.namevalue(gcds_data02.rowposition,"HLNO");
						}
					}

					if (confirm("저장 하시겠습니까?")){	
						gctr_data02.Action = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h050010_t2?";
          ...
          gcds_data01.namevalue(gcds_data01.rowposition,"APPLYID")=gusrid;
					...
					if (confirm("저장 하시겠습니까?")){	
						gctr_data01.Action = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h050010_t1?";
...
function ln_Excel(){
	if(gclx_lazcd.bindcolval=="G02"){ //국내출장
		gcgd_disp02.GridToExcel(gclx_lazcd.text,"",2);
	}else{
...
function ln_Chk_Otdt(){
	...
	strdt = gcds_data02.namevalue(gcds_data02.rowposition,"FRDT");
	stryy = strdt.substring(0,4);
	strmm = strdt.substring(4,6);
	strdd = strdt.substring(6,8);
	sdate ="";

  var cnt = ln_DataChk(gcds_data02.namevalue(gcds_data02.rowposition,"FRDT"),gcds_data02.namevalue(gcds_data02.rowposition,"TODT"));

  step2:
  for(k=1;k<=cnt;k++){
	  //날짜 계산
		if (k==1){
      date = new Date(stryy,strmm-1,strdd); //월에서는 1을 뺀다 
		}else{
		  strdd = parseInt(strdd,10)+1;
			date = new Date(stryy,strmm-1,strdd); //월에서는 1을 뺀다 
		}

    year = date.getYear()  // 년 구하기
		month = ln_Seqno_Format(date.getMonth()+1,2); // 월 구하기
		date = ln_Seqno_Format(date.getDate(),2);     // 일 구하기
	  sdate = ""+year+month+date;

		step1:
		for(j=1;j<=gcds_data03.countrow;j++){
			if(sdate==gcds_data03.namevalue(j,"OTDT")){
			  chk="1";
				break step1;
			}else{
				chk="0";
		...
		if (gcds_data03.countrow<1){
			var s_temp = "APPDT:STRING(8),EMPNO:STRING(7),LAZCD:STRING(4),FRDT:STRING(8),OTDT:STRING(8),"
			           + "OTSEQ:STRING(2),SECTION:STRING(20),TRAFFIC:STRING(4),CARFARE:DECIAML(7.0),INSTAMT:DECIAML(7.0),"
								 ...
								 + "ORGOTSEQ:STRING(2),DEPTCD:STRING(4),PAYGRD:STRING(4),MEMO:STRING(30),HLNO:STRING(14)";
...
function ln_Chk_Delete(gbn){
	...
	if(gbn=="G02"){//국내
		for(i=1;i<=gcds_data02.countrow;i++){
			if(gcds_data02.namevalue(i,"CHK")=="T"){
				if(fn_trim(gcds_data02.namevalue(i,"CLOSYN"))!=""&&gcds_data02.namevalue(i,"CLOSYN")!="N"&&gcds_data02.namevalue(i,"CLOSYN")!="R"&&gcds_data02.namevalue(i,"CLOSYN")!="B"&&gcds_data02.namevalue(i,"CLOSYN")!="H"){
					alert("삭제할 수 없습니다.");
					return false;
				}
				cnt+=1;
		...
		for(i=1;i<=gcds_data01.countrow;i++){
			if(gcds_data01.namevalue(i,"CHK")=="T"){
        if(fn_trim(gcds_data01.namevalue(i,"CLOSYN"))!=""&&gcds_data01.namevalue(i,"CLOSYN")!="N"&&gcds_data01.namevalue(i,"CLOSYN")!="R"&&gcds_data01.namevalue(i,"CLOSYN")!="B"&&gcds_data01.namevalue(i,"CLOSYN")!="H"){
					alert("삭제할 수 없습니다.");
					return false;
				}
				cnt+=1;
  ...
  <tr>
		<td>
			<table width="876" cellpadding="0" cellspacing="0" border="0" >
				<tr> 
					<td ><img src="../img/h050010_head.gif"></td>
		  ...
		  <div id="div_disp01" style="display:none;">
			<table>
					  ...
					  <!--Grid-->
						<table cellpadding="0" cellspacing="0" border="0" style="position:relative;left:-5px;width:280px;border:0 solid #708090;">
                          ...
                          <C> Name='달러'      ID=OVDSAMT   HeadAlign=Center HeadBgColor=#B9D4DC Width=60	  align=right  sort=false SumBgColor=#C3D0DB SumText=@sum</C> 
													...
													<C> Name='식비'      ID=OVFDAMT   HeadAlign=Center HeadBgColor=#B9D4DC Width=60	  align=right  sort=false SumBgColor=#C3D0DB </C> 
              ...
              <td class="tab13" style="text-align:left"><nobr>
						...
						<tr>
						  <td width="60px" height="25px" bgcolor="#eeeeee" class="tab19" align=center ><nobr>달러/원화</nobr></td>
							<td class="tab13" style="text-align:left"><nobr>
								<comment id="__NSID__">
									<object  id=gcem_ovdsamt_1 classid=CLSID:E6876E99-7C28-43AD-9088-315DC302C05F  style="width:70px;height:20px;position:relative;left:5px;top:2px">		
											<param name=Text					value=0>
											<param name=Alignment     value=2>
											<param name=Numeric       value="true">
											<param name=Border        value=true>
									 ...
									 <comment id="__NSID__">
									<object  id=gcem_ovwnamt_1 classid=CLSID:E6876E99-7C28-43AD-9088-315DC302C05F  style="width:70px;height:20px;position:relative;left:5px;top:2px">		
											<param name=Text					value=0>
											<param name=Alignment     value=2>
											<param name=Numeric       value="true">
											<param name=Border        value=true>
						...
						<!-- <tr>
						  <td width="80px" height="25px" bgcolor="#eeeeee" class="tab19" align=center ><nobr>원화</nobr></td>
							<td class="tab13" style="text-align:left"><nobr>
								<comment id="__NSID__">
									<object  id=gcem_ovwnamt_1 classid=CLSID:E6876E99-7C28-43AD-9088-315DC302C05F  style="width:100px;height:20px;position:relative;left:5px;top:0px">		
											<param name=Text					value=0>
											<param name=Alignment     value=2>
											<param name=Numeric       value="true">
											<param name=Border        value=true>
						...
						<tr style="text-align:left; border:10">
						  <td width="60px" height="25px" bgcolor="#eeeeee" class="tab19" align=center ><nobr>교통/식비</nobr></td>
							<td class="tab13" style="text-align:left; border:10" ><nobr>
								<comment id="__NSID__">
									<object  id=gcem_caramt_1 classid=CLSID:E6876E99-7C28-43AD-9088-315DC302C05F  width=70 style="height:20px;position:relative;left:5px;top:2px">		
											<param name=Text					value=0>
											<param name=Alignment     value=2>
											<param name=Numeric       value="true">
											<param name=Border        value=true>
									 ...
									 <comment id="__NSID__">
									<object  id=gcem_ovfdamt_1 classid=CLSID:E6876E99-7C28-43AD-9088-315DC302C05F  style="width:70px;height:20px;position:relative;left:5px;top:2px">		
											<param name=Text					value=0>
											<param name=Alignment     value=2>
											<param name=Numeric       value="true">
											<param name=Border        value=true>
						...
						<tr>
						  <td width="80px" height="20px" bgcolor="#eeeeee" align=center style="border:0 solid #708090;border-right-width:1px;"><nobr>결재상태</nobr></td>
							<td style="text-align:left"><nobr>
							<comment id="__NSID__">
								<object id=gclx_closeyn_1 classid=clsid:60109D65-70C0-425C-B3A4-4CB001513C69 width=100 class="txtbox"  style="position:relative;left:5px;top:2px" >
			...
			<table>
              ...
              <td class="tab13" style="text-align:left"><nobr>
                ...
                <object id=gclx_infdamt_2 classid=clsid:60109D65-70C0-425C-B3A4-4CB001513C69 width=70 class="txtbox"  style="position:relative;top:2px;left:5px;" >
										...
										<param name=SearchColumn		value="MINORNM">
										<param name=Sort						value="FALSE">
										<param name=ListExprFormat	value="MINORNM^0^70,MEMO^0^120">
              ...
              <comment id="__NSID__">
								<object id=gclx_dayamt_2 classid=clsid:60109D65-70C0-425C-B3A4-4CB001513C69 width=80 class="txtbox"  style="position:relative;top:2px;left:0px;" >
										...
										<param name=SearchColumn		value="MINORNM">
										<param name=Sort						value="FALSE">
										<param name=ListExprFormat	value="MINORNM^0^80,MEMO^0^100">
										<param name=BindColumn			value="MINORNM">
										<!-- <param name=Enable          value=false> -->
								</object></comment><script>__ws__(__NSID__);</script>
