...
function ln_Query(){
	...
	if(gcra_gubun.codevalue=="3"){ //부서장 
		...
		//본사
		gcds_data.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h060004_s6?v_str1="+gcem_yymm.text		//고과년월
    ...
    //prompt('',gcds_data.DataID);
		...
		ln_Reset('q');
		 ...
		 //본사외 
		gcds_data5.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h060004_s7?v_str1="+gcem_yymm.text		//고과년월
											+"&v_str2="+txt_empnoh.value //고과자	
											+"&v_str3="+txt_empnm.value	 //피고과자
											+"&v_str4="+gcra_gubun.CodeValue;	//정규직 계약직
    ...
    //본사
		gcds_data.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h060004_s1?v_str1="+gcem_yymm.text		//고과년월
														 +"&v_str2="+txt_empnoh.value	//고과자	
														 +"&v_str3="+txt_empnm.value	//피고과자
														 +"&v_str4="+gcra_gubun.CodeValue;	//정규직 계약직
		gcds_data.Reset(); 
		...
		ln_Reset('q');
    //본사외 
		gcds_data5.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h060004_s5?v_str1="+gcem_yymm.text		//고과년월
														 +"&v_str2="+txt_empnoh.value	//고과자	
														 +"&v_str3="+txt_empnm.value	//피고과자
														 +"&v_str4="+gcra_gubun.CodeValue;	//정규직 계약직
		gcds_data5.Reset(); 
...
function ln_Query2(){
	...
	if(gcra_gubun.codevalue=="3"){ //부서장 
		gcds_data2.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h060004_s8?v_str1="+gstr_yyyymm						//고과년월
		...
		gcds_data2.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h060004_s2?v_str1="+gstr_yyyymm						//고과년월
                                                          +"&v_str2="+gstr_empno						//피고과자사번
																													+"&v_str3="+gstr_grpcd					   //직급(과장이상/대리이하)
																													//+"&v_str4="+gcra_divcd.CodeValue//부문(능력고과/직업고과)
																													+"&v_str5="+gstr_jobgrph		      //공통&(사무직or기술직or영업직)
	...
	alert ("저장할 정보가 없습니다.");
		...
		if(ln_Chk()){
			if (confirm("저장하시겠습니까?")) {
				ln_Datasave();
				if(gcra_gubun.codevalue=="3"){ //부서장 
          gctr_data.action = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h060004_t2";
        }else{ //정규직, 계약직
					gctr_data.action = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h060004_t1";
				}
...
function ln_Corcal(){
  ...
  if(confirm("보정계산을 하시겠습니까?")){
		gcds_col.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h060004_p5?v_str1="+gcem_yymm.text		//고과년월
																												+"&v_str2="+txt_empnoh.value;	//고과자
		gcds_col.Reset(); 
	}
}
...
function ln_Corcal2(){
	...
	gcds_col2.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h060004_p6?v_str1="+gcem_yymm.text		//고과년월
														                          +"&v_str2="+txt_empnoh.value;	//고과자
	gcds_col2.Reset();
