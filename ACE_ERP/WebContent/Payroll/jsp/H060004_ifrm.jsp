...
function ln_Query(){
    ...
    if(gcds_dtl.namevalue(1,"WORKGB")==""&&gcds_dtl.namevalue(1,"WKCONT")==""&&gcds_dtl.namevalue(1,"IMPORT")==0){
			...
			//eval("txt_3"+i).value="※2007년 하반기 주요과제 및 실적(성과) 기술 \n (작성시 개인별 직무기술서, 업무추진계획등을 참조하여 작성)";
...
function ln_Data(i){
	...
	gcds_dtl.namevalue(i,"EMPNO") = gusrid;
	...
	gcds_dtl.namevalue(i,"IMPORT") = eval("txt_4"+i).value;
...
function ln_Print(){
  ...
  gcds_prthead.Addrow();
   ...
   //디테일
	...
	//예외처리
	if(parent.strParam[1]=="2030005"){
   gbn="4";
	}else if (parent.strParam[1]=="1990023"){
	 gbn="9";   //1페이지에 6칸 예외처리
	}
...
function ln_SetDataHeader(gbn){
	...
	if(gbn==1){       //업적신고MST
		...
		if (gcds_dtl.countrow<1){
			var s_temp = " EVAYM:STRING(6),EMPNO:STRING(7),SEQ2:DECIMAL(2),WORKGB:STRING(50),WKCONT:STRING(2048),"
