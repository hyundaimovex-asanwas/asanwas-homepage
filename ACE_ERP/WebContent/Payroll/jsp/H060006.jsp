...
<!-------------------------------------------------------------------------------
+ 시 스 템 명	:	인사고과관리  
+ 프로그램 ID	: H060006.html
+ 기 능 정 의	: 피고과자(부서장) - 고과자 지정 
+ 작  성   자 : 정 영 식
+ 작 성 일 자 : 2009.11.04
+ 변 경 이 력	: 
+ 서 블 릿 명	:                
...
function ln_Query(){
	...
	gcds_data.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h060006_s1?v_str1="+fn_trim(gcem_yymm.text)
...
function ln_Query2(){
	...
	gcds_data2.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h060006_s2?v_str1="+fn_trim(gcem_yymm.text)
																												+"&v_str2="+fn_trim(gclx_deptcd.bindcolval)
																												+"&v_str3="+fn_trim(gclx_paygrd.bindcolval)
																												+"&v_str4="+fn_trim(txt_name.value)
...
function ln_Save(){
	...
	if (gcds_data2.countrow<1){
		...
		if(ln_Chk()){
			if(ln_Chk2()){
			  if (confirm("저장하시겠습니까?")) {
					gctr_data.action = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h060006_t1";
					gctr_data.post();
					ln_Query2();
				}				
			} //if ln_Chk2
...
<table width="876" cellpadding="0" cellspacing="0" border="0" style="position:relative;left:3px">
  <tr> 
    <td  background="../../Common/img/com_t_bg.gif"><img src="../img/h060006_head.gif"></td>
