...
function ln_Query(){
	...
	gcds_data.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h060013_s1?v_str1="+fn_trim(gcem_yymm.text)
...
function ln_Query2(){
	...
	gcds_data2.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h060013_s2?v_str1="+fn_trim(gcem_yymm.text)
																												+"&v_str2="+fn_trim(gclx_deptcd.bindcolval)
																												+"&v_str3="+fn_trim(gclx_paygrd.bindcolval)
																												+"&v_str4="+fn_trim(txt_name.value)
		...
		if(ln_Chk()){
			if(ln_Chk2()){
			  if (confirm("저장하시겠습니까?")) {
					...
					gctr_data.action = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h060013_t1";
				//	prompt("gcds_data2",  gcds_data2.text );
				//
					gctr_data.post();
					ln_Query2();
