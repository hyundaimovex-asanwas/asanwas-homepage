...
<!-------------------------------------------------------------------------------
+ 시 스 템 명	:	계약직/급여항목제외관리
+ 프로그램 ID	: H100002
+ 기 능 정 의	: 조회|등록|저장|삭제
+ 변 경 이 력	: 이순미
+ 서 블 릿 명	: h100002_s1, h100002_t1
------------------------------------------------------------------------------>

<html>
<head>
    <jsp:include page="/Payroll/common/include/head.jsp"/>
<title>급여항목제외관리</title>
<!-----------------------------------------------------------------------------
...
function ln_Query(){
	...
	gcds_pay.DataID = "/servlet/Contract.h100002_s1?v_str1="+str1
																												+"&v_str2="+str2
																												+"&v_str3="+fn_trim(str3)
																												+"&v_str4="+str4
																												+"&v_str5="+str5;
...
//	prompt('',gcds_pay.text);
		if (confirm("입력하신 사항을 등록하시겠습니까?")){
			gctr_pay.KeyValue = "h100002_t1(I:USER=gcds_pay)";
			gctr_pay.Action = "/servlet/Contract.h100002_t1?v_str1="+str1+"&v_str2="+str2;
...
function ln_Delete() {
	...
	//prompt('',gcds_pay.text);
    gctr_pay.action = "/servlet/Contract.h100002_t1";
		gctr_pay.post();
		ln_Query();
	}
...
function ln_Luxequery1() { 
	...
	//prompt('gcds_paycd2.DataID:',gcds_paycd2.DataID);
	gcds_paycd2.Reset();

	//항목명
	gcds_paycd3.DataID = "/servlet/Contract.h100002_s2?v_str1="; 
	gcds_paycd3.Reset();
...
function ln_DeptQuery() { 
	//소속명
	//gcds_deptcd.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h010018_s3?v_str1=";
...
<table width="877" cellpadding="0" cellspacing="0" border="0" style="position:relative;left:3px">
  <tr> 
    <td background="../../Common/img/com_t_bg.gif"><img src="../img/h100002_head.gif"></td>
						...
						<td> 
							<comment id="__NSID__"><OBJECT id=gcgd_create classid=clsid:1F57AEAD-DB12-11D2-A4F9-00608CEBEE49 style="WIDTH:668px;HEIGHT:420px;border:1 solid #777777;display:block;"   viewastext>
								...
								<PARAM NAME="Format"			VALUE="  
									...
									<C> Name='지급구분'			ID=PAYDIV		HeadAlign=Center HeadBgColor=#B9D4DC Width=60		align=CENTER	Mask='정기급여'</C> 
									...
									<C> Name='항목명'				ID=PAYCD		HeadAlign=Center HeadBgColor=#B9D4DC Width=70		align=LEFT		Data='gcds_paycd3:PAYCD:PAYCDNM'		EditStyle=Lookup</C> 
			  ...
			  <fieldset id=ft_div1 style="width:670px;height:20px;background-color:#FFFFFF;border:1 solid #708090;border-top-width:0px;text-align:LEFT;">
                  ...
                  <td style="height:30px; border:0 solid #708090;border-bottom-width:1px;"><nobr>&nbsp;
										<comment id="__NSID__"><OBJECT id=gclx_paydiv classid=clsid:60109D65-70C0-425C-B3A4-4CB001513C69 height=106 width=110 style="position:relative;top:2px;" class="txtbox">
											<param name=CBData					value="T^정기급여,K^격려금">
											<param name=CBDataColumns		value="MINORCD,MINORNM">
											<param name=SearchColumn		value="MINORCD^MINORNM">
											<param name=Sort            value="TRUE">
											<param name=SortColumn			value="MINORCD">
