...
function ln_Start(){
	...
	if(window.dialogArguments[0]=="H050009"||window.dialogArguments[0]=="EMPNO"){
	   parentGubun ="";
	}else if(window.dialogArguments[0] != undefined && window.dialogArguments[0] != "H030003" && window.dialogArguments[0] != "H050005"&& window.dialogArguments[0] != "H050008" && window.dialogArguments[0] !="H100011" ) { 
	   parentGubun="P040001";
		...
		parentGubun="H050005";
	}else if(window.dialogArguments[0] == "H100011") {
		parentGubun="H100011";
		}
...
// alert("parentGubun::"+parentGubun+":::");
	...
	if(parentGubun=="") {	
		gcds_data.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h030003_s2?"
										 + "v_str1=" + txt_nm.value;
	}	else if(parentGubun == "H100011"){ //급여보험등록
			gcds_data.DataID = "/servlet/Contract.h100011_s4?"  //급여보험등록
											 + "v_str1=" + txt_nm.value; 
	  ...
	  if(window.dialogArguments[1]==""||window.dialogArguments[1]==undefined ){
			gcds_data.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h030003_s5?"  //야근특근 전체용
											 + "v_str1=" + txt_nm.value;
		}else{
			gcds_data.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h030003_s6?"  //야근특근 부서 관리자용
											 + "v_str1="+txt_nm.value
											 + "&v_str2="
		...
		if(window.dialogArguments[1]==""||window.dialogArguments[1]==undefined ){
			gcds_data.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h030003_s7?"  //근태 전체용
											 + "v_str1=" + txt_nm.value;
		}else{
			gcds_data.DataID = "<%=dirPath%><%=HDConstant.PATH_PERSON%>h030003_s8?"  //근태 부서 관리자용
											 + "v_str1="+txt_nm.value
											 + "&v_str2="
											 + "&v_str3="+window.dialogArguments[1];//사번
		}
