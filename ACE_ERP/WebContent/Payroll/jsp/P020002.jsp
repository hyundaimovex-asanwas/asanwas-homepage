...
<!-------------------------------------------------------------------------------
+ 시 스 템 명	:	급여/급여상여계산
+ 프로그램 ID	: p020002.HTML
+ 기 능 정 의	: 조회
+ 변 경 이 력	: 
+ 수 정 이 력	: 이순미 - 2005.08.02 - 소스 정리
+ 서 블 릿 명	: p020002_s1, p020002_s2
------------------------------------------------------------------------------>
...
function ln_Query(){
	...
	gcds_data.DataID = "<%=dirPath%><%=HDConstant.PATH_PAYROLL%>p020002_s2?"
									 + "v_str1=" + str1
									 + "&v_str2=" + str2;
	...
	for(i=1;i<=gcds_data.countrow;i++) {
		if (gcds_data.namevalue(row,"WRKSTS") == 'C') {
			...
			if (confirm("정보를 마감 하시겠습니까?")){
				...
				gctr_data.Action = "<%=dirPath%><%=HDConstant.PATH_PAYROLL%>p020002_t1?v_str1="+str1+"&v_str2="+str2;
				gctr_data.Post();
				ln_Query();
			}else{
				return false;
...
function ln_Create(){
	...
	if(str1.length < 6 ) 	{
		...
		gcds_data.ClearData();
		var prcedure_gubun="<%=dirPath%><%=HDConstant.PATH_PAYROLL%>p020002_s1?"
											 + "v_str1=" + str1
											 + "&v_str2=" + str2
											 + "&v_str3=" + str3;
	if(str2=="1") {
		prcedure_gubun+= "&v_str4=SP_P100000";
	}else if(str2=="2")	{		
		prcedure_gubun+= "&v_str4=SP_P200000";
	}else if(str2=="4")	{		
		prcedure_gubun+= "&v_str4=SP_P400000";
	}else if(str2=="5"||str2=="6")	{		
	 // alert("str2::"+str2);
		prcedure_gubun+= "&v_str4=SP_P500000";
		prcedure_gubun+= "&v_str5="+str4;
	}

		gcds_select_create.DataID =prcedure_gubun;
	//	prompt('',gcds_select_create.DataID);
...
<script language="javascript" for="gcds_select_create" event="onloadcompleted(row,colid)">
	...
	if(gcds_select_create.countrow <=0)
		...
		alert(gclx_select_ec01az.Text+" 계산이 정상적으로 구동 되었으니\n조회 버튼을 사용하여 진행상태를 조회하시기 바랍니다.");
		else
		alert(gclx_select_ec01az.Text+" 계산이 정상적으로 구동 되지 않았습니다.");
		}
	window.status="";
</script>
...
<table width="876" cellpadding="0" cellspacing="0" border="0" style="position:relative;left:3px">
  <tr> 
    <td ><img src="../img/p020002_head.gif"></td>
