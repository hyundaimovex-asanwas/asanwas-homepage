...
<!-------------------------------------------------------------------------------
+ 시 스 템 명	:	급여/급여상여세엑
+ 프로그램 ID	: p020004.HTML
+ 기 능 정 의	: 조회|계산
+ 변 경 이 력	: 
+ 수 정 이 력	: 이순미 - 2005.08.02 - 소스 정리
+ 서 블 릿 명	: p020004_s1, p020004_s2
------------------------------------------------------------------------------>
...
function ln_Query(){
	...
	gcds_data.DataID = "<%=dirPath%><%=HDConstant.PATH_PAYROLL%>p020004_s2?"
									 + "v_str1=" + str1
									 + "&v_str2=" + str2;
									 //+ "&v_str3=" + str3;
	gcds_data.Reset(); //그리드
...
function ln_Create(){
  ...
  if(str1.length < 6 ) 	{
	...
	/*}else if(str3.length < 8){
		...
		gcds_data.ClearData();
    var prcedure_gubun="<%=dirPath%><%=HDConstant.PATH_PAYROLL%>p020004_s1?"
											 + "v_str1=" + str1
											 + "&v_str2=" + str2
											 + "&v_str3=";

	 if(str2=="1") {
		prcedure_gubun += "&v_str4=SP_P100002";   //정기
	}else if(str2=="2")	 {		
		prcedure_gubun += "&v_str4=SP_P200002";  //소급
	}else if(str2=="4")	 {		
		prcedure_gubun += "&v_str4=SP_P400002";  //휴직
	}	else if(str2=="5"  ) {		
		prcedure_gubun += "&v_str4=SP_P500002";  //상여
	}
...
<script language="javascript" for="gcds_select_create" event="OnLoadStarted()">
	...
	window.status==gclx_select_ec01az.Text+" 세액 계산중입니다.";
	//document.all.LowerFrame.style.visibility="visible";
</script>

<script language="javascript" for="gcds_select_create" event="onloadcompleted(row,colid)">
	//ft_cnt1.innerText = "조회건수 : " + row + " 건";
	window.status=gclx_select_ec01az.Text+" 세액 계산 프로그램을 완료했습니다.";
	//document.all.LowerFrame.style.visibility="hidden";
	if(gcds_select_create.countrow <=0)
			alert("리턴값이 없습니다.");
	else {
		if(gcds_select_create.NameValue(1,"RETCOD") =="Y")
		alert(gclx_select_ec01az.Text+" 세액 계산 프로그램이 정상적으로 구동 되었으니\n조회 버튼을 사용하여 진행상태를 조회하시기 바랍니다.");
		else
		alert(gclx_select_ec01az.Text+" 세액 계산 프로그램이 정상적으로 구동 되지 않았습니다.");
	}
	window.status="";
</script>
...
<table width="876" cellpadding="0" cellspacing="0" border="0" style="position:relative;left:3px">
  <tr> 
    <td ><img src="../img/p020004_head.gif"></td>
					...
					<tr> 
						...
						<td style="height:30px;border:0 solid #708090;border-right-width:0px;"><nobr>
							<comment id="__NSID__"><object id=gclx_select_ec01az  name="gclx_select_ec01az" classid=clsid:60109D65-70C0-425C-B3A4-4CB001513C69 height=106 width=50 style="position:relative;left:8px;top:2px;font-size:12px;width:100px;height:200px;">
								...
								<param name=CBData					value="1^정기급여,2^소급급여,4^휴직급여,5^상여급여,,6^특별상여">
						...
						<td style="height:30px;border:0 solid #708090;border-right-width:0px;padding-left:3px;padding-right:3px;" bgcolor="#FFFFFF"><nobr>&nbsp;<!--지급일자-->&nbsp;</nobr></td>	
    ...
    <td colspan="2"> 
			<comment id="__NSID__"><OBJECT id=gcgd_hcl classid=clsid:1F57AEAD-DB12-11D2-A4F9-00608CEBEE49  style="WIDTH: 875px; HEIGHT: 423px;border:1 solid #777777" viewastext>
				...
				<PARAM NAME="Format"			VALUE="
					...
					<C> Name='시작시간'		ID=FRDT  		HeadAlign=Center HeadBgColor=#B9D4DC Width=70	 align=center Mask='XX:XX:XX' </C> 
