...
function ln_Query(){
  ...
  if (ln_Trim(txt_dong2.value) == "") {
		...
		gcds_zipcode.DataID = "/servlet/Person.h010002_s1?v_str1=" + v_str1; 
		gcds_zipcode.Reset();
	}
...
function ln_Query2(){
	...
	if (gcds_sendal.namevalue(gcds_sendal.rowposition,"USEYN") == "T") {
		...
		gcds_sendal.DataID = "/servlet/Person.h010002_s2?v_str1=";
		gcds_sendal.Reset(); 
...
function ln_Query3(){
		...
		gcds_holiday.DataID = "/servlet/Person.h010002_s3?v_str1="+str1; 
		gcds_holiday.Reset(); 
...
function ln_Query4(){
		...
		gcds_special.DataID = "/servlet/Person.h010002_s4?v_str1="; 
		gcds_special.Reset(); 
...
function ln_Save(){
	...
	if (gcds_zipcode.IsUpdated) {	
		if (confirm("입력하신 사항을 등록하시겠습니까?")){
			...
			gctr_zipcode.Action = "/servlet/Person.h010002_t1";
			gctr_zipcode.post();
			return ln_Query();
		}
	}
...
function ln_Save2(){
	...
	if (gcds_sendal.IsUpdated) {	
		if (confirm("입력하신 사항을 등록하시겠습니까?")){
			...
			gctr_sendal.Action = "/servlet/Person.h010002_t2";
			gctr_sendal.post();
			return ln_Query2();
		}
	}
...
function ln_Save3(){
	...
	if (gcds_holiday.IsUpdated) {	
		if (confirm("입력하신 사항을 등록하시겠습니까?")){
			...
			gctr_holiday.Action = "/servlet/Person.h010002_t3";
			gctr_holiday.post();
			return ln_Query3();
		}
	}
...
function ln_SaveBatch(){
	...
	if (gcds_batch.IsUpdated) {	
		if (confirm("입력하신 사항을 등록하시겠습니까?")){
			...
			gctr_batch.Action = "/servlet/Person.h010002_t5";
			gctr_batch.post();
			return ln_Query3();
		}
	}
...
function ln_Save4(){
	...
	if (gcds_special.IsUpdated) {	
		if (confirm("입력하신 사항을 등록하시겠습니까?")){
			...
			gctr_special.Action = "/servlet/Person.h010002_t4";
			gctr_special.post();
			return ln_Query4();
		}
	}
...
function ln_DelBatch(){
	...
	if (confirm("선택하신 사항을 삭제하시겠습니까?")){
			...
			gctr_holiday.Action = "/servlet/Person.h010002_t5?v_str1="+str1;
			gctr_holiday.post();
			return ln_Query3();
		}else gcds_holiday.undo(gcds_holiday.rowposition);		
...
function ln_Luxequery(){
	...
	//회사휴일관리 - 요일
	gcds_whatday.DataID = "/servlet/Person.hclcode_s1?v_str1=1151"; 
	gcds_whatday.Reset();

	//회사휴일관리 - 휴일구분
	gcds_holdiv.DataID = "/servlet/Person.hclcode_s1?v_str1=1150"; 
	gcds_holdiv.Reset();
