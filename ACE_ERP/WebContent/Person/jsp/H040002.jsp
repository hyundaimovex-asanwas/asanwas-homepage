...
function ln_Query(){
	...
	gcds_data.DataID = "/servlet/Person.h040002_s1?"
...
function ln_PrintQuery(){
	...
	gcds_data_p.DataID = "/servlet/Person.h040002_s3?"
								 + "v_str1=" + gclx_dept.bindcolval
								 + "&v_str2=" + gclx_ordcd.bindcolval
								 + "&v_str3=" + gcem_fr.text
								 + "&v_str4=" + gcem_to.text
...
//	} else {
		if (confirm("해당 인사 발령요청 사항을 저장하겠습니까 ?")) {
			gcds_tr.DataID = "/servlet/Person.h040002_s2?"
										 + "v_str1=" + gclx_dept.bindcolval
										 + "&v_str2=" + gclx_ordcd.bindcolval
										 + "&v_str3=" + gcem_fr.text
										 + "&v_str4=" + gcem_to.text
...
function ln_Delete(){
	...
	if(gcds_data.namevalue(row,"APPDT") == "") {			
		if (confirm("선택하신 사항을 삭제하시겠습니까?")) {
			...
			gctr_data.Action = "/servlet/Person.h040002_t1";
			gctr_data.post();
			return ln_Query();
		}
	} else {
...
function ln_Update(){
	...
	if (gcds_data.IsUpdated ) {
		if (confirm("입력하신 비고란을 저장하시겠습니까?")) {
				...
				gctr_data.Action = "/servlet/Person.h040002_t2";
				//prompt('',gcds_data.text);
				gctr_data.post();			
				//return ln_Query();
		}
