...
function fnOnLoad(tree_idx){
  ...
  gclx_closeyn.index=0;
	...
	gcem_frdt_2.text = gs_date2;
	gcem_todt_2.text = gs_date2;
}
...
function ln_Query(){
	...
	var v_str1=gcem_frdt.text;
	var v_str2=gcem_todt.text;
	var v_str4=v_str1.substring(0,4)+v_str1.substring(4,6);  //년월
	var v_str5=v_str2.substring(0,4)+v_str2.substring(4,6);  //년월
	var v_str6=fn_trim(gcem_empno_fr.Text);
	var v_str7=fn_trim(gcem_empno_to.Text);
	var v_str8="";//fn_trim(txt_empnmk.value);
   /*
	...
	gcds_data.DataID = "/servlet/Person.h050008_s1?"
	                 + "v_str1="  + gcem_frdt.text
									 + "&v_str2=" + gcem_todt.text
  ...
  /*
	...
	gcem_frtime_1.enable = false;			//출근시간
	gcem_totime_1.enabled = false;			//퇴근시간
	gcem_frdt_1.enable = false;				//시작일
...
function ln_Before(){
	...
	// 근무구분
	/*
	gcds_lazcd_1.DataID = "/servlet/Person.h050001_s2";
	gcds_lazcd_1.Reset();
  */
  //근태구분
	/*
	gcds_lazcd.DataID = "/servlet/Person.hclcode_s1?v_str1=1124&v_str2=S";
	gcds_lazcd.Reset();

	gcds_lazcd_1.DataID = "/servlet/Person.hclcode_s1?v_str1=1124&v_str2=S";
	gcds_lazcd_1.Reset();
  */
	...
	gcds_lazcd.DataID =  "/servlet/Person.h050001_s2";
	gcds_lazcd.Reset();
	...
	//휴일근무
	gcds_holiday.DataID = "/servlet/Person.h010002_s3?v_str1="+gs_date; 
	gcds_holiday.Reset();

			//당직관리자
	gcds_watch.DataID = "/servlet/Person.hclcode_s2?v_str1=2058&v_str2="+gusrid;
	gcds_watch.Reset();	
...
function ln_Save(){
	...
	if (gcds_data.IsUpdated) {
		if (confirm("저장 하시겠습니까?")){	
			gctr_data.Action = "/servlet/Person.h050001_t1?v_str1="+ v_str1+"&v_str2="+v_str2 + "&v_str3=" + v_str3;
 ...
 var row = gcds_data.rowposition;
  ...
  if(!ln_Chk_Delete())return;
	...
	if (confirm("선택한 정보를 삭제하시겠습니까?")){
		...
		gctr_data.action = "/servlet/Person.h050001_t1?v_str1=" + gcem_todt.text+ "&v_str2=" + str2;
		gctr_data.post();
		ln_Query();
	}
 }
...
******************************************************************************/
function ln_Excel(){
	gcgd_disp01.GridToExcel(gclx_lazcd.text,"",2);
}
...
function ln_Calendar(gbn){
	if(gbn=="fr"){
		...
		__GetCallCalendar('gcem_todt_2','Text');
	}
}
...
function ln_Amt_Setting(){
	...
	//특근(휴일)
	if (gcds_lazcd_1.namevalue(row,"LAZCD") == 'S05'||gcds_lazcd_1.namevalue(row,"LAZCD") == 'S07'||gcds_lazcd_1.namevalue(row,"LAZCD") == 'S08' ) {	
		if (gcds_data2.namevalue(gcds_data2.rowposition,"MEMO") == '20') {       //과장이상 지급금액
			gcem_amt_1.text = gcds_lazcd_1.namevalue(row,"MEMO");		               //40000
		}else if (gcds_data2.namevalue(gcds_data2.rowposition,"MEMO") == '22') { //대리이하 지급금액
			gcem_amt_1.text = gcds_lazcd_1.namevalue(row,"AMT");		               //30000
		}else{
      gcem_amt_1.text = 0;		
		} 
 	}else{ // 그 이외의 경우
	...
	if(gcds_lazcd_1.namevalue(row,"LAZCD")=="S10"||gcds_lazcd_1.namevalue(row,"LAZCD")=="S11"||gcds_lazcd_1.namevalue(row,"LAZCD")=="S12"||
	   gcds_lazcd_1.namevalue(row,"LAZCD")=="S13"||gcds_lazcd_1.namevalue(row,"LAZCD")=="S14"){
  ...
  if(gcem_frdt_2.text==""||gcem_todt_2.text==""){
	...
	if (gcds_batch.IsUpdated) {
		if(confirm("생성 하시겠습니까?")){	
			gctr_data2.Action = "/servlet/Person.h050008_t1?";
...
function ln_Chk_Create(){
	gcds_chk_batch.DataID = "/servlet/Person.h050008_s2?"
...
function ln_FindEmp(e,e2){
	...
	if(e2=='2')	{   
		...
		gcds_select_ec01az.DataID = "/servlet/Payroll.p040001_s1?"+ "v_str1=" + str1;
...
function ln_Chk_Submit(){
	...
	for(i=1;i<=gcds_data.countrow;i++){
		if(gcds_data.namevalue(i,"CHK")=="T"){
			...
			//야근특근의 경우 동일한 부서만 결재 가능...
			for(j=1;j<=gcds_data.countrow;j++){
				if(gcds_data.namevalue(j,"CHK")=="T"){
					if(gcds_data.namevalue(i,"DIVCD")!=gcds_data.namevalue(j,"DIVCD")){
						alert("동일한 부서만 결재 가능합니다.");
						return false;
					}
				}
			}
	...
	for(i=1;i<=gcds_data.countrow;i++){
       ...
       //출장관리에서 자동 생성된 경우 수정건의 경우 수정불가
			if(gcds_data.namevalue(i,"LAZCD")=="S05"&&(gcds_data.namevalue(i,"OTLAZCD")!=""&&gcds_data.namevalue(i,"OTLAZCD")!=null)){
				alert("출장관리에서 자동 생성된 건입니다. 삭제할 수 없습니다.");
				return false;
			}
			cnt+=1;
		}
...
/******************************************************************************
	Description : 당직자별 근태코드 설정
	Parameter :
******************************************************************************/
function ln_Lazcd_Setting(param_ds){

	if(gcds_watch.namevalue(gcds_watch.rowposition,"GUBUN")=="A"){       //당직관리자
		eval(param_ds).ClearAllMark();
		for(i=1;i<=eval(param_ds).countrow;i++){
			if(eval(param_ds).namevalue(i,"LAZCD")!="S08"){
				eval(param_ds).MarkRows(i,i);
			}
		}
		eval(param_ds).DeleteMarked();
	}else if(gcds_watch.namevalue(gcds_watch.rowposition,"GUBUN")=="B"){ // 영업당직(평일, 휴일)
		eval(param_ds).ClearAllMark();
		for(i=1;i<=eval(param_ds).countrow;i++){
			if(eval(param_ds).namevalue(i,"LAZCD")!="S06"&&eval(param_ds).namevalue(i,"LAZCD")!="S07"){
				eval(param_ds).MarkRows(i,i);
			}
		}
		eval(param_ds).DeleteMarked();
	}else if(gcds_watch.namevalue(gcds_watch.rowposition,"GUBUN")=="C"){ //신문당직
		eval(param_ds).ClearAllMark();
		for(i=1;i<=eval(param_ds).countrow;i++){
			if(eval(param_ds).namevalue(i,"LAZCD")!="S04"){
				eval(param_ds).MarkRows(i,i);
			}
		}
		eval(param_ds).DeleteMarked();
...
<!-- 현재 시간정보 -->
<OBJECT id=gcds_curtime classid=clsid:3267EA0D-B5D8-11D2-A4F9-00608CEBEE49 VIEWASTEXT>
	<PARAM NAME="SyncLoad" VALUE="true">
</OBJECT>

<!-- 휴일근무 -->
<OBJECT id=gcds_holiday classid=clsid:3267EA0D-B5D8-11D2-A4F9-00608CEBEE49 VIEWASTEXT>
	<PARAM NAME="SyncLoad"	VALUE="false">
</OBJECT>
...
<OBJECT id=gcds_manager classid=clsid:3267EA0D-B5D8-11D2-A4F9-00608CEBEE49 VIEWASTEXT>
	<PARAM NAME="SyncLoad" VALUE="false"><!-- 관리자구분 -->
</OBJECT>
...
<OBJECT id=gcds_watch classid=clsid:3267EA0D-B5D8-11D2-A4F9-00608CEBEE49 VIEWASTEXT>
	<PARAM NAME="SyncLoad" VALUE="false"><!--근태당직관리자-->
</OBJECT>
...
<script language="javascript" for="gcds_watch" event="onloadCompleted(row,colid)">
	//당직관리자 구분
	if(row>=1){
		ln_Lazcd_Setting(gcds_lazcd_1);
		ln_Lazcd_Setting(gcds_lazcd);
	}
</script>
          ...
          <td class="tab18" style="width:140px;" align=center><nobr>&nbsp;
				...
				<tr>
					...
					<td class="tab19" ><nobr>&nbsp;
						...
						style="position:relative;left:0px;top:2px;font-size:12px;width:190px;"  >
	  ...
	  <td COLSPAN=2>
			<table>
              ...
              <td class="tab13" style="text-align:left"><nobr>
						...
						<tr>
						  <td colspan=2 class="tab13" height=50px  style="text-align:left"><nobr>
							   <font style="position:relative;top:0px;left:0px; color=#0000FF;">[야근자동생성]</font>&nbsp;&nbsp;
								 <img src="../../Common/img/btn/com_b_create.gif"		style="cursor:hand;position:relative;top:2px;left:80px;" onclick="ln_Auto_Create()">
							</nobr></td>
						</tr>
						<tr>
              <td colspan=2 style="text-align:left"><nobr>
							 <font style="position:relative;top:-2px;left:2px; color=#0000FF;">일자:</font>
								<comment id="__NSID__">
									<object  id=gcem_frdt_2 classid=CLSID:E6876E99-7C28-43AD-9088-315DC302C05F  style="width:70px;height:20px;position:relative;left:0px;top:3px">		
										<param name=Text					value="">
										<param name=Alignment     value=0>
										<param name=Border        value=true>
										<param name=Format        value="YYYY/MM/DD">
									 ...
									 <img src="../img/calender.gif" value="calender.gif" id=img_calendar1 OnClick="ln_Calendar('fr2')" style="position:relative;width:20px;left:0px;cursor:hand;">~  
								<comment id="__NSID__">
									<object  id=gcem_todt_2 classid=CLSID:E6876E99-7C28-43AD-9088-315DC302C05F  style="width:70px;height:20px;position:relative;left:0px;top:3px">		
										<param name=Text					value="">
										<param name=Alignment     value=0>
										<param name=Border        value=true>
										<param name=Format        value="YYYY/MM/DD">
									 ...
									 <img src="../img/calender.gif" value="calender.gif" id=img_calendar1 OnClick="ln_Calendar('to2')" style="position:relative;width:20px;left:0px;cursor:hand;">  		
							</nobr></td>
            </tr>
            <tr>
						  <td><nobr>
