...
function ln_Query(){
	...
	gcds_data.DataID = "/servlet/Person.h060001_s1?v_str1=" + gcem_yymm.text+"&v_str2="+gclx_grpcd.bindcolval; 
    ...
    if (gcds_data.countrow<1){
		...
		if(ln_Chk()){
			if (confirm("저장하시겠습니까?")) {
				...
				gctr_pyo.action = "/servlet/Person.h060001_t1";
				gctr_pyo.post();
				ln_Query();
			}
		}//if ln_Chk
...
function ln_Batch(){
    ...
    gcem_yymm3.focus();
		...
		gcds_batchchk.DataID = "/servlet/Person.h060001_s3?v_str1=" + fn_trim(gcem_yymm3.text) ;
...
function ln_Luxecombo_load(){
 ...
 //과장이상 - 대리이하
	gcds_grpcd.DataID = "/servlet/Person.hclcode_s1?v_str1=2014"; 
	//prompt("gcds_grpcd", gcds_grpcd.DataID);
	gcds_grpcd.Reset();

	gcds_grpcd2.DataID = "/servlet/Person.hclcode_s1?v_str1=2014"; 
	//prompt("gcds_grpcd", gcds_grpcd.DataID);
	gcds_grpcd2.Reset();
  ...
  //공통-사무-기술-영업직
	gcds_jobgrphcd.DataID = "/servlet/Person.hclcode_s1?v_str1=2016"; 
	gcds_jobgrphcd.Reset();

  //평가요소
	gcds_grpitemcd_temp.DataID = "/servlet/Person.hclcode_s1?v_str1=2019"; 
  gcds_grpitemcd_temp.Reset();

	gcds_grpitemcd.DataID = "/servlet/Person.hclcode_s1?v_str1=2019"; 
	//prompt("",gcds_grpitemcd.Data);
	gcds_grpitemcd.Reset();

	gcds_samp.DataID = "/servlet/Person.hclcode_s1?v_str1=2019"; 
	gcds_samp.Reset();
...
<script language=JavaScript  for=gcds_batchchk event=OnLoadCompleted(rowcount)>
	...
	if(rowcount>=1) {
			...
			gcds_batch.DataID = "/servlet/Person.h060001_s4?";
	...
	if(rowcount>=1) {
		...
		if (confirm(gcem_yymm3.text+ "고과표를 생성하시겠습니까?")) {
          ...
          gcds_batch.namevalue(i,"EVAYM") =  gcem_yymm3.text;
			...
			gctr_batch.action = "/servlet/Person.h060001_t2";
