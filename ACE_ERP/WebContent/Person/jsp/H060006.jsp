...
function ln_Query(){
	...
	gcds_data.DataID = "/servlet/Person.h060006_s1?v_str1="+fn_trim(gcem_yymm.text)
...
function ln_Query2(){
	...
	gcds_data2.DataID = "/servlet/Person.h060006_s2?v_str1="+fn_trim(gcem_yymm.text)
																												+"&v_str2="+fn_trim(gclx_deptcd.bindcolval)
																												+"&v_str3="+fn_trim(gclx_paygrd.bindcolval)
																												+"&v_str4="+fn_trim(txt_name.value)
...
function ln_Save(){
	...
	if (gcds_data2.countrow<1){
		...
		if(ln_Chk()){
			if(ln_Chk2()){
			  if (confirm("저장하시겠습니까?")) {
					gctr_data.action = "/servlet/Person.h060006_t1";
					gctr_data.post();
					ln_Query2();
				}				
			} //if ln_Chk2
