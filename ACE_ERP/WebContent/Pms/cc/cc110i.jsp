...
<script language="javascript">
	...
	var v_temp1 = ""; //계정정보 Temp를 지속적으로 갖고 있기 위한 변수 선언
	var v_temp2 = ""; //계정정보 Temp를 지속적으로 갖고 있기 위한 변수 선언
	var v_temp3 = ""; //계정정보 Temp를 지속적으로 갖고 있기 위한 변수 선언
	var v_temp4 = ""; //계정정보 Temp를 지속적으로 갖고 있기 위한 변수 선언	
	var v_temp5 = ""; //계정정보 Temp를 지속적으로 갖고 있기 위한 변수 선언
	var v_temp6 = ""; //계정정보 Temp를 지속적으로 갖고 있기 위한 변수 선언
	var v_temp7 = ""; //계정정보 Temp를 지속적으로 갖고 있기 위한 변수 선언
	var v_temp8 = ""; //계정정보 Temp를 지속적으로 갖고 있기 위한 변수 선언	
	var v_temp9 = ""; //계정정보 Temp를 지속적으로 갖고 있기 위한 변수 선언	
	...
	function fnDelRowDt1(){
		...
		v_temp1 	= v_groupnm.value;
		v_temp2 	= v_atcodenm.value;
		v_temp3 	= v_atcode.value;
		v_temp4 	= v_remark.value;
		v_temp5 	= v_pl_dept_nm.value;
		v_temp6 	= v_pl_dept.value;
		v_temp7 	= v_pl_dept_div.value;
		v_temp8 	= v_refcdnm.value;
		v_temp9 	= v_refcd.value;			
		...
		if (ds_culd.IsUpdated||ds_cul.IsUpdated) {
			...
			if(ln_Chk()){
				...
				if(v_memory_chk.checked){//체크면. 1행의 값만 복사해서 보관
					...
					v_groupnm.value 	= v_temp1;
					v_atcodenm.value 	= v_temp2;
					v_atcode.value 		= v_temp3;
					v_remark.value 		= v_temp4;
					v_pl_dept_nm.value 	= v_temp5;
					v_pl_dept.value 	= v_temp6;
					v_pl_dept_div.value = v_temp7;
					v_refcdnm.value 	= v_temp8;
					v_refcd.value 		= v_temp9;
	...
	function ln_Chk() {
		...
		/* 2020.12.04 부분 취소, 부분 정산때문에 그냥 풀어줬다. 이창기 대리가 요청했고 이동훈이 수정해줌
		...
		*/
		...
		for(i=1;i<=row1;i++) {//계정|귀속정보 체크
			...
			//if(matching.match(' ')){
			//	alert("공백 blank를 빼시기 바랍니다");
			//	ds_culd.namevalue(i,"GROUP_NM") = ds_culd.namevalue(i,"GROUP_NM").replaceAll(' ', '');
			//}			
		...
		if(ds_culd.namevalue(1,"GROUP_NM") != undefined) {
			...
			if(v_memory_chk.checked){//체크면. 1행의 값만 복사해서 보관
				...
				v_temp1 		= ds_culd.namevalue(1,"GROUP_NM");
				v_temp2 		= ds_culd.namevalue(1,"ATCODENM");
				v_temp3 		= ds_culd.namevalue(1,"ATCODE");
				v_temp4 		= ds_culd.namevalue(1,"REMARK");
				v_temp5 		= ds_culd.namevalue(1,"PL_DEPT_NM");
				v_temp6			= ds_culd.namevalue(1,"PL_DEPT");
				v_temp7 		= ds_culd.namevalue(1,"PL_DEPT_DIV");
				v_temp8 		= ds_culd.namevalue(1,"REFCDNM");
				v_temp9 		= ds_culd.namevalue(1,"REFCD");					
			...
			/*
			alert("v_groupnm.value ==== " + v_groupnm.value);
...
<!--------------------------------- 코딩 부분 시작 ------------------------------->		
    <table border='0' cellpadding='0' cellspacing='0'> 
        ...
        <tr>
            <td class="r">
           		<table  border='0' cellpadding="0" cellspacing="1" bgcolor="#666666" width='1052px'>
           			<tr>
                        ...
                        <td bgcolor="#FFFFFF" >&nbsp;
                            <object id=v_frdt classid=<%=HDConstant.MxMaskEdit_CLSID_S%>  class="textbox" style= "width:90px;">
                            ...
                            <object id=v_todt classid=<%=HDConstant.MxMaskEdit_CLSID_S%>  class="textbox" style= "width:90px;">
                                <param name=Text        value="">
                                <param name=Alignment   value=1>
                                <param name=VAlign	    value=1>
                                <param name=Format      value="YYYY-MM-DD">
                        ...
                        <td align="left" bgcolor="#ffffff">&nbsp;
                            <object id=lc_gc_ccstat classid=<%=HDConstant.MxCombo_CLSID_S%> width=100 border="0"  align=absmiddle>
        ...
        <tr>
            <td valign="top">
                ...
                <object id=gr_cul classid=<%=HDConstant.MxGrid_CLSID_S%> width='1050px' height='370px' border='1'>
                    ...
                    <param name="Format"            value="
                        ...
                        <FC> name='카드번호'	 		ID='CARD_NUM'  	 	 	width=200 	align=CENTER	HeadBgColor='#5C7CAC' edit=none mask='XXXX-XXXX-XXXX-XXXX' sort=true </FC>
					  
					  <G> name='거래처 정보 (ERP)' 		 
                        <C> name='상호'					ID='VEND_NM'   		 	width=150 	align=left		edit=none sort=true bgColor={IF(ADDR1='미등록거래처','#ff3300',IF(ADDR1='(등록중)미등록거래처','#FFB4B4','#FFFFFF'))}  value={IF(ADDR1='미등록거래처','미등록거래처',IF(ADDR1='(등록중)미등록거래처','(등록중)미등록거래처',VEND_NM))}
                      ...
                      <G> name='이용정보'
                        ...
                        <C> name='금액'     			ID='AMT_TOTAL'			width=70 	align=right		Edit=true	 decao=0  sort=true  sumtext=@sum </C>
                      </G>
                      
                        <C> name='공급가액'     		ID='AMT_NET'			width=70 	align=right		Edit=true	 decao=0   sumtext=@sum		bgColor='#ffff00' sort=true</C>
					  ...
					  <G> name='가맹점 정보'
					    ...
					    <C> name='처리그룹'    			ID='GROUP_NM'			width=200 	align=left		edit=none	 </C>
					    <C> name='원가명'    			ID='PL_DEPT_NM'			width=200 	align=left		edit=none	 </C>					    
					    
					    
					    <C> name='계정코드'    			ID='ATCODE'				width=100 	align=center	edit=none	 </C>
					    <C> name='계정명'    			ID='ATCODENM'			width=200 	align=left		edit=none	 </C>
		...
		<tr>
			...
			<br>
				계정정보<input type="checkbox" name='v_memory_chk' id="v_memory_chk" onclick="fn_check();">
				<input type="text" name="v_groupnm" 	id="v_groupnm" 	   style="width:140px;" class="input01"  readOnly="readonly" value="">
				<input type="text" name="v_pl_dept_nm" 	id="v_pl_dept_nm"  style="width:110px;" class="input01"  readOnly="readonly" value="">
				<input type="text" name="v_pl_dept" 	id="v_pl_dept" 	   style="width:45px;" class="input01"  readOnly="readonly" value="">				
				...
				<input type="text" name="v_refcdnm" 	id="v_refcdnm"     style="width:100px;" class="input01"  readOnly="readonly" value="">				
				<input type="text" name="v_refcd" 		id="v_refcd" 	   style="width:45px;" class="input01"  readOnly="readonly" value="">
				<input type="text" name="v_remark" 		id="v_remark" 	   style="width:150px;" class="input01"  readOnly="readonly" value="">		
        ...
        <tr>
            <td valign="top">
                <object id=gr_culd classid=<%=HDConstant.MxGrid_CLSID_S%> width='1050px' height='50px' border='1'>
                    <param name="DataID"            value="ds_culd">
                    <param name="Editable"          value="true">
                    <param name="BorderStyle"       value="0">
                    <param name="SuppressOption"    value="1">
                    <param name="Format"            value="
						<C> name='No'				ID='{CurRow}'		width=40 	align=center 	HeadBgColor='#5C7CAC' edit=none sumtext=@cnt </FC>
						<C> name='처리 그룹명'		ID='GROUP_NM'		width=150 	align=left   	HeadBgColor='#5C7CAC' </FC>
                        <C> name='귀속정보(원가명)'	ID='PL_DEPT_NM' 	width=200 	align=left 		EditStyle=popup </C>
                        ...
                        <C> name='세목명'			ID='REFCDNM'		width=150 	align=left 		EditStyle=popup </C>
        ...
        <tr>
            <td valign="top"><br>
            	...
            	※ 처리 프로세스<br>
            　			1. 개인 사용　 : 비용 처리 하지 않을 사용 건을 먼저 [개인사용] 처리합니다.<br>
            　			2. 접 대 비　　: 접대비로 처리할 사용 건은 공제여부를 체크→체크해제하여 <b>부가세를 0</b>으로 만들어야 합니다.<BR>
 			　	3. 공제 처리<br>
			...
			　	4. 계정 정보　 : 공제여부 처리를 마친 후에 각 사용 건의 계정정보를 저장하면 상태가 [처리]로 변경됩니다.<br>
			　	5. 매입 생성　 : [일괄매입] 버튼을 누르면, 공제여부가 체크된 건 모두 계산서정보를 자동으로 생성합니다.
            </td>
        </tr>
