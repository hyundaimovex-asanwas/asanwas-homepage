	    ...
	    */
		...
		function fnSelect() {
	        ...
	        var param = "proType=<%=HDConstant.PROCESS_TYPE_SEARCH%>"
		     ...
		     ln_TRSetting(tr1,"<%=dirPath%><%=HDConstant.PATH_HELP%>Pr060H", "JSP(O:DS_DEFAULT1=ds_default1)",  param);
		     tr_post(tr1);			
	        ...
	        var param = "proType=<%=HDConstant.PROCESS_TYPE_SEARCH%>"
									            ...
									            + ",v_cd_site="+g_WinParam[4]
									            + ",v_no_mr="+g_WinParam[5];        
		     ln_TRSetting(tr1,"<%=dirPath%><%=HDConstant.PATH_HELP%>Pr060H", "JSP(O:DS_DEFAULT2=ds_default2)",  param);
		     tr_post(tr1);
		     ...
		     var param = "proType=<%=HDConstant.PROCESS_TYPE_SEARCH%>"
                 				+ ",v_ap_m_sid="+g_WinParam[3];
		     ln_TRSetting(tr1,"<%=dirPath%><%=HDConstant.PATH_HELP%>Pr060H", "JSP(O:DS_DEFAULT3=ds_default3)",  param);
		     tr_post(tr1);
		     
			 // (본사)결재정보		    
		     var param = "proType=<%=HDConstant.PROCESS_TYPE_SEARCH%>"
                 				+ ",v_ap_m_sid="+g_WinParam[3];
		     ln_TRSetting(tr1,"<%=dirPath%><%=HDConstant.PATH_HELP%>Pr060H", "JSP(O:DS_DEFAULT4=ds_default4)",  param);
		     tr_post(tr1);
		...
		function fnApply_2(){
			...
			if (confirm("반송하시겠습니까?")) {
				...
				ln_TRSetting(tr3,"<%=dirPath%><%=HDConstant.PATH_HELP%>Pr010H","JSP(O:msgDS=msgDS)", param);
				tr_post(tr3); 		
	...
	<td height=25 width=950 colspan =2>
		...
		<object id=gr_RcpDpt classid=<%=HDConstant.CT_GRID_CLSID%> width='1050px' height='180px' border='1'>
			...
			<param name="Format" 				value="
	    		 	 <FC> name='No' 							ID={CurRow}	   			width=20 		align=CENTER	edit=none</FC>
		        <FG> name='청구품목'
		             <FC> name='품목코드' 		ID='CD_ITEM'   		width=70 		align=CENTER		edit=none </FC>
		             <FC> name='명칭'    		ID='DS_ITEM'    	    width=150 		align=left	 		edit=none </FC>
		             <FC> name='규격'   			ID='SZ_ITEM'   		width=50 		align=CENTER	 	sumtext='@sum' sumtextalign='right'	 edit=none</FC>
		             <FC> name='단위'    		ID='UT_ITEM'    	    width=50 		align=CENTER	 	edit=none </FC>		             
		        </FG>		    		 	 
		        <FG> name='청구'
		             <FC> name='수량' 			ID='QN_ITEM'   		width=60 		align=CENTER		edit=none Value={Number(QN_ITEM)} Dec=0</FC>
		             <FC> name='단가'    		ID='UP_ITEM'    	    width=60 		align=CENTER	 	edit=none Value={Number(UP_ITEM)} Dec=0</FC>
		             <FC> name='금액'   			ID='AM_ITEM'   		width=80 		align=CENTER	 	edit=none Value={Number(AM_ITEM)} Dec=0</FC>
		        </FG>	
		        <G> name='기청구'
