	  ...
	  function fnOnLoad(){
		...
		function fnSelect() {
			...
			//기본사항 조회 
		     ...
		     //prompt(this, param);
		     ln_TRSetting(tr1,"<%=dirPath%><%=HDConstant.PATH_HELP%>pr010H", "JSP(O:DS_DEFAULT1=ds_default1)",  param);
		     //prompt(this, ds_default1.dataid);
		     tr_post(tr1);
	        ...
	        var param = "proType=<%=HDConstant.PROCESS_TYPE_SEARCH%>"
																		            	 ...
																		            	 + ",v_cd_site="+g_WinParam[4]
																					 	 + ",v_ym_work="+g_WinParam[5];	         
	        //prompt(this, param);
		    ln_TRSetting(tr1,"<%=dirPath%><%=HDConstant.PATH_HELP%>pr010H", "JSP(O:DS_DEFAULT2=ds_default2)",  param);
		    //prompt(this, ds_default2.dataid);
		    tr_post(tr1);
		     ...
		     var param = "proType=<%=HDConstant.PROCESS_TYPE_SEARCH%>"
														                 				+ ",v_ap_m_sid="+g_WinParam[3];
		     //prompt(this, param);
		     ln_TRSetting(tr1,"<%=dirPath%><%=HDConstant.PATH_HELP%>pr010H", "JSP(O:DS_DEFAULT3=ds_default3)",  param);
		     //prompt(this, ds_default3.dataid);
		     tr_post(tr1);
		...
		function fnApply_1(){
			...
			if (confirm("승인하시겠습니까?")) {
			    ...
			    ln_TRSetting(tr2,"<%=dirPath%><%=HDConstant.PATH_HELP%>pr750H","JSP(O:msgDS=msgDS)", param);
		...
		function fnApply_2(){
			...
			if (confirm("반송하시겠습니까?")) {
				...
				ln_TRSetting(tr3,"<%=dirPath%><%=HDConstant.PATH_HELP%>pr750H","JSP(O:msgDS=msgDS)", param);
	...
	<td height=25 width=950>
		<table width="950px" border="0" cellpadding="0" cellspacing="1"  bgcolor="#666666">
			<tr>
				...
				<td height="25px"  width="120px" bgcolor="#eeeeee" align=center class="text">가동월</td>
				<td height="25px"  bgcolor="#FFFFFF" align="left" valign=center>&nbsp;
...
<table border="0" cellpadding="0" cellspacing="0" align=center>
  <tr>
	<td height=30 align ="left">
	  	<img src="<%=dirPath%>/Sales/images/p_icon.gif" align=center> <font style="position:relative;top:1px;width:200;height:15;">가동일보</font>
	</td>
  </tr>
  <tr>
	<td height=25 width=950 colspan =2>
		...
		<object id=gr_RcpDpt classid=<%=HDConstant.CT_GRID_CLSID%> width='950px' height='300px' border='1'>
			...
			<param name="Format" 				value="
	             ...
	             <C> name='작업일자' 					ID='DT_WORK'	   			width=60 		align=CENTER	edit=none SumBgColor=#C3D0DB mask='XX-XX-XX' </C>
	             <C> name='장비명' 						ID='MD_EQUIP'    			width=80 		align=CENTER	edit=none SumBgColor=#C3D0DB </C>
	             <C> name='규격'							ID='SZ_EQUIP'  			width=100 		align=CENTER	edit=none SumBgColor=#C3D0DB </C>
	             <C> name='작업시간'					ID='QN_WORK'  			width=60 		align=CENTER	edit=none SumBgColor=#C3D0DB </C>	             
	             <C> name='단가'							ID='UP_WORK'  			width=80 		align=CENTER	edit=none SumBgColor=#C3D0DB Value={Number(UP_WORK)} Dec=0</C>        
	             <C> name='금액'							ID='AM_WORK'  			width=80 		align=CENTER	edit=none SumBgColor=#C3D0DB Value={Number(AM_WORK)} Dec=0</C>   	        
	             <C> name='사용장소'					ID='AT_WORK'  			width=100 		align=CENTER	edit=none SumBgColor=#C3D0DB </C>
	             <C> name='작업내용'					ID='DS_WORK'  			width=100 		align=CENTER	edit=none SumBgColor=#C3D0DB </C>
	             <C> name='공종코드'					ID='CD_COST'  			width=80 		align=CENTER	edit=none SumBgColor=#C3D0DB </C>	             
	             <C> name='공종명'						ID='DS_ITEM'  				width=100 		align=CENTER	edit=none SumBgColor=#C3D0DB </C>        
	             <C> name='업체명'						ID='DS_VENDOR'  			width=80 		align=CENTER	edit=none SumBgColor=#C3D0DB </C>   	 	             
...
<table border="0" cellpadding="0" cellspacing="0" align=center>
  ...
  <tr>
	<td height=25 width=950 colspan =2>
		<%=HDConstant.COMMENT_START%>
		<object id=gr_apline_rqst classid=<%=HDConstant.CT_GRID_CLSID%> width='950' height='150px' border='1'>
			...
			<param name="Format" 				value="
              ...
              <C> name='부서'    	ID=AP_DPTNM	   	                width=240 	align=center editlimit=16 show=true edit=none</C>
