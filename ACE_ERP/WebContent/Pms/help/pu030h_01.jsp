 ...
 * 작 성 일		: 2011-12-08
 * [ 수정일자 ][수정자] 내용
 * [2015-04-15][심동현] 품목 그리드 default8에서 각 품목별 대비% 표시 안되는 것 수정 
 *****************************************************************************/
%>
...
%>
   ...
   //alert("g_WinParam[0]::"+g_WinParam[0]+"::g_WinParam[1]::"+g_WinParam[1]+"::g_WinParam[2]::"+g_WinParam[2]+"::g_WinParam[3]::"+g_WinParam[3]);
	  ...
	  function fnOnLoad(){
		...
		function fnSelect() {
			...
			//기본사항 조회 
		     ...
		     ln_TRSetting(tr1,"<%=dirPath%><%=HDConstant.PATH_HELP%>Pu030H_01", "JSP(O:DS_DEFAULT6=ds_default6)",  param);
			...
			var sys_file3=sys_file10.slice(2,3);	
	         ...
	         var param = "proType=<%=HDConstant.PROCESS_TYPE_SEARCH%>"
		                       + ",v_est_sid="+g_WinParam[1];
		                      // alert(param);
		     ln_TRSetting(tr1,"<%=dirPath%><%=HDConstant.PATH_HELP%>Pu030H_01",
		    		 "JSP(O:DS_DEFAULT7=ds_default7 )",
		    		 param);
		    // prompt('',ds_default.dataid);   
		     tr_post(tr1);
		     ...
		     if(strAppline_gb=="1"){
				...
				if(0[4]==""||0[4]=="undefined"||array2[4]=="undefined"||array2[4]==""){ //결재선을 안만든 경우 
					 ...
					 var param = "proType=<%=HDConstant.PROCESS_TYPE_SEARCH%>,dsType=5"
				                     ...
				                     + ",v_empno3="+array4[3]
				                     + ",v_empno4="+array5[3];
				     ln_TRSetting(tr1,"<%=dirPath%><%=HDConstant.PATH_HELP%>Pu030H_01", "JSP(O:DS_DEFAULT5=ds_default2)",  param);
				     tr_post(tr1);
				    ...
				    var param = "proType=<%=HDConstant.PROCESS_TYPE_SEARCH%>,dsType=2"
				                     + ",v_sid="+array2[4];
				 	//alert(param);
				     ln_TRSetting(tr1,"<%=dirPath%><%=HDConstant.PATH_HELP%>Pu030H_01", "JSP(O:DS_DEFAULT2=ds_default2)",  param);
				     tr_post(tr1);
				}
		...
		function fnApply_1() {
		   ...
		   //prompt('',ds_approve.text);
		    ...
		    ln_TRSetting(tr3,"<%=dirPath%><%=HDConstant.PATH_HELP%>Pu030H_01", "JSP(I:DS_APPROVE=ds_approve,O:msgDS=msgDS)", "proType=A, dsType=1");
	        tr_post(tr3);
		}
	...
	function fnSelectDetail2(row) {
	     ...
	     ln_TRSetting(tr5,"<%=dirPath%><%=HDConstant.PATH_HELP%>Pu030H_01",
	...
	<td height=25 width=950>
		...
		<table width="950px" border="0" cellpadding="0" cellspacing="1"  bgcolor="#666666">
			...
			<tr>
				...
				<td  height="25px"  bgcolor="#FFFFFF" align="left" valign=bottom >&nbsp;&nbsp;<%=HDConstant.COMMENT_START%>
                    <object id=txt_prepay_amt classid=<%=HDConstant.CT_EMEDIT_CLSID%> height=19 width=120 align=center  >
  ...
  <tr>
	<td height=25 width=950 colspan =2>
		...
		<G> name='결정'
			 ...
			 <G> name='대비(%)'
				 <C> name='도급'    		ID='RATE_DG'	width=60 		align=RIGHT  show=true edit=none 	decao=1 dec=1	sumtext={sum(FN_SUM_AMT)/sum(DG_SUM_AMT)*100} sumtextalign='right'</C>
				 <C> name='예산'    		ID='RATE_YS'	width=60 		align=RIGHT  show=true edit=none 	 decao=1 dec=1	sumtext={sum(FN_SUM_AMT)/sum(YS_SUM_AMT)*100} sumtextalign='right'	</C>
			 </G>
	...
	<td height=25 width=950 colspan =2>
		...
		<object id=gr_apline classid=<%=HDConstant.CT_GRID_CLSID%> width='950px' height='100px' border='1'>
			...
			<param name="Format" 			value="
              ...
              <C> name='부서'    	ID=DEPTNM	   	                   width=250 	align=center editlimit=16 show=true edit=none</C>
              <C> name='상태'     	ID=AP_STATUS_D_NM         width=80 	align=center editlimit=30 show=true edit=none  </C>
              <C> name='의견'	    ID=AP_DESC     		           width=110	align=center editlimit=200 show=true edit=true</C>
              <C> name='결재일시'	ID=U_DATE			               width=250 	align=center editlimit=50 show=true edit=none</C>
			">
		</object>   
		<%=HDConstant.COMMENT_END%>		
