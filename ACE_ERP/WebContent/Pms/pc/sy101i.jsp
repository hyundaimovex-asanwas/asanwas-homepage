 ...
 * [2011-06-23][박경국] 공사관리시스템 공통코드화면 신규.
 *****************************************************************************/
%>
<%
///////////////////////////////////////////////////////////////////////////////
	...
	<script language="javascript">
		...
		function fnSelectDs1(){   
			if (ds1.IsUpdated || ds2.IsUpdated) {
				...
				ds1.DataId = "<%=dirPath%><%=HDConstant.PATH_PMS_SY%>Sy101I?proType=S&dsType=1&s_Head=" + txt_head.value + "&s_HeadName=" + txt_head_name.value;
				ds1.Reset();
			}
		} 
		...
		function fnSelectDs2(){
			if (ds1.IsUpdated || ds2.IsUpdated) {
				...
				ds2.DataID = "<%=dirPath%><%=HDConstant.PATH_PMS_SY%>Sy101I?proType=S&dsType=2&s_Head=" + ds1.namevalue(ds1.rowposition,"head");
				ds2.Reset();
			}
		}
		...
		function fnApplyDs1() {
			if (ds1.IsUpdated) {
					...
					if (ln_Chk())	{
						...
						tr1.Action = "<%=dirPath%><%=HDConstant.PATH_PMS_SY%>Sy101I?proType=A&dsNum=1";
						tr1.post();
					}
				}
		}
		...
		function fnApplyDs2() {
			if (ds2.IsUpdated) {
				if (ln_Chk()) {
					...
					tr2.Action = "<%=dirPath%><%=HDConstant.PATH_PMS_SY%>Sy101I?proType=A&dsNum=2";
					tr2.post();
				}
			}
		}
...
<jsp:include page="/Common/sys/body_s10.jsp"  lush="true"/>
