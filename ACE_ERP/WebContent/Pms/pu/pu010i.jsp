 ...
 * [ 수정일자 ][수정자] 내용
 * [2011-11-14][박경국] 신규
 * [2011-11-14][박경국] 품목 추가할 때 순번 넣기
 * [2013-11-05][박경국] 13 재구성-실행 이전
 * [2015-01-12][박경국] 결재선 창만 수정
 *****************************************************************************/
%>
<%
///////////////////////////////////////////////////////////////////////////////
...
// g_WinParam[0]=DOC_GU2='310'
// g_WinParam[1]=DOC_SID=rqst_sid
// g_WinParam[3]=s_ap_m_sid=전자결재sid
// 미결함_승인...sales.help>my400H.java....최종승인이고 부결일때 rqst_status 를 update
...
<script language="javascript">
    ...
    function fnInit() {
       ...
       var param = "proType=<%=HDConstant.PROCESS_TYPE_SEARCH%>"
 	      + ",v_empno="+ '6080001'
           + ",v_site_sid="+ 209
           + ",v_fr_rqst_date=" + '20000101'
           + ",v_to_rqst_date=" + '20000102'
           + ",v_rqst_status=" + 2 ;
 		//alert(param);
 		ln_TRSetting(tr1, 
	...
	function fnApply(){
		...
		//alert(lc_put_wh.ValueOfIndex("detail", lc_put_wh.Index) 				);
             ln_TRSetting(tr1, 
            "<%=dirPath%><%=HDConstant.PATH_PU%>Pu010I",
    ...
    function  fnSelSitePopup1(dirPath) {
        ...
        if (arrResult != null) {
            ...
            v_pu_empnm.value = arrParam[3];	//담당자명
	...
	function delResult(){
		...
		f.file1.value = "첨부파일은최대10M까지가능합니다.";
	}
	...
	*/
    ...
    function fnFile(){
		...
		if(real_file1==''){
			...
			//location.href = '/Pms/common/sys/download.jsp?dir=pu010&filename='+real_file1+'&sysfilename='+sys_file1; //로컬
		}
	}
	//첨부파일(2) 다운로드_열기
    function fnFile2(){
	...
	******************************************************************************/
	...
	function fnApproval() {
				...
				strPos = "dialogWidth:600px;dialogHeight:550px;status:no;scroll:yes;resizable:yes";
	...
	******************************************************************************/
	function fnDraft() {
		...
		//alert("1");
		strURL = "<%=dirPath%>/Pms/help/pu010h_v1.jsp?";
		...
		strPos = "dialogWidth:900px;dialogHeight:"+(screen.availHeight-52)+"px;status:no;scroll:no;resizable:no";
		//alert("2");
...
%>
	...
	<script language=javascript for=gr_tender event="OnExit(Row,Colid,OldData)">
	    if(Row==0) return;
	    if(Colid=="DG_AMT"){
		   	ds_tender.NameValue(Row, "DG_SUM_AMT") = 	ds_tender.NameValue(Row, "DG_AMT") * ds_tender.NameValue(Row, "DG_CNT");
...
<!--------------------------------- 코딩 부분 시작 ------------------------------->		
    ...
    <table border='0' cellpadding='0' cellspacing='0' width="845px"> 
                        ...
                        <td valign='top' width="300px">
                            ...
                            <object id=gr_default classid=<%=HDConstant.CT_GRID_CLSID%> width='300px' height='233px' border='1'>
                                ...
                                <param name="Format"            value="
                                     ...
                                     <C> name='투입창고'	ID='PUT_WH' 			width=120 align=CENTER	Edit=None	</C>                                     
                        ...
                        <td valign="top">
							<table border="0" cellpadding="0" cellspacing="1" width="100%" bgcolor="#666666" align="right">
								...
								<tr>
									...
									<td  colspan=2 width="265px"	bgcolor='#FFFOFO'>&nbsp;<input type="text" name="file1" id="file1" style="width:250px;" value="첨부파일은최대10M까지가능합니다." readonly  class=textbox> </td>
       ...
       <tr height='10px'>
        ...
        <tr>
            <td>
                <b>※청구순서:</b>1) <b>[신규버튼]</b>=>[사용처,현장명(팝업),투입창고,청구특기사항,파일첨부(파일찾기 후 업로드하기)]=><b>[저장버튼]</b>=>청구번호 자동생성 <br><br>
                  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;	2) <b>[행추가]</b>청구할 품목 입력=><b>[저장버튼]</b>
            </td>
        </tr>
    </table>
<%
