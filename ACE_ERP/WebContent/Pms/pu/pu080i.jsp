...
<script language="javascript">
	...
	function fnApply(){
		...
		function ln_Chk() {
			...
			var row2=ds_detail.countrow;
			
			for(i=1;i<=row1;i++) {
				if(ds_detail.namevalue(i,"DATE_CGC")=="" ) {
					alert("검수일자를 올바르게 입력해 주십시오");
					return false;
				}
			}
			return true;
	...
	function fnSettle() {
		...
		//if(v_job!="calc"){
			...
			//누계정산 수량계산
					getObjectFirst("ds_detail").NameValue(i, "CGC_SUM_CNT")=getObjectFirst("ds_detail").NameValue(i, "CGC_NOW_CNT") + getObjectFirst("ds_detail").NameValue(i, "CGC_SUM_CNT");
		    //누계정산 금액계산
					getObjectFirst("ds_detail").NameValue(i, "CGC_SUM_AMT")=getObjectFirst("ds_detail").NameValue(i, "FN_EST_AMT") * getObjectFirst("ds_detail").NameValue(i, "CGC_SUM_CNT");
			//전회정산 수량계산
			//		getObjectFirst("ds_detail").NameValue(i, "CGC_PRE_CNT")=getObjectFirst("ds_detail").NameValue(i, "CGC_NOW_CNT") ;
		    //전회정산 금액계산
					getObjectFirst("ds_detail").NameValue(i, "CGC_PRE_AMT")=getObjectFirst("ds_detail").NameValue(i, "FN_EST_AMT") * getObjectFirst("ds_detail").NameValue(i, "CGC_PRE_CNT");
			//금월정산 금액계산
					getObjectFirst("ds_detail").NameValue(i, "CGC_NOW_AMT")=getObjectFirst("ds_detail").NameValue(i, "FN_EST_AMT") * getObjectFirst("ds_detail").NameValue(i, "CGC_NOW_CNT");
			//잔여분 수량계산
					getObjectFirst("ds_detail").NameValue(i, "CGC_RES_CNT")=getObjectFirst("ds_detail").NameValue(i, "EST_CNT") - getObjectFirst("ds_detail").NameValue(i, "CGC_SUM_CNT");
			//잔여분 금액계산
					getObjectFirst("ds_detail").NameValue(i, "CGC_RES_AMT")=getObjectFirst("ds_detail").NameValue(i, "FN_EST_AMT") * getObjectFirst("ds_detail").NameValue(i, "CGC_RES_CNT");
		    }
	}
	...
	function fnAddRowDs2() {
		...
		if (ds_default.namevalue(ds_default.rowposition,"RQST_SID") == undefined || ds_default.namevalue(ds_default.rowposition,"RQST_SID") == "" || ds_default.namevalue(ds_default.rowposition,"RQST_SID") == 0 ) {
			alert("검수정산 할 정보를 조회 후 추가해 주세요.");
		} else {
			ds_detail.addrow();
			ds_detail.namevalue(ds_detail.RowPosition, "RQST_SID") = ds_default.namevalue(ds_default.rowposition,"RQST_SID");
			ds_detail.namevalue(ds_detail.RowPosition, "ITEM_SID") = ds_default.namevalue(ds_default.rowposition,"ITEM_SID");
			ds_detail.namevalue(ds_detail.RowPosition, "EST_SID") = ds_default.namevalue(ds_default.rowposition,"EST_SID");
			ds_detail.namevalue(ds_detail.RowPosition, "CON_SID") = ds_default.namevalue(ds_default.rowposition,"CON_SID");			
			ds_detail.namevalue(ds_detail.RowPosition, "VEND_CD") = ds_tender.namevalue(ds_tender.rowposition,"VEND_CD");									
...
<%//일괄수정 %>	
	 <script language="javascript"  for=gr_detail event=OnClick(row,colid)>
        if(row==0){
            if(ds_detail.CountRow==0) return;
            
            gr_detail.ReDraw = "False";
            if(colid=="DATE_CGC"){
                for(var i=1; i<=ds_detail.CountRow; i++){
                    ds_detail.NameValue(i, "DATE_CGC") = ds_detail.NameValue(1, "DATE_CGC");
                }
            } else if(colid=="CGC_NOW_CNT") {
                for(var i=1; i<=ds_detail.CountRow; i++){
                    ds_detail.NameValue(i, "CGC_NOW_CNT") = ds_detail.NameValue(1, "CGC_NOW_CNT");
               }
            }else if(colid=="CGC_PRE_CNT") {
                for(var i=1; i<=ds_detail.CountRow; i++){
                    ds_detail.NameValue(i, "CGC_PRE_CNT") = ds_detail.NameValue(1, "CGC_PRE_CNT");
               } 
            }else if(colid=="CGC_NOTE" ){
                for(var i=1; i<=ds_detail.CountRow; i++){
                    ds_detail.NameValue(i, "CGC_NOTE") = ds_detail.NameValue(1, "CGC_NOTE");
               }
            }
...
<!--------------------------------- 코딩 부분 시작 ------------------------------->		
            ...
            <td valign="top">
                ...
                <object id="gr_detail" classid=<%=HDConstant.CT_GRID_CLSID%> width='845px' height='300px' border='1'  bgcolor='green'>
                    ...
                    <param name="Format"            value="
                         ...
                         <C> name='검수일자V' 		ID='DATE_CGC'   	width=80 		align=CENTER	editlimit=8	edit=true EditStyle=popup show=true	bgColor=<%=HDConstant.GRID_ITEM%>	</C>
						...
						<G> name='전회정산'
	  						 <C> name='수량V' 		ID='CGC_PRE_CNT'	width=85 		align=right			show=false	decao=0 		edit=none</C>
    	                     <C> name='금액'    		ID='CGC_PRE_AMT'  width=85 		align=right	 		show=false	decao=0	 sumtext='@sum' sumtextalign='right'	edit=none</C>
						</G>
						<G> name='금월정산'
 							 <C> name='수량V' 		ID='CGC_NOW_CNT'	width=85 		align=right	 show=true	decao=0 	 	edit=true	bgColor=<%=HDConstant.GRID_ITEM%></C>
            	             <C> name='금액'    		ID='CGC_NOW_AMT'  	width=85 		align=right	 show=true	decao=0	 sumtext='@sum' sumtextalign='right'	edit=none	</C>
						</G>
						<G> name='누계정산'
							 <C> name='수량' 		ID='CGC_SUM_CNT'	width=85 		align=right	 show=true	decao=0 		edit=none</C>
                    	     <C> name='금액'    		ID='CGC_SUM_AMT'  width=85 		align=right	 show=true	decao=0	 sumtext='@sum' sumtextalign='right'	edit=none</C>
						</G>
						<G> name='잔여분'
							 <C> name='수량' 		ID='CGC_RES_CNT'	width=85 		align=right	 show=true	decao=0 	 	edit=none</C>
                         	<C> name='금액'    		ID='CGC_RES_AMT'  width=85 		align=right	 show=true	decao=0	 sumtext='@sum' sumtextalign='right'	edit=none</C>
						</G>
                         <C> name='검수특기사항V' 	ID='CGC_NOTE'   		width=200 		align=CENTER	edit=true show=true	bgColor=<%=HDConstant.GRID_ITEM%></C>                                                  
						<G> name='도급'
    ...
    <table>
    <BR>
    <B>** [조회] ==> [검수일자/금월정산수량/검수특기사항]입력 ==> [처리] ==> [저장]</B>
<%
/*=============================================================================
			Bind 선언
=============================================================================*/
