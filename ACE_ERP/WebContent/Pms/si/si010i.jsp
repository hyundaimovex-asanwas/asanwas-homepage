...
/*=============================================================================
 ...
 - [ 수정일자 ][수정자] 내용
 - [2012-05-21][정영식] 신규
 * [2013-11-06][심동현] 새 환경으로 이전 
 *						조회 프로시저 DB에서 테스트 완료, 저장 프로시저 수정 (시퀀스) 
 *						화면에서 테스트는 권한 문제로 불가
 =============================================================================*/
%>
<%
...
%>
	function fnAuthority(){
		gcds_empno_chk.DataID="<%=dirPath%><%=HDConstant.PATH_ACCOUNT%>a010010_s1?v_str1=&v_str2="+gusrid+"&v_str3=si020s&v_str4=Y";
    ...
    function fnSelect() {
		if (ds_default.IsUpdated ) {
	        ...
	        if(fr_date.length!=8) {
	            alert("출납일자(From)를 입력하시길 바랍니다.");
	            return;
	        }
	        
	        if(to_date.length!=8) {
	            alert("출납일자(To)를 입력하시길 바랍니다.");
	            return;
	    	}
			   ...
			   ************************************************/
			   ...
			   if(ds_default.namevalue(i,"INGB_CD")=="1") {  //법인카드
				   ...
				   if(ds_default.namevalue(i,"TAX_KND")=="8"){
			   		...
			   		//카드 존재하면 안됨 .
			   	 	if(ds_default.namevalue(i,"CARD_NUM")!="" && ds_default.namevalue(i,"CARD_NUM")!=" ") {
			   ...
			   if (ds_default.namevalue(i,"ATCODE")=="1220100"||ds_default.namevalue(i,"ATCODE")=="1220200"||
			       ...
			       ds_default.namevalue(i,"ATCODE")=="1222100"
			   	   ...
			   	   if(ds_default.namevalue(i,"H_ASTNBR")==""){
			   		 ...
			   		 if(ds_default.namevalue(i,"H_ASTNBR")!="" && ds_default.namevalue(i,"H_ASTNBR")!=" "){
    ...
    function fnSelEmpPopup(dirPath){
        ...
        //권한 체크         
        if(gstrAuthgr=="A" ||gstrAuthgr=="B" || gusrid == "2020013"){
