 ...
 * 기능정의     : 신용카드 결재
 * [수정일자][수정자] 내용
 * [20091221][박경국] 무통장입금 부분 입금내역 조회해서 선택 후 결재 하는 화면으로 수정
...
%>
    <script language="javascript">
        ...
        //결재 공통 모듈
        var c_pay_gubn          = "";   //결재수단
        var c_pay_manage_no     = "";   //결재 관리번호
        var c_van_cd            = "";   //van 구분
        var c_pay_amt           = "";   //거래금액
        var c_acct_join_no      = "";   //가맹점 ID
        var c_terminal_id       = "";   //단말기 아이디
        
        //가상계좌 / 계좌이체 결재 관련 연동 변수 선언
        //요청
        var r_order_nm          = "";   //주문자명
        ...
        function fnOnChangTab(tab_idx){
            if(tab_idx==2 && c_pay_amt<1000){
                alert("신용카드 결재는 1,000원 이상에서만 가능합니다.");
                return;
            }
            if(tab_idx==4){
...
//alert("strParam : " + strParam + " : " + strParam.length);
        ...
        function fnCardTrans(){
            ...
            if(c_pay_amt=="" || eval(c_pay_amt)<1000){
                alert("결재 금액 <%=HDConstant.A_MSG_MUST_INPUT%>");
...
//alert(document.acc_frm.action);
        ...
        function fnGiftTrans(){
            ...
            if(v_gift_amt!=c_pay_amt){
                alert("결재 요청 금액["+c_pay_amt+"]과 입금된 상품권 금액["+v_gift_amt+"]이 틀립니다.");
                return;
            }
        ...
        function fnGetManageNo(){
            ln_TRSetting(tr1, 
                ...
                "proType=<%=HDConstant.PROCESS_TYPE_SEARCH%>,dsType=1,v_client_cd="+v_client_cd);
            tr_post(tr1);
        }
        ...
        /*
         * 결재 연동페이지에서 처리완료 (상품권은 처리안함)
         * pay_gubn     : 결재수단 (020:가상계좌, 030:계좌이체 040:신용카드 060:상품권
         * retn_code    : 응답코드
         *                0000:정상응답
         *                9999:사용자취소
         *                etc :van 오류코드
        ...
        function fnAccountSuccess(pay_gubn, retn_code, result_param){
            ...
            if(retn_code=="0000"){  //정상응답
                ...
                //신용카드
                    ...
                    alert("알수 없는 결재구분수단입니다. ["+ pay_gubn +"]");
                    return;
                }
