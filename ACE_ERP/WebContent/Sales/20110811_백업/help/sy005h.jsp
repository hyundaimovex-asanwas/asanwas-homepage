...
%>
		<script language="javascript">		
			...
			function fnOnLoad(){
				...
				if(strParam[0]!=null) { //영업담당자sid
					txt_sal_dam_sid.value=strParam[0];
				}else {
				}
				if(strParam[1]!=null) { //영업담당자코드
					txt_sal_dam_cd.value=strParam[1];
				}else {
				}
				if(strParam[2]!=null) { //영업담당자명
					txt_sal_dam_nm.value=strParam[2];
				}else {
				}
			}
			...
			// 메인 조회
			function fnSelect() {
				ds1.Dataid  = "<%=dirPath%><%=HDConstant.PATH_HELP%>Sy005H?proType=S&dsType=1"
							+ "&s_SalDamSid="+txt_sal_dam_sid.value
							+ "&s_SalDamCd=" + txt_sal_dam_cd.value;
				ds1.Reset();
			}
			...
			function fnAdd() {
				...
				window.returnValue = ds1.namevalue(row, "sal_dam_sid") + ";" +
								 ds1.namevalue(row, "sal_dam_cd") + ";" + 
								 ds1.namevalue(row, "sal_dam_nm");
			 	fnClose();
			}
					...
					<table border="0" cellpadding="0" cellspacing="0" style="position:relative;left:1px;top:5px;width:219px;">
						<tr>
							<td colspan=2 height=25>
								<img src="<%=dirPath%>/Sales/images/p_icon.gif" align=center> <font style="position:relative;top:1px;width:70;height:15;">영업담당자</font>
								<input type="hidden" id="txt_sal_dam_sid" style="width:50;position:relative;top:2px;border:0;" readonly>
							</td>
						</tr>
						<tr>
							...
							<td width="110px" style="border:1 solid #708090;border-left-width:0px">
								<input type="text" id="txt_sal_dam_cd" name="txt_sal_dam_cd" class="textbox" style="position:relative;left:8px;width:100px;" onkeydown="if(event.keyCode==13) fnSelect();"  maxlength="30">
							</td>
						</tr>
					</table>
				</td>
...
%>
					<comment id="__NSID__"><object id=gr1 classid="clsid:1F57AEAD-DB12-11D2-A4F9-00608CEBEE49" style="width=479px; height:248px;border:1 solid #777777;">
						...
						<param name="Format" 		value="
							<c> Name='sid'		ID='sal_dam_sid'		HeadAlign=Center Width=100  align=center show=false</c>
							<c> Name='코        드'		ID='sal_dam_cd'	HeadAlign=Center Width=100  align=center </c>
							<c> Name='코   드   명' 	ID='sal_dam_nm'	HeadAlign=Center Width=349  LeftMargin=50 </c>
