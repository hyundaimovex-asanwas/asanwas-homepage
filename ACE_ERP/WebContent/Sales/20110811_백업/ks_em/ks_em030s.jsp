...
function ln_Query(){
		...
		gcds_data.DataID = "<%=dirPath%><%=HDConstant.PATH_KS_EM%>KS_Em030_S1"
											 + "?v_str1=" + fn_trim(gclx_dept_cd_s.bindcolval)	  // 부서         
											 + "&v_str3=" + fn_trim(txt_empno.value)			      // 사번       
											 + "&v_str4=" + fn_trim(txt_empnmk.value)	      		// 성명       
											 + "&v_str5=" + fn_trim(gcem_start_dt.text) 		  	// 휴가일자FR 
											 + "&v_str6=" + fn_trim(gcem_end_dt.text);			      // 휴가일자TO 										 
					...
					<tr>
						<td bgcolor="#FFFFFF" align="left" class="tab_z0101">
							...
							<PARAM NAME="Format"			VALUE="  
								...
								<FC>Name=직장,     		 ID=DEPT_NM,  width=100,HeadBgColor=#B9D4DC, bgcolor=#ffffff, align=left, edit=none </FC>
								...
								<FC>Name=요일,   		 ID=DAY_NM,  width=50,HeadBgColor=#B9D4DC, bgcolor=#ffffff, align=left, edit=none  </FC>
               					<FC>name=대체휴가일      ID=HOL_DT,  width=100 	HeadBgColor=#B9D4DC, align=center, mask='XXXX/XX/XX' </FC>   
								<FC>Name=요일,   		 ID=DAY_NM,  width=50,HeadBgColor=#B9D4DC, bgcolor=#ffffff, align=left, edit=none  </FC>

								<FC>Name=비고, 	 ID=REMARK,   width=160,HeadBgColor=#B9D4DC, bgcolor=#ffffff, align=center,   edit= none </FC>							
							">
							</OBJECT></comment><script>__ws__(__NSID__);</script>
