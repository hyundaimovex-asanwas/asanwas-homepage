...
<script language="javascript"> 	
   function view_send(brd_no,ref,ref_step){
		var readURL ="my710_read_pop.jsp?pageFrom=710&idx=400&brd_no="+brd_no+"&ref="+ref+"&ref_step="+ref_step;
   ...
   function write_ok(){
    var f = document.searchForm;
		f.action='my710_write.jsp?idx=400&work_flag=INSERT';
		f.target='';
		f.method = "POST";
		f.submit();
   }
			...
			<tr>
				<td width="92">
					 <SELECT name="search_key" ID="Select02" STYLE="width:87px;height:15px;display:;">
						...
						<OPTION VALUE='BRD_CREATOR_NAME' <%= key.equals("BRD_CREATOR_NAME") ? "selected" : "" %>>발신인 이름</OPTION>
					</SELECT>
				</td>
...
<!-- News LIST-->
  ...
  <%} else {
     ...
     <tr height="30" > 
		<td><input type=checkbox name="chk<%=bean3.get("brd_no")%>" id="chk<%=bean3.get("brd_no")%>"></td>
		<td><img src="http://static.naver.com/pwe/note/ico_note_on.gif" border=0></td>
		<!-- http://static.naver.com/pwe/note/ico_note_off.gif -->
		<td><img src="http://www.hdasan.com/img/common/bbs_bar_top.gif"></td>
  ...
  %>			  
	...
	<tr height=30>
		<td>
			<a href=""><img src="http://www.hdasan.com/img/common/btn_delete.gif" border="0"></a>  
		</td>
