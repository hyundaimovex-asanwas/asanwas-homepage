 ...
 * [2008-11-14][심동현] 판매가 수정 화면으로 전체 업데이트..ㅋ;;;
 ...
 * [2009-08-28][심동현] 예판 체크 로직 수행: 예약판매 View와 조인하여 해당되는 고객은 
 *						1) 할인유형 코드를 업뎃
 *						2) 입금액이 있는 경우 새로 예약한 RSV_SID로 잔액대체
 * [2009-11-17][심동현] 할인유형 코드가 관광객 것만 조회되지 않아서!
 *						공통코드 조회부분에서 파라미터를 모두 소문자로 맞추기로 통일!
 *****************************************************************************/
%>
<%
///////////////////////////////////////////////////////////////////////////////
...
%>
   ...
   /**
    ...
    //예약판매 체크 로직
    function fnPreRsv(){
	    if(v_accept_sid==""||v_accept_sid=="0"){
	    	alert("신청서번호가 정상 검색되지 않았습니다.\n다시 검색해주세요.");
	    }else{
	        v_job = "P";
	        ln_TRSetting(tr2,
	            "<%=dirPath%><%=HDConstant.PATH_AC%>Ac110I",
    ...
    <tr>
        <td>
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
                    ...
                    <td width=845>
                        ...
                        <object  classid=<%=HDConstant.CT_GRID_CLSID%>  id=gr1  style="width=845; height:400px" border="1">
                            ...
                            <param name="Format"            value="
                        		...
                        		<C> name='연령√'			ID='AGE_CD'         Width=60    align=center   editstyle=lookup data='codeDs6:detail:detail_nm' BgColor={IF(ACCT_CLOSE_YN='Y','#fefec0','' )} Sort=True</C>
								<C> name='표준요금'     	ID='STD_AMT'		Width=65   	align=Right    BgColor='#fefec0' SumText=@sum  Decao=0 Edit=none</C>
								<C> name='판매가'         	ID='USE_AMT'  		Width=65   	align=Right    SumText=@sum  Decao=0 BgColor='#fefec0' Edit=none</C>
								<C> name='입금액'         	ID='TOT_TOUR_AMT'   Width=65   	align=Right    Edit=none  BgColor=#fefec0 SumText=@sum   Decao=0</C>
								<C> name='상품'          	ID='GOODS_NM'       Width=70  	align=Left 	   Edit=none  BgColor=#fefec0  suppress=5</C>
                ...
                <tr>
                    <td><BR>
                    	　■ 신청서번호가 있는 경우에만 [판매가계산], [예약판매체크] 버튼이 나타납니다.<BR>
                    	　■ 할인상세내역의 5가지 조건에 따라 판매가가 적용됩니다. <BR>
                    	　■ 표준요금과 판매가를 직접 수정한 경우, 계산 버튼을 누르기 전에 한번 더 생각해 주세요.<BR>
                    	　■ 정확한 방문횟수 확인을 위해 [예약조회(퀵)] 메뉴를 활용해 주세요.<BR>
                    	　■ [예약판매체크]를 클릭하면 해당자를 조회 후 <b>자동으로 할인유형과 입금액을 대체</b>해 줍니다.<BR>
                    	　■ <b>2009년 예약판매 할인은 할인유형="일반"으로 판매가 계산<font color=red> 후 마지막에 할인율이 적용</font>됩니다.</b><BR>
