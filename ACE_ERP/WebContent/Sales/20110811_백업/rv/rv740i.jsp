...
%>		
		...
		function fnSelect2() {
			...
			var param = "proType=<%=HDConstant.PROCESS_TYPE_SEARCH%>"
			          ...
			          + ",sGoodSid=" + codeDs2.namevalue(codeDs2.rowposition,"goods_sid") 
		 ...
		 */
		function fnApply2() {
			if (codeDs2.namevalue(codeDs2.rowposition,"goods_sid")=="0"){
            ...
            if (ds2.IsUpdated){		
				...
				var param = "proType=<%=HDConstant.PROCESS_TYPE_APPLY%>"
				          ...
				          + ",sTDate=" + sTDate.Text
				          + ",sGoodSid=" + codeDs2.namevalue(codeDs2.rowposition,"goods_sid") 
				          + ",sClass=" + codeDs3.namevalue(codeDs3.rowposition,"class")
				          + ",sTeam=" + codeDs4.namevalue(codeDs3.rowposition,"teams") ;
				   //alert(param);
				ln_TRSetting(tr1, 
				    "<%=dirPath%><%=HDConstant.PATH_RV%>Rv740I",
			...
			if (ds1.IsUpdated) {
				...
				if(confirm("객실타입["+sFromRoomType.Text+"]에서 객실타입["+sToRoomType.Text+"]으로 자동배정합니다.\n 정말 진행하시겠습니까?")) {
					...
					var param = "proType=<%=HDConstant.PROCESS_TYPE_APPLY%>"
					          ...
					          + ",sUpjang="+ getObjectFirst("sUpjang").value				          				          
					          + ",sClass=" + codeDs3.namevalue(codeDs3.rowposition,"class")
					          + ",sTeam=" + codeDs4.namevalue(codeDs4.rowposition,"teams") 
					          + ",sAssign_YN="+ sAssign
		...
		function fnClass() {
			if (div == 'init') {
				ln_TRSetting(tr3, 
		            "<%=dirPath%><%=HDConstant.PATH_HELP%>CommonH",
		            "JSP(O:DS6=codeDs3)",
		            "proType=<%=HDConstant.PROCESS_TYPE_INIT%>,dsType=1,sDepartDate=<%=firstday%>,sSaupSid=1,sUpjang="+getObjectFirst("sUpjang").value +",sGoodsSid="+ codeDs2.namevalue(codeDs2.rowposition,"goods_sid"));
		        tr_post(tr3);
	            ...
	            "<%=dirPath%><%=HDConstant.PATH_HELP%>CommonH",
	            "JSP(O:DS6=codeDs3)",
	            "proType=<%=HDConstant.PROCESS_TYPE_INIT%>,dsType=1,sDepartDate="+sSDate.Text+",sSaupSid=1,sUpjang="+getObjectFirst("sUpjang").value +",sGoodsSid="+ codeDs2.namevalue(codeDs2.rowposition,"goods_sid"));
	        	tr_post(tr3);
			}
		...
		function fnTeam() {
			if (div == 'init') {
				ln_TRSetting(tr4, 
		            ...
		            "JSP(O:DS7=codeDs4)",
		            "proType=<%=HDConstant.PROCESS_TYPE_INIT%>,dsType=1,sDepartDate=<%=firstday%>,sSaupSid=1,sUpjang="+getObjectFirst("sUpjang").value 
		            		+",sGoodsSid="+ codeDs2.namevalue(codeDs2.rowposition,"goods_sid") + ",sClass=" + lc5.ValueOfIndex("class", lc5.Index));
		        tr_post(tr4);
			} else {
				ln_TRSetting(tr4, 
	            "<%=dirPath%><%=HDConstant.PATH_HELP%>CommonH",
...
/*        window.status = "<%=HDConstant.S_MSG_SAVE_ERR%>";
	...
	<script language=JavaScript for=codeDs2 event=OnLoadCompleted(rowcnt)> <!-- 상품 -->
		if (div == 'init') {
			fnClass();
		}
	</script>
	<script language=JavaScript for=codeDs3 event=OnLoadCompleted(rowcnt)> <!-- 반 -->
		if (div == 'init') {
			fnTeam();
		}
	</script>
...
<iframe id=roomInfoWin frameborder=0 marginwidth=0 marginheight=0 scrolling=no style="POSITION: absolute;DISPLAY:none;width:520;height:260;left: expression((document.body.clientWidth - 520)/2);top: expression((document.body.clientHeight - 260)/2);Z-INDEX: 9;"></iframe>
	...
	<table border='0' cellpadding='0' cellspacing='0' width='845'>	
		...
		<tr>
			<td align=center>
				<!-- 평면시작 div -->
				<div id=pageSelect style='width:845;height:200;overflow-y:auto;background:white;border:1 solid 	#9D9C9C;display:;position:relative;top:3px'>
