...
%>		
	    ...
	    function fnResultPop(gubun) {
	        ...
	        var param = "&sSaup_sid="+ lc1.ValueOfIndex("saup_sid", lc1.Index)
			                 ...
			                 + "&sSDate=" + ds1.namevalue(ds1.rowposition, "depart_date")
			                 + "&sUpjang_sid=" + ds1.namevalue(ds1.rowposition, "upjang_sid")
			                 + ",sCustGu=" + sCustGu.ValueOfIndex("detail", sCustGu.Index)
			                 + ",v_out_gubn=" + v_out_gubn.value;
...
<script language=JavaScript for=gr1 event=OnLButtonDblClk(row,colid,xpos,ypos)>
	...
	if (colid == "a_amt") {
		...
		if (ds1.namevalue(ds1.rowposition, "e_amt") == 0) {
			alert("조회할 내용이 없습니다!");
			return;
		}
		fnResultPop('080');
	} else if (colid == "f_amt") {	//기타대
		if (ds1.namevalue(ds1.rowposition, "f_amt") == 0) {
			alert("조회할 내용이 없습니다!");
			return;
		}
		fnResultPop('090');				
	}else if (colid == "refund_amt") {	//환불
		if (ds1.namevalue(ds1.rowposition, "refund_amt") == 0) {
			alert("조회할 내용이 없습니다!");
			return;
		}
		fnResultPop('3');				
	}else if (colid == "fund_amt") {	//환입
		if (ds1.namevalue(ds1.rowposition, "fund_amt") == 0) {
			alert("조회할 내용이 없습니다!");
			return;
		}
		fnResultPop('4');				
						...
						<td>
							...
							<object id=gr1 classid=<%=HDConstant.CT_GRID_CLSID%> width='845px' height='370px' border='1'>
								...
								<param name="Format" 			value="
									...
									<C> name='환불액'       ID='refund_amt' width=100	align=right editlimit=61 show=true SumText=@sum SumTextAlign=right decao=0</C>
									<C> name='환입액'       ID='fund_amt'  	width=100	align=right editlimit=61 show=true SumText=@sum SumTextAlign=right decao=0</C>
