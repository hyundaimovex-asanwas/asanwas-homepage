...
%>
			function fnSelectDs1(){   
				if(ds1.IsUpdated || ds2.IsUpdated) {
					...
					ds1.DataId = "<%=dirPath%><%=HDConstant.PATH_AC%>Sample?proType=S&dsType=1&s_Head=" + txt_head.value + "&s_HeadName=" + txt_head_name.value;
					ds1.Reset();
				}
			} 
<%
...
%>
			...
			function fnSelectDs2(){   
				if (ds1.IsUpdated || ds2.IsUpdated) {
					...
					ds2.DataID = "<%=dirPath%><%=HDConstant.PATH_AC%>Sample?proType=S&dsType=2&s_Head=" + ds1.namevalue(ds1.rowposition,"head");
					ds2.Reset();
				}
			}
...
%>
			...
			function fnAddRowDs1(){
				if(ds2.IsUpdated || ds1.IsUpdated) {
			...
			function fnApply(){
				...
				if (ds1.IsUpdated) {
					if (ln_Chk())	{
						...
						tr1.Action = "<%=dirPath%><%=HDConstant.PATH_AC%>Sample?proType=A&dsNum=1";
						tr1.post();
						fnSelectDs1();
					}
				}
				...
				if (ds2.IsUpdated) {
					if (ln_Chk()) {
						...
						tr2.Action = "<%=dirPath%><%=HDConstant.PATH_AC%>Sample?proType=A&dsNum=2";
						tr2.post();
						fnSelectDs2();
					}
				}
...
%>
			function fnPopup(){ 
				...
				strURL = "<%=dirPath%>/Sales/01 sample/sample_popup.jsp?head="+head.value+"&headName="+headName.value;
...
%>
		<script language="javascript"  for=gr1 event=OnClick(row,colid)>
			if(ds1.IsUpdated) {
				...
				ds1.rowposition=txt_focus.value;
				return;
			}
			if(ds2.IsUpdated) {
				alert("수정중인 행이 있으면 옮길 수 없습니다.");
		...
		<script language=JavaScript  for=ds1 event="OnLoadError()">
			alert("DS1 에러가 발생했습니다 : " + ds1.ErrorMsg );
...
%>	
		...
		<script language=JavaScript  for=ds2 event="OnLoadStarted()" >
 		...
 		<script language=JavaScript  for=ds2 event="OnLoadError()">
			alert("DS 2에러가 발생했습니다 : " + ds2.ErrorMsg );
			window.status="조회가 완료 되었습니다.";
			document.all.LowerFrame.style.visibility="hidden";
		</script>
