...
<script language="javascript">
	...
	******************************************************************************/
	function ln_Query(){
			...
			if(fn_trim(gcem_acdate_fr.text) == null || fn_trim(gcem_acdate_fr.text) == ""){
					alert("날짜필수 항목입니다.입력해주세요.");
//					gcem_acdate_fr.focus();
					return false;
			}

			var sdata = "/services/servlet/sales.bm.bm022i_s01"
								+ "?v_str1=" + fn_trim(gcem_vend_nm_fr.text)
								+ "&v_str2=" + fn_trim(gcem_acdate_fr.text)
								+ "&v_str3=" + fn_trim(txt_cdname_fr.value);
	...
	function ln_Save(){
		 ...
		 for(i=1;i<=gcds_data.countrow;i++) {
				if(gcds_data.namevalue(i,"CHK")=="T"){
					 gcds_data.namevalue(i,"LSTAY_REGI")=gs_date.substring(0,4)+gs_date.substring(5,7)+gs_date.substring(8,10);
				}else{ 
					 gcds_data.namevalue(i,"LSTAY_REGI")="";
					 gcds_data.namevalue(i,"U_EMPNO")=gusrid;	 
				} 

				gctr_data.KeyValue = "bm022i_t01(I:DATA=gcds_data)";
				gctr_data.Action = "/services/servlet/sales.bm.bm022i_t01";
				gctr_data.post();
		 }
		 //alert("작업완료");
	}
	...
	******************************************************************************/
	function ln_Excel(){
		  SetTitle("체류등록");
			...
			else gcgd_disp.GridToExcel("체류등록 ","C:\\Temp\\체류등록.xls",2);
	}
          ...
          <td>
						<comment id="__NSID__"><OBJECT  id=gcgd_disp class="tab_b0111" classid=clsid:1F57AEAD-DB12-11D2-A4F9-00608CEBEE49  style="width:877px;height:410px;" viewastext>
							...
							<param name="Format"				value="  
								...
								<C> Name='복귀예정일'	ID=ARRIVE_DATE	HeadAlign=Center HeadBgColor=#EEEEEE Width=80	 align=center, mask='XXXX/XX/XXXX' edit=none</C> 
								...
								<C> Name='체류등록일'	ID=LSTAY_REGI	  HeadAlign=Center HeadBgColor=#EEEEEE Width=100 align=center edit=none show=false</C> 
