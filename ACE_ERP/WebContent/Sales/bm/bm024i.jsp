  ...
  var gs_date = gn_Replace(gcurdate,"/");
	...
	******************************************************************************/
	function ln_Query(){
			...
			var sdata = "/services/servlet/sales.bm.bm024i_s01"
	...
	function ln_Save(){
		 ...
		 for(i=1;i<=gcds_data.countrow;i++) {
				if(gcds_data.namevalue(i,"CHK")=="T"){
					 gcds_data.namevalue(i,"ARRIVE_DATE")=gcds_data.namevalue(i,"ARRIVE_DATE");
					 gcds_data.namevalue(i,"ARRIVE_TIME")=gcds_data.namevalue(i,"ARRIVE_TIME");
					 gcds_data.namevalue(i,"U_EMPNO")=gusrid;
				}else{ 
					 gcds_data.namevalue(i,"LSTAY_DEL")="";
				} 
				gctr_data.KeyValue = "bm024i_t01(I:DATA=gcds_data)";
				gctr_data.Action = "/services/servlet/sales.bm.bm024i_t01";
				gctr_data.post();
		 }
	}
	/******************************************************************************
	...
	******************************************************************************/
	function ln_Excel(){
		  SetTitle("체류연장신청");
			...
			else gcgd_disp.GridToExcel("체류연장신청 ","C:\\Test\\dsh1033.xls",2);
	}
          ...
          <td>
						<comment id="__NSID__"><OBJECT  id=gcgd_disp class="tab_b0111" classid=clsid:1F57AEAD-DB12-11D2-A4F9-00608CEBEE49  style="width:877px;height:410px;" viewastext>
							...
							<param name="Format"				value="  
								...
								<C> Name='주민/여권번호' ID=MANAGE_NO	HeadAlign=Center HeadBgColor=#EEEEEE Width=100 align=center edit=none mask='XXXXXX-XXXXXXX'</C> 
								<C> Name='방북일'		  ID=DEPART_DATE	HeadAlign=Center HeadBgColor=#EEEEEE Width=80	 align=center edit=none mask='XXXX/XX/XXXX'</C> 
								<C> Name='시간'		    ID=DEPART_TIME	HeadAlign=Center HeadBgColor=#EEEEEE Width=50	 align=center edit=none mask='XX:XX'</C> 
								<C> Name='출경예정일'	ID=ARRIVE_DATE	HeadAlign=Center HeadBgColor=#EEEEEE Width=80	 align=center  mask='XXXX/XX/XXXX' bgcolor=#C4DDFF edit={decode(CHK,'T','true','F','false')}</C> 
								<C> Name='시간'		    ID=ARRIVE_TIME	HeadAlign=Center HeadBgColor=#EEEEEE Width=50	 align=center edit=true bgcolor=#C4DDFF edit={decode(CHK,'T','true','F','false')} editstyle='lookup' data='gcds_rm015:DETAIL:DETAIL_NM'</C> 
								
								<C> Name='예약SID'		ID=RSV_SID	    HeadAlign=Center HeadBgColor=#EEEEEE Width=50	 align=center edit=none show=false</C> 
								<C> Name='LSTAY_DEL'	ID=LSTAY_DEL	  HeadAlign=Center HeadBgColor=#EEEEEE Width=100 align=center edit=none show=false</C> 
