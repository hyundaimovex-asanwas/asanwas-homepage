...
+ 작 성 일 자   : 2008.01.18
+ 수 정 일 자   : 2008.01.19
+ 서 블 릿 명	: cm013i_s01,cm014i_s02, cm014i_t01
------------------------------------------------------------------------------>
<html>
<head>
...
<script language="javascript">
	...
	******************************************************************************/
	...
	//  prompt('',gcds_data01.DataID);
		...
		gcds_data02.DataID = "/services/servlet/sales.cm.cm014i_s02?"
												 + "&v_str1=0";
	...
	******************************************************************************/
	function ln_Valchk(){
		...
		for(i = 1 ; i <= rowcount ; i++){
			...
			if (fn_trim(gcds_data02.NameValue(i, "VIOL_DATE")) == ""  || gcds_data02.NameValue(i, "VIOL_DATE").length < 8) {
				alert("날짜를 입력하셔야 합니다.");
				return false;
			} 
		}
	...
	******************************************************************************/
	function ln_Excel(){
		gcds_data03.DataID = "/services/servlet/sales.cm.cm014i_s03";
		//prompt('',gcds_data03.DataID);
		gcds_data03.Reset();

		 SetTitle("위반내역조회");
		 if (gcds_data03.countrow<1) { //alert("해당날짜에 맞는 내용이 없습니다"); 
		 } else {gcgd_disp.GridToExcel("위반내역조회","C:\\Test\\위반내역조회.xls",2);}
	}
	/******************************************************************************
		Description : 저장
	******************************************************************************/
	function ln_Save(){
		...
		gctr_data.KeyValue = "cm014_t01(I:DATA=gcds_data02)";
	    gctr_data.Action = "/services/servlet/sales.cm.cm014i_t01";
		gctr_data.Parameters = "v_str1=" +  gcds_data02.namevalue(row,"VIOL_SID") + ", v_str2=" + gusrid;
...
<script language=JavaScript for=gcds_data02 event=OnColumnChanged(row,colid)>
	...
	if(colid == "VIOL_DATE"){
		chkValue = gcds_data02.NameValue(row, "VIOL_DATE");
		if(chkValue != ""){
//		alert("" + chkValue.substring(0, 2));
			if(!gv_date_check(chkValue)){
				alert("날짜 범위를 벗어났습니다.");
				gcds_data02.NameValue(row, "VIOL_DATE") = "";
				return;
			}
		}
	}	
	...
	if(colid == "BLACK_MARK"){
		markValue = gcds_data02.NameValue(row, "BLACK_MARK");

		for (i=0;i<markValue.length;i++){
			if(markValue.charAt(i)<="0" && markValue.charAt(i)>="9"){
				alert("벌점은 숫자 입니다.");
				break;
			}
		}
        ...
        <tr> 
          <td>
						<comment id="__NSID__"><OBJECT  id=gcgd_disp2 class="tab_b0111" classid=clsid:1F57AEAD-DB12-11D2-A4F9-00608CEBEE49  style="width:876px;height:200px;" viewastext>
							...
							<param name="Format"				value="  
								<F> Name='No.'				ID={CURROW} 			HeadAlign=Center HeadBgColor=#EEEEEE Width=30		align=center</F>
								<C> Name='위반이력'		ID=VIOL_SID				HeadAlign=Center HeadBgColor=#EEEEEE Width=0	align=center show=false edit=none</C> 
								<C> Name='CARNO_SID'	ID=CARNO_SID			HeadAlign=Center HeadBgColor=#EEEEEE Width=0	align=center show=false edit=none</C> 
								<C> Name='운전자'			ID=WORK_NO			HeadAlign=Center HeadBgColor=#EEEEEE Width=80	 align=center EditLimit=20</C> 
								<C> Name='위반일자'		ID=VIOL_DATE			HeadAlign=Center HeadBgColor=#EEEEEE Width=80 	align=center mask='XXXX/XX/XX' </C> 
								<C> Name='위반장소'	ID=VIOL_PLACE				HeadAlign=Center HeadBgColor=#EEEEEE Width=138	align=center EditLimit=30</C> 
								<C> Name='위반내용'	ID=VIOL_CONTENT	 		HeadAlign=Center HeadBgColor=#EEEEEE Width=180	align=center EditLimit=30</C> 
								<C> Name='벌점'		    ID=BLACK_MARK			HeadAlign=Center HeadBgColor=#EEEEEE Width=100	align=center</C> 
								<C> Name='조치사항'	ID=VIOL_FU					HeadAlign=Center HeadBgColor=#EEEEEE Width=250	align=center EditLimit=100</C> 
								">
			            </OBJECT></comment><script>__ws__(__NSID__);</script>
