...
function CallCalendar7(){ // 모달팝업 호출 함수7
__calendarRtn = window.showModalDialog("../common/html/com_calender.htm","","dialogWidth:180px;dialogHeight:230px;scroll:no;status:no;titlebar:no;center:yes;help:yes;");
}
...
//개체에 날짜선택 모달팝업에서 얻은 값 넣기 7 =====
function __GetCallCalendar7(objNameHead, objValueProp){
	CallCalendar7();
...
function __GetCallMonth(objNameHead, objValueProp){
	...
	//개체에 날짜선택 모달팝업에서 얻은 값 넣기 =====
function GetCallMonth(obj, objv, objf){
	CallMonth();

	if (__calMonthRtn!=null && __calMonthRtn.length>=4 ){
		eval( obj+"."+objv+" = __calMonthRtn" );
		eval( objf+"."+objv+" = gn_SetyyMm(__calMonthRtn,1)");
	...
	//개체에 날짜선택 모달팝업에서 얻은 값 넣기 =====
function GetCallMonth2(obj, objv, objf){
	CallMonth();

	if (__calMonthRtn!=null && __calMonthRtn.length>=4 ){
		eval( obj+"."+objv+" = __calMonthRtn" );
		eval( objf+"."+objv+" = __calMonthRtn");
