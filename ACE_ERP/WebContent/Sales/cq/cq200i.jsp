 ...
 *						체류현황(차량), 차량 체류변경 도 필요한거 아닌지..ㅋㅋㅋㅋ 내 촉이 맞지요 보통은.. 후;;
 * [2015-10-18][심동현] 인원 검색시 반/조 가 사업자 기준 이었던 듯.. ㅎ;; 반, 조 콤보 검색 조건
...
//			}
		...
		function eraseNO(){
			for (i = 0 ; i <= ds_cust.countrow; i++) {
				if (ds_cust.namevalue(i, "CHOICE")=="T") {
					...
					ds_cust.namevalue(i, "RIDE_CAR_NUM")= "";
					ds_cust.namevalue(i, "RIDE_CAR_NM")	= "";
	...
	function fnAssign() {
		...
		for( k=1; k<=ds_car.CountRow; k++ ) {
			ds_car.namevalue(k,"RIDE_CAR_NUM") = k;	//위에서부터 걍 차례대로~
		}
    ...
    function fnExcelDs1() {
        ...
        getObjectFirst("gr2").SetExcelTitle(1, "value:차량탑승(출경); font-face:; font-size:30pt; font-color:black;font-bold; bgcolor:white; align:center; line-color:black;line-width:2pt; skiprow:1;");
        getObjectFirst("gr2").GridToExcel("차량탑승(출경)", "차량탑승(출경).xls", 8);        
    }
 		...
 		if(colid=="RIDE_CAR_NUM"||colid=="RIDE_CAR_NM"){
			...
			for (i = 0 ; i <= ds_cust.countrow; i++) {	//ds_cust(고객정보)에서 선택된 손님 찾아서
				if (ds_cust.namevalue(i, "CHOICE")=="T") {
					...
					ds_cust.namevalue(i, "RIDE_CAR_NUM") = ds_car.namevalue(row, "RIDE_CAR_NUM");
					ds_cust.namevalue(i, "RIDE_CAR_NM")	= ds_car.namevalue(row, "RIDE_CAR_NM");
...
<!--------------------------------- 코딩 부분 시작 ------------------------------->		
	...
	<table  border='0' cellpadding='0' cellspacing='0' width='1002' >	
		<tr>
			<td align='right'>
				<img src="<%=dirPath%>/Sales/images/auto_assign.gif"  	style="cursor:pointer"	align=absmiddle onclick="fnAssign()">
				<img src="<%=dirPath%>/Sales/images/refer.gif"  	style="cursor:pointer"	align=absmiddle onclick="fnSelect1()"> 		<!-- 조회 -->
				<img src="<%=dirPath%>/Sales/images/save.gif"  	style="cursor:pointer"	align=absmiddle onclick="fnApply1()"> 		<!-- 저장 -->
				<img src="<%=dirPath%>/Sales/images/cancel.gif"  	style="cursor:pointer"	align=absmiddle onclick="fnCancel1()"> 		<!-- 취소 -->
			</td>
		</tr>
		...
		<tr>
			<td align=center>
				...
				<object id=gr1 classid=<%=HDConstant.CT_GRID_CLSID%> width=1000 height=240 border=1 style="display:;">
					...
					<G> name='출경시 차량정보'
						<C> name='호수' 		ID='RIDE_CAR_NUM'	Width=50   Edit=true align=CENTER  show=true BgColor='#fefec0' sort=true sumtext=@cnt SumTextAlign=center</C>
						<C> name='명칭' 		ID='RIDE_CAR_NM'	Width=70   Edit=true align=CENTER  show=true BgColor='#fefec0' sort=true SumText='대'  sumTextAlign=left </C>
					</G>
                        <C> name='협력사명'   ID='CLIENT_NM'		Width=80  Edit=none align=Left	show=false </C>
				...
				<table border=0 cellspacing=0 cellpadding=0>
					<tr>
						<td>				
							...
							<object id=gr2 classid=<%=HDConstant.CT_GRID_CLSID%> width=790 height=300 border=1 >
								...
								<G> name='출경시 차량정보'
									<C> name='호수' 		ID='RIDE_CAR_NUM'	Width=50   Edit=none align=CENTER  show=true sort=true SUMTEXT=명 sumTextAlign=left</C>
									<C> name='명칭' 		ID='RIDE_CAR_NM'	Width=70   Edit=none align=CENTER  show=true </C>
									<C> name='동승' 		ID='DONGSEUNG'     	width=45   	align=center editlimit=10   editstyle=lookup  data='ds_code2:detail:detail_nm' edit=true show=true  bgColor={IF(DONGSEUNG='Y','#ffcccc','#fefec0') }  </C>
									<C> name='등록번호'     ID='CAR_NO'         Width=80   Edit=none align=Left sort=true </C>									
								</G>
						...
						<td align=right valign=top width="200">
							<table  border=0 cellpadding='0' cellspacing='1' bgcolor=#666666>
								...
								<tr>
			                        ...
			                        <td align=left bgcolor="#ffffff">&nbsp;&nbsp;<input type=checkbox name="sAssign_YN" id="sAssign_YN">　　　<img src="<%=dirPath%>/Sales/images/eraser.gif" style="cursor:pointer"	align=absmiddle onclick="eraseNO()"></td>
								</tr>								
								<tr>
								...
								<tr height=24>
			                        <td align=center bgcolor="#ffffff" colspan=3><img src="<%=dirPath%>/Sales/images/refer.gif"  	style="cursor:pointer"	align=absmiddle onclick="fnSelect2()"> 		<!-- 조회 -->
										<img src="<%=dirPath%>/Sales/images/save.gif"  	style="cursor:pointer"	align=absmiddle onclick="fnApply2()"> 		<!-- 상품의 숙박일자 기준으로 저장 -->
										<img src="<%=dirPath%>/Sales/images/cancel.gif"  	style="cursor:pointer"	align=absmiddle onclick="fnCancel2()"> 		<!-- 취소 -->
									</td>
								</tr>
								<tr height=24>
