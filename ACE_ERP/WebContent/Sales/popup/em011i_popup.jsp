...
function ln_Query(e){
	if(parentGubun=="") {	
		gcds_data.DataID = "<%=dirPath%>/servlet/Person.h030003_s2?"
										 + "v_str1=" + txt_nm.value;
	}	else if(parentGubun=="P040001") {	//추계퇴직급조회
		gcds_data.DataID = "<%=dirPath%>/servlet/Payroll.p040001_pop_s1?"
									   + "v_str1=" + txt_nm.value;
	  ...
	  if(window.dialogArguments[1]==""||window.dialogArguments[1]==undefined ){
			gcds_data.DataID = "<%=dirPath%>/servlet/Person.h030003_s5?"  //야근특근 전체용
											 + "v_str1=" + txt_nm.value;
		}else{
			gcds_data.DataID = "<%=dirPath%>/servlet/Person.h030003_s6?"  //야근특근 부서 관리자용
											 + "v_str1="+txt_nm.value
											 + "&v_str2="
		...
		if(window.dialogArguments[1]==""||window.dialogArguments[1]==undefined ){
			gcds_data.DataID = "<%=dirPath%>/servlet/Person.h030003_s7?"  //근태 전체용
											 + "v_str1=" + txt_nm.value;
		}else{
			gcds_data.DataID = "<%=dirPath%>/servlet/Person.h030003_s8?"  //근태 부서 관리자용
											 + "v_str1="+txt_nm.value
											 + "&v_str2="
											 + "&v_str3="+window.dialogArguments[1];//사번
		}
