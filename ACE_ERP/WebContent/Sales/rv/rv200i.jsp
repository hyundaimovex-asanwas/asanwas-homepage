...
//            init_businessDs.Reset();
        ...
        /*
         * Sales\common\js\rv200i.js 의 소스를 가져옴. 예약쪽도 20151015
         */
         var v_cust_flag = "";
         var v_room_flag = "";
         var v_block_row = 0;
         ...
         /**
          * 출발일자로 조회시
          * 신청서 번호가 존재하면, 상품정보를 조회를 하면,
          * 그렇지 않을 경우엔 신규 등록으로 판단한다.
          */
...
<script language="javascript" for="tr_temp" event="OnSuccess()">
    ...
    var info_msg = "";
	//alert("ds_cust_result.text");
	//alert(ds_cust_result.text);
	//alert(ds_cust_result.CountRow);
    ...
    //alert("ds_cust_temp.text");
	//alert(ds_cust_temp.text);
	//alert(ds_cust_temp.CountRow);
    
    if(getObjectFirst("ds_cust_temp").CountRow==0){
        if((v_manage_no.Text!="") && (v_manage_no.Text!=null)){
...
<script language="javascript" for="tr_save" event="OnSuccess()">
     ...
     */
    msgTxt.innerHTML = "<%=HDConstant.S_MSG_SAVE_DONE%>";
    msgTxt.innerHTML = "tr_save OnSuccess!!!";
	fnHideLoading();
    ...
    //alert("ds_cust_result");
    //alert(ds_cust_result.text);
    //prompt('',ds_cust_result.text);
    //alert(ds_cust_result.CountRow);
    if(ds_cust_result.CountRow>0){
    	//alert("ds_cust_result.CountRow>0");
        for(var i=1; i<=ds_cust_result.CountRow; i++){
            if(ds_cust_result.NameValue(i, "MSG_CD")!="0000"){
    ...
    //alert("ds_room_result");
    //alert(ds_room_result.CountRow);
    if(ds_room_result.CountRow>0){
    	//alert("ds_room_result.CountRow>0");
    ...
    if(ds_amt_result.CountRow>0){
    	//alert("ds_amt_result.CountRow>0");
    ...
    if(ds_cust_result.CountRow>0){
    	//alert("ds_cust_result.CountRow>0");
        if((var_accept_no!="") && (var_accept_no!=null)){
...
<script language=javascript for="ds_search" event="OnLoadCompleted(row)">
    ...
    if((v_accept_sid.value!="") && (v_accept_sid.value!=null)){
        fnGetTour('<%=dirPath%>','<%=HDConstant.PATH_RV%>');
    	fnGetCar('<%=dirPath%>','<%=HDConstant.PATH_RV%>');
    }
</script>
...
<script language=javascript for="v_goods_nm" event="OnSelChange()">
    if(getObjectFirst("ds_search").CountRow<=1 || v_cust_flag=="" || v_cust_flag==null) return;
...
<script language=javascript for="v_blockType" event="OnSelChange()">
    if(getObjectFirst("init_blockTypeDs").CountRow==0 || v_cust_flag=="" || v_cust_flag==null) return;
    setTimeout("fnBlockType('<%=dirPath%>', '<%=HDConstant.PATH_RV%>')", 200);
</script>
<script language=javascript for="v_roomType" event="OnSelChange()">
    if(getObjectFirst("ds_room").CountRow==0 || v_cust_flag=="" || v_cust_flag==null) return;
                                ...
                                <td width="114" valign=top>
                                    <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                        ...
                                        <tr>
                                            <td valign=top>
                                                <!-- 객실상태: ds_block-->
                                                <object  classid=<%=HDConstant.CT_GRID_CLSID%>  id=gr_block  style="width=114px; height:417px" border="1">
                                                    ...
                                                    <tr>
                                                        <td>
                                                            ...
                                                            <object classid=<%=HDConstant.CT_GRID_CLSID%>  id=gr_cust  style="width=434px; height:200px" border="1">
                                                                ...
                                                                <%
                                                                    if((v_ut.equals(null)) && (v_ut.equals(""))){
                                                                %>
                                                        ...
                                                        <td width="326" valign="top">
                                                            <table border="0" cellspacing="1" cellpadding="0" bgcolor="#666666">
                                                                ...
                                                                <tr height="25">
                                                                    ...
                                                                    <td bgcolor="#FFFFFF">&nbsp;<input name="hold_empnm" id="hold_empnm" type="text" class="input01" size="8" readOnly> <img src="<%=dirPath%>/Sales/images/help.gif" id="hold_btn" border="0" align=absmiddle style="cursor:pointer;display:none" onclick="fnHoldEmp()"></td>
                ...
                <tr>
                    <td>
                        <table height="30" border="0" cellspacing="0" cellpadding="0" align=left>
                            <tr>                                
                                <td width="95"><img src="<%=dirPath%>/Sales/images/tab_01.gif" name="sub1" width="95" height="28" border="0" onMouseOver="changesub(1)" onMouseOut="changesubout(1)" style="cursor:pointer" onClick="changesubclick(1), fnOnChangTab('1')"></td>
                ...
                <tr>
                    <td height="497" valign=top style="padding-top:4px">
                        ...
                        <table id=tb_tourlist style="display:inline" border=0 cellspacing=0 cellpadding=0>
                            ...
                            <tr>
                                <td valign=top>
                                    <object classid=<%=HDConstant.CT_GRID_CLSID%> id=gr_tourlist  style="width:448; height:510" border="1">
                                        ...
                                        <param name="Format"         VALUE="
                                            ...
                                            <C> name='객실\\등급'   ID='ROOM_TYPE_CD'  Width=50   Edit=None   bgcolor={DECODE(MOVE_TEMP,'M','<%=HDConstant.GRID_ITEM%>',DECODE(JOIN_CD, '01', '<%=HDConstant.GRID_TOUR_COLOR%>', ''))} align=Left SumText='인원/금액' suppress=1 sort=true</C>
                                            <C> name='성명'        ID='CUST_NM'       Width=140   Edit=None   bgcolor={DECODE(MOVE_TEMP,'M','<%=HDConstant.GRID_ITEM%>',DECODE(JOIN_CD, '01', '<%=HDConstant.GRID_TOUR_COLOR%>', ''))} align=Left SumText=@count SumTextAlign=right sort=true</C>
                            ...
                            <tr>
                                <td align="right">
                                    <!--<img src="<%=dirPath%>/Sales/images/prize_cal.gif" align="absbottom" style="display:none; cursor:pointer" OnClick="fnAmtSave('<%=dirPath%>', '<%=HDConstant.PATH_RV%>')" id="btn_amtSave">-->
                                </td>
                            </tr>
                        </table>
