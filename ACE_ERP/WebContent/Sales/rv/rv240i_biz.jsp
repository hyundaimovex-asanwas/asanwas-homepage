 ...
 * [2008-04-22][심동현] 방북신청 마감 적용..
 * [2009-12-23][심동현] 차량신청부터 오픈
 *						-. 운전자 팝 닫기에서 운전자로 추가된 고객이 아닌데에도 하단 그리드에서 삭제된다!
 *****************************************************************************/
%>
<% 
///////////////////////////////////////////////////////////////////////////////
...
//   String firsttime = m_today.format(date.getTime()).substring(8,12);
	String nowYear = firstday.substring(0,4);
%>
...
//			to_accept_date.text="<%=firstday%>";  
	  		from_accept_date.text="20091218";	//신청일자 셋업
			to_accept_date.text="20091218";  

			    
			ln_TRSetting(tr1, 
	            "<%=dirPath%><%=HDConstant.PATH_RV%>Rv240I",
	            "JSP(O:RM014=codeDs1,O:RM015=codeDs2,O:SY010=codeDs4,O:CU022=codeDs5, O:DS1=ds1, O:DS2=ds2, O:DS3=ds3)",
		...
		//마감일자 확인: 2008-04-22 심동현
		function fnDeptDateChk(dept_date){
		 	var param = "proType=<%=HDConstant.PROCESS_TYPE_SEARCH%>,dsType=1"
		 				+",sClientSid=" + sClient_sid.value
		 				+",sDeptDate=" + dept_date;
		 	ln_TRSetting(tr2, 
			    "<%=dirPath%><%=HDConstant.PATH_RV%>Rv240I",
			    "JSP(O:RSV_CLOSE_DATE=ds_rsv_close)",
			    param);
			tr_post(tr2);	
		}
		...
		function fnDeleteRowDs3() {
			if(depart_date.Text!="" && depart_date.Text.length==8 && v_goods.Index!=-1 && v_goods.ValueOfIndex("GOODS_SID", v_goods.Index)!='0'){
				fnDeptDateChk(depart_date.Text);
			}
		...
		function fnAddRowDs1() {
			if (sClient_sid.value != null && sClient_sid.value != "" && sClient_sid.value.length >0 ) {
				...
				depart_date.text=from_accept_date.text; // 출발일자도 끌고 오자..;
				v_goods.Enable = "true";
		 ...
		 function fnAddRowDs3() {
			if(depart_date.Text!="" && depart_date.Text.length==8 && v_goods.Index!=-1 && v_goods.ValueOfIndex("GOODS_SID", v_goods.Index)!='0'){
		 	...
		 	if (ds1.rowposition <= 0) {
				...
				ds3.namevalue(ds3.rowposition,"long_stay_yn") = "N"	//장기여부 기본값 
				ds3.namevalue(ds3.rowposition,"n_card_gu") = "1";	//방북증 소재 기본값 
				ds3.namevalue(ds3.rowposition,"room_type_cd") = "KVLOO"	//희망객실 기본값  KVLOO
				ds3.namevalue(ds3.rowposition,"a_room_type_sid") = 213;	//희망객실sid 기본값 
...
//			strURL = "<%=dirPath%>/Sales/help/rv002h.jsp?sClientSid="+ sClient_sid.value+"&sClientNm="+ sClient_nm.value;
		...
		/*
		 * 방북증 팝업 : 인원 그리드(gr3)에서 
		 */
		function fnNcardPopup() {
			var arrParam	= new Array();
			var arrResult	= new Array();
			var strURL;	 
			var strPos;
			strURL = "<%=dirPath%>/Sales/help/rv016h.jsp?sCustSid="+ ds3.namevalue(ds3.rowposition,"cust_sid");
			strPos = "dialogWidth:495px;dialogHeight:379px;status:no;scroll:no;resizable:no";
			arrResult = showModalDialog(strURL,arrParam,strPos);	

			if (arrResult != null) {
				arrParam = arrResult.split(";");			
		   		ds3.namevalue(ds3.rowposition, "n_card_no") = arrParam[0];
		   		ds3.namevalue(ds3.rowposition,"north_purpose") = arrParam[1];
			} else {
				alert("방북증 정보가 없습니다.");
		   		ds3.namevalue(ds3.rowposition,"n_card_no") = "";
		   		ds3.namevalue(ds3.rowposition,"north_purpose") = "";
			}				
		}
		/*
		 * 희망객실 팝업 : 인원 그리드(gr3)에서 
		 */
		function fnRoomTypePopup() {
           ...
           var strPos;
               ...
               if (arrResult != undefined) {
                   arrParam = arrResult.split(";");
                   ds3.namevalue(ds3.RowPosition,"a_room_type_sid")=arrParam[0];
                   ds3.namevalue(ds3.RowPosition,"room_type_cd")=arrParam[1];
                   ...
                   ds3.namevalue(ds3.RowPosition,"a_room_type_sid")=0;
                   ds3.namevalue(ds3.RowPosition,"room_type_cd")='';
		...
		function fnChecked() {
			...
			if (ds3.IsUpdated) {
				for (j = 1; j <= ds3.countrow; j++) {
					...
					} else if (ds3.namevalue(j,"long_stay_yn") !="Y" && ds3.namevalue(j,"long_stay_yn") !="N") {
						alert("장기여부를 입력하세요!");
						gr3.SetColumn("long_stay_yn");
						return;

					} else if (ds3.namevalue(j,"manage_no") =="") {
						alert("주민/여권번호를을 입력하세요!");
						gr3.SetColumn("manage_no");
						return;

					} else if (ds3.namevalue(j,"n_card_gu") =="") {
						alert("방북증소재를 선택해주세요.");
						gr3.SetColumn("n_card_gu");
						return;

					} else if (ds3.namevalue(j,"north_purpose") =="") {
						alert("방북목적을 입력하세요!");
						gr3.SetColumn("north_purpose");
						return;
...
%>
	...
	<!--  출발일자 체크 : 인원그리드에서 비어 있을 경우 (발생하지 않음) -->
	...
	<!-- [장기]를 선택했을 때 다음해 12월 31로.. -->
	<script language=JavaScript for=gr3 event=OnCloseUp(row,colid)>
	    if(colid=="long_stay_yn"){
			if(ds3.namevalue(ds3.RowPosition,"long_stay_yn")=="Y"){
				var v1, v2, vs;
				vs = "";
//				v1 = ds3.namevalue(ds3.RowPosition,"arrive_date");
				v2 = parseInt("<%=nowYear%>") + 1;
				ds3.namevalue(ds3.RowPosition,"arrive_date") = vs.concat(v2 , "1231");
			}
	    }
	</script>
	...
	<script language=JavaScript for=ds_rsv_close event=OnLoadCompleted(rowcnt)>
		var closeYN =ds_rsv_close.namevalue(1,"DEPT_FLAG");

		if(closeYN=="Y"){
			alert("해당 출발일은 마감되었습니다.\n출입팀으로 문의하시거나 처음부터 다시 진행해주세요.");
		 	updateFlag = "";
		 	ds1.UndoAll();
		 	ds2.UndoAll();
		 	ds3.UndoAll();
   ...
   <script language=JavaScript for=from_accept_date event=OnKillFocus()>
       if( from_accept_date.Modified == true )
			to_accept_date.text = from_accept_date.text;
   </script>
...
<object id=codeDs5 classid=<%=HDConstant.CT_DATASET_CLSID%>> <!-- 방북증소재   -->
	<param name="SyncLoad" 	value="false">
</object>
...
<object id=ds_rsv_close classid=<%=HDConstant.CT_DATASET_CLSID%>> <!-- 방북신청 마감 체크   -->
	<param name="SyncLoad" 	value="false">
</object>
<object id=ds_temp classid=<%=HDConstant.CT_DATASET_CLSID%>> <!-- 상품정보 임시 보관 여부   -->
	<param name="SyncLoad" 	value="true">
</object>
...
<!--------------------------------- 코딩 부분 시작 ------------------------------->	
	...
	<table border='1' cellpadding='0' cellspacing='0' width='845' bordercolor=orange>	
		<tr>
			<td>
			...
			<td colspan="3"  border='0' cellpadding='0' cellspacing='0' width='845'>
				<table colspan="3"  border='0' cellpadding='0' cellspacing='1' width='845' bgcolor="#666666">
					<tr height="30px">
						...
						<td bgcolor="#FFFFFF" class="text" width="100px">출발일자</td>
						<td  bgcolor="#FFFFFF" align="center" width="200px">
							<%=HDConstant.COMMENT_START%>
		...
		<tr>
			<td width='210' valign="top">
				<table  border='0' cellpadding='0' cellspacing='0' width='210'>
					<%=HDConstant.COMMENT_START%>
					<object id=gr1 classid=<%=HDConstant.CT_GRID_CLSID%> width='210px' height='218px' border='1'>
						...
						<param name="Format" 			value="
			               ...
			               <C> name='신청서번호'	ID='accept_no'   	align=left width=105 	editlimit=15 	show=true mask='XXXXX-XXXXXX-XXX'</C>
			               <C> name='신청일자'  	ID='accept_date'	align=left width=70 	editlimit=8 	show=true  mask='XXXX-XX-XX' bgColor=<%=HDConstant.GRID_ITEM%></C>
						">											
					</object>	
					<%=HDConstant.COMMENT_END%>		
				</table>
			...
			<td valign="top">
				<table  border='0' cellpadding='0' cellspacing='0' width='605'>
					<tr>
						...
						<td align="right" width='200px' valign=bottom style="display:;">
							<img src="<%=dirPath%>/Sales/images/plus.gif"		style="cursor:hand" 	align=absmiddle onClick="fnAddRowDs2()" >		<%//차량추가%>
							<img src="<%=dirPath%>/Sales/images/minus.gif" 		style="cursor:hand" 	align=absmiddle onClick="fnDeleteRowDs2()" >	<%//차량삭제%>
						</td>
					</tr>
					<tr height="5px">
						<td></td>
					...
					<tr>
						<td colspan="2"  width='610px' style="display:">
							<%=HDConstant.COMMENT_START%>
							<object id=gr2 classid=<%=HDConstant.CT_GRID_CLSID%> width='620px' height='150px' border='1'>
			...
			<td valign="top" colspan=3 align=right>
				...
				<table  border='0' cellpadding='0' cellspacing='0' width="845">
					...
					<tr>
						<td>
							<%=HDConstant.COMMENT_START%>
							<object id=gr3 classid=<%=HDConstant.CT_GRID_CLSID%> width='845px' height='240px' border='1'>
								...
								<param name="Format" 			value="
									<G> name='방북 기간'
										<C> name='출발일자'	ID='depart_date'	align=left width=70 editlimit=8 	show=true mask='XXXX-XX-XX' edit=none</C>
										<C> name='출경시각'  	ID='depart_time'	align=left width=60 editlimit=4 	show=true editstyle=lookup  data='codeDs1:detail:detail_nm' edit=none</C>
										<C> name='입경일자'  	ID='arrive_date'	align=left width=70 editlimit=8 show=true mask='XXXX-XX-XX bgColor=<%=HDConstant.GRID_ITEM%>'</C>
										<C> name='입경시각'  	ID='arrive_time'	align=left width=60 editlimit=4 show=true editstyle=lookup  data='codeDs2:detail:detail_nm' bgColor=<%=HDConstant.GRID_ITEM%></C>
										<C> name='장기여부'  	ID='long_stay_yn'    align=left width=60 	editlimit=4 	show=true editstyle=lookup  data='ds_stay:detail:detail_nm' bgColor=<%=HDConstant.GRID_ITEM%></C>
									</G>
									<G> name='사업자 출입정보'
										<C> name='한글성명'	ID='cust_nm'     	align=left width=100 show=true editstyle=popup</C>
										<C> name='주민/여권번호'	ID='manage_no'   	align=left width=100 editlimit=20 show=true edit=none</C>
										<C> name='할인유형'	ID='cust_type'	align=left width=70 editlimit=8 	editstyle=lookup  data='ds_cust:detail:detail_nm'</C>
										<C> name='실무번호'	ID='work_no'	align=left width=100 show=true edit=none</C>
										<C> name='방북증번호'	ID='n_card_no'	align=left width=100 show=true editstyle=popup EDIT=NONE</C>
										<C> name='방북목적'	ID='north_purpose'	align=left width=110 editlimit=62 show=true</C>
										<C> name='운전여부'   ID='drive_yn'		align=left width=60 editlimit=1 show=false editstyle=lookup  data='codeDs4:detail:detail_nm' edit=none</C>
										<C> name='방북증소재'  ID='n_card_gu'		align=left width=70 editlimit=1 show=true editstyle=lookup  data='codeDs5:detail:detail_nm'</C>
									</G>
									<G> name='숙소 정보'
										<C> name='희망객실'	ID='room_type_cd'	align=left width=100 show=true   editstyle=popup edit=none</C>
										<C> name='참고사항'	ID='REMARKS'  	   Width=150   	align=left       editlimit=62 show=true</C>
									</G>
									<C> name='신청SID'  	ID='accept_sid'   	align=left width=100 editlimit=12 show=false</C>
									...
									<C> name='객실SID'   ID='a_room_type_sid'   align=left width=100 editlimit=12 show=false</C>
								">											
							</object>	
							<%=HDConstant.COMMENT_END%>							
						</td>
				...
				<tr>
					<td><BR>※ 현지체류하는 사업자께서는 하단 GRID의 <B>[방북기간>장기여부]</B> 에서 <B>장기</B>를 선택해주시기 바랍니다.
