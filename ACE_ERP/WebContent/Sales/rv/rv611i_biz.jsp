 ...
 * 작 성 자		 : 심동현
 * 작 성 일		 : 2008-04-21
 * 기능정의		 : 일자별 방북증 발급마감, 방북신청 마감
 * [수  정   일  자][수정자] 내용
 * [2008-04-21][심동현] 새로 생성
 * [2008-05-16][심동현] 관광객용 TL과 사업자용 TL 분리.
 * [2010-04-06][심동현] 방북마감을 분리, 금강산과 개성 따로.
 *****************************************************************************/
%>
<%
///////////////////////////////////////////////////////////////////////////////
...
<script language="javascript">
    ...
    function fnSelect() {
		if (ds_master.isUpdated || ds_detail.isUpdated) { 
	        ...
	        ln_TRSetting(tr1, 
	            "<%=dirPath%><%=HDConstant.PATH_RV%>Rv611I",
		...
		if (ds_master.IsUpdated || ds_detail.IsUpdated) {
            if(ds_master.IsUpdated){
                for(i=1;i<=ds_master.CountRow;i++) {
                    if(ds_master.NameValue(i, "N_RSV_CLOSE_DATE")=="" || ds_master.NameValue(i, "N_RSV_CLOSE_DATE").length!=8){
                        alert("금강산방북 마감일자 <%=HDConstant.A_MSG_MUST_INPUT%>");
                        gr_master.SetColumn("N_RSV_CLOSE_DATE");
                        ds_master.RowPosition = i;
                        return;
                    }
                    if(ds_master.NameValue(i, "GS_CLOSE_DATE")=="" || ds_master.NameValue(i, "GS_CLOSE_DATE").length!=8){
                        alert("개성방북 마감일자 <%=HDConstant.A_MSG_MUST_INPUT%>");
                        gr_master.SetColumn("GS_CLOSE_DATE");
                        ds_master.RowPosition = i;
                        return;
                    }
                }
            ...
            if(ds_detail.IsUpdated){
                for(i=1;i<=ds_detail.CountRow;i++) {
                    if(ds_detail.NameValue(i, "TL_DATE_BIZ")!="" && ds_detail.NameValue(i, "TL_DATE_BIZ").length!=8){
                        alert("T/L 금강산  <%=HDConstant.A_MSG_MUST_INPUT%>");
                        gr_detail.SetColumn("TL_DATE_BIZ");
                        ds_detail.RowPosition = i;
                        return;
                    }
                    if(ds_detail.NameValue(i, "TL_DATE_BIZ2")!="" && ds_detail.NameValue(i, "TL_DATE_BIZ2").length!=8){
                        alert("T/L 개성  <%=HDConstant.A_MSG_MUST_INPUT%>");
                        gr_detail.SetColumn("TL_DATE_BIZ");
                        ds_detail.RowPosition = i;
                        return;
                    }
...
%>
    ...
    <script language=JavaScript for=gr_detail event=OnClick(Row,Colid)>
        if(Row==0) {
            if(Colid == "TL_DATE_BIZ"){
                for(var i=1; i<=ds_detail.CountRow; i++){
                    ds_detail.NameValue(i, Colid)=ds_detail.NameValue(1, Colid);
                    ds_detail.NameValue(i, "YOIL_NM_BIZ")=ds_detail.NameValue(1, "YOIL_NM_BIZ");
                }
            }
            if(Colid == "TL_DATE_BIZ2"){
                for(var i=1; i<=ds_detail.CountRow; i++){
                    ds_detail.NameValue(i, Colid)=ds_detail.NameValue(1, Colid);
                    ds_detail.NameValue(i, "YOIL_NM_BIZ2")=ds_detail.NameValue(1, "YOIL_NM_BIZ2");
                }
            }            
            return;
        }
    ...
    <script language=JavaScript for=gr_detail event=OnExit(Row,Colid,olddata)>
        if(Row==0) return;
        if(Colid == "TL_DATE_BIZ2"){
            var v_tl_date = ds_detail.NameValue(Row, Colid);
            if(v_tl_date=="" || v_tl_date.length!=8) {
                ds_detail.NameValue(Row, "YOIL_NM_BIZ2") = "";
                return;
            }
            
            if(ds_detail.NameValue(Row, Colid)!=olddata){
                var first_date=new Date(v_tl_date.substring(0, 4),eval(v_tl_date.substring(4, 6))-1,eval(v_tl_date.substring(6)));
                switch(first_date.getDay()+1){
                    case 1 :
                        ds_detail.NameValue(Row, "YOIL_NM_BIZ2") = "일";
                        break;
                    case 2 :
                        ds_detail.NameValue(Row, "YOIL_NM_BIZ2") = "월";
                        break;
                    case 3 :
                        ds_detail.NameValue(Row, "YOIL_NM_BIZ2") = "화";
                        break;
                    case 4 :
                        ds_detail.NameValue(Row, "YOIL_NM_BIZ2") = "수";
                        break;
                    case 5 :
                        ds_detail.NameValue(Row, "YOIL_NM_BIZ2") = "목";
                        break;
                    case 6 :
                        ds_detail.NameValue(Row, "YOIL_NM_BIZ2") = "금";
                        break;
                    case 7 :
                        ds_detail.NameValue(Row, "YOIL_NM_BIZ2") = "토";
                        break;
                }
            }
        } else if(Colid=="TL_DATE_BIZ"){
            var v_tl_date = ds_detail.NameValue(Row, Colid);
            if(v_tl_date=="" || v_tl_date.length!=8) {
                ds_detail.NameValue(Row, "YOIL_NM_BIZ") = "";
                return;
            }
            ...
            if(ds_detail.NameValue(Row, Colid)!=olddata){
                ...
                switch(first_date.getDay()+1){
                    case 1 :
                        ds_detail.NameValue(Row, "YOIL_NM_BIZ") = "일";
                        break;
                    case 2 :
                        ds_detail.NameValue(Row, "YOIL_NM_BIZ") = "월";
                        break;
                    case 3 :
                        ds_detail.NameValue(Row, "YOIL_NM_BIZ") = "화";
                        break;
                    case 4 :
                        ds_detail.NameValue(Row, "YOIL_NM_BIZ") = "수";
                        break;
                    case 5 :
                        ds_detail.NameValue(Row, "YOIL_NM_BIZ") = "목";
                        break;
                    case 6 :
                        ds_detail.NameValue(Row, "YOIL_NM_BIZ") = "금";
                        break;
                    case 7 :
                        ds_detail.NameValue(Row, "YOIL_NM_BIZ") = "토";
                        break;
                }
            }
                ...
                <tr>
                    <td width="845px" colspan='2'>
                        <table  border="0" cellpadding="0" cellspacing="0" >
                            <tr valign="top"">
                                <td align="left" width="276px" border="0" cellpadding="0" cellspacing="0" bordercolor="#000000">
                                    <%=HDConstant.COMMENT_START%>
                                    <object  classid=<%=HDConstant.CT_GRID_CLSID%>  id=gr_master  style="width=276px; height:460px" border="1">
                                        ...
                                        <param name="Format"            value=" 
                                            <C> name='출발일자'     ID='CALN_DATE'         Width=70  align=center mask=XXXX-XX-XX Edit=none bgcolor='<%=HDConstant.GRID_ITEM%>'</C>
                                            <C> name='요일'       ID='YOIL_NM'            Width=30  align=center Edit=none bgcolor='<%=HDConstant.GRID_ITEM%>'</C>
                                            <C> name='방북 마감\\[금강산]'    ID='N_RSV_CLOSE_DATE'   Width=75  align=center mask=XXXX-XX-XX Edit=Numeric</C>
                                            <C> name='방북 마감\\[개성]'    ID='GS_CLOSE_DATE'     Width=75  align=center mask=XXXX-XX-XX Edit=Numeric</C>
                                        ">
                                    </object>
                                    <%=HDConstant.COMMENT_END%>
                                </td>       
                                <td width=10></td>
                                <td align="right" width="559px" border="0" cellpadding="0" cellspacing="0" bordercolor="#000000">
                                    <%=HDConstant.COMMENT_START%>
                                    <object  classid=<%=HDConstant.CT_GRID_CLSID%>  id=gr_detail  style="width:559px; height:460px" border="1">
                                        ...
                                        <param name="Format"            value=" 
                                            <C> name='코드'	ID='CLIENT_CD'	sort=true Width=55  align=center Edit=none bgcolor='<%=HDConstant.GRID_ITEM%>'</C>
                                            <C> name='대리점'      ID='CLIENT_NM'        sort=true Width=100  align=Left Edit=none bgcolor='<%=HDConstant.GRID_ITEM%>'</C>
                                            <C> name='T/L 방북√\\[금강산]'     ID='TL_DATE_BIZ'    Width=70  align=center mask=XXXX-XX-XX Edit=Numeric EditLimit=8</C>
                                            <C> name='요일'        ID='YOIL_NM_BIZ'		Width=30  align=center Edit=none bgcolor='<%=HDConstant.GRID_ITEM%>'</C>
                                            <C> name='T/L 방북√\\[개성]'     ID='TL_DATE_BIZ2'    Width=70  align=center mask=XXXX-XX-XX Edit=Numeric EditLimit=8</C>
                                            <C> name='요일'        ID='YOIL_NM_BIZ2'		Width=30  align=center Edit=none bgcolor='<%=HDConstant.GRID_ITEM%>'</C>
                                            <C> name='참고사항'     ID='REMARKS'    Width=185  align=left Edit=Any EditLimit=50</C>
                                        ">
                                    </object>
                                    <%=HDConstant.COMMENT_END%>
                                </td>
