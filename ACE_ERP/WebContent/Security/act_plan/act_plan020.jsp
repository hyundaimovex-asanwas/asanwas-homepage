...
jui.ready([ "grid.xtable" ], function(xtable) {
        ...
        event: {
        	 ...
        	 click:function(e){ 
					...
					if(e.index == "0"){
						...
						var url = "/HDAsan/Security/act_plan/act_plan200.jsp";
						
					   	window.open(url,"memoRead_from","scrollbars=no,toolbar=no,location=no,directories=no,noresize");							
						
					}else if(e.index == "1"){
						
						var url = "/HDAsan/Security/act_plan/act_plan220.jsp";
						
					   	window.open(url,"memoRead_from","scrollbars=no,toolbar=no,location=no,directories=no,noresize");	
						
					}else if(e.index == "2"){
						
						var url = "/HDAsan/Security/act_plan/act_plan240.jsp";
						
					   	window.open(url,"memoRead_from","scrollbars=no,toolbar=no,location=no,directories=no,noresize");						
						
					}else if(e.index == "3"){
						
						var url = "/HDAsan/Security/act_plan/act_plan260.jsp";
						
					   	window.open(url,"memoRead_from","scrollbars=no,toolbar=no,location=no,directories=no,noresize");	
						
					}else if(e.index == "4"){
						
						var url = "/HDAsan/Security/act_plan/act_plan280.jsp";
    ...
    xtable_1_submit = function() {
        ...
        var result = [];
			...
			result.push({ folder_name : "관광운영"});
			result.push({ folder_name : "영업 및 마케팅"});
			result.push({ folder_name : "시설 개보수"});
			result.push({ folder_name : "조직 및 인력"});
			result.push({ folder_name : "협력회사"});
