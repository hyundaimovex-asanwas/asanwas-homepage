...
<OBJECT id=gcDs_etc33 classid=clsid:3267EA0D-B5D8-11D2-A4F9-00608CEBEE49 >
	 <param name=SyncLoad value="false">
</OBJECT>
...
<OBJECT id=gcDsgetdate classid=clsid:3267EA0D-B5D8-11D2-A4F9-00608CEBEE49 >
	 <param name=SyncLoad value="true">
</OBJECT>
...
------------------------------------------------------------------------------>
<%=HDConstant.COMMENT_START%>
<OBJECT id="gcTr1" classid="clsid:0A2233AD-E771-11D2-973D-00104B15E56F" >
...
<SCRIPT language=JavaScript>
	...
	******************************************************************************/
	function Start(){
		...
		prompt('gcDs_etc11.DataID ',gcDs_etc11.DataID );
		alert("countrow::"+gcDs_etc11.countrow);
		gcDs_etc11.Reset();
		gcDs_etc1.DataId = "<%=dirPath%><%=HDConstant.PATH_TRANSFER%>tr00501_s1?v_str1=0002"; //Project
		gcDs_etc1.Reset();
		alert("gcDs_etc1countrow::"+gcDs_etc1.countrow);
		...
		if (str1=="" || str1==undefined){
			...
			//prompt('0000', gcDsgetdate.DataId);
			gcDsgetdate.Reset() ;	
			//prompt('',gcDsgetdate.NameValue(1,"APP_DT1"));
	...
	******************************************************************************/
    ...
    var strChkGb="";      //체크박스 구분
		...
		if (confirm("선택하신 주문정보를 삭제하시겠습니까 ?")){
			...
			//prompt("11",gcDs1.text);
			gcTr1.post();
		}
	}

	/******************************************************************************
		Description : 그리드 포멧 변경    A:주문별 , B:차량별 , C:상하차지별
	******************************************************************************/
		...
		if(format_type != SEARCH_TYPE){
			if(SEARCH_TYPE.BindColVal =="A"){
				gcGR1.Format  = "<F> Name=''		 ID=RATE_FLAG 	HeadAlign=Center HeadBgColor=#E8E8E8 Width=20	 align=center editstyle=checkbox, HeadCheck=false HeadCheckShow=true, cursor=hand</F> "
							  ...
							  + "<C> Name='상태'      ID=ORDER_STS	HeadAlign=Center HeadBgColor=#eeeeee Width=50  align=center , editstyle=LookUp Data='gcDs_etc3:MINORCD:MINORNM' </C> "
							  + "<C> Name='반출일자'   ID=ETD_DT		HeadAlign=Center HeadBgColor=#eeeeee Width=80  align=center MASK ='XXXX-XX-XX'</C> "
							  + "<C> Name='사업명'     ID=LINE_PART	HeadAlign=Center HeadBgColor=#eeeeee Width=100  align=left EditStyle=Lookup Data='gcDs_etc1:MINORCD:MINORNM'</C> "
							  + "<C> Name='물류비 부담' ID=CDNAM		HeadAlign=Center HeadBgColor=#eeeeee Width=100  align=left</C> "
							  + "<C> Name='신청업체명'  ID=CUST_NM		HeadAlign=Center HeadBgColor=#eeeeee Width=100  align=left</C> "
							  + "<C> Name='담당자'     ID=CUST_PRSN  	HeadAlign=Center HeadBgColor=#eeeeee Width=70  align=left  show=false</C> "
							  + "<C> Name='화주'      ID=SHIPPER_NM	HeadAlign=Center HeadBgColor=#eeeeee Width=100  align=left</C> "
						   	  + "<C> Name='담당자'     ID=SHIPPERPS  	HeadAlign=Center HeadBgColor=#eeeeee Width=70  align=left show=false</C> "
							  + "<C> Name='적재구분'   ID=CARGO_TYPE  HeadAlign=Center HeadBgColor=#eeeeee Width=90  align=left , editstyle=LookUp Data='gcDs_etc2:MINORCD:MINORNM'</C> "
			...
			} else if(SEARCH_TYPE.BindColVal =="B") { //차량별
				gcGR1.Format  = "<F> Name=''		 ID=RATE_FLAG 	HeadAlign=Center HeadBgColor=#E8E8E8 Width=20	 align=center editstyle=checkbox, HeadCheck=false HeadCheckShow=true, cursor=hand</F> "
							  + "<F> Name='No.'		 ID='SeralNo'   HeadAlign=Center HeadBgColor=#E8E8E8 Width=30	 align=center Bgcolor='#EFEFEF' Value={ToString(RowCount-CurRow+1)}	show=false</F> "
							  + "<C> Name='주문번호'   ID=ORDER_NO	HeadAlign=Center HeadBgColor=#eeeeee Width=80  align=center suppress=1 show=false</C>	"
							  + "<C> Name='상태'      ID=ORDER_STS	HeadAlign=Center HeadBgColor=#eeeeee Width=50  align=center , editstyle=LookUp Data='gcDs_etc3:MINORCD:MINORNM' suppress=1</C> "
							  + "<C> Name='반출일자'   ID=ETD_DT		HeadAlign=Center HeadBgColor=#eeeeee Width=80  align=center MASK ='XXXX-XX-XX' suppress=1</C> "
							  + "<C> Name='신청업체명'  ID=CUST_NM		HeadAlign=Center HeadBgColor=#eeeeee Width=130  align=left   show=true</C> "
							  + "<C> Name='실화주'     ID=SHIPPER_NM  HeadAlign=Center HeadBgColor=#eeeeee Width=130  align=left</C> "
							  + "<C> Name='상차지'     ID=LDCARGO     HeadAlign=Center HeadBgColor=#eeeeee Width=80  align=left , editstyle=LookUp Data='gcDs_etc5:MINORCD:MINORNM' </C> "
							  + "<C> Name='하차지'     ID=OFFCARGO    HeadAlign=Center HeadBgColor=#eeeeee Width=80  align=left, editstyle=LookUp Data='gcDs_etc6:MINORCD:MINORNM' </C> "
							  ...
							  + "<C> Name='대표품명'	 ID=ARTC_NM     HeadAlign=Center HeadBgColor=#eeeeee Width=80  align=left   </C> "
							  + "<C> Name='수량'		 ID=PUNIT_CNT   HeadAlign=Center HeadBgColor=#eeeeee Width=80  align=right  show=false </C> "
							  + "<C> Name='적재구분'	 ID=CARGO_TYPE  HeadAlign=Center HeadBgColor=#eeeeee Width=90  align=left , editstyle=LookUp Data='gcDs_etc2:MINORCD:MINORNM'</C> "
							  + "<C> Name='Project명' ID=LINE_PART	HeadAlign=Center HeadBgColor=#eeeeee Width=100  align=left EditStyle=Lookup Data='gcDs_etc1:MINORCD:MINORNM' suppress=1 show=false</C> "
							  + "<C> Name='물류비 부담' ID=CDNAM		HeadAlign=Center HeadBgColor=#eeeeee Width=140  align=left show=false</C> "
							  + "<C> Name='담당자'     ID=CUST_PRSN  	HeadAlign=Center HeadBgColor=#eeeeee Width=75  align=left show=false</C> "
							  + "<C> Name='담당자'     ID=SHIPPERPS  	HeadAlign=Center HeadBgColor=#eeeeee Width=75  align=left show=false</C> "
							  + "<C> Name='전송상태'   ID=EDI_STS		HeadAlign=Center HeadBgColor=#eeeeee Width=60  align=left EditStyle=Combo Data='1:미전송,S:전송중,T:전송완료' </C> "
							  + "<C> Name='수정상태'   ID=ING_STS		HeadAlign=Center HeadBgColor=#eeeeee Width=60  align=left EditStyle=Combo Data='X:삭제,M:수정,C:취소' </C> "
				...
				gcGR1.Format= "<F> Name=''			  ID=RATE_FLAG 	HeadAlign=Center HeadBgColor=#E8E8E8 Width=20	 align=center editstyle=checkbox, HeadCheck=false HeadCheckShow=true, cursor=hand</F> "
							...
							+ "<C> Name='단위'		  ID=ARTC_UNIT  HeadAlign=Center HeadBgColor=#eeeeee Width=80  align=left,	editstyle=LookUp Data='gcDs_etc7:MINORCD:MINORNM'</C> "
							...
							+ "  <C> Name='수량단위'	  ID=PUNIT_UNIT   HeadAlign=Center HeadBgColor=#eeeeee Width=100  align=center, show=false </C>"
							...
							+ "  <C> Name='용적(CBM)' ID=PKG_CBM     HeadAlign=Center HeadBgColor=#eeeeee Width=91  align=right  </C> 	"
	...
	******************************************************************************/
	function ln_RegOrder(){
		...
		window.location.href="<%=dirPath%>/Transfer/jsp/tr00102.jsp";
	...
	******************************************************************************/
	function ln_Print(){
		...
		alert("gcds_report01::"+gcds_report01.countrow);
		alert("gcds_report02::"+gcds_report02.countrow);
	...
	******************************************************************************/
	function ln_PrintFR(){
		...
		//alert("1gcds_report01::"+gcds_report01.countrow);
		//alert("1gcds_report02::"+gcds_report02.countrow);
...
<script language=JavaScript for=gcGR1 event=OnDblClick(row,colid)>
	    ...
	    gcDs1.namevalue(row,"ORDER_NO") ="O100500063";
	    arrParam[1]=gcDs1.namevalue(row,"ORDER_NO");
	    

		strURL = "<%=dirPath%>/Transfer/jsp/tr00102.jsp?gstr1=update1&gstr2="+gcDs1.NameValue(row,"ORDER_NO") +"&A" ;  
...
<BODY>
	<table width="876" cellpadding="0" cellspacing="0" border="0" style="position:relative;left:3px">
		<tr>
			<td width="876" align="right" background="<%=dirPath%>Common/img/com_t_bg.gif" style="padding-top:4px;" align='right'>
				<img src="<%=dirPath%>/Common/img/btn/btn_rtn.gif" id=img_rtn style="cursor:hand;display:none" onclick="ln_HistBack()">
				<img src="<%=dirPath%>/Common/img/btn/btn_cargomark.gif" style="cursor:hand" onclick="ln_CargoMark()">
				<!--<img src="../../../Common/img/btn/btn_order.gif" style="cursor:hand" onclick="ln_RegOrder()">-->
				...
				<img src="<%=dirPath%>/Common/img/com_b_excel.gif" style="cursor:hand" onclick="if (gcDs1.countrow>0) ExcelDialog(gcGR1,'주문정보');">
			...
			<td colspan=2>
				<table cellpadding="0" cellspacing="0" border="0" style="width:876px;height:30px;font-size:9pt;background-color:#ffffff;border:1 solid #708090">
					<tr>
						...
						<td class="tab11_left" bgcolor="#eeeeee">&nbsp;사업명1</td>
						...
						<td class="tab13_left" style="width:240px;">
							...
							<OBJECT id=SEARCH_TYPE classid=clsid:60109D65-70C0-425C-B3A4-4CB001513C69 style='width:100;position:relative;left:8px;top:2px'>
								<param name=CBData					value="B^차량별,A^주문별,C^상하차지별">
					...
					<tr>
						...
						<td class="tab11_left" align='left'><input id="CUST_CDNM" name="CUST_CDNM" type="text" class="txt11" style='width:150;position:relative;left:8px;top:0px;ime-mode:active'></td>
