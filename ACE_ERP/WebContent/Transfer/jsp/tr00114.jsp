...
/*--------------------------------------------------------------------------------
+ 시 스 템 명	:	현대아산  반출주문조회
+ 프로그램 ID	: TR00114.HTML
+ 기 능 정 의	: 주문상태변경
+ 변 경 이 력	: 정미선
-----------------------------------------------------------------------------
+ 수 정 내 용 :	EDI 전송시 취소 Flag 설정 추가 
+ 수   정  자 : 정영식
+ 수 정 일 자 : 2007.08.17 
----------------------------------------------------------------------------------
+ 서블릿명 :
...
<SCRIPT language=JavaScript> 
	...
	******************************************************************************/ 
	function ln_Query(){
		...
		gcDs1.DataId = "<%=dirPath%><%=HDConstant.PATH_TRANSFER%>dsOrderSts?NOH=Y&gstr0=O&gstr1="+ls_from_date+"&gstr2="+ls_to_date+"&gstr3="+ls_project+"&gstr4="+ls_ordno+"&gstr5=" + ls_ordsts ; 
	...
	******************************************************************************/ 
	function ln_Save(){
		...
		//신규 접수
		if (confirm("선택하신 주문정보의 상태를 변경하시겠습니까1 ?")){
			//gcTr01.KeyValue = "trOrderRegMst(I:USER=gcDs1)";
			gcTr01.KeyValue = "trOrderSts(I:USER=gcDs1)";
...
/******************************************************************************
	Description : 전송 후 수정건(2008.09.10)
	전송 후 수정한 건에 대해서만 수정처리 하는 건이다.
******************************************************************************/
function ln_Update(){

	if(!ln_Update_chk()) return ;

	//체크한 건의 전송상태가 [진행중]이 아니면 빠져나옴.
	
	if (confirm("선택하신 주문정보의 상태를 [수정]으로 변경하시겠습니까 ?")){
		gcTr01.KeyValue = "trOrderUpdate(I:USER=gcDs1)";
		gcTr01.Action = "<%=dirPath%><%=HDConstant.PATH_TRANSFER%>trOrderUpdate";
...
/******************************************************************************
	Description : 수정건에 대한 체크(2008.09.10) ==> 2008.11.06 수정건없음.
  -- 전송하고 난 다음 수정된 건에 대해서 관리자가 확인 후 직접 수정 체크하여 저장한다.
  -- 이때 적용은 
	   물류 테이블에만 EDI_STS='N' ING_STS ='U', 로 변경한다.
******************************************************************************/

function ln_Update_Chk(){

  for(i=1;i<=gcDs1.countrow;i++){
		if(gcDs1.namevalue(i,"CHK")=="T"){ //체크이고, 전송상태가 전송 또는 완료로 되어있는 경우만 재전송 한다.
			if(gcDs1.namevalue(i,"EDI_STS")=="S"||gcDs1.namevalue(i,"EDI_STS")=="R"){
...
<script language=JavaScript for=gcGR1 event=OnDblClick(row,colid)>
	...
	if (colid=="CHK") return;
		...
		strURL = "<%=dirPath%>/Transfer/jsp/tr00102.jsp?gstr1=update&gstr2="+gcDs1.NameValue(row,"ORDER_NO") +"&A" ;  
	...
	<table width="876" cellpadding="0" cellspacing="0" border="0" style="position:relative;left:3px">
		<tr> 
			<td width="876" align="right" background="<%=dirPath%>/Common/img/com_t_bg.gif" style="padding-top:4px;">
			  	<img src="<%=dirPath%>/Common/img/com_b_cancle.gif" style="cursor:hand" onclick="ln_Cancel()">
				<img src="<%=dirPath%>/Common/img/btn/b_save.gif" style="cursor:hand" onclick="ln_Save()"> 
				<img src="<%=dirPath%>/Common/img/com_b_delete.gif" style="cursor:hand" onclick="ln_Delete()">
				<img src="<%=dirPath%>/Common/img/com_b_query.gif" style="cursor:hand" onclick="ln_Query()">
				<img src="<%=dirPath%>/Common/img/btn_rtn.gif" id=img_rtn style="cursor:hand;display:none" onclick="ln_HistBack()"> 				
			</td>
		</tr>
		...
		<tr>
			<td colspan=2>
			  <table cellpadding="0" cellspacing="0" border="0" style="width:876px;height:30px;font-size:9pt;background-color:#ffffff;border:1 solid #708090">
					<tr>
						<td class="tab11" style="height:30px;" bgcolor="#eeeeee" width=75>&nbsp;반출일자</td>
						...
						<td class="tab11" bgcolor="#eeeeee" width=75>&nbsp;사업명</td>
						<td class="tab21" >&nbsp;
							<%=HDConstant.COMMENT_START%>
					...
					<tr>
						...
						<td class="tab12" style="text-align:left">&nbsp;
							<%=HDConstant.COMMENT_START%>
							<OBJECT id=ORDER_STS  style='border:0;position:relative;left:0px;top:2px;width:120px;height:140' classid=clsid:60109D65-70C0-425C-B3A4-4CB001513C69>
				...
				<table  cellpadding="0" cellspacing="0" border="0" style="width:876px;height:300px;font-size:9pt;background-color:#ffffff;border:1 solid #708090" >
					<tr>
						<td bgcolor="#eeeeee" align=right class=tab11 colspan=2 style="display:none">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
							확인자&nbsp;&nbsp;:
							<input id="MANAGER" name="MANAGER" type="text" class="txt11" style='width:130;position:relative;left:0px;top:-1px'>&nbsp;&nbsp; 확인일시&nbsp;: &nbsp; <%=HDConstant.COMMENT_START%><OBJECT id=curr_date classid=CLSID:E6876E99-7C28-43AD-9088-315DC302C05F  tabindex="1"
							...
							<input id="ORDER_STS12" name="ORDER_STS12" type="text" class="txt11" style='width:0;position:relative;left:0px;top:-1px' style=dispaly:none>등록 -- > 접수							<%=HDConstant.COMMENT_START%><OBJECT id=ORDER_STS1  style='border:0;position:relative;left:0px;top:2px;height:110' classid=clsid:60109D65-70C0-425C-B3A4-4CB001513C69  width=0 height=100  >
					...
					<tr>
						<td colspan=2 >
							...
							<OBJECT classid=clsid:1F57AEAD-DB12-11D2-A4F9-00608CEBEE49 id=gcGR1 style="width:874;height:402px;border:0 solid #777777;display:block" viewastext>
								...
								<PARAM NAME="Format"			VALUE="  
									<F>  Name=''						ID=CHK,   width=20 EditStyle=CheckBox	Pointer=Hand 	HeadBgColor=#eeeeee HeadCheckShow=true align=center  bgcolor={IF(ING_STS='C','Gold')}</F>
									...
									<C> Name='반출일자'			ID=EDT_DATE		HeadAlign=Center HeadBgColor=#eeeeee Width=70   align=center MASK='XXXX-XX-XX' edit=none, suppress=2  bgcolor={IF(ING_STS='C','Gold')} </C>
									...
									<C> Name='실화주'				ID=SHIPPER		HeadAlign=Center HeadBgColor=#eeeeee Width=100  align=left,  suppress=3 edit=none   bgcolor={IF(ING_STS='C','Gold')}</C> 
									<C> Name='상차지'				ID=LD_CARGO		HeadAlign=Center HeadBgColor=#eeeeee Width=50   align=left, suppress=4 edit=none,	 editstyle=LookUp Data='gcDs_etc4:MINORCD:MINORNM' bgcolor={IF(ING_STS='C','Gold')} </C> 
									<C> Name='하차지'				ID=OFF_CARGO	HeadAlign=Center HeadBgColor=#eeeeee Width=50   align=left, suppress=5 edit=none,	 editstyle=LookUp Data='gcDs_etc5:MINORCD:MINORNM' bgcolor={IF(ING_STS='C','Gold')} </C> 
									<C> Name='배정번호'			ID=CARINFO		HeadAlign=Center HeadBgColor=#eeeeee Width=190  align=left, edit=none suppress=6  bgcolor={IF(ING_STS='C','Gold')}</C> 
									...
									<C> Name='총중량(KG)'		ID=PKG_WGHT		HeadAlign=Center HeadBgColor=#eeeeee Width=60   align=right edit=none  bgcolor={IF(ING_STS='C','Gold')}</C>
									<C> Name='총용적(CBM)'		ID=PKG_CBM		HeadAlign=Center HeadBgColor=#eeeeee Width=50   align=right edit=none  bgcolor={IF(ING_STS='C','Gold')}</C>
						...
						<td> <br>
							< 검색조건의 구분 설명 ><br>
							* 주문 : 주문접수할 때 사용하며, 해당 주문번호가 동일한 건이 일괄선택됩니다.<br>
							* 취소 : EDI 전송 후 주문 취소건에 대하여 해당 주문번호가 동일한 건이 일괄선택됩니다.
						</td>
					</tr>
					</tr>
