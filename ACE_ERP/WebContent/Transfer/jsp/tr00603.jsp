...
<%
/*--------------------------------------------------------------------------------
+ 시 스 템 명	:	물류관리 - 고객의소리관리
+ 프로그램 ID	: TR00603.HTML
+ 기 능 정 의	: 조회,저장,삭제
+ 변 경 이 력	: 정미선
+ 작성일자 :
----------------------------------------------------------------------------------
...
******************************************************************************/
function ln_Query(){
	gcds_data.DataID = "<%=dirPath%><%=HDConstant.PATH_TRANSFER%>tr00602_s01?v_str1=2&v_str2=";
	gcds_data.Reset();
}
...
function ln_Add(){
	...
	var re = gcds_data.NameValue(row,"MAIREP");

	gcds_data.insertrow(row+1);
	gcds_data.NameValue(row+1,"MAIYMD") = gcds_data.NameValue(row,"MAIYMD");			//입력일자
	gcds_data.NameValue(row+1,"MAISEQ") = gcds_data.NameValue(row,"MAISEQ");		  //순번
	gcds_data.NameValue(row+1,"MAIREP") = 9;			//
	gcds_data.NameValue(row+1,"REGNM") = gusrnm;	//답변자
	
	if (re != "0") gcds_data.NameValue(row+1,"MAISUBJ") = "  "+ gcds_data.NameValue(row,"MAISUBJ");		//답변순번
	else gcds_data.NameValue(row+1,"MAISUBJ") = " [Re] "+ gcds_data.NameValue(row,"MAISUBJ");		//답변순번
...
function ln_Save(){
	...
	if (gcds_data.countrow > 0) {
		...
		for (var j=1; j<=gcds_data.countrow; j++){
			if (gcds_data.sysStatus(j)=="1"){
				gcds_data.NameValue(j,"MAIGBN") = "2";	        //입력구분
				...
				ls_curdate = gcds_data.NameValue(j,"MAIYMD");
			}
		}
	} 
	...
	if (confirm("해당 입력정보를 저장하겠습니까 ?")) {
		...
		if (tfile == null || tfile == "")	{
			gctr_nfile.Parameters = "v_str1=" + "2" + ",v_str2=" + ls_curdate;
			gctr_nfile.Post();		  //첨부파일이 없는 경우
		} else {
			gctr_tfile.Parameters = "v_str1=" + "2" + ",v_str2=" + ls_curdate;
...
<OBJECT id=gctr_nfile classid=clsid:0A2233AD-E771-11D2-973D-00104B15E56F VIEWASTEXT>
  <PARAM NAME="Action" VALUE="<%=dirPath%><%=HDConstant.PATH_TRANSFER%>tr00602_t03">
...
<OBJECT id=gctr_tfile classid=clsid:0A2233AD-E771-11D2-973D-00104B15E56F VIEWASTEXT>
  <PARAM NAME="Action" VALUE="<%=dirPath%><%=HDConstant.PATH_TRANSFER%>tr00602_t04">
	<PARAM NAME="KeyName"  VALUE="toinb_dataid4">
  ...
  <tr> 
	...
	<tr>
		<td valign="top" colspan=2>
			<%=HDConstant.COMMENT_START%><OBJECT classid=clsid:1F57AEAD-DB12-11D2-A4F9-00608CEBEE49 id=gcgd_disp2  
			    ...
			    <PARAM NAME="DataID"			VALUE="gcds_data">
				  ...
				  <PARAM NAME="Format"			VALUE="  
						...
						<C> Name='제 목'  ID=MAISUBJ  HeadAlign=Center HeadBgColor=#B9D4DC Width=507 align=left</C> 
						<C> Name='작성일' ID=MAIYMD  	HeadAlign=Center HeadBgColor=#B9D4DC Width=100 align=center, mask='XXXX/XX/XX'</C> 
          ...
          <td> 
            <table border="0" cellspacing="0" cellpadding="0" style="width:100%;border:1 solid #708090;border-right-width:0px;border-bottom-width:0px;">
              <tr> 
                ...
                <td colspan="3" class="tab21">
                  <input type="text" class="txt11" id=txt_filenm readonly style="width:685px">
					<%=HDConstant.COMMENT_START%>
					<OBJECT id=gcip_file classid=CLSID:69F1348F-3EBE-11D3-973D-0060979E2A03 style="position:relative;left:9px;width:80; height:20;" align="absmiddle">
              ...
              <tr style="height:300px"> 
                ...
                <td colspan="3" class="tab21">
                <TEXTAREA id=txt_text class="txt11" style="width:775px; height:290px;border:0;overflow:auto;IME-MODE: active"></TEXTAREA>
				</td>
              </tr>
            </table>
          </td>
