...
public class a020014_t1 extends HttpServlet{
	/**
	 * 
	 */
        ...
        /************************************************************************************
         @
              ...
              StringBuffer sql_mst = new StringBuffer();
							...
							sql_mst.append( "          nvl(COCODE,''),nvl(DEPTCD,''),'"+str2+"',nvl(DIVCD,''), 0 FSAMT,     	\n");
							sql_mst.append( "          FSVAT, 0 DETOT, 0 CRTOT,REMARK,'', 											\n");
						...
						//System.out.println("# sql_mst Query : \n " + sql_mst);						
					 ...
					 //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
					        StringBuffer sql_dtl = new StringBuffer();
									...
									sql_dtl.append( "        ATDECR,	0 FSAMT,	  ACNNBR,	  BANJAEYN,	PRIMCOST,	 		\n");
						...
						//System.out.println("# sql_dtl Query : \n " + sql_dtl);	
					 ...
					 //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
						...
						//System.out.println("# sql_ref Query : \n " + sql_ref);	
