...
public class a030008_s2_n extends HttpServlet {
	/**
	 * 
	 */
...
**********************************************************************************************/
				...
				if (!GauceReq.isBuilderRequest()) {
					...
					if(str5.equals("A")){ //전체
						...
						sql.append(" SELECT B.ATCODE ATCODE, C.ATKORNAM ATKORNAM, C.ATPRTSEQ, C.ATUSEYN ATUSEYN,           \n");
						sql.append("        CASE WHEN C.ATDECR='1' THEN  nvl(B.DETOT - B.CRTOT,0) ELSE 0 END DEVAL,     \n");
						sql.append("        COALESCE(B.DETOT,0) DETOT,                                                     		\n");
						sql.append("        COALESCE(B.CRTOT,0) CRTOT,                                                     		\n");
						sql.append("        CASE WHEN C.ATDECR='2' THEN  nvl(B.CRTOT - B.DETOT,0) ELSE 0 END CRVAL      \n");
						sql.append("  FROM(                                                                                					\n");
						sql.append("        SELECT Y.ATCODE, SUM(COALESCE(Y.DETOT,0)) DETOT, SUM(COALESCE(Y.CRTOT,0))CRTOT \n");
						sql.append("        FROM (                                                                         									\n");
						...
						sql.append(" 					SELECT A.ATCODE ATCODE,                                                      				\n");
						sql.append(" 								SUM(COALESCE(A.FODETOT,0)) DETOT,                                      	\n");
						sql.append(" 								SUM(COALESCE(A.FOCRTOT,0)) CRTOT                                       	\n");
						sql.append(" 					 FROM ACCOUNT.MTOTALBAL A                                                    			\n");
						sql.append(" 					WHERE A.FDCODE= '03'                                                         					\n");
						sql.append(" 						AND (A.ACTYYMM >= '"+str4+"00' AND A.ACTYYMM <= '"+str2+"')                	\n");
						sql.append(" 					GROUP BY A.ATCODE                                                            					\n");
						//전체 조회시점에 금강산 원화의 외화환산이익은 조회 되면 안됨. 당기순이익/순손실과 상계
						//2011.07.18
						sql.append(" 					)Y                                                                           			\n");
