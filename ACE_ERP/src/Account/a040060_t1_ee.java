...
public class a040060_t1_ee extends HttpServlet{
	/**
	 * 
	 */
			...
			try {
				...
				for (int j = 0; j < rows.length; j++){
					if(rows[j].getJobType() == GauceDataRow.TB_JOB_UPDATE) {
						...
						sql.append( " SELECT HDASAN_EDI.FN_CONVERSATION_SEQ('"+strRegno+"'||TRIM(B.VEND_ID)||A.TAXDAT) SEQ, TRIM(B.VEND_ID), A.TAXDAT   \n" ); 
						...
						InsertSql.append( " CREATED_BY,      CREATION_DATE,  LAST_UPDATED_BY, LAST_UPDATE_DATE,   AMEND_CODE, REMARK,REMARK2   				   \n" );
						...
						if(strTaxGb.equals("D")){ // 개성 건은 구현진 씨....
						...
						InsertSql.append( "        '' AS CREATED_BY, sysdate  ,  '' AS  LAST_UPDATED_BY,  sysdate, A.AMEND_CODE, A.AMEND_REMARK ,A.REMARK2 \n" );
						...
						InsertSql.append( "   LEFT JOIN PAYROLL.V_HIPERSON Y ON Y.EMPNO ='3080087'                                                       \n" );
						InsertSql.append( "   LEFT JOIN PAYROLL.HCDEPT H ON H.DEPTCD =A.DEPTCD                                                           \n" );
						
						if(strTaxGb.equals("D")){ // 개성 건은 구현진 씨....
							InsertSql.append( "   LEFT JOIN HDASAN_EDI.XXSB_DTI_SM_USER X ON X.FND_USER = '3080087' AND X.FND_REGNO = '"+strRegno+"'       \n" );
						}else{
