...
public class a040060_t1_ee_A extends HttpServlet{
	/**
	 * 
	 */
	...
	public void doPost(HttpServletRequest req,HttpServletResponse res) {
		...
		try{
			...
			try {
				...
				//sql0.append( " SELECT CAST(REPLACE(CHAR(CURRENT DATE,ISO),'-','')||REPLACE(CHAR(CURRENT TIME,ISO),'.','')||SUBSTR(MICROSECOND(CURRENT TIMESTAMP),1,1) AS NUMERIC(15))  \n" ); 
				...
				for (int j = 0; j < rows.length; j++){
					if(rows[j].getJobType() == GauceDataRow.TB_JOB_UPDATE) {
						...
						strTaxGb = rows[j].getString(idxTAXNBR).substring(0,1);
						...
						InsertSql.append( " SUP_BIZPLACE_CODE, BYR_BIZPLACE_CODE, ORI_ISSUE_ID   \n" );
						
						/*
						if(strEmpno.equals("4090003")){
							InsertSql.append( " ,BYR_BIZPLACE_CODE \n" );
						}else if (strEmpno.equals("2020013")){
							InsertSql.append( " ,BYR_BIZPLACE_CODE \n" );
						}else if (strEmpno.equals("2110005")){
							InsertSql.append( " ,BYR_BIZPLACE_CODE \n" );	
						}
						*/
						...
						InsertSql.append( "        CASE WHEN A.TAXTYPE ='Y' THEN '1'                          --영수                                                                           \n" );
						InsertSql.append( "             WHEN A.TAXTYPE ='N' THEN '18' END AS TAX_DEMAND,      --청구                                                                           \n" );
						...
						//2016.09.06 JYS 환경공단 예외처리 (종사업장 대표자 별도지정 )
						InsertSql.append( "         TRIM(B.VEND_ID)   AS BYR_COM_REGNO,    \n" );
						InsertSql.append( "         CASE WHEN TRIM(B.VEND_ID)='1378207000' AND A.B_BIZPLACE='0002' THEN '김영기' ELSE B.VD_DIRECT END AS BYR_REP_NAME,      \n" );
						InsertSql.append( "         B.VEND_NM   AS BYR_COM_NAME,\n" );
						
						InsertSql.append( "         B.BSNS_CND  AS BYR_COM_TYPE,     B.BSNS_KND  AS BYR_COM_CLASSIFY,                                    \n" );
						InsertSql.append( "        ----------------------------------------------------------------------------------------------------- \n" );
						
						InsertSql.append( "        TRIM(B.ADDRESS1)||TRIM(B.ADDRESS2) AS BYR_COM_ADDR,   \n" );
						//InsertSql.append( "        CASE WHEN TRIM(A.B_BIZPLACE)='' OR TRIM(A.B_BIZPLACE) IS NULL THEN  TRIM(B.ADDRESS1)||TRIM(B.ADDRESS2) ELSE TRIM(M.ADDRESS1)||TRIM(M.ADDRESS2) END AS BYR_COM_ADDR,   \n" );
						
						InsertSql.append( "        J.DEPTNM AS BYR_DEPT_NAME,                       \n" );
						
						InsertSql.append( "        J.EMPNM  AS BYR_EMP_NAME,         J.TELNO  AS BYR_TEL_NUM,       J.EMAIL  AS BYR_EMAIL,               \n" );
						//InsertSql.append( "        J.EMPNM  AS BYR_EMP_NAME,         J.TELNO  AS BYR_TEL_NUM,      'happyjin3@hyundai-securities.co.kr'  AS BYR_EMAIL,               \n" );
						InsertSql.append( "        ----------------------------------------------------------------------------------------------------- \n" );
						...
						InsertSql.append( "        '' AS CREATED_BY, sysdate  ,  '' AS  LAST_UPDATED_BY,  sysdate, A.AMEND_CODE, A.AMEND_REMARK, \n" );
						
						/*
						if(strEmpno.equals("2110005")){
							InsertSql.append( "        '현대증권 가락지점/서울시 송파구 중대로 80(문정동, 롯데마트3층)' ,\n" );
							//InsertSql.append( "        '경기도 수원시 팔달구 경수대로 472' ,\n" );
						}else{
							InsertSql.append( "        A.AMEND_REMARK ,\n" );
						}
						*/
						
						InsertSql.append( "        A.REMARK2,A.REMARK3, \n" );
						
						InsertSql.append( "        A.S_BIZPLACE, A.B_BIZPLACE, A.ORI_ISSUE_ID \n" );
						...
						if(strEmpno.equals("4090003")){
							...
							InsertSql.append( "         WHEN TRIM(B.VEND_ID) IN ('3058213158') THEN '0002' ELSE '' END  BYR_BIZPLACE_CODE  \n" );
						...
						*/
						...
						//공급받는자 종사업장 주소 변경  2017.12.26 다시막음
						//InsertSql.append( "   LEFT JOIN ACCOUNT.MINORPLACE M ON A.VEND_CD = M.VEND_CD AND A.B_BIZPLACE = M.MINOR_CD                     \n" );
						
						InsertSql.append( "  WHERE A.TAXNBR =?                                                                                           \n" );
						InsertSql.append( "    AND A.VEND_CD = B.VEND_CD                                                                                 \n" );			
						InsertSql.append( "    AND A.EXTCHK<>'1'                                                                                         \n" );
						InsertSql.append( "    AND ((A.EBILLGB <>'2' AND A.EBILLGB<>'9') OR A.EBILLGB IS NULL)                                           \n" );
						...
						InsertSql2.append( "    AND ((A.EBILLGB <>'2' AND A.EBILLGB<>'9') OR A.EBILLGB IS NULL)                                        \n" );  
						//ITEM
						...
						InsertSql3.append( "   AND ((A.EBILLGB <>'2' AND A.EBILLGB<>'9') OR A.EBILLGB IS NULL)                                        \n" ); 
