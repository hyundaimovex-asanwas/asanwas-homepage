...
public class a040065_t3_ee extends HttpServlet{
	/**
	 * 
	 */
				...
				for (int j = 0; j < rows.length; j++){
					if(rows[j].getJobType() == GauceDataRow.TB_JOB_UPDATE) {
                        ...
                        */
						...
						sql.append( "   FROM ACCOUNT.ATTAXMST A, ACCOUNT.ATTAXDTL B  \n" );
						sql.append( "  WHERE A.TAXNBR = B.TAXNBR            \n" );
						sql.append( "    AND A.TAXNBR LIKE '"+strTaxdat+"%' \n" );
						...
						InsertSql.append( " GUBUN,		AMEND_CODE,    AMEND_REMARK, EXTCHK,     REMARK2, S_BIZPLACE,          							 \n" );
						InsertSql.append( " EBILLGB,	TAXSTS          							 \n" );
						InsertSql.append( " )												 														     \n" );
						...
						//2018.02.02 역발행매출 
						InsertSql.append( "        CASE WHEN A.SUP_BIZPLACE_CODE ='0001' THEN '02'                                                        \n" );
						InsertSql.append( "             WHEN A.SUP_BIZPLACE_CODE ='0003' THEN '04'                                                        \n" );
						InsertSql.append( "             ELSE '01' END COCODE,                                                                            \n" );
						...
						InsertSql.append( "        G.VEND_CD,  TO_CHAR(DTI_WDATE,'YYYYMMDD') AS TAXDAT,                                                  \n" );
						InsertSql.append( "        A.SUP_AMOUNT AS TAXSUM,                                                                               \n" );
						InsertSql.append( "        ----------------------------------------------------------------------------------------------------- \n" ); 
						InsertSql.append( "        A.TAX_AMOUNT AS TAXVATAMT, A.TOTAL_AMOUNT AS TAXTOT,X.ITEM_NAME AS REMARK,'' TAXCDNBR, '1110310' ATCODE,\n" );
						InsertSql.append( "        ----------------------------------------------------------------------------------------------------- \n" ); 
						...
						InsertSql.append( "        CASE WHEN SUBSTR( TO_CHAR(DTI_WDATE,'YYYYMMDD'),5,2) IN ('01','02','03') THEN '1'   \n" );
						InsertSql.append( "             WHEN SUBSTR( TO_CHAR(DTI_WDATE,'YYYYMMDD'),5,2) IN ('04','05','06') THEN '2'   \n" );
						InsertSql.append( "             WHEN SUBSTR( TO_CHAR(DTI_WDATE,'YYYYMMDD'),5,2) IN ('07','08','09') THEN '3'   \n" );
						InsertSql.append( "             WHEN SUBSTR( TO_CHAR(DTI_WDATE,'YYYYMMDD'),5,2) IN ('10','11','12') THEN '4'   \n" );
						...
						InsertSql.append( "        ----------------------------------------------------------------------------------------------------- \n" );
						InsertSql.append( "        '1' AS GUBUN,  A.AMEND_CODE,  A.REMARK AS AMEND_REMARK , '0' EXTCHK,                                  \n" );
						//InsertSql.append( "        CASE WHEN A.SUP_BIZPLACE_CODE ='0001' THEN '현대아산주식회사(지점) 서울 종로 연지 1-7' ELSE '' END REMARK2, \n" );
						InsertSql.append( "        CASE WHEN A.SUP_BIZPLACE_CODE ='0001' THEN '현대아산주식회사(지점) 서울 종로 연지 1-7'  \n" );
						InsertSql.append( "             WHEN A.SUP_BIZPLACE_CODE ='0003' THEN '현대아산주식회사 충청남도 홍성군 홍북면 충남대로 36(빌앤더스오피스텔)' ELSE '' END REMARK2, \n" );
						InsertSql.append( "        A.SUP_BIZPLACE_CODE AS S_BIZPLACE, '1' EBILLGB, 'N' TAXSTS                                            \n" );
						//2013.12.09 JYS 수정
						//InsertSql.append( "   FROM HDASAN_EDI.XXSB_DTI_MAIN A, HDASAN_EDI.XXSB_DTI_STATUS C                                              \n" );
						InsertSql.append( "   FROM HDASAN_EDI.XXSB_DTI_MAIN A                               \n" );
						InsertSql.append( "   LEFT JOIN HDASAN_EDI.XXSB_DTI_STATUS C ON C.DTI_STATUS ='V'   \n" );
						InsertSql.append( "   LEFT JOIN PAYROLL.V_HIPERSON V ON UPPER(V.EMAIL) = UPPER(A.SUP_EMAIL)                                      \n" );
						...
						InsertSql.append( "                  AND RTRIM(VEND_ID) IS NOT NULL                                                              \n" );
						InsertSql.append( "                  AND RTRIM(VEND_CD) IS NOT NULL                                                              \n" );
						InsertSql.append( "                GROUP BY VEND_ID                                                                              \n" );
						InsertSql.append( "              )G ON G.VEND_ID = A.BYR_COM_REGNO                                                               \n" );
						InsertSql.append( "   LEFT JOIN ACCOUNT.CUSTOMER K ON K.VEND_CD =G.VEND_CD AND UPPER(K.EMAIL) = UPPER(A.BYR_EMAIL)               \n" );						
						InsertSql.append( "   LEFT JOIN ( SELECT CONVERSATION_ID, SUPBUY_TYPE,DIRECTION,DTI_LINE_NUM,COALESCE(ITEM_NAME,'') ITEM_NAME      \n" );						
						InsertSql.append( "                   FROM HDASAN_EDI.XXSB_DTI_ITEM                                                              \n" );						
						InsertSql.append( "                  WHERE CONVERSATION_ID='"+rows[j].getString(idxCONVERSATION_ID)+"'                           \n" );						
						InsertSql.append( "                    AND SUPBUY_TYPE ='"+rows[j].getString(idxSUPBUY_TYPE)+"'                                  \n" );						
						InsertSql.append( "   				   AND DIRECTION ='"+rows[j].getString(idxDIRECTION)+"'                                      \n" );						
						InsertSql.append( "   		       ) X ON X.CONVERSATION_ID=A.CONVERSATION_ID                                                    \n" );						
						InsertSql.append( "                 AND X.SUPBUY_TYPE = A.SUPBUY_TYPE                                                            \n" );						
						InsertSql.append( "                 AND X.DIRECTION = A.DIRECTION                                                                \n" );						
						InsertSql.append( "		            AND X.DTI_LINE_NUM = 1                                                                       \n" );						
						...
						InsertSql2.append( " SELECT '"+strTaxnbr+"' TAXNBR,                                                   \n" );
						InsertSql2.append( "        LPAD(TO_NUMBER(NVL(DTI_LINE_NUM,0)),3,0),                                 \n" );
