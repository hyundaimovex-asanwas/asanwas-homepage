...
public class a070021_s12 extends HttpServlet {
	/**
	 * 
	 */
...
**********************************************************************************************/
				...
				if (!GauceReq.isBuilderRequest()) {	
					...
					sql.append( "            0 CRAMT, MAX(A.REMARK)REMARK, '' DOCUMCD , '' DOCUVAL                               \n");
					sql.append( "       FROM ACCOUNT.AETCPAY A                                                                   \n");
					sql.append( "      WHERE A.CHKCASH ='T'                                                                      \n");
					...
					sql.append( "     SELECT '2' SEQ, '1110150' ATCODE, 'A00003' VEND_CD, 0 DEAMT, SUM(A.JANAMT)CRAMT,           \n");
					sql.append( "            '기타업체 대금지급' REMARK, '' DOCUMCD , '' DOCUVAL                \n");
					sql.append( "       FROM ACCOUNT.AETCPAY A                                                                   \n");
					sql.append( "      WHERE A.CHKCASH ='T'                                                                      \n");
					sql.append( "        AND A.FDCODE ='"+str1+"'                                                                \n");
					sql.append( "        AND (A.ACTDAT LIKE  '"+str2+"%' OR A.ACTDAT LIKE  '"+str6+"%' )                         \n");
					...
					sql.append( "            X.REMARK, C.ATDEBTYN, X.DOCUMCD, X.DOCUVAL                                          \n");
					sql.append( "  ORDER BY  X.SEQ ASC, D.VEND_NM, X.ATCODE                                                      \n");

					//logger.dbg.println( this, sql.toString() ); 
					//System.out.println("a070021_s12:::"+sql.toString());
/*********************************************************************************************/
