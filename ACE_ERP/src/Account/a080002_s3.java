...
public class a080002_s3 extends HttpServlet {
	/**
	 * 
	 */
...
**********************************************************************************************/
				...
				str[3] = req.getParameter("v_str5");	//예산부서
				...
				for (int i=0; i<strArrCN.length; i++) { //set column column 
					...
					if (!GauceReq.isBuilderRequest()) {					
						...
						int preyear = Integer.parseInt(str[1]) - 1;
						sql.append( " SELECT																																		\n");	
						...
						sql.append( " LEFT JOIN ACCOUNT.ACTCODE F  ON A.FDCODE=F.FDCODE 											\n");
						...
						if(!str[1].equals("")) sql.append( " AND A.BGTYY   = '"+preyear+"'									\n");	//예산년도
						if(!str[2].equals("")) sql.append( " AND A.BGTDPT  = '"+str[2]+"'									\n");	//예산부서
						if(!str[3].equals("")) sql.append( " AND A.APPROVAL LIKE '%"+str[3]+"'							\n");	//예산부서
