...
public class a090004_s1 extends HttpServlet {
	/**
	 * 
	 */
...
**********************************************************************************************/
				...
				String[] strArrCN = new String[]{ "ASETNO", "ASTNAME", "ASAQSDAT", "ATCODE", "ASTSTAT", "ASTUNIT", "ASTQTY", "ASTAQAMT","ASTYUSE","MNGDPT","USEDPT", "CDNAM"};  

				int[] intArrCN = new int[]{ 11, 30, 8, 7, 1, 2, 10, 13, 3, 4, 4, 30 }; 
					
				int[] intArrCN2 = new int[]{ -1, -1, -1, -1, -1, -1, 0, 0, 0, -1, -1, -1 }; 
				...
				if (!GauceReq.isBuilderRequest()) {	
					...
					sql.append( " COALESCE(A.ASAQSDAT,'') as ASAQSDAT, A.ATCODE, COALESCE(B.ASTSTAT,'') as ASTSTAT, "); 
					sql.append( " COALESCE(B.ASTUNIT,'') as ASTUNIT, COALESCE(B.ASTQTY,0) as ASTQTY, COALESCE(B.ASTAQAMT,0) as ASTAQAMT, ");
					sql.append( " COALESCE(B.ASTYUSE,0) as ASTYUSE, COALESCE(B.MNGDPT,'') as MNGDPT, COALESCE(B.USEDPT,'') as USEDPT, E.CDNAM ");		    
					...
					sql.append( " LEFT JOIN ACCOUNT.COMMDTIL E ON A.ASTNAME=E.CDCODE AND E.CMTYPE='2161' \n");
					if (!str6.equals("")) sql.append( " WHERE A.ASAQSDAT BETWEEN '" + str6 + "' " );
