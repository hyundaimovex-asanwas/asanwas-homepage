...
public class a090004_s4 extends HttpServlet {
	/**
	 * 
	 */
...
**********************************************************************************************/
				...
				String  str5	= req.getParameter("v_str5");    //수리일자from
				String  str6	= req.getParameter("v_str6");    //수리일자to
				...
				String[] strArrCN = new String[]{ "ASETNO", "ASTNAME", "REPAIRDATE", "REPAIRPART", "DEFECTNOTE", "REPAIRAMT", "WRDT", "WRID", "CDNAM"};  

				int[] intArrCN = new int[]{ 11, 30, 8, 13, 30, 13, 8, 7, 30}; 
					
				int[] intArrCN2 = new int[]{ -1, -1, -1, -1, -1, 0, -1, -1, -1}; 
				...
				if (!GauceReq.isBuilderRequest()) {	
					...
					sql.append( " COALESCE(B.ASTNAME,'') as ASTNAME, COALESCE(A.REPAIRDATE,'') as REPAIRDATE, ");
					sql.append( " COALESCE(A.REPAIRPART,'') as REPAIRPART, COALESCE( A.DEFECTNOTE,'') AS DEFECTNOTE, "); 
					sql.append( " COALESCE(A.REPAIRAMT,0) AS REPAIRAMT, COALESCE(A.WRDT,'') AS WRDT, COALESCE(A.WRID,'') AS WRID, X.CDNAM ");	    
					sql.append( " FROM ACCOUNT.ASTREPAIR A LEFT OUTER JOIN ACCOUNT.ASTMST B ON ");
					...
					if (!str5.equals("")) sql.append( " WHERE A.REPAIRDATE BETWEEN '" + str5 + "' " );
