...
public class a090004_s5 extends HttpServlet {
	/**
	 * 
	 */
...
**********************************************************************************************/
				...
				String  str5	= req.getParameter("v_str5");    //보험일자from
				String  str6	= req.getParameter("v_str6");    //보험일자to
				String  str7	= req.getParameter("v_str7");    //보험회사
				...
				String[] strArrCN = new String[]{ "ASETNO", "ASTNAME", "INSUREDATE", "INSHEETNO", "INSSTARTDT", "INSENDDT", "INSTERM", "INSAMT","INSEDAMT","INSFIRM", "CDNAM"};  

				int[] intArrCN = new int[]{ 11, 30, 8, 20, 8, 8, 20, 13, 13, 13, 30}; 
				...
				if (!GauceReq.isBuilderRequest()) {	
					...
					sql.append( " COALESCE(A.INSUREDATE,'') as INSUREDATE, COALESCE(A.INSHEETNO,'') as INSHEETNO, "); 
					sql.append( " COALESCE(A.INSSTARTDT,'') AS INSSTARTDT, COALESCE(A.INSENDDT,'') AS INSENDDT, ");
					sql.append( " COALESCE(A.INSTERM,'') AS INSTERM, COALESCE(A.INSAMT,0) AS INSAMT, ");		    
					sql.append( " COALESCE(A.INSEDAMT,0) AS INSEDAMT, COALESCE(A.INSFIRM,'')as INSFIRM, X.CDNAM ");
					sql.append( " FROM ACCOUNT.ASTINSURE A LEFT OUTER JOIN ACCOUNT.ASTMST B ON ");
					...
					if (!str5.equals("")) sql.append( " WHERE A.INSUREDATE BETWEEN '" + str5 + "' " );
					...
					if (!str7.equals("")) sql.append( " AND A.INSFIRM = '" + str7 + "' " );					
