...
public class a090004_s6 extends HttpServlet {
	/**
	 * 
	 */
...
**********************************************************************************************/
				...
				String  str5	= req.getParameter("v_str5");    //취득일자from
				String  str6	= req.getParameter("v_str6");    //취득일자to
				String  str7	= req.getParameter("v_str7");    //임대차처
				...
				String[] strArrCN = new String[]{ "ASETNO", "ASTNAME", "LEASEDATE", "LEASECLS", "LEASEHOLD", "LEASETERM", "LEASEAMT", "LEASEGAMT","LEASEWHY","LEASESTS","LEASEPYUNG", "CDNAM"};  

				int[] intArrCN = new int[]{ 11, 30, 8, 1, 13, 20, 13, 13, 20, 2, 5, 30}; 
					
				int[] intArrCN2 = new int[]{ -1, -1, -1, -1, -1, -1, 0, 0, -1, -1, 0, -1}; 
				...
				if (!GauceReq.isBuilderRequest()) {	
					...
					sql.append( " COALESCE(B.ASTNAME,'') as ASTNAME, ");
					sql.append( " COALESCE(A.LEASEDATE,'') AS LEASEDATE, COALESCE(A.LEASECLS,'') AS LEASECLS, "); 
					sql.append( " COALESCE(A.LEASEHOLD,'') AS LEASEHOLD, COALESCE(A.LEASETERM,'') AS LEASETERM, ");
					sql.append( " COALESCE(A.LEASEAMT,0) as LEASEAMT, COALESCE(A.LEASEGAMT,0) AS LEASEGAMT, ");
					sql.append( " COALESCE(A.LEASEWHY,'') AS LEASEWHY, COALESCE(A.LEASESTS,'') AS LEASESTS, ");
					sql.append( " COALESCE(A.LEASEPYUNG,0) AS LEASEPYUNG, X.CDNAM ");
					sql.append( " FROM ACCOUNT.ASTLEASE A LEFT OUTER JOIN ACCOUNT.ASTMST B ON ");
					...
					if (!str5.equals("")) sql.append( " WHERE A.LEASEDATE BETWEEN '" + str5 + "' " );
