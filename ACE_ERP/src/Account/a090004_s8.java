...
public class a090004_s8 extends HttpServlet {
	/**
	 * 
	 */
...
**********************************************************************************************/
				...
				String  str5	= req.getParameter("v_str5");    //재평가일자from
				String  str6	= req.getParameter("v_str6");    //재평가일자to
				...
				String[] strArrCN = new String[]{ "ASETNO", "ASTNAME", "EVALDATE", "BEVALAMT", "EVALAMT", "DIFFAMT", "EVALAFYEAR", "CDNAM"};  

				int[] intArrCN = new int[]{ 11, 30, 8, 13, 13, 13, 3, 30}; 
					
				int[] intArrCN2 = new int[]{ -1, -1, -1, 0, 0, 0, 0, -1}; 
				...
				if (!GauceReq.isBuilderRequest()) {	
					...
					sql.append( " COALESCE(A.EVALDATE,'') AS EVALDATE, COALESCE(A.BEVALAMT,0) AS BEVALAMT, ");
					sql.append( " COALESCE(A.EVALAMT,0) AS EVALAMT, COALESCE(A.DIFFAMT,0) AS DIFFAMT, "); 
					sql.append( " COALESCE(A.EVALAFYEAR,0) AS EVALAFYEAR, X.CDNAM ");	    
					sql.append( " FROM ACCOUNT.ASTEVAL A LEFT OUTER JOIN ACCOUNT.ASTMST B ON ");
					...
					if (!str5.equals("")) sql.append( " WHERE A.EVALDATE BETWEEN '" + str5 + "' " );
