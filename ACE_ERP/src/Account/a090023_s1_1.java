...
public class a090023_s1_1 extends HttpServlet {
	/**
	 * 
	 */
...
**********************************************************************************************/
				...
				if (!GauceReq.isBuilderRequest()) {	
					...
					sql.append( " SELECT B.ASTNBR,     																   \n");
					sql.append( "        TRIM(E.ASTNM) AS ASNAM, B.ASAQSDAT, COALESCE(C.ASTYUSE,0) AS ASLIFYER,        \n");
					sql.append( "        COALESCE(C.ASTAQAMT,0) AS ASTAQAMT,   COALESCE(A.ABDEPRAMT,0)AS ABDEPRAMT,    \n");
					sql.append( "        COALESCE(A.ACDEPRAMT,0) AS ACDEPRAMT, COALESCE(A.ASTRMAMT,0) AS ASTRMAMT,     \n");
					...
					sql.append( "       'N' AS MODIFYGB, A.ASTDEPRST         										   \n");
					...
					sql.append( "   LEFT JOIN ACCOUNT.ASTNAME E ON B.AST2ND=E.AST2ND AND B.AST3RD = E.AST3RD AND B.ASTNAME = E.ASTCD \n"); 
					...
					if(str7.equals("")||str7.equals("0")){
						...
						sql.append( "       (A.AST1ST||A.AST2ND||A.AST3RD||A.ASTSEQ) AS ASTNUM,                         \n");
						sql.append( "        B.FDCODE, B.ATCODE, B.AST1ST, B.AST2ND, B.AST3RD, B.ASTSEQ,                \n");
						sql.append( "       'Y' AS MODIFYGB, A.ASTDEPRST         										\n");
						sql.append( "   FROM ACCOUNT.ASTDEPR A                                                          \n");
						sql.append( "   LEFT JOIN ACCOUNT.ASTMSTN B ON A.FDCODE=B.FDCODE AND A.ATCODE=B.ATCODE          \n");
						sql.append( "                              AND A.AST1ST=B.AST1ST AND A.AST2ND=B.AST2ND          \n");
						sql.append( "                              AND A.AST3RD=B.AST3RD AND A.ASTSEQ=B.ASTSEQ          \n");
