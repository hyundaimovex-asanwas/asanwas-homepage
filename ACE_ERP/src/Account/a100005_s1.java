...
public class a100005_s1 extends HttpServlet {
	/**
	 * 
	 */
...
**********************************************************************************************/
				...
				int[] intArrCN = new int[]{  7, 66, 6, 200, 5,
					                          15
				}; 
				...
				if (!GauceReq.isBuilderRequest()) {					
					...
					sql.append( "     FROM(                                                                                                    \n " );
					sql.append( "      --매출액                                                                                                                                                                                                   \n " );
					sql.append( "       SELECT '0000001' ATCODE, '매출액' PRTNAM, A.FSREFVAL, C.GBCD||C.PSEQ||C.CDNAM FSREFNM, '00001'PRTSEQ, B.ATDECR,  \n " );
					...
					sql.append( "       GROUP BY B.ATDECR, A.FSREFVAL,C.GBCD||C.PSEQ||C.CDNAM                                                  \n " );
					...
					sql.append( "              A.FSREFVAL, C.GBCD||C.PSEQ||C.CDNAM FSREFNM,                                                    \n " );
					...
					if(!str[4].equals(""))sql.append( " AND A.FSREFVAL ='"+str[4]+"'                                                           \n " );										
					sql.append( "          AND B.ATUSEYN='Y'                                                                                   \n " );
					sql.append( "      GROUP BY A.ATCODE,B.ATDECR, A.FSREFVAL,C.GBCD||C.PSEQ||C.CDNAM                                          \n " );
					sql.append( "     UNION ALL                                                                                                \n " );					
					sql.append( "      --매출원가 전체                                                                                         		                                           \n " );
					sql.append( "      SELECT '0000002' ATCODE, '매출원가' PRTNAM, A.FSREFVAL, C.GBCD||C.PSEQ||C.CDNAM FSREFNM,'00009' PRTSEQ, B.ATDECR,       \n " );
					sql.append( "             CASE WHEN B.ATDECR='1' THEN SUM(A.DETOT)-SUM(A.CRTOT) ELSE SUM(A.CRTOT)-SUM(A.DETOT) END AMT     \n " );
					sql.append( "          FROM ACCOUNT.MFSREFTOT A                                   									       \n " );
					sql.append( "          LEFT JOIN ACCOUNT.ACTCODE B ON A.ATCODE = B.ATCODE                                                  \n " );
					...
					sql.append( "			        nvl(Y.FSREFVAL,CASE WHEN '01' = '"+str[3]+"' THEN '061001'                                 \n " );
					sql.append( "			                            WHEN '02' = '"+str[3]+"' THEN '066001'                                 \n " );
					sql.append( "			                            WHEN '03' = '"+str[3]+"' THEN '069001'                                 \n " );
					sql.append( "			                            ELSE '061001' END) FSREFVAL,                                           \n " );
					sql.append( "			        nvl(Y.FSREFNM,CASE WHEN '01' = '"+str[3]+ "' THEN '개성시범공장건축공사(공통)'                   \n " );
					...
					sql.append( "			 	   CASE WHEN Y.ATDECR='1' THEN COALESCE(Y.DETOT-Y.CRTOT,0) ELSE COALESCE(Y.CRTOT-Y.DETOT,0) END AMT  \n " );
					...
					sql.append( "			 														C.FSREFVAL, K.GBCD||K.PSEQ||K.CDNAM FSREFNM      \n " );
					sql.append( "			 											 FROM ACCOUNT.MFSREFTOT C                                    \n " );
					sql.append( "			 											 LEFT JOIN ACCOUNT.ACTCODE B ON C.ATCODE = B.ATCODE          \n " );
					...
					sql.append( "      UNION ALL                                                                                                     \n " );
					sql.append( "      --원가                                                                                                                                                                                                                  \n " );
					sql.append( "      SELECT A.ATCODE, F.PRTNAM, A.FSREFVAL, C.GBCD||C.PSEQ||C.CDNAM FSREFNM, F.PRTSEQ, B.ATDECR ,                  \n " );
					sql.append( "             CASE WHEN B.ATDECR='1' THEN SUM(A.DETOT)-SUM(A.CRTOT) ELSE SUM(A.CRTOT)-SUM(A.DETOT) END AMT           \n " );
					sql.append( "          FROM ACCOUNT.MFSREFTOT A                                                                                  \n " );
					sql.append( "          LEFT JOIN ACCOUNT.ACTCODE B ON A.ATCODE = B.ATCODE                                                        \n " );
					sql.append( "          LEFT JOIN ACCOUNT.RPTPRTACT F ON A.ATCODE = F.ATCODE                                                      \n " );
					...
					sql.append( "           AND B.ATCODE = F.ATCODE                                                                                  \n " );
					sql.append( "           AND F.PRTCODE='08'                                                                                       \n " );
					sql.append( "       GROUP BY A.ATCODE,A.ACTYYMM, F.PRTNAM,F.PRTSEQ,B.ATDECR, A.FSREFVAL,C.GBCD||C.PSEQ||C.CDNAM                  \n " );
					sql.append( "	    UNION ALL                                                                                                    \n " );
					sql.append( "		SELECT '9999999' ATCODE, '매출총이익' PRTNAM, Z.FSREFVAL,Z.FSREFNM,Z.PRTSEQ,''ATDECR,                          \n " );
					sql.append( "		        SUM(AMT1) - SUM(AMT2) AS AMT                                                                         \n " );
					sql.append( "		  FROM(                                                                                                      \n " );
					sql.append( "			    --매출액                                                                                                                                                                                             \n " );
					sql.append( "			    SELECT '0000001' ATCODE, '매출액' PRTNAM, A.FSREFVAL, C.GBCD||C.PSEQ||C.CDNAM FSREFNM, '99999' PRTSEQ, B.ATDECR, \n " );
					sql.append( "			           CASE WHEN B.ATDECR='1' THEN SUM(A.DETOT)-SUM(A.CRTOT) ELSE SUM(A.CRTOT)-SUM(A.DETOT) END AMT1, 0 AMT2    \n " );
					sql.append( "			      FROM ACCOUNT.MFSREFTOT A                                                             			     \n " );
					sql.append( "			      LEFT JOIN ACCOUNT.ACTCODE B ON A.ATCODE = B.ATCODE                                                 \n " );
					sql.append( "			      LEFT JOIN ACCOUNT.COMMDTIL C ON C.CDCODE = A.FSREFVAL AND C.CMTYPE='0030'                          \n " );
					sql.append( "			     WHERE A.ATCODE LIKE '41%'                                                                           \n " );
					...
					sql.append( "			     UNION ALL                                                                                           \n " );
					sql.append( "			    --매출원가 전체                                                                                                                                                                                   \n " );
					sql.append( "			    SELECT '0000002' ATCODE, '매출원가' PRTNAM, A.FSREFVAL, C.GBCD||C.PSEQ||C.CDNAM FSREFNM,'99999' PRTSEQ, B.ATDECR, 0 AMT1,    \n " );
					sql.append( "			           CASE WHEN B.ATDECR='1' THEN SUM(A.DETOT)-SUM(A.CRTOT) ELSE SUM(A.CRTOT)-SUM(A.DETOT) END AMT2 \n " );
					sql.append( "			      FROM ACCOUNT.MFSREFTOT A                                         							         \n " );
					sql.append( "			      LEFT JOIN ACCOUNT.ACTCODE B ON A.ATCODE = B.ATCODE                                                 \n " );
					sql.append( "			      LEFT JOIN ACCOUNT.RPTPRTACT F ON A.ATCODE = F.ATCODE                                               \n " );
					sql.append( "			      LEFT JOIN ACCOUNT.COMMDTIL C ON C.CDCODE = A.FSREFVAL AND C.CMTYPE='0030'                          \n " );
					...
					//System.out.println("# Query a100005_s1: \n" + sql);
