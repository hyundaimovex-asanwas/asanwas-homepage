...
public class a160004_s1 extends HttpServlet {
	/**
	 * 
	 */
...
**********************************************************************************************/
				...
				if (!GauceReq.isBuilderRequest()) {					
					...
					sql.append( "  A.EMPNO, \n");								/* 사번 */
					sql.append( "  A.EMPNMK, \n");							/*한글명*/
					sql.append( "  D.MINORNM AS PAYDIVNM, \n"); /*지급구분*/
					sql.append( "  B.DEPTCD, \n");							/* 부서코드 */
					sql.append( "  C.DEPTNM AS DEPTNM, \n");		/* 부서명*/
					sql.append( "  E.MINORNM AS MINORNM, \n");		/* 직위명*/
					sql.append( "  F.MINORNM AS TMINORNM, \n");		/* 근무지*/
					sql.append( "  B.PAYDT, ");		/*  지급일*/
					sql.append( " H.GP35000, H.BP35000, H.GP24000, H.BP24000, \n");	
					sql.append( "COALESCE((B.P10000+B.P11000+B.P12000+B.P21000+B.P22000+B.P23000+H.GP24000+B.P25000+B.P26000+B.P27000+B.P28000+B.P29000+B.P30000+B.P31000+B.P32000+B.P33000+B.P34000+H.GP35000+B.P36000+B.P37000+B.P38000+B.P39000+B.PX0010+B.PX0020+B.PX0030+B.PX0040),0) AS PAYSUM, \n");
					...
					sql.append( "                FROM PAYROLL.PBPAY G)H ON A.EMPNO = H.EMPNO AND H.APPDT ='"  + str[0] + "'  \n");
					if (str[1].equals("1")) sql.append( " AND H.PAYDIV IN ('1','4') ");   //정기급여
					if (str[1].equals("5")) sql.append( " AND H.PAYDIV IN('5') ");       //상여
					sql.append( " LEFT JOIN PAYROLL.HCDEPT C ON  B.DEPTCD=C.DEPTCD \n");
					sql.append( " LEFT JOIN PAYROLL.HCLCODE D ON  B.PAYDIV=D.MINORCD AND D.MAJORCD='1120' \n"); /*지급구분*/
					sql.append( " LEFT JOIN PAYROLL.HCLCODE E ON  B.PAYGRD=E.MINORCD AND E.MAJORCD='2002' \n"); /*직위*/
					sql.append( " LEFT JOIN PAYROLL.HCLCODE F ON  C.DEPTGB=F.MINORCD AND F.MAJORCD='2050' \n"); /*근무지*/
					sql.append( " WHERE A.EMPNO=B.EMPNO ");
					if (!str[0].equals("")) sql.append( " AND B.APPDT= '"  + str[0] + "' ");  //지급년월
					if (str[1].equals("1")) sql.append( " AND B.PAYDIV IN('1','4') ");   //지급구분
