...
public class a160004_s7 extends HttpServlet {
	/**
	 * 
	 */
...
**********************************************************************************************/
				...
				if (!GauceReq.isBuilderRequest()) {					
					...
					sql.append("  COALESCE(B.COUNTR,0) AS DCOUNTR, --현재월 인원수 */						    \n ");
					sql.append("  COALESCE(B.PAYSUMR,0) AS DPAYSUMR, -- 총급여*/                                \n ");
					sql.append("  COALESCE(B.BP35000R,0) AS DBP35000R, -- 식대 */                               \n ");
					sql.append("  COALESCE(B.S91000R,0) AS DS91000R, -- 차량+식대 */                              \n ");
					sql.append("  COALESCE(B.T11000R,0) AS DT11000R, -- 소득세*/                                \n ");
					sql.append("  COALESCE(B.T12000R,0) AS DT12000R, -- 주민세*/                                \n ");
					sql.append("  COALESCE(B.BXTOTALR,0) AS DBXTOTALR,   --기타공제계*/                         \n ");
					sql.append("  COALESCE(B.S93000R,0) AS DS93000R, -- 공제합계*/                              \n ");
					sql.append("  COALESCE(B.S94000R,0) AS DS94000R, -- 실지급액*/                              \n ");
					sql.append("  '"+str[0].substring(0,4)+"' AS DCYYYY, --현재년 */                            \n ");
					sql.append("  '"+str[0].substring(4,6)+"' AS DCMM   --현재월 */                            \n ");
					sql.append("    FROM																		\n ");
					sql.append(" (SELECT																		\n ");
					...
					sql.append("  SUM(B.PX0010)+							--전월소급*/						\n ");
					sql.append("  SUM(B.PX0020)+							--전월소급*/						\n ");
					sql.append("  SUM(B.PX0030)+							--전월소급*/						\n ");
					sql.append("  SUM(B.PX0040))						--전월소급*/							\n ");
					...
					sql.append( " AND B.EMPNO NOT IN ('1990005','2000001 ')										\n"); //관광경협 임의 별도로 보이게 함.
					if (str[1].equals("T")) sql.append( " AND B.PAYDIV IN( 'T')									\n ");  //지급구분
					...
					//2010.01.18 JYS 예외처리 ==============>
					sql.append("  SELECT                                                                                             \n ");
					sql.append("     CASE WHEN B.EMPNO='1990005' THEN D.SEQ ELSE E.SEQ END SEQ ,                                     \n ");
					sql.append("     CASE WHEN B.EMPNO='1990005' THEN D.MINORCD ELSE E.MINORCD END MINORCD,                          \n ");
					sql.append("     CASE WHEN B.EMPNO='1990005' THEN RTRIM(D.MINORNM) ELSE RTRIM(E.MINORNM) END DEPTNM,             \n ");
					sql.append("  COUNT(B.EMPNO) AS COUNTR,														\n ");
					sql.append("  SUM(B.P35000) AS P35000R,-- 비과세 식대*/				 						\n ");
					sql.append("  SUM(H.BP35000) AS BP35000R,-- 비과세 식대*/									\n ");
					sql.append("  SUM(H.GP35000) AS GP35000R,-- 과세 식	*/										\n ");
					...
					sql.append( "        FROM PAYROLL.PBPAY G)H ON B.EMPNO = H.EMPNO AND H.APPDT ='"  + str[0] + "' AND H.PAYDIV ='"+str[1]+"' \n");					
					sql.append("     LEFT OUTER JOIN PAYROLL.HCDEPT C ON  B.DEPTCD = C.DEPTCD                                        \n ");
					sql.append("     LEFT JOIN PAYROLL.HCLCODE D ON D.MINORCD = '94' AND D.MAJORCD = '2051'                          \n ");
					sql.append("     LEFT JOIN PAYROLL.HCLCODE E ON E.MINORCD = '95' AND E.MAJORCD = '2051'                          \n ");
					sql.append("     WHERE B.APPDT='"+str[0]+"'     	                                                               \n ");
					sql.append("     AND (B.JTGB<>2 AND B.JTGB<>3 AND B.JTGB<>4 AND B.JTGB<>5 AND B.JTGB<>6)                         \n ");
					sql.append("     AND B.DEPTCD NOT IN( SELECT DEPTCD FROM PAYROLL.HCDEPT WHERE GROUPID = '01'  AND USEYN='T' )    \n ");
					if (str[1].equals("T")) sql.append( " AND B.PAYDIV IN( 'T') \n ");  //지급구분
					sql.append("     AND B.EMPNO IN ('1990005','2000001')                                                            \n ");
					sql.append("     GROUP BY  B.EMPNO, D.SEQ,E.SEQ, D.MINORCD, D.MINORNM,E.MINORCD, E.MINORNM                       \n ");
