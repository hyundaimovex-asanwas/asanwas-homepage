...
public class a400030_s11 extends HttpServlet {
	/**
	 * 
	 */
...
**********************************************************************************************/
				...
				String[] strArrCN = new String[]{ 
					...
					"BRCHNM",   "ACCTGBNM",
					"WRID",     "WRDT",      "UPID",     "UPDT",  "BALAMTS_SUM"
					 }; 
				...
				for (int i=0; i<strArrCN.length; i++) { //set column column 
					...
					if (!GauceReq.isBuilderRequest()) {					
						...
						sql.append( "       ( SELECT ROUND(SUM(CASE WHEN DEPOGB='1'THEN BALAMTS ELSE 0 END)/1000000)+ \n");
						sql.append( "                ROUND(SUM(CASE WHEN DEPOGB='2'THEN BALAMTS ELSE 0 END)/1000000)+ \n");
						sql.append( "                ROUND(SUM(CASE WHEN DEPOGB='3'THEN BALAMTS ELSE 0 END)/1000000)+ \n");
						sql.append( "                ROUND(SUM(CASE WHEN DEPOGB='4'THEN BALAMTS ELSE 0 END)/1000000)  \n");
						sql.append( "           FROM ACCOUNT.ACDPSUM              \n");
						sql.append( "          WHERE DEDATE ='"+str[2] +"') BALAMTS_SUM     \n");
						sql.append( "   FROM ACCOUNT.ACDPSUM A, ACCOUNT.COMMDTIL C,  ACCOUNT.COMMDTIL D   \n");
						sql.append( "  WHERE A.DEPOGB ='1'                                                \n");
						...
						System.out.println("# a400030_s11 Query : \n" + sql);
