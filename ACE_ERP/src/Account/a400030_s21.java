...
public class a400030_s21 extends HttpServlet {
	/**
	 * 
	 */
...
**********************************************************************************************/
				...
				for (int i=0; i<strArrCN.length; i++) { //set column column 
					...
					if (!GauceReq.isBuilderRequest()) {					
						...
						sql.append( " 	 ''CURUNIT,   ROUND((X.A01+X.A02-X.A03)/1000000) AS BALAMTS2, X.A01+X.A02-X.A03 AS BALAMTS, 0 FBALAMTS,       \n");
						sql.append( " 	 ''REMARK,  X.ACCTCD, X.ACCTNO, '' INRATE, '' ACCTUSE, ''CHKBOX , NVL(Y.CLOSEYN,'N')CLOSEYN                   \n");
						sql.append( " FROM (                                                                                                          \n");
						sql.append( " --원화                                                                                                                                                       \n");
						sql.append( "   SELECT  '"+str[2]+"'AS DEDATE, '현금(원화)' AS BRCHNM, '' AS ACCTGBNM , 'WON' AS GB,  '1'ACCTNO,  '1' ACCTCD,   \n");
						...
						sql.append( "  SELECT                                                                                                                                                      \n");
						sql.append( "    '"+str[2]+"'AS DEDATE, '현금(외화)' AS BRCHNM, '' AS ACCTGBNM ,'FOR' AS GB, '2' ACCTNO, '2' ACCTCD,                                                                                         \n");
