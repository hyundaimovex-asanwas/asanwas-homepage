...
public class a400320_s1 extends HttpServlet {
	/**
	 * 
	 */
...
**********************************************************************************************/
				...
				String[] strArrCN = new String[]{ "FDCODE",  "FSDAT",    "FSNBR",    "FSSEQ",    "FSNUM",  "SEQ01",
				...
				int[] intArrCN = new int[]{  2,  9,  6,  5,   3, 10,  
						                      8,  4,  7, 66,   1,
						                     15, 15, 13, 62, 132, 
					                          3, 20,  1,  4, 
					                          2,  2,  2,  2,   2, 
				...
				if (!GauceReq.isBuilderRequest()) {					
					...
					sql.append( " SELECT  '00'  AS FDCODE , 'A00000000'   AS FSDAT, '000000' AS FSNBR, '00000' AS FSSEQ, '000' AS FSNUM, X.SEQ01,     \n");
					...
					sql.append( "         SELECT CASE WHEN SUBSTR(P.ATCODE,1,2) ='43' THEN 'A'||P.ATCODE                                              \n");
					sql.append( "                     WHEN SUBSTR(P.ATCODE,1,2) ='44' THEN 'B'||P.ATCODE                                              \n");
					sql.append( "                     ELSE 'C'||P.ATCODE END SEQ01,                                                                   \n");
					sql.append( "                P.ATCODE||'1' AS SEQ, P.ATCODE,                                                                      \n");
					sql.append( "                CASE WHEN Z.ATDECR ='1' THEN P.SUPAMT ELSE 0 END AS DEAMT,                                           \n");
					sql.append( "                CASE WHEN Z.ATDECR ='2' THEN P.SUPAMT ELSE 0 END AS CRAMT,                                           \n");
					sql.append( "                A.VENDCD, A.VENDNM, P.COSTGB, P.COSTCT AS COSTBGT,                                                    \n");
					...
					sql.append( "           FROM ACCOUNT.ACAPPFUND A,  ACCOUNT.ACAPPCOST P, ACCOUNT.ATTAXMST B, ACCOUNT.ACTCODE Z                     \n");
					sql.append( "          WHERE A.APPNO  = P.APPNO                                                                                   \n");
					sql.append( "            AND P.ATCODE = Z.ATCODE(+)                                                                               \n");
					...
					sql.append( "                     WHEN SUBSTR(P.ATCODE,1,2) ='44' THEN 'B'||P.ATCODE                                              \n");
					sql.append( "                     ELSE 'C'||P.ATCODE END SEQ01,                                                                   \n");
					sql.append( "                P.ATCODE||'2' AS SEQ,                                                                                \n");
					sql.append( "                CASE WHEN SUBSTR(P.ATCODE,1,2)='44' THEN  '2100510' ELSE '2100110' END ATCODE,                       \n");
					sql.append( "                CASE WHEN Z.ATDECR ='1' THEN 0 ELSE P.SUPAMT  END AS DEAMT,                                         \n");
					sql.append( "                CASE WHEN Z.ATDECR ='2' THEN 0 ELSE CASE WHEN ( A.PURGB!='T' OR A.PURGB IS NULL)  THEN P.SUPAMT  ELSE (P.SUPAMT + P.SUPAMT * 0.1) END  END AS CRAMT,\n");
					...
					sql.append( "           FROM ACCOUNT.ACAPPFUND A, ACCOUNT.ACAPPCOST P, ACCOUNT.ACTCODE Z                                          \n");
					sql.append( "          WHERE A.APPNO =P.APPNO                                                                                     \n");
					sql.append( "            AND P.ATCODE = Z.ATCODE(+)                                                                               \n");
					sql.append( "            AND A.APPNO IN ("+str[4]+")                                                                              \n");
					sql.append( "          UNION ALL                                                                                                  \n");
					sql.append( "          --선급부가가치세                                                                                                                                                                                             \n");
					sql.append( "        SELECT 'C'||'1112300' AS SEQ01,                                                                              \n");
					sql.append( "               A.ATCODE||'3' AS SEQ,                                                                                 \n");
					...
					sql.append( "           AND Z.ATCODE = '1112300'                                                                                  \n");
					sql.append( "           AND A.TAXNBR = B.TAXNBR                                                                                   \n");
					...
					sql.append( "                                                                                                                     \n");
					sql.append( "  GROUP BY X.SEQ01,X.SEQ,X.SEQ2, X.ATCODE, C.ATKORNAM, C.ATDECR, X.DEAMT, X.CRAMT,                                   \n");
					sql.append( "           X.VENDCD, X.VENDNM, X.REMARK, C.ATDEBTYN, X.DOCUMCD, X.DOCUVAL,X.VENDREAMRK, X.COSTGB, X.COSTBGT,         \n");
					sql.append( "           X.TAXNBR,X.TAXKND, X.TAXDAT, X.PAYGB,    X.CURUNITCD, X.CURUNIT, X.FOUTAMT                                \n");
					sql.append( "  ORDER BY X.VENDCD, X.SEQ01, X.REMARK, X.SEQ2, X.SEQ                                                                \n");
								
					//System.out.println("a400320_S1 : " + sql);
