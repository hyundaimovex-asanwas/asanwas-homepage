...
public class a500010_t1 extends HttpServlet{
	/**
	 * 
	 */
	...
	public void doPost(HttpServletRequest req,HttpServletResponse res) {
		...
		try{
			...
			try {
				...
				int idxV_ACCTNO_CHK = userSet.indexOfColumn("V_ACCTNO_CHK");
				int idxSYS_FILE_NAME= userSet.indexOfColumn("SYS_FILE_NAME");
				...
				InsertSql.append( "	VEND_CD,	VEND_NM,	VEND_ID,	SEQ,  					\n");
				InsertSql.append( "	BANK_CD,	BANK_NM, 										\n");				
				InsertSql.append( "	ACCT_NO,	ACCT_AUTH,	REASON,	GUBUN,	STATUS,	USEYN,		\n");	
				InsertSql.append( "	ATT_GB,		ATT_FILE_NAME, V_ACCTNO_CHK, SYS_FILE_NAME,     \n");
				...
				UpdateSql.append( " ,V_ACCTNO_CHK = ? 					    					\n");
				UpdateSql.append( " ,SYS_FILE_NAME = ? 					    					\n");
				UpdateSql.append( " ,I_EMPNO = '"+str1+"'										\n");
				UpdateSql.append( " ,I_DATE  =	sysdate 										\n");
				...
				for (int j = 0; j < rows.length; j++){
					...
					if(rows[j].getJobType() == GauceDataRow.TB_JOB_INSERT) {
						...
						sql0.append( "        ELSE (MAX(SEQ)+1) END  								\n");
						...
						if(rs0.next()){
							intVend_SEQ=  rs0.getInt(1);
						}
						
						rs0.close();
						stmt0.close();
						
					    rows[j].setDouble(idxSEQ, intVend_SEQ);		
						...
						gsmt.bindColumn(12, idxATT_GB);
						gsmt.bindColumn(13, idxATT_FILE_NAME);
						gsmt.bindColumn(14, idxV_ACCTNO_CHK);
						gsmt.bindColumn(15, idxSYS_FILE_NAME);
					...
					if(rows[j].getJobType() == GauceDataRow.TB_JOB_UPDATE) {
						...
						gsmt.bindColumn(10, idxATT_GB);
						gsmt.bindColumn(11, idxATT_FILE_NAME);
						gsmt.bindColumn(12, idxV_ACCTNO_CHK);
						gsmt.bindColumn(13, idxSYS_FILE_NAME);
						gsmt.bindColumn(14, idxVEND_CD);	
						gsmt.bindColumn(15, idxSEQ);
