...
public class comm_func_s2 extends HttpServlet {
	/**
	 * 
	 */
...
**********************************************************************************************/
        ...
        str[5]	= req.getParameter("v_str6"); 	//회계년월fr
				...
				if (!GauceReq.isBuilderRequest()) {					
					...
					sql.append( " SELECT '' FDCODE,X.ATCODE,                                                           \n" );
					sql.append( "       SUM(CASE WHEN B.ATDECR ='1' THEN X.DETOT - X.CRTOT                             \n" );
					sql.append( "                WHEN B.ATDECR ='2' THEN X.CRTOT - X.DETOT ELSE 0 END)  TOTALVAL       \n" );
					sql.append( "   FROM (                                                                             \n" );
					sql.append( "        --금강산 외(본사,서울,개성)                                                   \n" );
					sql.append( "        SELECT A.FDCODE, A.ATCODE, A.DETOT, A.CRTOT                                   \n" );
					sql.append( "          FROM ACCOUNT.MTOTALBAL A                                                    \n" );
					sql.append( "         WHERE A.ACTYYMM>='"+str[5]+"'                                            \n" );
					...
					sql.append( "         --금강산                                                                \n" );
					sql.append( "        SELECT A.FDCODE, A.ATCODE, A.FODETOT DETOT, A.FOCRTOT CRTOT                   \n" );
					sql.append( "          FROM ACCOUNT.MTOTALBAL A                                                    \n" );
					sql.append( "         WHERE A.ACTYYMM>='"+str[5]+"'                                            \n" );
					sql.append( "           AND A.ACTYYMM<='"+str[1]+"'                                            \n" );
					sql.append( "           AND A.ATCODE='"+str[2]+"'                                              \n" );
					...
					sql.append( "         UNION ALL                                                                    \n" );
					sql.append( "         --수정분개                                                                   \n" );
					sql.append( "        SELECT  A.FDCODE FDCODE,A.ATCODE, A.DETOT, A.CRTOT                            \n" );
					sql.append( "          FROM ACCOUNT.MDMTOTAL A                                                     \n" );
					sql.append( "         WHERE A.ACTYYMM>='"+str[5]+"'                                            \n" );
					sql.append( "           AND A.ACTYYMM<='"+str[1]+"'                                            \n" );
					sql.append( "           AND A.ATCODE='"+str[2]+"'                                              \n" );
					sql.append( "           AND A.FDCODE IN ('01','02','03','04')                                      \n" );
					if(!str[3].equals("")) sql.append(" AND A.COCODE = '"+str[3]+"' \n");
					if(!str[4].equals("")) sql.append(" AND A.DIVCD = '"+str[4]+"'  \n");
					sql.append( "         ) X, ACCOUNT.ACTCODE B                                                       \n" );
					sql.append( " WHERE X.ATCODE = B.ATCODE                                                            \n" );
					sql.append( " GROUP BY  X.ATCODE                                                                   \n" );
