...
public class commdtil_s1 extends HttpServlet {
	/**
	 * 
	 */
...
**********************************************************************************************/
				...
				String  str4	= req.getParameter("v_str4");  //폐기일자 구분 C이면 폐기일자가 없는건만 가져옴 
				String  str5	= req.getParameter("v_str5");  //본부(출력순)으로 중복값 체크된 건만 가져옴
				String  str6	= req.getParameter("v_str6");  //본부(출력순)으로 중복값 체크된 건만 가져옴
				...
				if (!GauceReq.isBuilderRequest()) {					
					...
					if (!str2.equals(""))sql.append( "   AND CDCODE = '" + str2 + "' \n" );
					if (!str3.equals(""))sql.append( "   AND CDNAM  LIKE '%" + str3 + "%' \n" );
					if (!str4.equals(""))sql.append( "   AND (RTRIM(DSUDT) ='' OR RTRIM(DSUDT) IS NULL) \n");
					if (!str5.equals(""))sql.append( "   AND PSEQ <>'' \n");
					if (!str6.equals(""))sql.append( "   AND GBCD = '"+ str6 +"' \n");

					//logger.dbg.println(this,sql.toString());
                    //System.out.println("commdtil_s1::"+sql.toString());
/*********************************************************************************************/
