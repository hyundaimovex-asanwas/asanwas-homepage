...
public class discount_popup_t1 extends HttpServlet{
	/**
	 * 
	 */
		...
		try{
			...
			try {
				...
				String[] StrArrCN = new String[] {
					...
					"BANCOD","DCRATE","DCDAYS","DCAMT","SLIPYN",//"UPDT","UPID"
				};
				...
				for (int j = 0; j < rows.length; j++){
					if(rows[j].getJobType() == GauceDataRow.TB_JOB_INSERT) {
						...
						********************************************************************/
						...
						InsertSql.append( " INSERT INTO ACCOUNT.ABINHAL (						");
						InsertSql.append( " FDCODE,ATCODE,FUNDDIV,REFCODE2,STRDATE,	");
						InsertSql.append( " BANCOD,DCRATE,DCDAYS,DCAMT,SLIPYN				");
					...
					if(rows[j].getJobType() == GauceDataRow.TB_JOB_UPDATE) {
						...
						UpdateSql.append( " UPDATE ACCOUNT.ABINHAL SET													");
						UpdateSql.append( " STRDATE=?, BANCOD=?, DCRATE=?, DCDAYS=?, DCAMT=?, SLIPYN=?,	");
					...
					if(rows[j].getJobType() == GauceDataRow.TB_JOB_DELETE) {
						...
						DeleteSql.append( " DELETE FROM ACCOUNT.ABINHAL 															");
