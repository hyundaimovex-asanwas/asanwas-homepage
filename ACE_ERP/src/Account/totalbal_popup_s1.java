...
public class totalbal_popup_s1 extends HttpServlet {
	/**
	 * 
	 */
...
**********************************************************************************************/
				...
				String[] strArrCN = new String[]{ "ATDECR","ATCODE","ATKORNAM","ACTDAT","DETOT",
																					"CRTOT","SLPVLA","CUMULTAL","REMARK","VENDER",
																					"LEVELS","FDCODE","FSDAT","FSNBR"
				}; 
				...
				if (!GauceReq.isBuilderRequest()) {					
					...
					sql.append( "         X.CUMULTAL AS CUMULTAL,X.REMARK AS REMARK, X.VENDER AS VENDER, X.LEVELS AS LEVELS,                \n");
					sql.append( "         X.FDCODE,X.FSDAT,X.FSNBR																												                  \n");
					sql.append( "  FROM (                                                                                                  	\n");	
					...
					sql.append( "         0 AS CUMULTAL,'' AS REMARK,'' AS VENDER,SUBSTR('"+str[4]+"',3,2)||'0000' LEVELS,                  \n");
					...
					if(!str[1].equals(""))	sql.append( " AND A.ACTDAT BETWEEN '"+str[4]+"0101' AND '"+str[6]+"'                            \n");
					if(!str[3].equals(""))	sql.append( " AND A.ATCODE BETWEEN '"+str[3]+"' AND '"+str[3]+"'                                \n");
					sql.append( " GROUP BY B.ATDECR, B.ATCODE, B.ATKORNAM, ACTDAT                                                         	\n");
					sql.append( " ) X                                                         																							\n");
					sql.append( " GROUP BY X.ATDECR, X.ATCODE, X.ATKORNAM, X.ACTDAT, X.CUMULTAL,X.REMARK, X.VENDER,X.LEVELS,X.FDCODE,X.FSDAT,X.FSNBR \n");
					...
					sql.append( "        G.CUMULTAL AS CUMULTAL,G.REMARK AS REMARK,H.VEND_NM AS VENDER,G.LEVELS	AS LEVELS,															\n");
					sql.append( "        G.FDCODE,G.FSDAT,G.FSNBR																																												\n");
					sql.append( "	FROM (SELECT E.ATDECR, E.ATCODE, E.ATKORNAM,E.ACTDAT,E.DETOT,E.CRTOT,E.SLPVLA,																				\n");
					sql.append( "	             E.CUMULTAL,E.REMARK,F.FSREFVAL,E.LEVELS,E.FDCODE,E.FSDAT,E.FSNBR																					\n");
					sql.append( "   FROM(SELECT C.ATDECR,C.ATCODE,C.ATKORNAM,C.ACTDAT,C.DETOT,C.CRTOT,C.SLPVLA,																					\n");
					sql.append( "               C.CUMULTAL, D.REMARK, C.FDCODE,C.FSDAT,C.FSNBR,C.FSSEQ,C.LEVELS																					\n");
					...
					sql.append( "  GROUP BY G.ATDECR,G.ATCODE,G.ATKORNAM,G.ACTDAT,G.DETOT,G.CRTOT,G.SLPVLA,G.CUMULTAL,G.REMARK,H.VEND_NM,G.LEVELS,			  ");
					sql.append( "  G.FDCODE,G.FSDAT,G.FSNBR																																															\n");
					...
					sql.append( " SUM(A.DETOT) AS DETOT, SUM(A.CRTOT) AS CRTOT,																													\n");
					sql.append( "        SUM(CASE WHEN B.ATDECR='1' THEN A.DETOT-A.CRTOT ELSE A.CRTOT-A.DETOT END) AS SLPVLA,           \n");
					sql.append( "        0 AS CUMULTAL, '' AS REMARK, '' AS VENDER, SUBSTR(A.ACTDAT,3,4)||'32' LEVELS,									\n");
					sql.append( "        '' AS FDCODE,'' AS FSDAT,'' AS FSNBR																														\n");
					sql.append( " FROM ACCOUNT.TOTALBAL A, ACCOUNT.ACTCODE B                                                            \n");
					sql.append( " WHERE A.ATCODE=B.ATCODE                                                                               \n");
					if(!str[0].equals(""))  sql.append( " AND A.FDCODE='"+str[0]+"'                                                     \n");
					...
					sql.append( "             ) S ) AS CUMULTAL,'' AS REMARK, '' AS VENDER, SUBSTR(A.ACTDAT,3,4)||'33' LEVELS,  				\n");
					sql.append( "             '' AS FDCODE,'' AS FSDAT,'' AS FSNBR																											\n");
					sql.append( "     FROM ACCOUNT.TOTALBAL A, ACCOUNT.ACTCODE B																												\n");
					sql.append( "     WHERE A.ATCODE=B.ATCODE																																						\n");
					if(!str[0].equals(""))  sql.append( " AND A.FDCODE='"+str[0]+"'																											\n");
		            ...
		            //System.out.println("totalbal_popup_s1:"+ sql.toString());
/*********************************************************************************************/
