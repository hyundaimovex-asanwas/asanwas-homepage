...
public class trusttake_popup_t1 extends HttpServlet{
	/**
	 * 
	 */
		...
		try{
			...
			try {
				...
				String[] StrArrCN = new String[] {
					...
					"BANCOD","ORDDATE","ORDAMT","SLIPYN"//,"UPDT","UPID"
				};
				...
				for (int j = 0; j < rows.length; j++){
					if(rows[j].getJobType() == GauceDataRow.TB_JOB_INSERT) {
						...
						********************************************************************/
						...
						InsertSql.append( " INSERT INTO ACCOUNT.ABINSUT (						");
						InsertSql.append( " FDCODE,ATCODE,FUNDDIV,REFCODE2,STRDATE,	");
						InsertSql.append( " BANCOD,ORDDATE,ORDAMT,SLIPYN						");
					...
					if(rows[j].getJobType() == GauceDataRow.TB_JOB_UPDATE) {
						...
						UpdateSql.append( " UPDATE ACCOUNT.ABINSUT SET													");
						UpdateSql.append( " STRDATE=?, BANCOD=?, ORDDATE=?, ORDAMT=?, SLIPYN=?,	");
					...
					if(rows[j].getJobType() == GauceDataRow.TB_JOB_DELETE) {
						...
						DeleteSql.append( " DELETE FROM ACCOUNT.ABINSUT 															");
