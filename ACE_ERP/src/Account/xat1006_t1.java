...
public class xat1006_t1 extends HttpServlet{
	/**
	 * 
	 */
	...
	public void doPost(HttpServletRequest req,HttpServletResponse res) {
		...
		*************************************************************************************/
		...
		/************************************************************************************
		 @ POSDATA에 UPDATE 처리   ( FSDAT, FSNBR ) 신청접수번호
		************************************************************************************/
		 ...
		 if (userSet7 != null) {
				...
				int idx7DFSCD           = userSet7.indexOfColumn("DFSCD");
				int idx7SALEDT          = userSet7.indexOfColumn("SALEDT");
				int idx7POSNO           = userSet7.indexOfColumn("POSNO");
				int idx7RECNO           = userSet7.indexOfColumn("RECNO");
				int idx7GOODCD          = userSet7.indexOfColumn("GOODCD");
			    ...
			    // 자금신청으로 변경해야 함.  
				UpdateSql7.append( " UPDATE ACCOUNT.TBIF_POSDATA " );
				UpdateSql7.append( "    SET FDCODE =?" );
			    UpdateSql7.append( "       ,FSDAT = ?" );
			    UpdateSql7.append( "       ,FSNBR = ?" );
				UpdateSql7.append( "  WHERE DFSCD=?" );
				UpdateSql7.append( "    AND SALEDT=?" );
				UpdateSql7.append( "    AND POSNO=?" );
				UpdateSql7.append( "    AND RECNO=?" );
				UpdateSql7.append( "    AND GOODCD=?" );
				...
				for (int b=0;b<rows7.length;b++){
					...
					gsmt7.bindColumn(4,idx7DFSCD);
					gsmt7.bindColumn(5,idx7SALEDT);
					gsmt7.bindColumn(6,idx7POSNO);
					gsmt7.bindColumn(7,idx7RECNO);
					gsmt7.bindColumn(8,idx7GOODCD);
