...
public class h080001_t2 extends HttpServlet{
	...
	public void doPost(HttpServletRequest req,HttpServletResponse res) {
        ...
        for (int i=0; i<strArrCN.length; i++) {
				...
				sql.append( " SELECT MAX(SEQ)+1 CNT FROM PAYROLL.T_HIFAMILY WHERE EMPNO = '" + str1 + "'" );
				...
				for (int j = 0; j < rows.length; j++){
					...
					if(rows[j].getJobType() == GauceDataRow.TB_JOB_INSERT) {
						...
						InsertSql.append( " INSERT INTO PAYROLL.T_HIFAMILY " );
						InsertSql.append( " ( " );
					...
					if(rows[j].getJobType() == GauceDataRow.TB_JOB_UPDATE) {
						UpdateSql = new StringBuffer();
						UpdateSql.append( " UPDATE PAYROLL.T_HIFAMILY " );
					...
					if(rows[j].getJobType() == GauceDataRow.TB_JOB_DELETE ) {
						...
						DeleteSql.append( " DELETE FROM PAYROLL.T_HIFAMILY ");
