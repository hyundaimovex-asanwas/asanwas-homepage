...
public class h080002_t1 extends HttpServlet{
	...
	public void doPost(HttpServletRequest req,HttpServletResponse res) {
        ...
        int[] idx = new int[92];
				...
				idx[91] = userSet.indexOfColumn(strArrCN[91]);   //JOBKIND
              ...
              sql.append( "  WHERE SUBSTR(EMPNO,1,1)='5' AND  SUBSTR(EMPNO,2,2) = '" + str1 + "'" );
							...
							if (maxCnt!=null){
								if (!maxCnt.equals("0000")) {
									...
									ichk++;
									if (ichk<10) maxCnt = "5" + str1 + "000" + ichk;	//5+05+000+2
									else if (ichk<100 && ichk>9) maxCnt = "5" + str1 + "00" + ichk;
									else if (ichk<1000 && ichk>99) maxCnt = "5" + str1 + "0" + ichk;
									else if (ichk>999) maxCnt = "5" + str1 + ichk;
								} else {
									maxCnt = "5" + str1 + "0001";
								}
							} else {
						...
						GauceStatement gsmt4 = conn.getGauceStatement(InsertSql2.toString());
						gsmt4.setGauceDataRow(rows[j]);

						gsmt4.bindColumn(1, idx[89]);
						gsmt4.bindColumn(2, idx[90]);
					...
					if(rows[j].getJobType() == GauceDataRow.TB_JOB_UPDATE) {
						...
						arr_sql = "SELECT EMPNO FROM PAYROLL.T_HIASSUR WHERE EMPNO = '"+rows[j].getString(idx[0])+"'";

						ResultSet rs2 = stmt2.executeQuery(arr_sql);
						...
						if (rs2.next()) {
							...
							InsertSql3.append( " INSERT INTO PAYROLL.T_HIASSUR " );
							InsertSql3.append( " ( EMPNO,HEIGHT,WEIGHT )" );
							...
							gsmt3.bindColumn(2, idx[89]);
							gsmt3.bindColumn(3, idx[90]);
						...
						UpdateSql.append( "				 JOBKIND = ? ");//---------------------------------------------------------[3-88]
						UpdateSql.append( "  WHERE EMPNO = ? " );//----------------------------------------------------------[1-91]
						...
						gsmt.bindColumn(88, idx[91]);	//JOBKIND
						gsmt.bindColumn(89, idx[0]);	//EMPNO
