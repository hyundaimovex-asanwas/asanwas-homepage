...
public class h080004_s3 extends HttpServlet {
...
**********************************************************************************************/
				...
				str[19] = req.getParameter("v_str20");	//성별
				str[20] = req.getParameter("v_str21");	//호봉
				str[21] = req.getParameter("v_str22");	//호봉 ODER BY (ASC/DESC)
				...
				String[] strArrCN = new String[]{ "EMPNMK","NME","NMC","EMPNO","RESINO",
																					...
																					"TODT","STRTDT","ENDDT","EDIVNM","SCHNM",
																					"BLOOD","TELNO","EMPADD","PICFILE","RILIGNM","GRDDIVNM"
																					,"TREECD","SEQ"
																				};
				...
				if (!GauceReq.isBuilderRequest()) {					
					...
					sql.append( " RTRIM(A.TODT) AS TODT, \n");															//--계약만료일
					...
					sql.append( " 'http://203.242.32.198/services/servlet/Contract.h080001_s0?v_str1=' || A.EMPNO AS PICFILE, \n ");	//--사진
					sql.append( " RTRIM(K.MINORNM) AS RILIGNM, \n");												//--종교
					sql.append( " RTRIM(J.MINORNM) AS GRDDIVNM \n");												//--직급
					sql.append( " , B.TREECD, IFNULL(C.SEQ,0)SEQ ");																			//--조직도,SEQ
...
//					sql.append( " LEFT JOIN PAYROLL.HCLCODE E ON E.MINORCD = A.BIRCITY AND E.MAJORCD = '2011' \n");	//--출신도
					...
					sql.append( " LEFT JOIN PAYROLL.HCLCODE J ON J.MINORCD = A.GRDDIV  AND J.MAJORCD='2001' \n");	//--직급
					sql.append( " LEFT JOIN PAYROLL.HCLCODE K ON K.MINORCD = A.RILIGN  AND K.MAJORCD='1117' \n");	//--종교
					sql.append( " LEFT JOIN PAYROLL.T_HIEDUC G ON G.EMPNO = A.EMPNO \n");
					sql.append( "														 AND G.GRAYY = (SELECT MAX(Z.GRAYY) AS GRAYY \n");
					sql.append( "														 FROM PAYROLL.T_HIEDUC Z \n");
					sql.append( "														 WHERE Z.EMPNO = A.EMPNO) \n");
					sql.append( " LEFT JOIN PAYROLL.T_HIASSUR H ON H.EMPNO = A.EMPNO \n");	//--혈액형
