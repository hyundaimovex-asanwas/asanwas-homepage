...
public class h080006_t1 extends HttpServlet{
			...
			try {
				...
				for (int j = 0; j < rows.length; j++){
					...
					if(rows[j].getJobType() == GauceDataRow.TB_JOB_INSERT) {
						...
						sql.append( " SELECT CAST(MAX(A.CERTINO) AS INTEGER)+1 CNT ");
						sql.append( " FROM PAYROLL.T_HICERTI A, PAYROLL.T_HIPERSON B ");
            sql.append( " WHERE A.EMPNO=B.EMPNO AND A.CERKIND  = '" + strCerkind + "' ");
            sql.append( " AND SUBSTR(A.CERDATE,1,4) = SUBSTR('" + str1 + "',1,4) ");
						...
						InsertSql.append( " INSERT INTO PAYROLL.T_HICERTI " );
						InsertSql.append( " ( " );
					...
					if(rows[j].getJobType() == GauceDataRow.TB_JOB_DELETE ) {
						...
						DeleteSql.append( " DELETE FROM PAYROLL.T_HICERTI ");
						DeleteSql.append( "  WHERE EMPNO = '" + strEmpno + "' ");
						DeleteSql.append( "    AND CERTINO = '" + strCertino + "' ");
						DeleteSql.append( "    AND CERKIND = '" + strCerkind + "' ");
