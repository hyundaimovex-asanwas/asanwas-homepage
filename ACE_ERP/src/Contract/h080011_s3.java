...
public class h080011_s3 extends HttpServlet {
...
**********************************************************************************************/
				...
				if (!GauceReq.isBuilderRequest()) {					
					...
					sql.append( "SELECT  ");
					sql.append( "	RTRIM(A.EMPNO) AS EMPNO ,RTRIM(A.ORDDT) AS ORDDT,A.SEQ,RTRIM(A.ORDCD) AS ORDCD,RTRIM(A.DEPTCD) AS DEPTCD,   ");
					...
					sql.append( "	RTRIM(E.MINORNM) AS DEPTPRT,RTRIM(B.JOBDTY) AS JOBDTY,RTRIM(G.DEPTNM) AS PREDEPT,RTRIM(B.GRDDIV) AS GRDDIV ");
					...
					if (str2.equals("")) {
						sql.append( " AND A.ORDCD LIKE '%' ");
					}else {
						sql.append( " AND A.ORDCD = '"+str2+"' ");
					}
