...
public class h100006_s1 extends HttpServlet {
...
**********************************************************************************************/
        ...
        "AMT_2",   //공조회대출
          ...
          sql.append( "  A.RESINO,  RTRIM(M.MINORNM) AS GROUPNM,  ");   //주민번호 / 구분
					//고용보험(회사부담분 ), 공조회대출 추가 2011.03.03
					sql.append( "  B.B23000, COALESCE((B.COAAMT+B.COLAMT),0) AMT_2, ");	//
					sql.append( "  B.COACNT, B.COAAMT, B.COJAMT, B.COLAMT ");	

					sql.append( " from  PAYROLL.PBPAY B "); 
					sql.append( " LEFT OUTER  JOIN PAYROLL.T_HIPERSON A ON  B.EMPNO = A.EMPNO   ");
					sql.append( " LEFT OUTER  JOIN PAYROLL.HCDEPT C ON  B.DEPTCD = C.DEPTCD AND C.CHGYM = (SELECT MAX(CHGYM) FROM PAYROLL.HCDEPT WHERE CHGYM <='"+str[1]+"')   ");
					...
					sql.append( " LEFT OUTER  JOIN PAYROLL.HCLCODE M ON  C.GROUPID = M.MINORCD  AND M.MAJORCD = '2051'  ");
...
*/				
					if (str[10].equals("1")) sql.append( " AND SUBSTR(C.TREECD,1,2)= '" + str[11] +"'  \n");
					if (str[10].equals("2")) sql.append( " AND SUBSTR(C.TREECD,1,2)= '" + str[11] +"'  \n");
					if (str[10].equals("3")) sql.append( " AND SUBSTR(C.TREECD,1,4)= '" + str[11] + "' \n");
					if (str[10].equals("4")) sql.append( " AND SUBSTR(C.TREECD,1,6)= '" + str[11] +"'  \n");
					if (str[10].equals("5")) sql.append( " AND RTRIM(C.TREECD)= '" + str[11] + "'      \n");
