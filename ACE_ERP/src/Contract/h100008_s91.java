...
public class h100008_s91 extends HttpServlet {
...
**********************************************************************************************/
				...
				"TIMAMT",	 /* 시급*/    
        "SANGRAT",
        "B23000","COACNT","COAAMT","COJAMT","COLAMT"
          ...
          9,3,9,9,9   
				...
				if (!GauceReq.isBuilderRequest()) {					
					...
					sql.append("\n").append( "  RTRIM(J.MINORNM) AS JOBKINDNM, " );	/*  직종명*/
					...
					sql.append("\n").append( "  B.P23005, " );	/* 장애*/
					sql.append("\n").append( "  B.P23006," );	/* 경로*/	
					sql.append("\n").append( "  B.PR0020, " );	/* 기타지급1*/
					sql.append("\n").append( "  B.PR0030, " );	/* 기타지급2*/
					sql.append("\n").append( "  B.PR0040, " );	/* 기타지급3*/
					sql.append("\n").append( "  B.BR0020, " );	/* 기타공제1*/
					sql.append("\n").append( "  B.BR0030, " );	/* 기타공제2*/
					sql.append("\n").append( "  B.BR0040, " );	/* 기타공제3*/
					//sql.append("\n").append( "  I2.SEQ,    " );	/* 출력순서*/
					// 2008.08.18 재택근무자 별도로 정렬...
          //sql.append(" CASE WHEN A.HSKSCR=2 THEN 99 ELSE I2.SEQ END SEQ,   ");	 /* 경로*/
					//sql.append(" CASE WHEN A.JTGB IN (2,3,4,5,6) THEN 99 ELSE I2.SEQ END SEQ,   ");	 /* 경로*/
          
					sql.append(" B.PRTSEQ AS SEQ, ");	 	          
					sql.append("\n").append( "  COALESCE(I.TIMAMT,0) TIMAMT," );	/* 시급*/
					sql.append("\n").append( "  B.SANGRAT, " );	/* 기타공제3*/
          sql.append("\n").append( "  B.B23000,  B.COACNT,  B.COAAMT,  B.COJAMT,  B.COLAMT " );	/* 기타공제3*/
					...
					sql.append("\n").append( " LEFT JOIN PAYROLL.HCLCODE J ON A.JOBKIND = J.MINORCD AND J.MAJORCD = '2037'" );		/*직종*/
					...
					sql.append("\n").append( " LEFT JOIN PAYROLL.T_PCPAYTBL I ON SUBSTR(I.STDYM,1,6)=(SELECT MAX(STDYM) STDYM FROM PAYROLL.T_PCPAYTBL)" );		/*호봉*/
					sql.append("\n").append( "													AND B.GRDDIV=I.PAYGRD AND B.PAYSEQ=I.PAYSEQ					");		/*호봉MAX건*/
					...
					//sql.append("\n").append( "  ORDER BY I2.SEQ, D.SEQ,   B.GRDDIV, CAST(b.payseq AS NUMERIC) DESC  , A.EMPNO" ); /*지급구분*/
					//sql.append("\n").append( "  ORDER BY SEQ, D.SEQ,   B.GRDDIV, CAST(b.payseq AS NUMERIC) DESC  , A.EMPNO" ); /*지급구분*/
					sql.append("\n").append( "  ORDER BY B.PRTSEQ, D.SEQ,   B.GRDDIV, CAST(b.payseq AS NUMERIC) DESC  , A.EMPNO" ); /*지급구분*/
