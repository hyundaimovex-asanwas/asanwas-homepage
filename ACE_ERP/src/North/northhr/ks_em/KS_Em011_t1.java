...
public class KS_Em011_t1 extends HttpServlet{
	...
	public void doPost(HttpServletRequest req,HttpServletResponse res) {
		...
		try{
			...
			try {
				...
				String[] strArrCN = new String[]{ "SEQ",  "EMPNO",	
        ...
        for (int i=0; i<strArrCN.length; i++) {
			...
			//발령순번 관리
					...
					stmt.close();
					 ...
					 rows[j].setInt(idx[0],intSeq);
					 
					 
           //발령 Insert
					 StringBuffer InsertSql = new StringBuffer();
					 InsertSql.append( " INSERT INTO NORTHHR.KST015                          						\n" )
										.append( "( SEQ,	EMPNO,	       	 															\n" )
										.append( "  ORD_DT,	 ORD_CD,       	 													\n" )
										.append( "  DEPT_CD,  VEND_CD,   	  	 												\n" )
										.append( "  C_VEND_CD, JOB_CD,	 														\n" )
...
/*
					 ...
					 logger.dbg.println(this,"SEQ,					rows[j].getInt(idx[0])::"+rows[j].getInt(idx[0]));         
					 logger.dbg.println(this,"EMPNO,				rows[j].getString(idx[1])::"+rows[j].getString(idx[1]));         	
					 logger.dbg.println(this,"ORD_DT,           rows[j].getString(idx[2])::"+rows[j].getString(idx[2]));         
					 logger.dbg.println(this,"ORD_CD,			rows[j].getString(idx[3])::"+rows[j].getString(idx[3]));         
					 logger.dbg.println(this,"DEPT_CD,			rows[j].getString(idx[4])::"+rows[j].getString(idx[4]));          
					 ...
					 logger.dbg.println(this,"JOB_CD,    		rows[j].getString(idx[7])::"+rows[j].getString(idx[7]));                   
					 logger.dbg.println(this,"I_EMPNO,    		rows[j].getString(idx[8])::"+rows[j].getString(idx[8]));     
					 logger.dbg.println(this,"U_EMPNO,      	rows[j].getString(idx[10])::"+rows[j].getString(idx[10]));         
*/
						...
						if(rows[j].getString(idx[3]).equals("2")){  //조동일경우 인사기본정보에 반영
						
						//인사MST UPDATE
            			 StringBuffer UpdateSql0 = new StringBuffer();	
						 
							UpdateSql0.append( " UPDATE NORTHHR.KST010									\n")
											.append( " SET																		\n")
											.append( " DEPT_CD=?,															\n")		//1
											.append( " VEND_CD=?,														\n")		//2
											.append( " C_VEND_CD=?,	 												\n")		//3
							...
							GauceStatement gsmt2 = conn.getGauceStatement(UpdateSql0.toString());
							gsmt2.setGauceDataRow(rows[j]);

							gsmt2.bindColumn(1, idx[4]);			//VEND_CD
							gsmt2.bindColumn(2, idx[5]); 			//DEPT_CD
							gsmt2.bindColumn(3, idx[6]);			//C_VEND_CD
							gsmt2.bindColumn(4, idx[7]);			//JOB_CD															
						...
						if(rows[j].getString(idx[3]).equals("3")){  //퇴사일경우 인사기본정보에 반영

						//인사MST UPDATE
            			 StringBuffer UpdateSql = new StringBuffer();	
						 
						 UpdateSql.append( " UPDATE NORTHHR.KST010										 \n")
											.append( " SET																		 \n")
											.append( " USESTS='3',															 \n")
											.append( " END_DT= '"+rows[j].getString(idx[2])+"'				 \n")
											.append( "WHERE EMPNO = '"+rows[j].getString(idx[1])+"'	 \n");


							GauceStatement gsmt3 = conn.getGauceStatement(UpdateSql.toString());
						...
						if(rows[j].getString(idx[3]).equals("4")){  //재입사일경우 인사기본정보에 반영
						...
						//인사MST UPDATE
						 ...
						 UpdateSql.append( " UPDATE NORTHHR.KST010										 \n")
							...
							GauceStatement gsmt4 = conn.getGauceStatement(UpdateSql.toString());
							gsmt4.setGauceDataRow(rows[j]);

							gsmt4.bindColumn(1, idx[4]);			//DEPT_CD
							gsmt4.bindColumn(2, idx[5]); 			//VEND_CD
							gsmt4.bindColumn(3, idx[6]);			//C_VEND_CD
							gsmt4.bindColumn(4, idx[7]);			//JOB_CD																		
...
/*
						...
						//인사MST UPDATE
            			 StringBuffer UpdateSql = new StringBuffer();			
						 UpdateSql.append( " UPDATE NORTHHR.TCM300	\n")
											.append( " SET                  \n")
											.append( " VEND_CD=?,           \n")
											.append( " DEPT_CD=?,           \n")
											.append( " TEAM_CD=?,           \n")
             ...
             UpdateSql.append( " B_CON_YEAR=1, \n");
						...
						if(rows[j].getString(idx[4]).equals("007")){  //퇴사일 경우 계약에 반영
							...
							//퇴직시 - 계약 UPDATE (CLOSE_YN =Y로 만듬)
							 ...
							 UpdateSql3.append( " UPDATE NORTHHR.TCM345 A	\n")
												 .append( "    SET A.CLOSE_YN='Y',\n")
												 .append( "        A.U_EMPNO=?,   \n")
												 .append( "        A.U_DATE=CURRENT TIMESTAMP \n")
												 .append( "  WHERE A.EMPNO=?				\n")
							           .append( "    AND A.SEQ= (SELECT MAX(SEQ) FROM NORTHHR.TCM345 B WHERE B.EMPNO = A.EMPNO) \n");
...
*/
          ...
          /*
					if(rows[j].getJobType() == GauceDataRow.TB_JOB_DELETE) {
						...
						DeleteSql.append( " DELETE FROM NORTHHR.TCM305		\n")
										 .append( "	 WHERE EMPNO=?							\n")
				  					 .append( "    AND SEQ=?							  \n");
