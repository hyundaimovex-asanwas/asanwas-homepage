...
public class KS_Em029_S1 extends HttpServlet {
        ...
        for(int i=0;i<5;i++){
				...
				for (int i=0; i<strArrCN.length; i++) { // set column column 
					...
					if (!GauceReq.isBuilderRequest()) {					
						...
						sql.append( "	   FROM SALES.KST160 A                                                                        \n");						
						sql.append( "	   LEFT JOIN SALES.KST010 B ON B.EMPNO = A.EMPNO                                              \n");						
						sql.append( "	   LEFT JOIN SALES.TSY010 C ON C.DETAIL =A.DEPT_CD AND C.HEAD = 'KS006' AND TRIM(C.DETAIL)<>''\n");
						sql.append( "	   LEFT JOIN SALES.TSY010 D ON D.DETAIL =A.DAY_CD AND D.HEAD = 'KS020' AND TRIM(D.DETAIL)<>''\n");
