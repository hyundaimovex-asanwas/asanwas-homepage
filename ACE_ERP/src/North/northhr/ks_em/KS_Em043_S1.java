...
public class KS_Em043_S1 extends HttpServlet {
...
**********************************************************************************************/
				...
				for (int i=0; i<strArrCN.length; i++) { // set column column 
					...
					if (!GauceReq.isBuilderRequest()) {					
						...
						sql.append( "			 FROM ATP.KST200 A											 						\n");					
						sql.append( "			   LEFT JOIN ATP.KST010 B ON A.VEND_CD = B.VEND_CD AND A.DEPT_CD = B.DEPT_CD  AND A.EMPNO = B.EMPNO    \n");  
						...
						sql.append( "			)X																 														\n");											  
						sql.append( "		LEFT JOIN ATP.KST200 C ON C.VEND_CD = X.VEND_CD 	AND C.DEPT_CD = X.DEPT_CD AND C.PAY_YM =  '"+str1+"'	 					\n");												     
						sql.append( "		LEFT JOIN ATP.TSY010 I ON RTRIM(I.DETAIL) = RTRIM(C.DEPT_CD)   AND I.HEAD='KS006'   AND RTRIM(I.DETAIL) IS NOT NULL        	\n");
						sql.append( "		LEFT JOIN ATP.TSY010 J ON RTRIM(J.DETAIL) = RTRIM(X.VEND_CD)   AND J.HEAD='KS007'   AND RTRIM(J.DETAIL) IS NOT NULL      	\n");   
						sql.append( "		LEFT JOIN ATP.TSY010 K ON RTRIM(K.DETAIL) = RTRIM(X.C_VEND_CD)   AND K.HEAD='KS008'   AND RTRIM(K.DETAIL) IS NOT NULL      \n");   
