...
**********************************************************************************************/
        ...
        "AMT_2",   //공조회대출
          ...
          sql.append( "  A.RESINO,  ");   //주민번호
					...
					sql.append( "  COALESCE((B.APLAMT+B.APLRAT),0) AMT_1, RTRIM(M.MINORNM) AS GROUPNM ,");	//상환액
					//sql.append( "  COALESCE((L.APLAMT+L.APLRAT),0) AMT_1 ");	//상환액
					//유보금액추가 2009.04.17
					sql.append( "  COALESCE(B.UBAMT,0) UBAMT, ");	//유보금액
        ...
        */
				...
				//	sql.append( " AND  J.SEQ = (SELECT IFNULL(MAX(SEQ),'') ORDDT FROM PAYROLL.HOORDER  WHERE EMPNO = B.EMPNO )  ");
					...
					if (str[10].equals("5")) sql.append( " AND RTRIM(B.TREECD)= '" + str[11] + "'");
