...
**********************************************************************************************/
          ...
          4, // 근무지코드   
				...
				if (!GauceReq.isBuilderRequest()) {					
					...
					sql.append( " AND B.DEPTCD NOT IN( SELECT DEPTCD FROM PAYROLL.HCDEPT WHERE GROUPID = '01'  AND USEYN='T' ) "); //본사가 아님
					 //2010.01.20 JYS
					sql.append( " AND B.EMPNO NOT IN ('1990005','2000001') "); //관광경협 임의 별도로 보이게 함.
					if (str[2].equals("1")) sql.append( " AND B.PAYDIV IN( '1','3','4') \n ");  //지급구분
					if (str[2].equals("5")) sql.append( " AND B.PAYDIV IN( '5')					\n ");	//지급구분
          if (str[2].equals("6")) sql.append( " AND B.PAYDIV IN( '6')					\n ");	//지급구분
          ...
          sql.append("  SELECT																																			 \n ");
					...
					//2008.12.19 JYS
          sql.append(" CASE WHEN B.JTGB =2 THEN 995                           \n ");
					sql.append(" 	 WHEN B.JTGB = 3 THEN 996                                   \n ");
					sql.append(" 	 WHEN B.JTGB = 4 THEN 997                                   \n ");
					sql.append(" 	 WHEN B.JTGB = 5 THEN 998                                   \n ");
					sql.append(" 	 WHEN B.JTGB = 6 THEN 999 END AS SEQ,                       \n ");
					sql.append(" CASE WHEN B.JTGB =2 THEN 'Z5'                           \n ");
					sql.append(" 	 WHEN B.JTGB = 3 THEN 'Z6'                                   \n ");
          sql.append(" 	 WHEN B.JTGB = 4 THEN 'Z7'                                   \n ");
					sql.append(" 	 WHEN B.JTGB = 5 THEN 'Z8'                                   \n ");
					sql.append(" 	 WHEN B.JTGB = 6 THEN 'Z9' END AS MINORCD,                   \n ");
          ...
          sql.append("  SELECT                                                                                             \n ");
					...
					sql.append("     FROM PAYROLL.PBPAY B                                                                            \n ");
					sql.append("     LEFT OUTER JOIN PAYROLL.HCDEPT C ON  B.DEPTCD = C.DEPTCD                                        \n ");
					sql.append("     LEFT JOIN PAYROLL.HCLCODE D ON D.MINORCD = '30' AND D.MAJORCD = '2051'                          \n ");
					sql.append("     LEFT JOIN PAYROLL.HCLCODE E ON E.MINORCD = '31' AND E.MAJORCD = '2051'                          \n ");
					sql.append("     WHERE B.APPDT='"+str[1]+"'     	                                                               \n ");
					sql.append("     AND (B.JTGB<>2 AND B.JTGB<>3 AND B.JTGB<>4 AND B.JTGB<>5 AND B.JTGB<>6)                         \n ");
          ...
          sql.append(" SELECT                                                                                       \n ");
					...
					sql.append("    LEFT OUTER JOIN PAYROLL.HCDEPT C ON  B.DEPTCD = C.DEPTCD                                  \n ");
					sql.append("    --LEFT JOIN PAYROLL.HCLCODE D ON C.GROUPID = D.MINORCD AND D.MAJORCD = '2051'             \n ");
					sql.append("    --LEFT JOIN PAYROLL.HCLCODE E ON C.GROUPID = D.MINORCD AND D.MAJORCD = '2051'             \n ");
					sql.append("    LEFT JOIN PAYROLL.HCLCODE D ON D.MINORCD = '30' AND D.MAJORCD = '2051'                    \n ");
					sql.append("    LEFT JOIN PAYROLL.HCLCODE E ON E.MINORCD = '31' AND E.MAJORCD = '2051'                    \n ");
					sql.append("    WHERE B.APPDT='"  + str[0] + "'	                                                          \n ");
					sql.append("    AND (B.JTGB<>2 AND B.JTGB<>3 AND B.JTGB<>4 AND B.JTGB<>5 AND B.JTGB<>6)                   \n ");
	        ...
	        if (str[2].equals("6")) sql.append( " AND B.PAYDIV IN( '6') ");					//지급구분
					...
					sql.append("    AND B.EMPNO IN ('1990005','2000001')                                                      \n ");
					sql.append("    GROUP BY B.EMPNO, D.SEQ, D.MINORCD, D.MINORNM,E.SEQ, E.MINORCD, E.MINORNM  \n ");
