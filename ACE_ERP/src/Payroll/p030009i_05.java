...
public class p030009i_05 extends SuperServlet {
	...
	public void makeSql() {
		...
		// 카드/현금 테이블  정보 조회
		arr_sql[0].append("{CALL PAYROLL.PR_P030009I_05s (?,?,?) }");

		// 저장/수정
		arr_sql[1].append("{CALL PAYROLL.PR_P030009I_05i (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) }");
			...
			while(it.hasNext()){
				...
				if(ds1!=null){
					if ("DS1".equals(gauceName)) {
						...
						int colNm_CARD11  		= ds1.indexOfColumn("CARD11");	//국세청카드  
						int colNm_CARD12  		= ds1.indexOfColumn("CARD12");	//국세청지로  
						int colNm_CARD13  		= ds1.indexOfColumn("CARD13");	//국세청현금  
						int colNm_CARD14  		= ds1.indexOfColumn("CARD14");	//국세청의료비이중공제배제 
						int colNm_CARD21  		= ds1.indexOfColumn("CARD21");	//카드외    
						int colNm_CARD22  		= ds1.indexOfColumn("CARD22");	//지로외    
						int colNm_CARD24  		= ds1.indexOfColumn("CARD24");	//의료비이중공제배제 외
						int colNm_CARD25  		= ds1.indexOfColumn("CARD25");	//사업관련비용						
						...
						for(int i=0; i<rows.length; i++){
							...
							cstmt.setInt   (11,rows[i].getInt   (colNm_CARD11) );//국세청카드  
							cstmt.setInt   (12,rows[i].getInt   (colNm_CARD12) );//국세청지로  
							cstmt.setInt   (13,rows[i].getInt   (colNm_CARD13) );//국세청현금  
							cstmt.setInt   (14,rows[i].getInt   (colNm_CARD14) );//국세청의료비이중공제배제  
							cstmt.setInt   (15,rows[i].getInt   (colNm_CARD21) );//카드외      
							cstmt.setInt   (16,rows[i].getInt   (colNm_CARD22) );//지로외   
							cstmt.setInt   (17,rows[i].getInt   (colNm_CARD24) );//의료비이중공제배제외
							cstmt.setInt   (18,rows[i].getInt   (colNm_CARD25) );//사업관련비용
