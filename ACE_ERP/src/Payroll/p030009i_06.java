...
public class p030009i_06 extends SuperServlet {
	...
	public void makeSql() {
		...
		// 의료비 테이블  정보 조회
		arr_sql[0].append("{CALL PAYROLL.PR_P030009I_06s (?,?) }");

		// 저장/수정
		arr_sql[1].append("{CALL PAYROLL.PR_P030009I_06i (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) }");
	...
	public void select(GauceDBConnection conn, GauceRequest req,
		...
		//String eGIVCOD	        = HDUtil.nullCheckStr(req.getParameter("eGIVCOD"));	  // 기부금코드
				...
				if(ds1!=null){
					if ("DS1".equals(gauceName)) {
						...
						GauceDataColumn[] 	cols = ds1.getDataColumns();
   						...
   						int colNm_GIVMONTH		= ds1.indexOfColumn("GIVMONTH");//기부월
   						...
   						int colNm_GIVREF	  	= ds1.indexOfColumn("GIVREF");	//비고 
							int colNm_GIVNO	    	= ds1.indexOfColumn("GIVNO");	//기부금영수증일련번호
						...
						for(int i=0; i<rows.length; i++){
							...
							cstmt.setString( 3,rows[i].getString(colNm_GIVYEAR) + rows[i].getString(colNm_GIVMONTH));//기부년월      
							cstmt.setString( 4,rows[i].getString(colNm_GIVCOD));//코드
							//StringUtil.printMsg("setp1", "" ,this);
							if ("10".equals(rows[i].getString(colNm_GIVCOD))){
								givtype = "법정기부금";
							}else if ("20".equals(rows[i].getString(colNm_GIVCOD))) {
								givtype = "정치자금";
							}else if ("21".equals(rows[i].getString(colNm_GIVCOD))) {
								givtype = "진흥기금";
								...
								givtype = "종교단체";	
							}else if ("50".equals(rows[i].getString(colNm_GIVCOD))) {
								givtype = "기타의 기부금";											
							};
							//StringUtil.printMsg("setp2", "" ,this);
							...
							cstmt.setString(16,rows[i].getString(colNm_GIVREF));//비고         
							cstmt.setString(17,rows[i].getString(colNm_GIVNO));//기부금영수증일련번호  
							cstmt.setInt   (18,rows[i].getInt   (colNm_GIVAMT2));//금액-그밖의   세액공제
							...
							//StringUtil.printMsg("setp99", "" ,this);
	
							cstmt.executeUpdate();

							//StringUtil.printMsg("setp100", "" ,this);
