...
public class p030010i_04 extends SuperServlet {
	...
	public void makeSql() {
		...
		// 교육비공제 테이블  정보 조회
		arr_sql[0].append("{CALL PAYROLL.PR_P030010I_04s (?,?,?) }");

		// 저장/수정
		arr_sql[1].append("{CALL PAYROLL.PR_P030010I_04i (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) }");
	...
	public void select(GauceDBConnection conn, GauceRequest req,
		...
		String eJuno	        = HDUtil.nullCheckStr(req.getParameter("eJuno"));	  // 공제자주민번호
			...
			while(it.hasNext()){
				...
				if(ds1!=null){
					if ("DS1".equals(gauceName)) {
						...
						int colNm_FAMRELA_CD	= ds1.indexOfColumn("FAMRELA_CD");		//공제대상 관계
						...
						int colNm_EDU11				= ds1.indexOfColumn("EDU11");				//국세청초중고교육비
						int colNm_EDU12				= ds1.indexOfColumn("EDU12");				//국세청대학생교육비
						int colNm_EDU13				= ds1.indexOfColumn("EDU13");				//국세청장애인교육비
						int colNm_EDU20				= ds1.indexOfColumn("EDU20");				//유치원비외
						int colNm_EDU21				= ds1.indexOfColumn("EDU21");				//초중고교육외    
						int colNm_EDU22				= ds1.indexOfColumn("EDU22");				//대학생교육외    
						int colNm_EDU23				= ds1.indexOfColumn("EDU23");				//장애인교육외    
						...
						for(int i=0; i<rows.length; i++){
							...
							cstmt.setInt   (11,rows[i].getInt   (colNm_EDU10) );				//국세청취학전교육비
							cstmt.setInt   (12,rows[i].getInt   (colNm_EDU11) );				//국세청초중고교육비
							cstmt.setInt   (13,rows[i].getInt   (colNm_EDU12) );				//국세청대학생교육비
							cstmt.setInt   (14,rows[i].getInt   (colNm_EDU13) );				//국세청장애인교육비
							cstmt.setInt   (15,rows[i].getInt   (colNm_EDU20) );				//취학전교육비외    
							cstmt.setInt   (16,rows[i].getInt   (colNm_EDU21) );				//초중고교육비외    
							cstmt.setInt   (17,rows[i].getInt   (colNm_EDU22) );				//대학생교육비외    
							cstmt.setInt   (18,rows[i].getInt   (colNm_EDU23) );				//장애인교육비외    
