...
public class p050002_s4 extends HttpServlet {
...
**********************************************************************************************/
          ...
          9,  9, 100, 3, 4,
				...
				if (!GauceReq.isBuilderRequest()) {					
					...
					sql.append( " SELECT '"+str[0]+"' APPYM, Y.GROUPID,X.MEDSNO, X.EMPNO,                                          \n ");
					...
					sql.append( "        MIN(X.REMARK)REMARK,IFNULL(RTRIM(D.SEQ),'') AS SEQ2, MAX(X.PAYSEQ)PAYSEQ,MAX(RTRIM(I.MINORNM)) GROUPNM   \n ");
					sql.append( "   FROM                                                                                         \n ");
					sql.append( " (                                                                                              \n ");
					sql.append( " /*이전월*/                                                                                     \n ");
          ...
          sql.append( "   			  CASE WHEN (C.EMPNMK=''OR C.EMPNMK IS NULL) THEN D.DEPTCD ELSE C.DEPTCD END DEPTCD,  \n ");
					sql.append( "   			  CASE WHEN (C.EMPNMK=''OR C.EMPNMK IS NULL) THEN D.PAYGRD ELSE C.PAYGRD END PAYGRD,  \n ");
					sql.append( "   			  CASE WHEN (C.EMPNMK=''OR C.EMPNMK IS NULL) THEN D.PAYSEQ ELSE C.PAYSEQ END PAYSEQ,  \n ");
          sql.append( "   			  0 AS PRE_STDAMT, 0 AS PRE_INSAMT, A.STDAMT,  A.INSAMT,                              \n ");
					sql.append( "   			  A.REMARK                                                                           \n ");
					sql.append( "   	 FROM PAYROLL.PWINDIPN A                                                                  \n ");
          sql.append( "   	 LEFT JOIN PAYROLL.HIPERSON C ON A.EMPNO = C.EMPNO                                        \n ");
					sql.append( "   	 LEFT JOIN PAYROLL.T_HIPERSON D ON A.EMPNO = D.EMPNO                                      \n ");
          ...
          sql.append( "   \n ");
					...
					sql.append( "   HAVING  (IFNULL(SUM(X.PRE_STDAMT),0)<>0 OR         \n ");
					sql.append( "            IFNULL(SUM(X.PRE_INSAMT),0)<>0 OR         \n ");
					sql.append( "            IFNULL(SUM(X.STDAMT),0)<>0 OR             \n ");
					sql.append( "            IFNULL(SUM(X.INSAMT),0)<>0)               \n ");

					sql.append( " ORDER BY I.SEQ,INTEGER(D.SEQ), MAX(X.PAYSEQ) DESC, X.EMPNO                                     \n ");
