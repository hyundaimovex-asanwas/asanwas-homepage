...
public class p050018_pop_s1 extends HttpServlet {
...
**********************************************************************************************/
				...
				if (!GauceReq.isBuilderRequest()) {					
					StringBuffer sql = new StringBuffer();
						...
						sql.append( "       RTRIM(B.MINORNM) AS PAYGNM,																															\n");
						sql.append( "       CASE WHEN A.USESTS='3' AND A.ENDDT<'20080107' THEN RTRIM(D.DEPTNM) ELSE RTRIM(C.DEPTNM) END  DEPTNM			\n");
						sql.append( "	FROM PAYROLL.HIPERSON A,																																	\n");
						sql.append( "       PAYROLL.HCLCODE B																																			\n");
						sql.append( "	LEFT JOIN PAYROLL.HCDEPT C ON A.DEPTCD=C.DEPTCD																								\n");
						sql.append( "	LEFT JOIN PAYROLL.HCDEPT_99 D ON A.DEPTCD=D.DEPTCD																							\n");
						sql.append( " WHERE A.PAYGRD = B.MINORCD																																\n");
						sql.append( "		AND B.MAJORCD = '2002'																																	\n");
						sql.append( "		AND A.USESTS IN ( '1', '2')																																	\n");
if(!str1.equals(""))sql.append( "		AND A.EMPNMK LIKE '" + str1 + "%'																														\n");
						sql.append( " UNION																																									\n");
						sql.append( " SELECT																																								\n");
						sql.append( "		RTRIM(A.EMPNO) AS EMPNO,RTRIM(A.EMPNMK) AS EMPNMK,																					\n");
						sql.append( "       RTRIM(B.MINORNM) AS PAYGNM,																															\n");
