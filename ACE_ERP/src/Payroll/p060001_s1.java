...
public class p060001_s1 extends HttpServlet {
...
**********************************************************************************************/
				...
				for (int i=0; i<strArrCN.length; i++) { // set column column 
					...
					if (!GauceReq.isBuilderRequest()) {					
						...
						sql.append( " SELECT " );	//U
						sql.append( " RTRIM(PAYDIV) PAYDIV, RTRIM(PBDIV) PBDIV, RTRIM(PAYCD) PAYCD, ");
						sql.append( " RTRIM(PAYCDNM) PAYCDNM, RTRIM(CALDIV) CALDIV, " );
						sql.append( " RATEAMT, RTRIM(PGMID) PGMID, RTRIM(AUTOYN) AUTOYN, TAX, RTRIM(FRDT) FRDT, " );
						sql.append( " RTRIM(TODT) TODT, RTRIM(MEMO)	MEMO " );
						sql.append( " from PAYROLL.pcpaycd ");
						sql.append( " where PAYDIV <> ' ' " );
						
						if (!str1.equals("") && !str1.equals("0") )
							sql.append( " AND PAYDIV = '" + str1 + "' " );
