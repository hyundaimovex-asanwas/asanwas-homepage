...
public class h090005_t1 extends HttpServlet{
	...
	public void doPost(HttpServletRequest req,HttpServletResponse res) {
		...
		/*		int idxDEPTCD	= userSet.indexOfColumn("DEPTCD");
		  ...
		  */
				...
				UpdateSql.append( " AGRDT = '"+str2+"', AGRID = '"+str1+"'  " );
				UpdateSql.append( " WHERE APPDT = ? AND EMPNO = ? AND LAZCD = ? " );
				}else if (str3.equals("A")) {
				UpdateSql.append( " UPDATE PAYROLL.T_HLDUTY SET " );
				UpdateSql.append( " AGRDT = '99999999', AGRID = '"+str1+"'  " );
				UpdateSql.append( " WHERE APPDT = ? AND EMPNO = ? AND LAZCD = ? " );
				} else { 
				UpdateSql.append(" SELECT * FROM PAYROLL.T_HLDUTY"); }
				
				DeleteSql = new StringBuffer();
				DeleteSql.append( "DELETE FROM PAYROLL.T_HLDUTY WHERE EMPNO = '"+str1+"' " );
