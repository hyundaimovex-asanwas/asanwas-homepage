	...
	public void doPost(HttpServletRequest req,HttpServletResponse res) {
		...
		try{
			...
			try {
				...
				int idxCHGYM   = userSet.indexOfColumn("CHGYM");
				int idxORG_CHGYM   = userSet.indexOfColumn("ORG_CHGYM");
				int idxSITE   = userSet.indexOfColumn("SITE");
				...
				InsertSql.append( " TREECD, FRDT,	TODT   , USEYN  , REDTAXYN, LEVEL, DEPTGB, GROUPID, CHGYM, SITE " );
				...
				UpdateSql.append( " CHGYM = ?, " );
				UpdateSql.append( " SITE = ? " );
				UpdateSql.append( " WHERE DEPTCD = ? " );
				UpdateSql.append( "   AND CHGYM = ? " );
 
				DeleteSql = new StringBuffer();
				DeleteSql.append( "DELETE FROM PAYROLL.HCDEPT WHERE DEPTCD = ? AND CHGYM =? " );
				...
				for (int j = 0; j < rows.length; j++){
					...
					if(rows[j].getJobType() == GauceDataRow.TB_JOB_INSERT) {
  					...
  					gsmt.bindColumn(14, idxCHGYM);
  					gsmt.bindColumn(15, idxSITE);
						gsmt.executeUpdate();
						gsmt.close();
					}
					...
					if(rows[j].getJobType() == GauceDataRow.TB_JOB_UPDATE) {
						...
						gsmt.bindColumn(13, idxCHGYM);
						gsmt.bindColumn(14, idxSITE);
						gsmt.bindColumn(15, idxDEPTCD);
						gsmt.bindColumn(16, idxORG_CHGYM);
					...
					if(rows[j].getJobType() == GauceDataRow.TB_JOB_DELETE ) {
						...
						gsmt.bindColumn(1, idxDEPTCD);
						gsmt.bindColumn(2, idxORG_CHGYM);
						gsmt.executeUpdate();
						gsmt.close();
					}
				}
