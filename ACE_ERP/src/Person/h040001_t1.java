				...
				//	EndDt = rows[j].getString(idx[1]);
					...
					if(rows[j].getJobType() == GauceDataRow.TB_JOB_INSERT) {
						...
						Ordcd= rows[j].getString(idx[3]);
							...
							sql1.append( "UPDATE PAYROLL.HOORDER SET ENDDT=REPLACE(CHAR(DATE(SUBSTRING('"+rows[j].getString(idx[1])+"',1,4)||'-'||SUBSTRING('"+rows[j].getString(idx[1])+"',5,2)||'-'||SUBSTRING('"+rows[j].getString(idx[1])+"',7,2)) -1 DAY, ISO),'-','')\n"); 
							sql1.append( "WHERE EMPNO ='"+rows[j].getString(idx[0])+"'\n"); 
							sql1.append( "	AND ORDDT=( SELECT MAX(ORDDT) \n");
							sql1.append( "              FROM PAYROLL.HOORDER \n");
						...
						//	 logger.dbg.println(this, "Ordcd	::"+rows[j].getString(idx[3]));
							...
							sql2.append( "UPDATE PAYROLL.HOORDER SET ENDDT='"+rows[j].getString(idx[1])+"'\n"); 
							...
							sql2.append( "			 FROM PAYROLL.HOORDER						 \n");
						...
						}							//MAX값 SEQ 조회 2008.08.27/////////////////////////////////////////////// 
						Statement stmt0 = conn.createStatement();
						StringBuffer sql0 = new StringBuffer();
						sql0.append( " SELECT COALESCE(MAX(SEQ),0)+1 FROM PAYROLL.HOORDER "); 
