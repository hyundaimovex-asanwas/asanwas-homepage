...
**********************************************************************************************/
				...
				if (!GauceReq.isBuilderRequest()) {					
					...
					sql.append( "        RTRIM(MAX(X.SEQ1)) AS SEQ1,    \n");
					sql.append( "        CASE WHEN MAX(H.EMPNMK)='' OR  MAX(H.EMPNMK) IS NULL THEN  MAX(L.EMPNMK) ELSE MAX(H.EMPNMK) END EMPNMK1,   \n");
					sql.append( "        MAX(X.SEQ2)SEQ2, \n");
					sql.append( "        CASE WHEN MAX(I.EMPNMK)='' OR  MAX(I.EMPNMK) IS NULL THEN  MAX(M.EMPNMK) ELSE MAX(I.EMPNMK) END EMPNMK2,   \n");
					sql.append( "        MAX(X.SEQ3)SEQ3,\n");
					sql.append( "        CASE WHEN MAX(J.EMPNMK)='' OR  MAX(J.EMPNMK) IS NULL THEN  MAX(N.EMPNMK) ELSE MAX(J.EMPNMK) END EMPNMK3,   \n");
					sql.append( "        MAX(X.SEQ4)SEQ4,\n");
					sql.append( "        CASE WHEN MAX(K.EMPNMK)='' OR  MAX(K.EMPNMK) IS NULL THEN  MAX(O.EMPNMK) ELSE MAX(K.EMPNMK) END EMPNMK4,   \n");
					sql.append( "        MAX(X.TREECD) TREECD,  \n");
          ...
          sql.append( "               WHERE (C.ENDDT='' OR SUBSTR(C.ENDDT,1,6)> '" + str1 +"')  \n");
					sql.append( "                 AND A.GRSREF='1' \n" );
					 ...
					 //2008.06.04. 정영식 추가 ( 파견직 중 전산팀을 정규직 피고과자를 만든다.
					sql.append( "              UNION ALL   \n"); 
					sql.append( "               SELECT 'F' CHK, A.EVAYM, A.EMPNO, C.EMPNMK, A.DEPTCD, D.DEPTNM ,A.PAYGRD, (E.MINORNM) PAYGRDNM,A.GRSTRTDT,A.STRTDT,\n");
          ...
          sql.append( "                   LEFT JOIN PAYROLL.HCDEPT   D ON A.DEPTCD = D.DEPTCD  \n");
          sql.append( "                   LEFT JOIN PAYROLL.HCLCODE  E ON A.PAYGRD = E.MINORCD AND E.MAJORCD='2002'  \n");
          sql.append( "      WHERE (C.ENDDT='' OR SUBSTR(C.ENDDT,1,6)> '200811')  \n");
          sql.append( "        AND  A.GRSREF='1'  \n");
              //END
					sql.append( "                    ) X    \n");
          ...
          sql.append( "           LEFT JOIN PAYROLL.T_HIPERSON L ON X.SEQ1  = L.EMPNO \n");
					sql.append( "           LEFT JOIN PAYROLL.T_HIPERSON M ON X.SEQ2  = M.EMPNO \n");
					sql.append( "           LEFT JOIN PAYROLL.T_HIPERSON N ON X.SEQ3  = N.EMPNO \n");
					sql.append( "           LEFT JOIN PAYROLL.T_HIPERSON O ON X.SEQ4  = O.EMPNO \n");
				...
				//	sql.append( "       AND  X.DEPTCD LIKE '" + str2 +"%'" );
					...
					if (str5.equals("3")) sql.append( " AND SUBSTR(X.TREECD,1,4) = '"+ str6 + "' \n");
					if (str5.equals("4")) sql.append( " AND SUBSTR(X.TREECD,1,6) = '"+ str6 + "' \n");
					if (str5.equals("5")) sql.append( " AND RTRIM(X.TREECD) = '" + str6 + "'     \n");
          ...
          sql.append( "              WHERE (C.ENDDT='' OR SUBSTR(C.ENDDT,1,6)> '" + str1 +"')  \n");
				  ...
				  //2008.05.27. 정영식 추가 ( 정규직 중 운전직을 계약직을 피고과자를 만든다.
					sql.append( "              UNION ALL   \n"); 
					sql.append( "               SELECT 'F' CHK, A.EVAYM, A.EMPNO, C.EMPNMK, A.DEPTCD, D.DEPTNM ,A.PAYGRD, (E.MINORNM) PAYGRDNM,A.GRSTRTDT,A.STRTDT,\n");
					//sql.append( "                           ''SEQ1,''SEQ2,''SEQ3,''SEQ4, D.TREECD, E.SEQ, C.PAYSEQ \n"); 
					//sql.append( "                  FROM PAYROLL.HVEMP A         \n"); 
