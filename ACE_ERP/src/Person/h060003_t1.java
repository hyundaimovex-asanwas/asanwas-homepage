...
public class h060003_t1 extends HttpServlet{
	...
	public void doPost(HttpServletRequest req,HttpServletResponse res) {
		...
		try{
			...
			try {
				...
				InsertSql.append( " INSERT INTO PAYROLL.HVHEMP( EVAYM,EMPNOH,EMPNO,EVASEQ,DEPTCD,DEPTNM,PAYGRD) " );
				InsertSql.append( "                                 values  (?,?,?,?,?,?,?)" );
  
				UpdateSql = new StringBuffer();
				UpdateSql.append( " UPDATE PAYROLL.HVHEMP SET EMPNOH=?,EVASEQ=?, DEPTCD=?,DEPTNM=?, PAYGRD=?" );
			  UpdateSql.append( "  WHERE EVAYM = ?" );
				...
				DeleteSql.append( "  DELETE FROM PAYROLL.HVHEMP " );
