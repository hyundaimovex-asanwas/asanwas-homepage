...
public class h060005_s2 extends HttpServlet {
...
**********************************************************************************************/
          ...
          if (str[9].equals("1")){ //정규직
            ...
            sql.append( "  SELECT RTRIM(A.EVAYM) EVAYM,  RTRIM(D.MINORNM) GRPNM, A.EMPNO, RTRIM(E.EMPNMK) EMPNMK,                          \n");
						...
						//2009.11.24 JYS 고과 반영비율 예외자 처리 
						//2010.06.10 현업담당자가 1차, 2차 고과를 임의로 조정하여 일괄적용 가능하게 되어 막음.
						/***
						sql.append( "          CASE WHEN A.EMPNO IN ( '2020013','2030016','2040025','2040028','2050046','2060001','2070002','2080009','2080019','2080028') THEN \n");
						sql.append( "							  CASE WHEN (RTRIM(F.EMPNOH)<>'' AND RTRIM(G.EMPNOH)<>'' AND RTRIM(H.EMPNOH)<>'') THEN (A.SCR1*0.4+A.SCR2*0.3+A.SCR3*0.3)                                                \n");
						sql.append( "								     WHEN (RTRIM(F.EMPNOH)<>'' AND RTRIM(G.EMPNOH)<>'' AND(RTRIM(H.EMPNOH)IS NULL OR RTRIM(H.EMPNOH)=''))  THEN (A.SCR1*0.4+A.SCR2*0.6+A.SCR3*0)                       \n");
						sql.append( "								     WHEN (RTRIM(F.EMPNOH)<>'' AND (RTRIM(G.EMPNOH)IS NULL OR RTRIM(G.EMPNOH)='')AND(RTRIM(H.EMPNOH)IS NULL OR RTRIM(H.EMPNOH)=''))  THEN (A.SCR1*1+A.SCR2*0+A.SCR3*0) \n");
						sql.append( "								ELSE 0  END  \n");
						sql.append( "          ELSE \n");
						sql.append( "							  CASE WHEN (RTRIM(F.EMPNOH)<>'' AND RTRIM(G.EMPNOH)<>'' AND RTRIM(H.EMPNOH)<>'') THEN (A.SCR1*0.4+A.SCR2*0.3+A.SCR3*0.3)                                                \n");
						sql.append( "							       WHEN (RTRIM(F.EMPNOH)<>'' AND RTRIM(G.EMPNOH)<>'' AND(RTRIM(H.EMPNOH)IS NULL OR RTRIM(H.EMPNOH)=''))  THEN (A.SCR1*0.6+A.SCR2*0.4+A.SCR3*0)                       \n");
						sql.append( "							       WHEN (RTRIM(F.EMPNOH)<>'' AND (RTRIM(G.EMPNOH)IS NULL OR RTRIM(G.EMPNOH)='')AND(RTRIM(H.EMPNOH)IS NULL OR RTRIM(H.EMPNOH)=''))  THEN (A.SCR1*1+A.SCR2*0+A.SCR3*0) \n");
						sql.append( "							   ELSE 0 END   \n");                                                                                                                                     
						sql.append( "          END AS SCRAVG , \n");
						sql.append( "          CASE WHEN A.EMPNO IN ( '2020013','2030016','2040025','2040028','2050046','2060001','2070002','2080009','2080019','2080028') THEN \n");
						sql.append( "								CASE WHEN RTRIM(F.EMPNOH)<>'' AND RTRIM(G.EMPNOH)<>'' AND RTRIM(H.EMPNOH)<>'' THEN (A.AVG1*0.4+A.AVG2*0.3+A.AVG3*0.3)                                                \n");
						sql.append( "          					 WHEN (RTRIM(F.EMPNOH)<>'' AND RTRIM(G.EMPNOH)<>'' AND(RTRIM(H.EMPNOH)IS NULL OR RTRIM(H.EMPNOH)='')) THEN  (A.AVG1*0.4+A.AVG2*0.6+A.AVG3*0)                     \n");
						sql.append( "          					 WHEN (RTRIM(F.EMPNOH)<>'' AND (RTRIM(G.EMPNOH)IS NULL OR RTRIM(G.EMPNOH)='')AND(RTRIM(H.EMPNOH)IS NULL OR RTRIM(H.EMPNOH)='')) THEN (A.AVG1*1+A.AVG2*0+A.AVG3*0)\n");
						sql.append( "								ELSE 0 END                                                                                                                                                           \n");
						sql.append( "          ELSE \n");
						sql.append( "								CASE WHEN RTRIM(F.EMPNOH)<>'' AND RTRIM(G.EMPNOH)<>'' AND RTRIM(H.EMPNOH)<>'' THEN (A.AVG1*0.4+A.AVG2*0.3+A.AVG3*0.3)                                                  \n");
						sql.append( "										 WHEN (RTRIM(F.EMPNOH)<>'' AND RTRIM(G.EMPNOH)<>'' AND(RTRIM(H.EMPNOH)IS NULL OR RTRIM(H.EMPNOH)='')) THEN  (A.AVG1*0.6+A.AVG2*0.4+A.AVG3*0)                       \n");
						sql.append( "										 WHEN (RTRIM(F.EMPNOH)<>'' AND (RTRIM(G.EMPNOH)IS NULL OR RTRIM(G.EMPNOH)='')AND(RTRIM(H.EMPNOH)IS NULL OR RTRIM(H.EMPNOH)='')) THEN (A.AVG1*1+A.AVG2*0+A.AVG3*0)  \n");
						sql.append( "               ELSE 0 END  \n");                                                                                                                                      
						sql.append( "          END AS CORAVG, \n");
					  ***/
						...
						sql.append( "          COALESCE(A.TOTAVG,0) TOTAVG, E.PAYSEQ PAYSEQ, A.STRTDT STRTDT,COALESCE(Y.CORAVG_SUM,0)CORAVG_SUM, COALESCE(Y.TOTAVG_SUM,0)TOTAVG_SUM,  \n");
						sql.append( "          COALESCE( Y.GOGAGB_SUM,0) GOGAGB_SUM, A.GRADE,  A.GRADE2 , A.GRADE3,  COALESCE(P.DPTAVG,0) DPTAVG \n");
            ...
            sql.append( "   LEFT JOIN PAYROLL.HCDEPT_99 Z ON A.DEPTCD = Z.DEPTCD                                                                                                                             \n");
						...
						sql.append( "   							FROM (                                                                             \n");
						sql.append( "   										 SELECT A.EMPNO,COUNT(A.EMPNO)CNT, SUM(COALESCE(A.TOTAVG,0)) AS TOTAVG,  SUM(COALESCE(A.TOTAVG,0)) AS CORAVG     \n");
						...
						sql.append( "   LEFT JOIN ( SELECT A.EVAYM, A.EMPNO,                                                                    \n");
						sql.append( "                      ROUND(COALESCE((A.AVG1+A.AVG2+A.AVG3+A.AVG4+A.AVG5                                   \n");
						sql.append( "                                     +A.AVG6+A.AVG7+A.AVG8+A.AVG9+A.AVG10                                  \n");
						sql.append( "                                     +A.AVG11+A.AVG12+A.AVG13+A.AVG14+A.AVG15)/V.CNT,0),3) AS DPTAVG       \n");
						sql.append( "                 FROM PAYROLL.HVEMP_DPT A                                                                  \n");
						sql.append( "                 LEFT JOIN ( SELECT EVAYM, EMPNO, COUNT(EMPNOH) CNT                                        \n");
						sql.append( "                               FROM PAYROLL.HVHEMP_DPT                                                     \n");
						sql.append( "                              WHERE EVAYM='" + str[0] +"'	                                                \n");
						sql.append( "                              GROUP BY EVAYM, EMPNO                                                        \n");
						sql.append( "                            )V ON A.EVAYM = V.EVAYM AND A.EMPNO = V.EMPNO                                  \n");
						sql.append( "              )P ON P.EVAYM = A.EVAYM AND P.EMPNO = A.EMPNO                                                \n");
            ...
            sql.append( " SELECT RTRIM(A.EVAYM) EVAYM,  RTRIM(D.MINORNM) GRPNM, A.EMPNO, RTRIM(E.EMPNMK) EMPNMK,                                                                                                                     \n");
						...
						//2010.06.10 현업담당자가 1차, 2차 고과를 임의로 조정하여 일괄적용 가능하게 되어 막음.
						/***
						sql.append( "          CASE WHEN A.EMPNO IN ( '3040038','3050022','3080086','3080166','3090004') THEN \n");
						sql.append( "							  CASE WHEN (RTRIM(F.EMPNOH)<>'' AND RTRIM(G.EMPNOH)<>'' AND RTRIM(H.EMPNOH)<>'') THEN (IFNULL(A.SCR1,0)*0.4+IFNULL(A.SCR2,0)*0.3+IFNULL(A.SCR3,0)*0.3)                                                \n");
						sql.append( "								     WHEN (RTRIM(F.EMPNOH)<>'' AND RTRIM(G.EMPNOH)<>'' AND(RTRIM(H.EMPNOH)IS NULL OR RTRIM(H.EMPNOH)=''))  THEN (IFNULL(A.SCR1,0)*0.4+IFNULL(A.SCR2,0)*0.6+IFNULL(A.SCR3,0)*0)                       \n");
						sql.append( "								     WHEN (RTRIM(F.EMPNOH)<>'' AND (RTRIM(G.EMPNOH)IS NULL OR RTRIM(G.EMPNOH)='')AND(RTRIM(H.EMPNOH)IS NULL OR RTRIM(H.EMPNOH)=''))  THEN (IFNULL(A.SCR1,0)*1+IFNULL(A.SCR2,0)*0+IFNULL(A.SCR3,0)*0) \n");
						sql.append( "								ELSE 0 END   \n");                                                                                                                                                                      
						sql.append( "          ELSE \n");
						sql.append( "							  CASE WHEN (RTRIM(F.EMPNOH)<>'' AND RTRIM(G.EMPNOH)<>'' AND RTRIM(H.EMPNOH)<>'') THEN (IFNULL(A.SCR1,0)*0.4+IFNULL(A.SCR2,0)*0.3+IFNULL(A.SCR3,0)*0.3)                                               \n");
						sql.append( "							       WHEN (RTRIM(F.EMPNOH)<>'' AND RTRIM(G.EMPNOH)<>'' AND(RTRIM(H.EMPNOH)IS NULL OR RTRIM(H.EMPNOH)=''))  THEN (IFNULL(A.SCR1,0)*0.6+IFNULL(A.SCR2,0)*0.4+IFNULL(A.SCR3,0)*0)                      \n");
						sql.append( "							       WHEN (RTRIM(F.EMPNOH)<>'' AND (RTRIM(G.EMPNOH)IS NULL OR RTRIM(G.EMPNOH)='')AND(RTRIM(H.EMPNOH)IS NULL OR RTRIM(H.EMPNOH)=''))  THEN (IFNULL(A.SCR1,0)*1+IFNULL(A.SCR2,0)*0+IFNULL(A.SCR3,0)*0)\n");
						sql.append( "							  ELSE 0 END   \n");                                                                                                                                                                      
						sql.append( "          END AS SCRAVG , \n");
						sql.append( "          CASE WHEN A.EMPNO IN ( '3040038','3050022','3080086','3080166','3090004') THEN \n");
						sql.append( "								CASE WHEN RTRIM(F.EMPNOH)<>''  AND RTRIM(G.EMPNOH)<>'' AND RTRIM(H.EMPNOH)<>'' THEN (IFNULL(A.AVG1,0)*0.4+IFNULL(A.AVG2,0)*0.3+IFNULL(A.AVG3,0)*0.3)                                                \n");
						sql.append( "          			     WHEN (RTRIM(F.EMPNOH)<>'' AND RTRIM(G.EMPNOH)<>'' AND(RTRIM(H.EMPNOH)IS NULL OR RTRIM(H.EMPNOH)='')) THEN  (IFNULL(A.AVG1,0)*0.4+IFNULL(A.AVG2,0)*0.6+IFNULL(A.AVG3,0)*0)                      \n");
						sql.append( "          			     WHEN (RTRIM(F.EMPNOH)<>'' AND (RTRIM(G.EMPNOH)IS NULL OR RTRIM(G.EMPNOH)='')AND(RTRIM(H.EMPNOH)IS NULL OR RTRIM(H.EMPNOH)='')) THEN (IFNULL(A.AVG1,0)*1+IFNULL(A.AVG2,0)*0+IFNULL(A.AVG3,0)*0) \n");
						sql.append( "								 ELSE 0 END   \n");                                                                                                                                                                     
						sql.append( "          ELSE \n");
						sql.append( "								CASE WHEN RTRIM(F.EMPNOH)<>''  AND RTRIM(G.EMPNOH)<>'' AND RTRIM(H.EMPNOH)<>'' THEN (IFNULL(A.AVG1,0)*0.4+IFNULL(A.AVG2,0)*0.3+IFNULL(A.AVG3,0)*0.3)                                                 \n");
						sql.append( "								     WHEN (RTRIM(F.EMPNOH)<>'' AND RTRIM(G.EMPNOH)<>'' AND(RTRIM(H.EMPNOH)IS NULL OR RTRIM(H.EMPNOH)='')) THEN  (IFNULL(A.AVG1,0)*0.6+IFNULL(A.AVG2,0)*0.4+IFNULL(A.AVG3,0)*0)                       \n");
						sql.append( "								     WHEN (RTRIM(F.EMPNOH)<>'' AND (RTRIM(G.EMPNOH)IS NULL OR RTRIM(G.EMPNOH)='')AND(RTRIM(H.EMPNOH)IS NULL OR RTRIM(H.EMPNOH)='')) THEN (IFNULL(A.AVG1,0)*1+IFNULL(A.AVG2,0)*0+IFNULL(A.AVG3,0)*0)  \n");
						sql.append( "                ELSE 0 END   \n");                                                                                                                                                                     
						sql.append( "          END AS CORAVG, \n");
						***/
            ...
            sql.append( "         ELSE 0 END AS SCRAVG,    \n");
						...
						sql.append( "         COALESCE(A.TOTAVG,0) TOTAVG, E.PAYSEQ PAYSEQ, A.STRTDT STRTDT,COALESCE(Y.CORAVG_SUM,0)CORAVG_SUM, COALESCE(Y.TOTAVG_SUM,0)TOTAVG_SUM,                        \n");
						sql.append( "         COALESCE( Y.GOGAGB_SUM,0) GOGAGB_SUM ,A.GRADE,  A.GRADE2 , A.GRADE3,  0 DPTAVG  \n");
						sql.append( "  FROM PAYROLL.HVEMP A ,PAYROLL.cIPERSON E                                                                                                                                                                  \n");
						sql.append( "  LEFT JOIN PAYROLL.HCDEPT   B ON A.DEPTCD = B.DEPTCD                                                                                                                                                        \n");
						sql.append( "  LEFT JOIN PAYROLL.HCLCODE C ON A.PAYGRD = C.MINORCD AND C.MAJORCD='2002'                                                                                                                                   \n");
						sql.append( "  LEFT JOIN PAYROLL.HCLCODE D ON C.MEMO      =   D.MEMO      AND D.MAJORCD='2014'                                                                                                                            \n");
