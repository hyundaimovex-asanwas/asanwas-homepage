...
public class h060005_s3 extends HttpServlet {
...
**********************************************************************************************/
        ...
        for(int i=0;i<=1;i++){
				...
				String[] strArrCN = new String[]{ "PAYGRD2","S","A","B","C","D" }; 
				int[] intArrCN = new int[]{ 4, 3, 3, 3, 3, 3	}; 	
				...
				if (!GauceReq.isBuilderRequest()) {					
					...
					sql.append( " SELECT Y.PAYGRD2,                                                                                            \n");
					sql.append( "        CAST(ROUND(Y.CNT*5.00 /100.00,0)AS INTEGER )AS S,                                                     \n");
					sql.append( "        CAST(ROUND(Y.CNT*15.00/100.00,0)AS INTEGER )AS A,                                                     \n");
					sql.append( "        Y.CNT - ( CAST(ROUND(Y.CNT*5.00 /100.00,0)AS INTEGER )+CAST(ROUND(Y.CNT*15.00/100.00,0)AS INTEGER )+  \n");
					sql.append( "                  CAST(ROUND(Y.CNT*7.00 /100.00,0)AS INTEGER )+CAST(ROUND(Y.CNT*3.00 /100.00,0)AS INTEGER )   \n");
					sql.append( "                )AS B,                                                                                        \n");
					sql.append( "        CAST(ROUND(Y.CNT*7.00 /100.00,0)AS INTEGER )AS C,                                                     \n");
					sql.append( "        CAST(ROUND(Y.CNT*3.00 /100.00,0)AS INTEGER )AS D                                                      \n");
					sql.append( " FROM (                                                                                                       \n");
					sql.append( "        SELECT DISTINCT PAYROLL.FN_MAXCNT(X.EVAYM, X.GRSREF, X.PAYGRD2) CNT, X.PAYGRD2                        \n");
