...
**********************************************************************************************/
				...
				if (!GauceReq.isBuilderRequest()) {					
					...
					sql.append( "        A.DEPTCD, RTRIM(B.DEPTNM) DEPTNM, A.PAYGRD, RTRIM(C.MINORNM) PAYGRDNM,				\n"); 
          ...
          sql.append( "        CASE WHEN RTRIM(F.EMPNOH)<>'' AND RTRIM(G.EMPNOH)<>'' AND RTRIM(H.EMPNOH)<>'' THEN (A.AVG1+A.AVG2+A.AVG3)/3                                                       ");
					...
					sql.append( "        COALESCE(A.TOTAVG,0) TOTAVG																																			\n");
					sql.append( " FROM PAYROLL.HVEMP A  \n");
					if (str[9].equals("1"))  sql.append( " ,PAYROLL.HIPERSON E	\n");
					if (str[9].equals("2"))  sql.append( " ,PAYROLL.T_HIPERSON E	\n");
