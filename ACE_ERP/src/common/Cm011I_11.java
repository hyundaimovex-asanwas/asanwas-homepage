...
public class Cm011I_11 extends SuperServlet {
	...
	public void makeSql(){
		...
		//메뉴 트리 만들기
		arr_sql[0].append ("{CALL COMMON.PR_CM010S_11(?,?)}");	//권한 * Action Plan - 조회용
	...
	public void select(Connection conn, HttpGauceRequest greq, HttpGauceResponse gres)  throws ServletException, Exception {
		...
		try{
          ...
          //Component 로부터 전송된 입력스트림의 Output GauceDataSet의 배열을 반환
			...
			String menu_nm = "";
			String full_dir_nm = "";
			//StringUtil.printMsg(" 디버깅== ","for문 직전",this);
			//StringUtil.printMsg(" 컴포넌트의 데이터셋 갯수 ",dSets.length,this);
			//StringUtil.printMsg(" 디버깅== ","뭐가 null인가요?",this);
			//StringUtil.printMsg(" 컴포넌트의 데이터셋 네임 ",dSets[0].getName(),this);
            for(int i=0; i<dSets.length;i++){
            	//StringUtil.printMsg(" 디버깅== ","갯수가 1이면 여기도 찍히지?",this);
				//StringUtil.printMsg(" 컴포넌트의 데이터셋 네임 ",dSets[i].getName(),this);
				//StringUtil.printMsg(" 디버깅== ","그럼 null은 겟네임?",this);
