...
public class HdasanPwd {

	/**
	 * 회원 정보
	 */
	public int Pwd(HttpServletRequest request, HttpServletResponse response) {

		CallableStatement cstmt = null;
		CallableStatement cstmt2 = null;
		String aed 		= SecUtil.sqlFilter(StringUtil.encoding(request.getParameter("txt_aed")));
		String jminno 	= SecUtil.sqlFilter(request.getParameter("txt_jminno"));
		String empnm 	= SecUtil.sqlFilter(StringUtil.encoding(request.getParameter("txt_empnm")));
		...
		System.out.println("empnm1=="+ request.getParameter("txt_empnm")) ;
		*/
		...
		try{
			...
			//StringUtil.printMsg("COMMON.PR_CM100I_01(01)", "COMMON.PR_CM100I_01(01)", this);
			...
			cstmt.setString (3, empnm);	//pEMPNM			VARCHAR(24),		-- 3성명,
			...
			if(cstmt.getString(8).equals("Y")){	// 성공여부
				...
				//StringUtil.printMsg("COMMON.PR_CM100I_01(02)", "COMMON.PR_CM100I_01(02)", this);
	 ...
	 * 비밀번호 설정
	 */
	public int SetPwd(HttpServletRequest request, HttpServletResponse response) {
		...
		try{
			...
			cstmt = conn.prepareCall(qry);
			cstmt.setString (1, aed);		//pAED			VARCHAR(10),		-- 1사번,
			//StringUtil.printMsg("1", aed, this);	
	 ...
	 * 세션확인
	 */
	public String Ssn(HttpServletRequest request, HttpServletResponse response) {
		CallableStatement cstmt = null;
		//String aed 		= SecUtil.sqlFilter(request.getParameter("aed"));
		String aed	 			= request.getParameter("SAed");
		String qry ="";
		System.out.println("aed=="+aed);
		Connection conn = null;
		ResultSet rs    = null;
