...
public class Sy101I extends HttpServlet {
	...
	public void select(HttpServletRequest req, HttpServletResponse res)  throws ServletException {
		...
		try {
			...
			//System.out.println(s_HeadName);
			//System.out.println(s_Detail);
			// dataset 구분 param
			...
			switch(dsType) {
				case 1 :
					...
					ds1.addDataColumn(new GauceDataColumn("head_nm", 		GauceDataColumn.TB_STRING, 50));
...
//					 쿼리 실행
				...
				case 2:
					...
					ds2.addDataColumn(new GauceDataColumn("item1", 			GauceDataColumn.TB_STRING, 30));
					ds2.addDataColumn(new GauceDataColumn("item2", 			GauceDataColumn.TB_STRING, 30));
					ds2.addDataColumn(new GauceDataColumn("item3", 			GauceDataColumn.TB_STRING, 30));
					ds2.addDataColumn(new GauceDataColumn("item4", 			GauceDataColumn.TB_STRING, 30));
					ds2.addDataColumn(new GauceDataColumn("item5", 			GauceDataColumn.TB_STRING, 30));
					ds2.addDataColumn(new GauceDataColumn("item6", 			GauceDataColumn.TB_STRING, 30));
					ds2.addDataColumn(new GauceDataColumn("item7", 			GauceDataColumn.TB_STRING, 30));
					ds2.addDataColumn(new GauceDataColumn("item8", 			GauceDataColumn.TB_STRING, 30));
					ds2.addDataColumn(new GauceDataColumn("item9", 			GauceDataColumn.TB_STRING, 30));
					ds2.addDataColumn(new GauceDataColumn("item10", 		GauceDataColumn.TB_STRING, 30));
					ds2.addDataColumn(new GauceDataColumn("item11", 		GauceDataColumn.TB_STRING, 30));
					ds2.addDataColumn(new GauceDataColumn("item12", 		GauceDataColumn.TB_STRING, 30));
					ds2.addDataColumn(new GauceDataColumn("item13", 		GauceDataColumn.TB_STRING, 30));
					ds2.addDataColumn(new GauceDataColumn("item14", 		GauceDataColumn.TB_STRING, 30));
					ds2.addDataColumn(new GauceDataColumn("item15", 		GauceDataColumn.TB_STRING, 50));
...
//			int iParamUpjangSid = HDUtil.convertNum(req.getParameter("upjang_sid"));	// 영업장 SID
			...
			int colNm_HeadName 	= inputDs1.indexOfColumn("head_nm");	// 분류코드 
			GauceDataRow[] rows = inputDs1.getDataRows();
			
			insertQuery += "\n" 		
						+ "\n\t" + "INSERT INTO CONSTRUCT.TSY011 "
						+ "\n\t" + "	("
						+ "\n\t" + "		head, "
						+ "\n\t" + "		head_nm, " 
						+ "\n\t" + "		u_empno, " 
...
//			int iParamUpjangSid = HDUtil.convertNum(req.getParameter("upjang_sid"));	// 영업장 SID
			...
			int colNm_DetailName 	= inputDs1.indexOfColumn("detail_nm");	//   
			...
			for(int i = 0; i < rows.length; i++) {
				...
				if (rows[i].getJobType() == GauceDataRow.TB_JOB_UPDATE) {
					...
					stmt.setString(uCnt++, iParamEmpno);
					stmt.setString(uCnt++, iParamIp);
					stmt.bindColumn(uCnt++, colNm_Head);		// HEAD
				...
				if (rows[i].getJobType() == GauceDataRow.TB_JOB_DELETE) {
					...
					System.out.println(rows[i].getColumnValue(colNm_Head));
					System.out.println("# Value detail 	="	 + rows[i].getColumnValue(colNm_Detail));
					stmt.bindColumn(dCnt++, colNm_Head);		// HEAD
