 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class FileBoard extends DAOHome{
	...
	public int  getTotalCount(String part, String keyword) throws  DbException , Exception  { 
        ...
        sql.append(" WHERE BOARD_TYPE='03' ");
        sql.append(subSql);
        /*
        sql.append(" SELECT   \n");
		sql.append("   count(*)  \n");
		...
		sql.append("     FROM TBRD_BOARD_MA  WHERE BOARD_TYPE = '03'  \n");
		sql.append(subSql);
		sql.append(" ) A,(  \n");
    ...
    throws  DbException , Exception {
       ...
       FROM( 
        SELECT  
         ...
         BOARD_TOP_NO , READ_NUM , REG_DATE, RECOMM_NUM
       FROM TBRD_BOARD_MA  WHERE BOARD_TYPE = '03'
       ) A,(
        SELECT MIN(BOARD_SEQ) AS BOARD_SEQ ,
        COUNT(BOARD_SEQ) FILE_NUM 
        FROM 
        TBRD_ATTACH_FILE_DE 
        GROUP BY BOARD_SEQ
       ) B
       ...
       */
		...
		sql.append(" NVL(B.FILE_NUM,0) AS FILE_NUM , A.USER_NAME  \n");
		sql.append(" FROM( \n");
		sql.append("     SELECT \n");
		sql.append("       BOARD_SEQ, REF_NO , LEVEL_NO , SORT_NO , TITLE,  \n");
		sql.append("       CONTENT, USER_ID , USER_NICK, USE_YN,  \n");
		...
		sql.append(subSql);
		sql.append(" ) A,(  \n");
		sql.append("  SELECT /*+ INDEX(ATTACH_FILE_SEQ) */ MIN(BOARD_SEQ) AS BOARD_SEQ ,  \n");
		sql.append("      COUNT(BOARD_SEQ) FILE_NUM  \n");
		sql.append("  FROM  \n");
		sql.append("  TBRD_ATTACH_FILE_DE \n" );
		sql.append("  GROUP BY BOARD_SEQ \n");
    ...
    */
        ...
        throws DbException , Exception 
    {
        ...
        updateSql.append(" WHERE REF_NO=? AND SORT_NO > ? AND BOARD_TYPE = '03' ").toString();;
