 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class KsTourPrRoom  extends DAOHome{
    ...
    public Tbrd_board_ma_bean[] getList(int pageSize, int requestedPage,  String part, String keyword) throws  Exception{
		...
		sql.append("     B.THUMBNAIL_REAL_NAME, B.THUMBNAIL_VIRTUAL_NAME                         \n");
		sql.append(" FROM asan.TBRD_BOARD_MA  A left  outer join (                               \n");
		sql.append("     SELECT A.BOARD_SEQ ,A.REAL_NAME, A.VIRTUAL_NAME ,A.THUMBNAIL_REAL_NAME, \n");
		sql.append("     A.THUMBNAIL_VIRTUAL_NAME, A.ATTACH_FILE_SEQ                                   \n");
		...
		sql.append(" WHERE A.BOARD_TYPE='16' AND USE_YN='1'                                            \n");
		sql.append(subSql);
       ...
       //		 페이징 처리
        if(chk_page > 0){
			...
			query.append(" WHERE A.BOARD_TYPE='16' AND A.USE_YN='1'                                      \n");
     ...
     */
	 public int getTotalCount(String part, String keyword ) throws  Exception{
		...
		int totalCount = 0;
		 ...
		 if(!keyword.equals("")){
				if(part.equals("1")){
					...
					subSql = " AND TYPE_CODE LIKE '%"+keyword+"%' \n";
				}else{
		...
		.append(" WHERE BOARD_TYPE = '16' AND USE_YN='1' \n")
        ...
        throws DbException , Exception 
    {
		...
		String query = new StringBuffer()
			.append(" SELECT	                                    ")												
			.append(" ATTACH_FILE_SEQ, THUMBNAIL_REAL_NAME, THUMBNAIL_VIRTUAL_NAME, ")
		     .append(" REAL_NAME, VIRTUAL_NAME		")		
	...
	public int insert(Tbrd_board_ma_bean data , Tbrd_attach_file_de_bean fileBean ) throws DbException , Exception {
	 ...
	 private int insertAttachFile(Connection dbconn, Tbrd_attach_file_de_bean fileBean) throws DbException , Exception {
       ...
       int attache_seq = commonDao.getAttachFileNextSeq(dbconn);
		try {     
			...
			.append("  	(ATTACH_FILE_SEQ, REAL_NAME, VIRTUAL_NAME, THUMBNAIL_REAL_NAME,  ")
			.append("    THUMBNAIL_VIRTUAL_NAME, BOARD_SEQ)                              ")
			.append("  VALUES                                                            ")
			.append("  	(?, ?, ?, ?, ?, ?)                                               ").toString(); 
			...
			if(fileBean != null){	
                   //첨부 파일 정보가 없을 경우에도 real_name 과 vertual_name 이 없는 ROW 가 항상  추가 된다. 	
					...
					pstmt.setString(4,fileBean.getThumbnail_real_name());
					pstmt.setString(5,fileBean.getThumbnail_virtual_name());					
					pstmt.setInt(6,fileBean.getBoard_seq());
		...
		try { 
			...
			if(data != null){
				...
				if(result > 0){
					  ...
					  //썸네일 + 이미지
				      if( fileBean.getReal_name() != "" && fileBean.getThumbnail_real_name() != ""){
					      attach_result = updateAttachFileAll(dbconn, fileBean);
					  //썸네일
				      }else if( fileBean.getReal_name() == "" && fileBean.getThumbnail_real_name() != "" ){

				      	  attach_result = updateAttachFileOne(dbconn, fileBean);
                      //이미지
				      }else if( fileBean.getThumbnail_real_name() == "" &&  fileBean.getReal_name() !=""){

				      	  attach_result = updateAttachFileTwo(dbconn, fileBean);
				      }else{
      ...
      * @author 박상배 
      */
	 private int updateAttachFileAll(Connection dbconn, Tbrd_attach_file_de_bean fileBean) throws DbException , Exception {
        ...
        .append(" 	  VIRTUAL_NAME=?,					    ")
        .append(" 	  REAL_NAME=?,					        ")
		.append(" 	  THUMBNAIL_VIRTUAL_NAME=?,				")
		.append(" 	  THUMBNAIL_REAL_NAME=?  				")		
		...
		try {
			if(fileBean != null){
					...
					pstmt.setString(2,fileBean.getReal_name());	
					pstmt.setString(3,fileBean.getThumbnail_virtual_name());	
					pstmt.setString(4,fileBean.getThumbnail_real_name());	
					pstmt.setInt(5,fileBean.getAttach_file_seq());
					result = pstmt.executeUpdate();
      ...
      * @author 박상배 
      */
	 private int updateAttachFileOne(Connection dbconn, Tbrd_attach_file_de_bean fileBean) throws DbException , Exception {
		...
		try {
			if(fileBean != null){
					pstmt = dbconn.prepareStatement(query);
					// 변수 바인딩
					pstmt.setString(1,fileBean.getThumbnail_virtual_name());	
					pstmt.setString(2,fileBean.getThumbnail_real_name());	
      ...
      * @author 박상배 
      */
	 private int updateAttachFileTwo(Connection dbconn, Tbrd_attach_file_de_bean fileBean) throws DbException , Exception {
		...
		try {
			if(fileBean != null){
					...
					//System.out.println("------------"+fileBean.getVirtual_name());
					//System.out.println("------------"+fileBean.getReal_name());
					//System.out.println("------------"+fileBean.getAttach_file_seq());
	 ...
	 public Tbrd_board_ma_bean [] getPrevAndNextBoard(int board_seq) throws DbException , Exception {
		...
		sql.append(" WHERE BOARD_TYPE ='16' and BOARD_SEQ = ?			      ");
