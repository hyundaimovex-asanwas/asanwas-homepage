...
public class GoodsInfo  extends DAOHome{  
    ...
    public Goods_bean[] getGoodsList(int requestedPage, int pageSize, String part) throws  Exception{
		...
		if(chk_page > 0){		
				query = new StringBuffer();	
				query.append(" SELECT IDX, G_CORNER, G_IMGFILE , G_NAME , G_STD , G_PRICE        \n");
				query.append(" FROM ASAN.NORTHGOODS WHERE IDX NOT IN          \n");
				query.append(" ( SELECT IDX FROM           \n");
				query.append("    ( SELECT IDX FROM ASAN.NORTHGOODS  WHERE IDX > 0        \n");
				query.append(  subSql);
				query.append(" ORDER BY IDX DESC          \n");		
				query.append(" FETCH FIRST " + (requestedPage - 1) * pageSize + " ROWS ONLY  ) AS TMP )         \n");
				query.append(  subSql + " ORDER BY IDX DESC  \n");
