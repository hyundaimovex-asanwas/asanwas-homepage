 ...
 * 20131021	superServelt도 각 업무단별로 1개씩.. 왜냐면 여기서 JNDI를 맺기 때문에
 * 
 * */
...
public abstract class SuperServlet extends HttpServlet{
    ...
    public static GauceDataSet getDataSet(ResultSet rs, GauceDataSet ds) throws ServletException, Exception{
		...
		for (int i = 0; i < columnName.length; i++) {
			...
			//System.out.println(" 컬럼명"+i+"==="+ columnName[i]);
			
			//20151218 숫자타입 처리 : ms-sql쪽 데이터 타입 추가
			if(rsmd.getColumnTypeName(i+1).equals("NUMBER")||rsmd.getColumnTypeName(i+1).equals("bigint")||rsmd.getColumnTypeName(i+1).equals("float")||rsmd.getColumnTypeName(i+1).equals("numeric") ){
				if(columnName[i].indexOf("_AMT")!= -1 || columnName[i].indexOf("_RATE")!= -1 || columnName[i].indexOf("AMT_")!= -1 || columnName[i].indexOf("RATE_")!= -1 || columnName[i].indexOf("RT_")!= -1) {//RT_ 추가 20151218
					//System.out.println("상수[i]"+i+"::::::::::::::"+columnName[i]);
					
					ds.addDataColumn(new GauceDataColumn(columnName[i], GauceDataColumn.TB_DECIMAL, rsmd.getColumnDisplaySize(i+1)));
					//System.out.println("상수끝[i]"+i+"::::::::::::::"+columnName[i]);
				}
				else{ 
					//StringUtil.printMsg(" 컬럼명"+i+"===", columnName[i] ,this);
					//System.out.println("정수[i]"+i+"::::::::::::::"+columnName[i]);
					//System.out.println("GauceDataColumn.TB_DECIMAL"+"::::::::::::::::"+GauceDataColumn.TB_DECIMAL);
					//System.out.println("rsmd.getColumnDisplaySize(i+1)"+":::::::::::::::::"+rsmd.getColumnDisplaySize(i+1));
					
					ds.addDataColumn(new GauceDataColumn(columnName[i], GauceDataColumn.TB_DECIMAL, rsmd.getColumnDisplaySize(i+1),0 ));
					
					//System.out.println("정수끝[i]"+i+"::::::::::::::"+columnName[i]);
				}
			} else {
				//StringUtil.printMsg(" 컬럼명"+i+"===", columnName[i] ,this);
				//System.out.println("문자[i]"+i+"::::::::::::::"+columnName[i]);
				ds.addDataColumn(new GauceDataColumn(columnName[i], GauceDataColumn.TB_STRING, rsmd.getColumnDisplaySize(i+1)));
				//System.out.println("문자끝[i]"+i+"::::::::::::::"+columnName[i]);
			}
		}
		...
		while(rs.next()) {
			for(int i=0; i<columnName.length; i++) {
				columnName[i] = rsmd.getColumnName(i+1); // 컬럼 이름
				//System.out.println(" 타  입"+i+"==="+ rsmd.getColumnTypeName(i+1));	//이건 잘 찍힙니다.
				
			
				if(rsmd.getColumnTypeName(i+1).equals("NUMBER")||rsmd.getColumnTypeName(i+1).equals("bigint")||rsmd.getColumnTypeName(i+1).equals("float")||rsmd.getColumnTypeName(i+1).equals("numeric") ){
					//StringUtil.printMsg(" 숫자값 ===", rs.getInt(columnName[i]),this);					
					if(columnName[i].indexOf("_AMT")!= -1 || columnName[i].indexOf("AMT_")!= -1 || columnName[i].indexOf("_RATE")!= -1 || columnName[i].indexOf("RATE_")!= -1 || columnName[i].indexOf("RT_")!= -1) {//RT_ 추가 20151218
						//System.out.println(" 컬럼명"+i+"==="+ columnName[i] );
					...
					//System.out.println(" 컬럼명"+i+"==="+ columnName[i]);
					//System.out.println(" 문자값"+i+"==="+HDUtil.nullCheckStr(rs.getString(columnName[i])) );
					//문자인 경우
