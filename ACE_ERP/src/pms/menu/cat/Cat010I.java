...
public class Cat010I extends SuperServlet {
		...
		try {
            ...
            for(int i=0; i<dSets.length;i++){
				...
				if (ds_xlD != null && dSets[i].getName().equals("excelDs")) {
					...
					ds_xlD.addDataColumn(new GauceDataColumn("RATE_EXPECT",	  GauceDataColumn.TB_DECIMAL, 10,4)); 
	...
	public void apply(Connection conn, HttpGauceRequest greq, HttpGauceResponse gres)  throws ServletException, Exception {
		...
		try {
			...
			if (ds_xlsM != null) {
				...
				cstmt.setLong(6, rows[0].getLong(colNm_AMT_BASE));
				cstmt.setLong(7, rows[0].getLong(colNm_AMT_WIN));
			...
			if (ds_xls != null) {
				...
				for(int i=0; i<rows.length; i++){
					...
					//StringUtil.printMsg("INSERT 1= ",rows[i].getString(colNm_COMPANY_NM),this);	//거래일시 (은행에서 받은)
					...
					//StringUtil.printMsg("업체명=**",rows[i].getString(colNm_COMPANY_NM),this);
					//StringUtil.printMsg("예가대비율=**",rows[i].getFloat(colNm_RATE_EXPECT)+"***",this);
					//StringUtil.printMsg("기초대비투찰율 =**",rows[i].getFloat(colNm_RATE_BASE)+"***",this);
					...
					cstmt.setLong(6, rows[i].getLong(colNm_AMT_TENDER) );
					cstmt.setFloat(7, rows[i].getFloat(colNm_RATE_EXPECT) );
					cstmt.setFloat(8, rows[i].getFloat(colNm_RATE_BASE) );
					cstmt.setFloat(9, rows[i].getFloat(colNm_RATE_SHOOT) );
					cstmt.setString(10, rows[i].getString(colNm_DATE_TENDER));
					cstmt.setLong(11, rows[i].getLong(colNm_AMT_DIFF_MIN) );
