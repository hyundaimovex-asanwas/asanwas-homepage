...
public class Cc120I_t1 extends HttpServlet{

	/**	전표 저장 서블릿
	 *		공사/출납등록(by 정영식) 버전을 계속 복사해서 사용중 
	 */
	...
	public void doPost(HttpServletRequest req,HttpServletResponse res) {
		...
		try{
			...
			try {
				...
				*************************************************************************************/
				...
				//logger.dbg.println(this,"step2::전표MST시작");
				...
				//logger.dbg.println(this,"step2-3::");
			...
			*************************************************************************************/
			   //logger.dbg.println(this,"step3::전표DTL시작");
				...
				//System.out.println("Si050I_t1 1");

				GauceDataRow[]rows4 = userSet4.getDataRows();
				
				//System.out.println("Si050I_t1 2");
        		...
        		for (int j = 0; j < rows3.length; j++){
					...
					**/							
						...
						if(rows3[j].getJobType() == GauceDataRow.TB_JOB_INSERT) {
							...
							//System.out.println("Si050I_t1 8"+InsertSql.toString());
		...
		*************************************************************************************/
		...
		/************************************************************************************
		 @ 전표 작업 후 업무테이블(Tcc120-계정|귀속정보)에 전표 접수번호 UPDATE 처리 by 심동현 20150904
		  	20160111 그룹명 추가
		************************************************************************************/
		 GauceDataSet userSet7 = reqGauce.getGauceDataSet("USER7");//ds_cus
		 if (userSet7 != null) {
			int idx7ACQUIRE_SID    	= userSet7.indexOfColumn("ACQUIRE_SID");
			int idx7GROUP_NM		= userSet7.indexOfColumn("GROUP_NM");
			int idx7PL_DEPT	    	= userSet7.indexOfColumn("PL_DEPT");
			int idx7ATCODE	    	= userSet7.indexOfColumn("ATCODE");
		    ...
		    //쿼리문 만들어주고
			UpdateSql7.append( " UPDATE PMS.TCC120 SET " );
			UpdateSql7.append( "               FDCODE = ? " );
		    UpdateSql7.append( "              ,FSDAT = ? " );
		    UpdateSql7.append( "              ,FSNBR = ? " );
			UpdateSql7.append( "  WHERE ACQUIRE_SID = ? " );
			UpdateSql7.append( "    AND GROUP_NM = ? " );
			UpdateSql7.append( "    AND PL_DEPT = ? " );
			UpdateSql7.append( "    AND ATCODE = ? " );
			
			//System.out.println("UpdateSql7.toString()::"+UpdateSql7.toString());
			//System.out.println("rows7.length::"+rows7.length);
			...
			for (int b=0;b<rows7.length;b++){	
				...
				gsmt7.bindColumn(4,idx7ACQUIRE_SID);
				gsmt7.bindColumn(5,idx7GROUP_NM);
				gsmt7.bindColumn(6,idx7PL_DEPT);
				gsmt7.bindColumn(7,idx7ATCODE);
				gsmt7.executeUpdate();
				...
				UpdateSql8.append( " UPDATE PMS.TCC110 SET STAT='04' WHERE ACQUIRE_SID = ? " );
				GauceStatement gsmt8 = conn.getGauceStatement(UpdateSql8.toString());
				gsmt8.setGauceDataRow(rows7[b]);
				gsmt8.bindColumn(1,idx7ACQUIRE_SID);
				gsmt8.executeUpdate();
				gsmt8.close();
				gsmt8=null;				
				//TCC110 상태 업데이트 --------------------------------------
