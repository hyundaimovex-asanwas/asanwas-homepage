...
public class Pu020H extends SuperServlet {
	...
	public void makeSql(){
		...
		//	자재청구_전자결재기안 기본사항 조회_품목 LIST 
		arr_sql[6].append ("{call PMS.PR_PU010HS_06(?,?) }" );
			...
			try {
				...
				for(int i=0; i<dSets.length;i++){
					...
					if(dSets[i].getName().equals("ds_default")){
						GauceDataSet ds1 = gis.read("DS_DEFAULT");            //자재청구 기본조회 
					...
					}else if(dSets[i].getName().equals("ds_default2") & dsType==2){	//결재선 조회 : 내 결재선 sid
