...
public class Pu030H extends SuperServlet {
	...
	public void makeSql(){
		...
		arr_sql[4].append ("{call PMS.PR_PU030HS_04(?,?) }" );
		//	품의서작성_전자결재기안 기본사항 조회_견적내역업체별 LIST 
		arr_sql[5].append ("{call PMS.PR_PU030HS_05(?,?,?) }" );
		//	품의서작성_전자결재기안 기본사항 조회_품목 LIST 
		arr_sql[6].append ("{call PMS.PR_PU030HS_06(?,?,?) }" );
		//청구_현장결재정보
		arr_sql[7].append ("{call PMS.PR_PU030HS_07(?,?) }" );
		//품의_본사결재정보
		arr_sql[8].append ("{call PMS.PR_PU030HS_08(?,?) }" );
			...
			for(int i=0; i<dSets.length;i++){
				...
				if(dSets[i].getName().equals("ds_default6")){
					...
					GauceDataSet ds1 = gis.read("DS_DEFAULT9"); 
						...
						rs = (ResultSet)cstmt.getObject(2);	//커서 OUT param. 			
						gos.write(getDataSet(rs, ds1));
				}else if(dSets[i].getName().equals("ds_default9_1")){
					GauceDataSet ds1 = gis.read("DS_DEFAULT9_1");  
						cstmt = conn.prepareCall(arr_sql[8].toString());
						int  v_con_sid	= HDUtil.nullCheckNum(greq.getParameter("v_rqst_sid")); 		// 청구sid
