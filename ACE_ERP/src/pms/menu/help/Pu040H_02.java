...
public class Pu040H_02 extends SuperServlet {
	...
	public void makeSql(){
		...
		//	구매검수_전자결재기안 기본사항 조회_견적내역업체별 LIST  
		arr_sql[5].append ("{call PMS.PR_PU040HS_02_05(?,?,?) }" );
		//		구매검수_전자결재기안 기본사항 조회_품목 LIST  
			...
			for(int i=0; i<dSets.length;i++){
				...
				if(dSets[i].getName().equals("ds_default6")){
					GauceDataSet ds1 = gis.read("DS_DEFAULT6");  
						...
						int  v_con_sid		= HDUtil.nullCheckNum(greq.getParameter("v_con_sid")); 			// 검수마스터sid
						int  v_cgcd_sid		= HDUtil.nullCheckNum(greq.getParameter("v_cgcd_sid")); 		// 검수디테일sid
						String  v_date_cgcd	= HDUtil.nullCheckStr(greq.getParameter("v_date_cgcd")); 		// 검수일자sid
					...
					GauceDataSet ds1 = gis.read("DS_DEFAULT7");   
						...
						int  v_rqst_sid	= HDUtil.nullCheckNum(greq.getParameter("v_rqst_sid")); 		// 견적sid
					...
					GauceDataSet ds1 = gis.read("DS_DEFAULT8");  
						...
						int  v_con_sid	= HDUtil.nullCheckNum(greq.getParameter("v_con_sid")); 		// 품의sid
					...
					GauceDataSet ds1 = gis.read("DS_DEFAULT99");   
