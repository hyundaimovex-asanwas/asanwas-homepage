...
public class Pu070H extends SuperServlet {
	...
	public void makeSql(){
		...
		//정보등록_전자결재기안 기본사항 조회 
		//arr_sql[0].append ("{call PMS.PR_LI0100HS_01(?) }" );
		
		//정보등록_전자결재기안 결재선 조회 
		//arr_sql[1].append ("{call PMS.PR_LI0100HS_02(?) }" );
		//정보등록_전자결재기안 결재선 조회 
		//arr_sql[6].append ("{call PMS.PR_LI0100HS_03(?,?,?,?) }" );
		
		
	   //배포등록_전자결재기안 기본사항 조회 
		//arr_sql[2].append ("{call PMS.PR_LI0200HS_01(?) }" );
	    
	   //수신_전자결재기안 기본사항 조회 
		//arr_sql[3].append ("{call PMS.PR_LI0300HS_01(?,?) }" );
	    
		//배포등록_전자결재기안 기본사항 조회_수신부서 LIST 
		//arr_sql[5].append ("{call PMS.PR_LI0200HS_02(?) }" );
		
		//정보등록 상신 / 배포등록 상신 / 열람등록 상신  
		//arr_sql[4].append ("{call PMS.PR_LI0100HI_01(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?,?) }");
		...
		try {
			...
			while (it.hasNext()) {
				...
				if(ds1!=null){
					...
					if(gauceName.equals("DS_DEFAULT")){              //자재청구 기본조회 
						cstmt = conn.prepareCall(arr_sql[4].toString());
						
						int  v_rqst_sid		= HDUtil.nullCheckNum(req.getParameter("v_rqst_sid")); 		// 청구sid

						StringUtil.printMsg("DS_DEFAULT_v_rqst_sid",v_rqst_sid,this);
					...
					if(gauceName.equals("DS_DEFAULT3")){              //배포정보 조회 
						cstmt = conn.prepareCall(arr_sql[2].toString());
	
						//StringUtil.printMsg("배포조회[2]::","ㅇㅇ",this);
	
						int  v_dtb_mag_sid		= HDUtil.nullCheckNum(req.getParameter("v_dtb_mag_sid")); 		// 배포번호
					...
					if(gauceName.equals("DS_DEFAULT4")){              //수신 조회 
						cstmt = conn.prepareCall(arr_sql[3].toString());

						//StringUtil.printMsg("수신조회::","DS_DEFAULT4",this);

						int  txt_rcp_rdg_sid		= HDUtil.nullCheckNum(req.getParameter("v_rcp_rdg_sid")); 		//수신열람번호 
						String  txt_rcp_dpt     	= HDUtil.nullCheckStr(req.getParameter("v_rcp_dpt")); 	        	//수신부서
					...
					if(gauceName.equals("DS_DEFAULT9")){              //자재청구_품목조회 
						cstmt = conn.prepareCall(arr_sql[5].toString());

						//StringUtil.printMsg("배포조회[5]::","ㅇㅇ",this);

						int  v_rqst_sid	= HDUtil.nullCheckNum(req.getParameter("v_rqst_sid")); 		// 청구sid
						StringUtil.printMsg("DS_DEFAULT9_v_rqst_sid",v_rqst_sid,this);
						cstmt.setInt(1, v_rqst_sid);
						//StringUtil.printMsg("v_dtb_mag_sid",v_dtb_mag_sid,this);
						rs = cstmt.executeQuery();
						getDataSet(rs, ds1).flush();
