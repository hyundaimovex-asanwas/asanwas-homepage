...
public class Pc010I1 extends SuperServlet {
	...
	public void makeSql(){
		...
		//마스터 조회
		arr_sql[0].append ("{call PMS.PR_PC010I_01(?,?,?,?,?, ?,?) }" );

		//마스터 조회
		arr_sql[1].append ("{call PMS.PR_PC010I_02(?, ?) }" );

		//Main 저장
		arr_sql[2].append ("{call PMS.PR_PC010I_03(?,?,?,?,?,   ?,?,?,?,?,   ?,?,?,?,?,  ?,?) }");
		//Detail 저장
		arr_sql[3].append ("{call PMS.PR_PC010I_04(?,?,?,?,?,   ?,?,?,?,?,   ?,?,?,?,?,  ?,?) }");
		...
		try {
          ...
          //Component 로부터 전송된 입력스트림의 Output GauceDataSet의 배열을 반환
            ...
            for(int i=0; i<dSets.length;i++){
            	...
            	//O:DS_DEFAULT=ds_default,O:DS_TENDER=ds_tender)
		...
		try {
          ...
          //Component 로부터 전송된 입력스트림의 Output GauceDataSet의 배열을 반환
            ...
            for(int i=0; i<dSets.length;i++){
            	...
            	if(dSets[i].getName().equals("ds_default")){
					GauceDataSet ds1 = gis.read("DS_DEFAULT");
					//StringUtil.printMsg("DS DS_DEFAULT","조회",this);
					int v_site_sid 			= HDUtil.nullCheckNum(greq.getParameter("v_site_sid"));			// 현장 SID
					String v_bid_kind_cd 	= HDUtil.nullCheckStr(greq.getParameter("v_bid_kind_cd")); 	//  공종 (공통)
	...
	public void apply(Connection conn, HttpGauceRequest greq, HttpGauceResponse gres)  throws ServletException, Exception {
		/* 2013.10.22 심동현
		 * 이렇게 TR을 호출합니다. 
		 * 		 * I:DS_DEFAULT=ds_default,I:DS_TENDER=ds_tender
...
//		int fSiteSid = 0;
		try{
			if (ds1 != null) {
				...
				int colNm_DATE_BEGIN     = ds1.indexOfColumn("DATE_BEGIN");    //하도급 시작일
				int colNm_DATE_END       = ds1.indexOfColumn("DATE_END");     //하도급 종료일
				int colNm_CT_KIND        = ds1.indexOfColumn("CT_KIND");  //공종
