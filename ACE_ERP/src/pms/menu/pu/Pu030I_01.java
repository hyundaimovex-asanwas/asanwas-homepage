...
public class Pu030I_01 extends SuperServlet {
	...
	public void select(Connection conn, HttpGauceRequest greq, HttpGauceResponse gres) 
		...
		String v_item_nm 	= HDUtil.nullCheckStr(greq.getParameter("v_item_nm"));
		try {
			GauceInputStream gis = greq.getGauceInputStream();
			GauceOutputStream gos = gres.getGauceOutputStream();
			//Component 로부터 전송된 입력스트림의 Output GauceDataSet의 배열을 반환
				...
				for(int i=0; i<rows.length; i++){
					...
					switch(rows[i].getJobType()){
						 ...
						 */
							...
							cstmt.setString(7, "1");
							cstmt.setInt(8, rows[i].getInt(colNm_TEM_KIND));
