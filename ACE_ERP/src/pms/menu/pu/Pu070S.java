...
public class Pu070S extends SuperServlet {
	...
	public void makeSql(){
		...
		//마스터 조회
		arr_sql[0].append ("{call PMS.PR_PU070S_01(?,?,?,?,?     ,?) }" );

		//TENDER 조회
		arr_sql[1].append ("{call PMS.PR_PU070S_02(?,?,?) }" );

		//DETAIL 조회
		arr_sql[2].append ("{call PMS.PR_PU070S_03(?,?,?) }" );

		//Main 저장
		//arr_sql[3].append ("{call PMS.PR_PU070S_04(?,?,?,?,?,   ?,?,?) }");
		
		//Tender 저장
		//arr_sql[4].append ("{call PMS.PR_PU070S_05(?,?,?,?,?,   ?,?,?,?) }");

		//Detail 저장
		arr_sql[5].append ("{call PMS.PR_PU070S_04(?,?,?,?,?,   ?,?,?,?,?,     ?,?,?,?,?,      ?,?,?,?,?,      ?,?,?,?,?        ,?,?,?) }");
	}
