...
public class Pu110I extends SuperServlet {
	...
	public void makeSql(){
		...
		//마스터 조회
		arr_sql[0].append ("{call PMS.PR_PU110I_01(?,?,?,?,?) }" );

		//Detail 조회
		arr_sql[1].append ("{call PMS.PR_PU110I_02(?,?) }" );

		//Main 저장
		arr_sql[2].append ("{call PMS.PR_PU110I_03(?,?,?,?,?,   ?,?,?,?,?,   ?,?,?,?,?            ,?) }");
		//Detail 저장
		arr_sql[3].append ("{call PMS.PR_PU110I_04(?,?,?,?,?,   ?,?,?,?,?,   ?,?,?,?,?                ,?) }");
	...
	public void apply(Connection conn, HttpGauceRequest greq, HttpGauceResponse gres) throws ServletException, Exception {
		...
		try{
			if (ds1 != null) {
				...
				int colNm_RQST_MEMO 		= ds1.indexOfColumn("RQST_MEMO");  	// 청구특이사항
