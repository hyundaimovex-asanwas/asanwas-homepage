...
public class Si060I1 extends SuperServlet {
	...
	public void makeSql(){
		...
		//마스터 조회
		arr_sql[0].append ("{call PMS.PR_SI060I_01(?,?,?,?,?,?,?) }" );

		//Main 저장
		arr_sql[1].append ("{call PMS.PR_SI060I_02(?,?,?,?,?,   ?,?,?,?,?,   ?,?,?,?,?,  ?,?,?,?,?,   ?,?,?,?,?,?,?,?,?,?,?,?,?)}" );
...
//		int fSiteSid = 0;
		try{
			if (ds1 != null) {
				...
				int colNm_TAXNBR           = ds1.indexOfColumn("TAXNBR");		//세부공정SID  
				int colNm_FDCODE           = ds1.indexOfColumn("FDCODE");		//세부공정SID  
				int colNm_FSDAT              = ds1.indexOfColumn("FSDAT");		//세부공정SID  
				int colNm_FSNBR              = ds1.indexOfColumn("FSNBR");		//세부공정SID  
				...
				for(int i=0; i<rows.length; i++){
					...
					switch(rows[i].getJobType()){
						case GauceDataRow.TB_JOB_INSERT:
						    ...
						    cstmt.setString(27, rows[i].getString(colNm_TAXNBR));
						    cstmt.setString(28, rows[i].getString(colNm_FDCODE));
						    cstmt.setString(29, rows[i].getString(colNm_FSDAT));
						    cstmt.setString(30, rows[i].getString(colNm_FSNBR));
						    
							cstmt.setString(31, iParamEmpno);
							cstmt.setString(32, iParamIp);
