...
public class Sy101I extends SuperServlet {
	...
	public void makeSql(){
		...
		//마스터 apply-입력
		arr_sql[2].append (" INSERT INTO PMS.TSY011 	(head, head_nm, u_empno, u_date,  u_ip ) \n")
				  .append (" VALUES ( ?, ?, ?, SYSTIMESTAMP, ? )  \n");
		
		//마스터 apply-수정
		arr_sql[3].append (" UPDATE PMS.TSY011   \n")
				  .append ("    SET head_nm = ?, u_empno = ?, u_date = SYSTIMESTAMP, u_ip = ?	\n")  
				  .append ("  WHERE head = ?  \n");
				  
		//마스터 apply-삭제
		arr_sql[4].append (" DELETE FROM PMS.TSY011 WHERE head = ?  \n");
		
		
		
		//디테일 apply-입력
		arr_sql[5].append (" INSERT INTO PMS.TSY010 (head, detail, detail_nm,  \n")
		...
		//디테일 apply-수정
		arr_sql[6].append (" UPDATE PMS.TSY010   \n")
		...
		//디테일 apply-삭제
		arr_sql[7].append (" DELETE FROM PMS.TSY010 WHERE head= ? and detail= ?   \n");
