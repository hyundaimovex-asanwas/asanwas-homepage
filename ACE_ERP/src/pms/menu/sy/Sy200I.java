...
public class Sy200I extends SuperServlet {
	...
	public void makeSql(){
		...
		//card list : Master 조회
		arr_sql[0].append ("{call PMS.PR_SY200I_01(?,?,?,?,?,  ?,?) }" );
		...
		try {
          ...
          //Component 로부터 전송된 입력스트림의 Output GauceDataSet의 배열을 반환
            ...
            for(int i=0; i<dSets.length;i++){
            	if(dSets[i].getName().equals("ds_pl")){
					GauceDataSet ds1 = gis.read("DS_PL");
	...
	public void apply(Connection conn, HttpGauceRequest greq, HttpGauceResponse gres)  throws ServletException, Exception {
		...
		try{
			if (ds_cl != null) {
				...
				int colNm_CABANKNO     	= ds_cl.indexOfColumn("CABANKNO");//꼐좌번호    
