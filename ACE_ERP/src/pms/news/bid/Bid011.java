	...
	public void apply(Connection conn, HttpGauceRequest greq, HttpGauceResponse gres)  throws ServletException, Exception {
		...
		try{
			...
			if (ds_default != null) {
				...
				int colNm_BID_CD				= ds_default.indexOfColumn("BID_CD");        		// 프로젝트코드            
				int colNm_BID_NM       			= ds_default.indexOfColumn("BID_NM");   			// 차수       
				int colNm_BID_DT				= ds_default.indexOfColumn("BID_DT");				// 상세분류코드
				int colNm_ESTIMATE_AMT    		= ds_default.indexOfColumn("ESTIMATE_AMT");     	// 품목명
				int colNm_YN_RESULT    			= ds_default.indexOfColumn("YN_RESULT");     		// node,leaf 여부
				int jobtype 	= 3;
