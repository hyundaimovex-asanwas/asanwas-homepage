...
public class Bd010 extends SuperServlet {
	...
	public void makeSql(){
		for(int i=0; i<arr_sql.length; i++)
			arr_sql[i] = new StringBuffer();
				...
				//	bd010.jsp 입력값 조회
				arr_sql[1].append ("{call PMS.PR_NEW_BD010_02(?,?)}");
//				// bd011.jsp Detail 조회
				arr_sql[2].append ("{call PMS.PR_NEW_BD010_03(?,?,?)}");
//				// 프로젝트명 검색 하는 쿼리 
				arr_sql[3].append ("{call PMS.PR_NEW_RQ010S_02(?,?) }" );
//				//	insert 쿼리
				arr_sql[4].append ("{call PMS.PR_NEW_BD010_04(?,?,?,?)}");
//				
				//tree default 조회 
				arr_sql[5].append ("{call PMS.PR_NEW_BD010_05(?,?,?) }" );
				//tree ds_detail 조회
				arr_sql[6].append ("{call PMS.PR_NEW_BD010_06(?,?,?)}");
				//tree ds_detail 저장
				arr_sql[7].append ("{call PMS.PR_NEW_BD010_07(?,?,?,?,?,?,?,?) }");
