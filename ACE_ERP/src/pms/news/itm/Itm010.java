...
public class Itm010 extends SuperServlet {
	...
	public void makeSql(){
		...
		for(int i=0; i<arr_sql.length; i++)
			arr_sql[i] = new StringBuffer();
				...
				//default 조회 //추가
				arr_sql[0].append ("{call PMS.PR_NEW_ITM010S_01(?) }" );
				
				
				//	ds_detail 조회
				arr_sql[1].append ("{call PMS.PR_NEW_ITM010S_02(?,?,?)}");
	...
	public void init(Connection conn, HttpGauceRequest greq, HttpGauceResponse gres)  throws ServletException, Exception {
		...
		try {
          ...
          //Component 로부터 전송된 입력스트림의 Output GauceDataSet의 배열을 반환
            ...
            String v_overValue1 				= HDUtil.nullCheckStr(greq.getParameter("v_overValue1"));//
            
            
    		//StringUtil.printMsg("v_project",v_project,this);
            ...
            for(int i=0; i<dSets.length;i++){
            	...
            	}else if(dSets[i].getName().equals("ds_item_d")){

					GauceDataSet ds2 = gis.read("DS_ITEM_D");
					
					cstmt = conn.prepareCall(arr_sql[1].toString());
					
					System.out.println("181234");
