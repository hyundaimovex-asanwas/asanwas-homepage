...
public class Sta010 extends SuperServlet {
	...
	public void makeSql(){
		...
		for(int i=0; i<arr_sql.length; i++)
			...
			arr_sql[i] = new StringBuffer();
				...
				//ds_default 조회
				arr_sql[1].append ("{call PMS.PR_NEW_STA010S_01(?,?)}");
				

				//ds_default 저장
				arr_sql[2].append ("{call PMS.PR_NEW_STA010S_02(?,?,?,?,?,?,?,?,?,?,?,?)}");
	...
	public void select(Connection conn, HttpGauceRequest greq, HttpGauceResponse gres)  throws ServletException, Exception {
		...
		String v_journal			= HDUtil.nullCheckStr(greq.getParameter("v_journal"));				//분개정보
