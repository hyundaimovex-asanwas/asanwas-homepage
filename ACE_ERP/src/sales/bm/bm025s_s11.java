...
public class bm025s_s11 extends HttpServlet {
	...
	public void doGet(HttpServletRequest req, HttpServletResponse res) {
		...
		try {
			...
			try {
				...
				sql.append( "          INNER JOIN SALES.TCM040 T4 ON T1.RSV_SID = T4.RSV_SID	\n" );//추가

				sql.append( "     WHERE 1=1																	\n" );
				if (!str1.equals("")) sql.append( "  AND T1.DEPART_DATE = = '" + str1 + "'	\n");
				sql.append( "  ORDER BY V1.CLIENT_NM, T3.CUST_NM									");
				//logger.dbg.println("bm025s_s11",sql.toString());
