 ...
 * 20131021	superServelt도 각 업무단별로 1개씩.. 왜냐면 여기서 JNDI를 맺기 ��문에
 * 
 * */
    ...
    public static GauceDataSet getDataSet(ResultSet rs, GauceDataSet ds) throws ServletException, Exception{
		...
		for (int i = 0; i < columnName.length; i++) {
			...
			if(rsmd.getColumnTypeName(i+1).equals("NUMBER") ){
				...
				//System.out.println(" 사이즈?"+i+"==="+ rsmd.getColumnDisplaySize(i+1) );
				//2014-02-04 : 이 사이즈가 테이블의 컬럼사이즈를 따라오는게 맞음.
				//			   다만, 테이블에 없이 쿼리문에서 만들어진 컬럼은 한글1자를 2사이즈가 아닌 1로 인식... 
				//			   따라서, CASE문 같은걸로 그냥 만들어지는 컬럼은 한글 뒤에 공백을 붙여 사이즈를 강제로 늘려주자.
				//			   PR_AC820B_02 참고...
