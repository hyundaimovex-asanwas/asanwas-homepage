        ...
        for (int i=0; i<strArrCN.length; i++) {
            ...
            strYYmmdd2 =  strYYmmdd.substring(0,4)+"-"+strYYmmdd.substring(4,6)+"-"+"01";
						...
						sql0.append( " SELECT SUBSTR(REPLACE(CHAR((CAST('"+strYYmmdd2+"' AS DATE) - 1 MONTH ),ISO),'-',''),1,6),  \n" )
						    .append( "        SUBSTR(REPLACE(CHAR((CAST('"+strYYmmdd2+"' AS DATE) - 2 MONTH ) ,ISO),'-',''),1,6)   \n" )
            ...
            /*+++++++++++++++++++++++++++++++++++++++++++++++ 
              급여계산작업 시작 : 전화비 유보
							--2008.05.07 . 전화비 유보 계산 정의
							휴가자 또는 퇴사자는 
							휴가일자 또는 퇴사일자가 15일 이전(14일까지)는 
							이전월에서 제외관리로 빼고, 
							15일 이후는 시스템에서 자동으로 뺌
					  +++++++++++++++++++++++++++++++++++++++++++++++*/
