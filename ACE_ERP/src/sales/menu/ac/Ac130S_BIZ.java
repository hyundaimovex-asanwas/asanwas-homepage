...
public class Ac130S_BIZ extends SuperServlet {
	...
	public void makeSql() {
		...
		//조회
		arr_sql[1].append("CALL SALES.PR_AC130S_01_BIZ(?,?,?,?,?, ?,?,?,?,?)");
		//리포트
		arr_sql[2].append("CALL SALES.PR_AC130S_02_BIZ(?,?,?,?,?,   ?,?,?,?,?,   ?,?)"); //신청서번호, 비용부담회사 추가
	...
	public void select(GauceDBConnection conn, GauceRequest req,
		...
		String sAccept_no	= HDUtil.nullCheckStr(req.getParameter("v_accept_no"));	// 11 선택된 신청서 번호. 
		int sReal_client 		= HDUtil.nullCheckNum(req.getParameter("v_real_client"));	//12 비용부담회사.
		...
		//StringUtil.printMsg("신청서번호",sAccept_no+"",this);
		//StringUtil.printMsg("비용부담회사",sReal_client+"",this);
		try {
			int sCnt1 = 1;
			...
			while (it.hasNext()) {
				...
				if(ds1!=null){
					...
					case 2:
							...
							cstmt.setString(9, v_wrk_gubn);
							cstmt.setInt(10, sSalDamSid);
							cstmt.setString(11, sAccept_no);
							cstmt.setInt(12, sReal_client);
