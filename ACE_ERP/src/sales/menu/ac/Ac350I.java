...
public class Ac350I extends SuperServlet {
				...
				if(ds1!=null){
					switch(dsType){
						case 1:
							...
							StringUtil.printMsg("call","SALES.PR_AC350i_01",this);
							cstmt = conn.prepareCall("{call SALES.PR_AC350i_01(?, ?, ?, ?, ?) }");
				...
				inner : {
					for(int i=0; i<rows.length; i++){
						...
						case GauceDataRow.TB_JOB_UPDATE:
							StringUtil.printMsg("call","SALES.PR_AC350i_02-UPDATE",this);
							cstmt = conn.prepareCall("{call SALES.PR_AC350i_02(?, ?, ?, ?, ?, ?, ?, ?, ?) }");
