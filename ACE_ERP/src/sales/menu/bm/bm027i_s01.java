...
public class bm027i_s01 extends HttpServlet {
	...
	public void doGet(HttpServletRequest req, HttpServletResponse res) {
		...
		try {
			...
			try {
				...
				sql.append( "     FROM SALES.TCM060 T3											\n" );
				sql.append( "         INNER JOIN SALES.TCU010 T2									\n" );
				sql.append( "             ON T3.CUST_SID = T2.CUST_SID						\n" );
				sql.append( "         LEFT JOIN SALES.TCM010 T1									\n" );
				sql.append( "             ON T3.CUST_SID = T1.CUST_SID						\n" );
				sql.append( "     WHERE T3.CUST_SID = '"+str1+"'								\n" );
				sql.append( "     ORDER BY T3.NS_PLAN DESC											" );

				//logger.dbg.println("bm027i_s01",sql.toString());
