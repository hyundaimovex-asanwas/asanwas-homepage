...
public class Cq040S extends SuperServlet {
	...
	//저장 _출력인원_tcm040의 snp_yn=>Y
		arr_sql[2].append("{call SALES.PR_CQ040S_02 (?,?,?,?,?,   ?,?,? ) }");			
	//	저장 _미출력인원_tcm040의 snp_yn=>N
...
//									System.out.println("ColumnType:"+i+rsmd.getColumnTypeName(i+1));
				...
				if(dSets[j].getName().equals("ds9")){
					...
					//StringUtil.printMsg("조회쿼리 ds9","출력",this);
					switch(dsType){
						case 1:
						break;
						case 2:
						...
						//StringUtil.printMsg("조회쿼리 arr_sql[1].toString()",arr_sql[1].toString(),this);
						if (0 != sSaupSid) 
...
//								System.out.println("ColumnName:"+i+rsmd.getColumnName(i+1)+"끝");									
	...
	public void apply(Connection conn, HttpGauceRequest greq, HttpGauceResponse gres)  throws ServletException, Exception {
		...
		//StringUtil.printMsg("저장 - ", "저장", this);
		GauceInputStream gis = greq.getGauceInputStream();
        GauceOutputStream gos = gres.getGauceOutputStream();
