...
public class Cq200I extends SuperServlet {
	...
	public void makeSql() {
		...
		//차량탑승_차량조회
		arr_sql[0].append("{call SALES.PR_CQ200I_01 (?,?,?,?,?,   ?,? ) }");
		
		//차량탑승_차량 호차 저장 
		arr_sql[2].append("{call SALES.PR_CQ200I_02 (?,?,?,?,?,   ?,?,?,?,?,  ?) }");		

		// 차량탑승_고객정보 조회
		arr_sql[3].append("{call SALES.PR_CQ200I_03 (?,?,?,?,?,   ?,?,?,?,?,  ?,? ) }");
				
		// 차량탑승_고객정보 저장 
		arr_sql[4].append("{call SALES.PR_CQ200I_04 (?,?,?,?,?,   ?,?,?,?) }");

		// 차량탑승_액셀용_고객정보 조회
		arr_sql[5].append("{call SALES.PR_CQ200I_05 (?,?,?,?,?,   ?,?,?,?,?,  ?,? ) }");
	...
	public void select(Connection conn, HttpGauceRequest greq, HttpGauceResponse gres)  throws ServletException, Exception {
		...
		//StringUtil.printMsg(" sRide", sRide,this);
		...
		try {
			...
			if (ds_car != null) {// 차량그리드 호수/명칭 저장
				...
				for(int i=0; i<rows.length; i++){
					...
					cstmt.setInt(2, rows[i].getInt(colNm_RideCarNum));
					cstmt.setString(3, rows[i].getString(colNm_RideCar_Nm));
					cstmt.setInt(4, rows[i].getInt(colNm_RideCarNum2));
			...
			if (ds_cust != null) {// 인원그리드 탑승정보 저장 
				...
				int colNm_DriveYn  			= ds_cust.indexOfColumn("DONGSEUNG");
