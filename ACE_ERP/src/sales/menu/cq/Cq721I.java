		...
		try{
			if(ds1!=null){
				...
				for(int i=0; i<row.length; i++){
					...
					cstmt.setInt	(2, row[i].getInt(ds1.indexOfColumn("CAR_RSV_SID")));		//차량 예약SID
					cstmt.setInt	(3, row[i].getInt(ds1.indexOfColumn("INOUT_SID")));		//입출경SID
					cstmt.setString (4, row[i].getString(ds1.indexOfColumn("ARRIVE_DATE")));	//입경일자
					cstmt.setString (5, row[i].getString(ds1.indexOfColumn("ARRIVE_TIME")));	//입경시간
					cstmt.setString (6, row[i].getString(ds1.indexOfColumn("REMARKS")));	//참고사항
