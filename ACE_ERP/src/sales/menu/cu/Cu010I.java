...
public class Cu010I extends SuperServlet{
	/**
	 * 
	 */
    ...
    public void init(Connection conn, HttpGauceRequest greq, HttpGauceResponse gres) throws ServletException, Exception {
		...
		try {
		    ...
		    for(int i=0; i<dSets.length;i++){
		   		if(dSets[i].getName().equals("searchds1")){//고객구분
				...
				} else if(dSets[i].getName().equals("codeDs3")){//고객구분 //ac030s
		    		GauceDataSet ds1 = gis.read("CUSTALL");
		    		cstmt = conn.prepareCall(arr_sql[0].toString());
					cstmt.setString(1, "CU001");
					rs = cstmt.executeQuery(); 
					...
					//StringUtil.printMsg("initds4","initds4",this);
					...
					rs = cstmt.executeQuery(); 
		   			...
		   			rsmd = rs.getMetaData();
					columnName = new String[rsmd.getColumnCount()];
				} else if(dSets[i].getName().equals("initds1") && dsType!=2){//국적구분
					GauceDataSet ds1 = gis.read("NATION");
					//StringUtil.printMsg("NATION",arr_sql[1].toString(),this);
					cstmt = conn.prepareCall(arr_sql[1].toString());
					cstmt.setString(1, "CU009");
					rs = cstmt.executeQuery(); 
		   			gos.write(getDataSet(rs, ds1));// DataSet set
		   			rsmd = rs.getMetaData();
					columnName = new String[rsmd.getColumnCount()];
				}else if(dSets[i].getName().equals("initds1") && dsType==2){//예약조회에서 사용..홈>관광>예약관리>예약신청>예약조회
					GauceDataSet ds1 = gis.read("CUST");
		    		cstmt = conn.prepareCall(arr_sql[1].toString());
					cstmt.setString(1, "CU001");
					rs = cstmt.executeQuery(); 
		   			gos.write(getDataSet(rs, ds1));// DataSet set
				} else if(dSets[i].getName().equals("initds2") && dsType!=2){//인증여부
					...
					rs = cstmt.executeQuery(); 
		   			...
		   			rsmd = rs.getMetaData();
					columnName = new String[rsmd.getColumnCount()];
				}else if(dSets[i].getName().equals("initds2") && dsType==2){//예약조회에서 사용..홈>관광>예약관리>예약신청>예약조회
					GauceDataSet ds1 = gis.read("NATION");
					//StringUtil.printMsg("NATION",arr_sql[1].toString(),this);
					cstmt = conn.prepareCall(arr_sql[1].toString());
					cstmt.setString(1, "CU009");
					rs = cstmt.executeQuery(); 
		   			gos.write(getDataSet(rs, ds1));// DataSet set
				} else if(dSets[i].getName().equals("initds3")  && dsType!=2){//성별
