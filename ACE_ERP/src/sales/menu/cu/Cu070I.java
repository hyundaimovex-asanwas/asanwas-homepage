...
public class Cu070I extends SuperServlet {
    ...
    public void makeSql(){
		 ...
		 */
		arr_sql[0].append("{call SALES.PR_CU070I_01( ?, ?, ?, ?, ?,  ? ) }");	 
		 
		 
		// 신청 저장
		arr_sql[1].append("{call SALES.PR_CU070I_02( ?,?,?,?,?,  ?,?,?,?,?, ?,?,?,?,?, ?,? ) }");
			...
			while(it.hasNext()){
				...
				if(ds1!=null){
					...
					if ("DEFAULT".equals(gauceName)) {
						...
						cstmt.setString(5, sVisitObjCd);	//IN 방북목적코드
		...
		try{
			...
			if (ds1 != null) {
				...
				for(int i = 0; i < rows.length; i++) {
					...
					cstmt.setString     (5, rows[i].getString(colNm_ReqDt));		//교육신청일
					//StringUtil.printMsg("5",rows[i].getString(colNm_ReqDt) , this);
					cstmt.setString     (6, rows[i].getString(colNm_Remarks));		//참고
					//StringUtil.printMsg("6",rows[i].getString(colNm_Remarks) , this);
					cstmt.setInt     	(7, rows[i].getInt(colNm_CustSid));			//고객SID
					//StringUtil.printMsg("7",rows[i].getInt(colNm_CustSid) , this);
					cstmt.setString     (8, rows[i].getString(colNm_NEduc_yn));		//방북교육유무 CU014
					//StringUtil.printMsg("8",rows[i].getString(colNm_NEduc_yn) , this);
					cstmt.setString     (9, rows[i].getString(colNm_NJobGu));		//박북신청작업구분 CU015 (최초/추가/재/이전)
					//StringUtil.printMsg("9",rows[i].getString(colNm_NJobGu) , this);
					cstmt.setString     (10, rows[i].getString(colNm_VisitObject));	//방북목적
					//StringUtil.printMsg("10",rows[i].getString(colNm_VisitObject) , this);
					cstmt.setInt        (11, rows[i].getInt(colNm_SaupSid));	//지역 : 사업소sid
					//StringUtil.printMsg("11",rows[i].getInt(colNm_SaupSid) , this);
					cstmt.setString     (12, fParamEmpno);							//수정사번
					cstmt.setString     (13, fParamIp);								//수정ip
					cstmt.setString     (14, v_edit_styatus);						//편집상태
