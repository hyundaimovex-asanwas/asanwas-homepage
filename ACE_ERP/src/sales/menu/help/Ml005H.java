...
public class Ml005H extends SuperServlet {
...
//				  .append ("  AND T1.EFFICACY_DAY  >= TO_CHAR(CURRENT_DATE,'YYYYMMDD') 	        	\n")
		...
		arr_sql[17].append ("INSERT INTO SALES.TRV210  												\n")
		  .append ("(	VIRTUAL_ID,     PAY_MANAGE_NO,	PAY_CD,			PAY_DATE,		PAY_TIME,           \n")
		  .append ("	ORDER_NM,		ORDER_TEL_NO,	ORDER_E_MAIL,	PAY_AMT,            \n")
		  .append ("	TERMINAL_ID,	VAN_IDX_NO,		ACCT_JOIN_NO,	AGREE_TIME,			\n")
		  .append ("	AGREE_CUST_NM,  AGREE_AMT,		BANK_CD,		BANK_NM,			\n")
		...
		//온라인 다이렉트 : 해당 결제관리번호에 아이디 패스워드를 업데이트합니다. 
		// 그렇지만 이것만으로는 부족하군요.. 내부처리 프로세스가.. 20111102
		arr_sql[18].append  (" UPDATE SALES.TPU010 	\n")
					.append (" SET DIRECT_ID= ? , DIRECT_PASS=	?\n")
					.append (" WHERE PAY_MANAGE_NO= ?	\n");
	}
	...
	public void select(Connection conn, HttpGauceRequest greq, HttpGauceResponse gres)  throws ServletException, Exception {
		...
		int		v_pay_amt_tot	= HDUtil.nullCheckNum(greq.getParameter("v_pay_amt_tot"));
		String	v_sales_gubn	= HDUtil.nullCheckStr(greq.getParameter("v_sales_gubn"));
		try{
            GauceInputStream gis = greq.getGauceInputStream();
            GauceOutputStream gos = gres.getGauceOutputStream();
          ...
          //Component 로부터 전송된 입력스트림의 Output GauceDataSet의 배열을 반환
            ...
            for(int i=0; i<dSets.length;i++){
            	...
            	if(dSets[i].getName().equals("ds_manage_no")){
					...
					cstmt.setInt(2, v_pay_amt_tot);	//결제금액
					...
					cstmt.setString(4, v_sales_gubn);	//영업매출구분: PU001. 01관광요금, 02현지매출, 03유통
	...
	public void apply(Connection conn, HttpGauceRequest greq, HttpGauceResponse gres)  throws ServletException, Exception {
		...
		String v_login_id		= HDUtil.nullCheckStr(greq.getParameter("v_login_id"));
		String v_login_pass		= HDUtil.nullCheckStr(greq.getParameter("v_login_pass"));
		...
		try{
			...
			if (ds_tongjang != null) {
				...
				//StringUtil.printMsg("동현: 무통장", "start", this);
				  ...
				  StringUtil.printMsg("row[i].getString(0)", row[i].getString(0), this);
				  StringUtil.printMsg("row[i].getString(1)", row[i].getString(1), this);
				  StringUtil.printMsg("row[i].getString(2)", row[i].getString(2), this);
				  StringUtil.printMsg("row[i].getString(3)", row[i].getString(3), this);
				  StringUtil.printMsg("row[i].getString(4)", row[i].getString(4), this);
				  StringUtil.printMsg("row[i].getString(5)", row[i].getString(5), this);
				  StringUtil.printMsg("row[i].getString(6)", row[i].getString(6), this);
				  StringUtil.printMsg("row[i].getString(7)", row[i].getString(7), this);
				  StringUtil.printMsg("row[i].getString(8)", row[i].getString(8), this);
				  StringUtil.printMsg("row[i].getString(9)", row[i].getString(9), this);
				  StringUtil.printMsg("row[i].getString(10)", row[i].getString(10), this);
				  StringUtil.printMsg("row[i].getString(11)", row[i].getString(11), this);
				  */
			...
			/*if(gauceName.equals("DIRECT")){	//온라인 다이렉트
				stmt = conn.getGauceStatement(arr_sql[18].toString());
				
				stmt.setString(1, v_login_id);
				stmt.setString(2, v_login_pass);
				stmt.setString(3, v_pay_manage_no);
				stmt.executeUpdate();
