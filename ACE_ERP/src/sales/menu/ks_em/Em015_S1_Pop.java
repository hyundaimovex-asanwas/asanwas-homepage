...
**********************************************************************************************/
				...
				str[3] =  req.getParameter("v_str4");	// 조기휴가자
        ...
        for(int i=0;i<=3;i++){
				...
				String[] strArrCN = new String[]{  "CHK",     "EMPNO",  "EMPNMK", "SEQ",    "CUST_SID",
                                           ...
                                           "START_DT","END_DT", "CONT_GB","CONT_GBNM",
					                                 "VEND_CD", "WORK_CD", "CON_YEAR", "H_YN","REWARD_YM", "BON_YM",
                                           "I_EMPNO", "U_EMPNO","I_DATE", "U_DATE", "CON_YEARNM", "SSTART_DT"                         
																		  	}; //
            ...
            sql.append( "	        CASE WHEN A.SEQ>=2 THEN (SELECT  X.BON_YM  FROM SALES.TCM345 X  WHERE X.EMPNO = A.EMPNO AND X.SEQ = A.SEQ + 1) ELSE '' END BON_YM,  \n");
						sql.append( "	       A.I_EMPNO,  A.I_DATE, A.U_EMPNO, A.U_DATE, I.DETAIL_NM AS CON_YEARNM, C.START_DT AS SSTART_DT   \n");
						...
						sql.append( "	  LEFT JOIN SALES.TSY010 I ON I.DETAIL = A.CON_YEAR AND I.HEAD = 'CM017' AND TRIM(I.DETAIL)<>''\n"); 
						sql.append( "	 WHERE C.USESTS='1'                      /* 재직자 */                      \n");
						sql.append( "	   AND (D.H_YN <>'Y' OR D.H_YN IS NULL)  /* 휴가 안간 사람 */              \n");

						if(!str[3].equals("1")){ //조기휴가자 일경우 안탐.
