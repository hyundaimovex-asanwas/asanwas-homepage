	...
	public void doPost(HttpServletRequest req,HttpServletResponse res) {
		...
		try{
			...
			try {
				...
				String[] strArrCN = new String[]{ "EMPNO", "CONT_GB", "REWARD_YM", "I_EMPNO", "U_EMPNO", "BON_YM", "SEQ"
																			  };
        ...
        for (int i=0; i<strArrCN.length; i++) {
  				 ...
  				 StringBuffer InsertSql = new StringBuffer();
					 InsertSql.append("  INSERT INTO SALES.TCM345                                           \n" )
						        ...
						        .append("         C.DEPT_CD, C.TEAM_CD, C.WORK_CD, '"+rows[j].getString(idx[1])+"' AS CONT_GB,  \n" )
										.append("         REPLACE(CHAR((CAST(SUBSTR(A.START_DT,1,4)||'-'||SUBSTR(A.START_DT,5,2)||'-'||SUBSTR(A.START_DT,7,2) AS DATE)+1 YEAR ),ISO),'-','') AS START_DT,     \n" )
										.append("         REPLACE(CHAR((CAST(SUBSTR(A.END_DT,1,4)||'-'||SUBSTR(A.END_DT,5,2)||'-'||SUBSTR(A.END_DT,7,2) AS DATE)+1 YEAR),ISO),'-','') AS END_DT,             \n" )
										.append("         ''C_EMPNO,''C_POSI_CD,''C_OPI,   \n" )
										.append("         ''T_EMPNO,''T_POSI_CD,''T_OPI,   \n" )
            ...
            StringBuffer sql_1 = new StringBuffer();
						...
						sql_1.append( " SELECT CON_YEAR,  CON_YEAR-1, START_DT,  A.SEQ-1 AS P_SEQ     \n" )
						...
						while (rs_1.next())	{
							...
							intBCon_Year =rs_1.getInt(2);
							strChgymd =rs_1.getString(3);
