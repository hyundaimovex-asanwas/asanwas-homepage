...
public class Ml130I extends SuperServlet {
	...
	public void makeSql() {
		...
		arr_sql[1].append ("SELECT  \n")           
					  ...
					  .append (" ,ML_GUBN,AMT_COST,REMARKS 					\n")
					  .append (" FROM SALES.TML100	\n");
				  
		arr_sql[2].append("CALL SALES.PR_ML120I_01(?,?,?,?, ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");		//업데이트  20개		  		
		arr_sql[3].append("CALL SALES.PR_ML120I_02(?,?,?,?, ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");		//인서트하기  19개	
	...
	public void apply(GauceDBConnection conn, GauceRequest req,
		...
		try{
			...
			if (ds1 != null) {
				...
				int colNm_ml_gubn		= ds1.indexOfColumn("ML_GUBN");							 
					...
					/*		StringUtil.printMsg("회차       - ",rows[i].getString(colNm_use_seq)	, this); 
						...
						*/
						...
						//	cstmt.setString(7, rows[i].getString(colNm_dsp_yn));								
							...
							cstmt.setString(19, rows[i].getString(colNm_remarks));
							cstmt.setInt(20, rows[i].getInt(colNm_item_sid));		
							cstmt.executeUpdate();
							if(cstmt!=null) cstmt.close();
						 }//job_update				
