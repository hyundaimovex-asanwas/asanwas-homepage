...
public class My720DAO extends CommonDAO {
    public My720DAO() {
		super(); 
    }
    
    //게시물 리스트 조회시
    public ArrayList list(My720DTO entity, String where_sql,int requestedPage,int pageSize ,String v_login_id,String v_login_name,Connection conn) throws SQLException {
        ...
        try {
			...
			// 페이징 처리
			//	보낸쪽지함으로 로긴 사용자가 보낸 쪽지만 보여줌.20100520_박경국
			if(chk_page > 0){	//1페이지가 아닌 경우
				...
				sql.append("  AND YN_DEL_CREATOR    = 'N' 	");//보낸 쪽지함에서 삭제하지 않고
				sql.append("  AND YN_KEEP_CREATOR    = 'N' 	");
				sql.append("  AND REF_STEP = 0 		");
				sql.append("  AND BRD_CREATOR = ? 		");
				sql.append(where_sql);	//검색 조건절 부분			
				sql.append("  AND BRD_NO NOT IN 	");
				sql.append("	  (SELECT BRD_NO FROM( ");
				sql.append("		   SELECT BRD_NO FROM SALES.TMY070 WHERE BRD_GU = '01' AND YN_DEL_CREATOR = 'N' AND REF_STEP = 0 ");
				sql.append(				where_sql);	//검색 조건절 부분			
				...
				sql.append(" FROM SALES.TMY070   	");
				sql.append("WHERE BRD_GU   = '01' 	");
				sql.append("  AND YN_DEL_CREATOR    = 'N' 	");
				sql.append("  AND YN_KEEP_CREATOR    = 'N' 	");
				sql.append("  AND REF_STEP = 0 		");
				sql.append("  AND BRD_CREATOR =? 		");
				sql.append(where_sql);	//검색 조건절 부분
   ...
   //보낸쪽지함으로 로긴 사용자가 보낸 쪽지의  개수만 카우트.20100520_박경국
	public ArrayList alllist(My720DTO entity, String where_sql,int requestedPage,int pageSize ,String v_login_id,String v_login_name,Connection conn) throws SQLException {
		java.sql.PreparedStatement pstmt = null;
		ResultSet rs = null;
		StringBuffer sql = new StringBuffer();
		ArrayList arrBeans = new ArrayList();
		...
		try {
			...
			sql.append("  AND YN_DEL_CREATOR    = 'N' 	");	// 삭제 아니고
			sql.append("  AND YN_KEEP_CREATOR    = 'N' 	");				//보관함 아니고
    ...
    public ArrayList mainlist(My720DTO entity, String where_sql,String v_login_id,String v_login_name,Connection conn) throws SQLException {
        java.sql.PreparedStatement pstmt = null;
        ResultSet rs = null;
        StringBuffer sql = new StringBuffer();
        ArrayList arrBeans = new ArrayList();
	...
	//게시물 조회시
    public My720DTO select(My720DTO entity, Connection conn) {
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        StringBuffer sql = new StringBuffer();
        StringBuffer sql1 = new StringBuffer();
        My720DTO dto = new My720DTO();

        try {
			//조회수 증가시키는 부분 : 보낸 쪽지함에서는 생략임
			...
			sql1.append(" BRD_VIEWCNT, BRD_PASSWD, REF, REF_STEP, REF_LEVEL, YN_DEL_CREATOR ");
...
//	보관함 조회시 : 
	  public My720DTO keep(My720DTO entity, Connection conn) {
		  PreparedStatement pstmt = null;
		  ResultSet rs = null;
		  StringBuffer sql = new StringBuffer();
		  StringBuffer sql1 = new StringBuffer();
...
//	System.out.println("710DAO");
    ...
    public long insert(My720DTO dto, Connection conn) {
        PreparedStatement pstmt = null;
    ...
    public long rep_insert(My720DTO dto, Connection conn) {
        PreparedStatement pstmt = null;
        StringBuffer sql = new StringBuffer();
        StringBuffer sql2 = new StringBuffer();
        ...
        try {
            ...
            pstmt.setString(8, StringUtil.strReplace(StringUtil.strReplace(dto.getBrd_contents(), "<P>", ""), "</P>", "<br>"));
            pstmt.setString(9,dto.getReal_file());
            pstmt.setString(10,dto.getSys_file());
            pstmt.setLong(11,dto.getBrd_viewcnt());
            pstmt.setString(12,dto.getBrd_passwd());
            pstmt.setLong(13,dto.getRef());
            pstmt.setLong(14,dto.getRef_step() + 1);
            pstmt.setLong(15,dto.getRef_level() + 1);
    ...
    public long update(My720DTO dto, Connection conn) {
        PreparedStatement pstmt = null;
        StringBuffer sql = new StringBuffer();
        ResultSet rs = null;
        long ret = 0;
    ...
    public long delete(My720DTO dto, Connection conn)  {
        PreparedStatement pstmt = null;
        StringBuffer sql = new StringBuffer();
        long ret = 0;
	...
	public long delete2(My720DTO dto, Connection conn)  {
			PreparedStatement pstmt = null;
			StringBuffer sql = new StringBuffer();
			long ret = 0;
