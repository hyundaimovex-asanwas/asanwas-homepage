			...
			if (ds1 != null) {
				...
				for(int i = 0; i < rows.length; i++) {
					...
					if(rows[i].getJobType()==GauceDataRow.TB_JOB_UPDATE) {
						...
						cstmt.setString     (4, fParamEmpno);							//수정사번
						cstmt.setString     (5, fParamIp);								//수정ip
						cstmt.setInt        (6, rows[i].getInt(colNm_D_RSV_SID));		//받는사람 예약SID
						//StringUtil.printMsg("6",rows[i].getInt(colNm_D_RSV_SID) , this);
						cstmt.setInt        (7, rows[i].getInt(colNm_D_PAY_AMT));		// 대체 할 금액
