...
public class Rv540I extends SuperServlet {
				...
				if(ds1!=null){
					switch(dsType){
						case 1:
							...
							//StringUtil.printMsg("call","SALES.PR_RV540i_01",this);
//							cstmt = conn.prepareCall("{call SALES.PR_RV530i_01(0, '', '20060101', '20061231') }");
							cstmt = conn.prepareCall("{call SALES.PR_RV540I_01(?, ?, ?, ?) }");
	...
	public void apply(GauceDBConnection conn, GauceRequest req, GauceResponse res)  throws ServletException, Exception {
		...
		try{
			if(inputDs!=null){
				...
				int colNm_TsalAmt               = inputDs.indexOfColumn("TSAL_AMT");
				...
				for(int i=0; i<rows.length; i++){
					...
					case GauceDataRow.TB_JOB_INSERT:
						...
						cstmt = conn.prepareCall("{call SALES.PR_RV540i_02(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) }");
						...
						cstmt.setInt(9, rows[i].getInt(colNm_SalAmt));
						cstmt.setInt(10, rows[i].getInt(colNm_TsalAmt));
						cstmt.setInt(11, rows[i].getInt(colNm_TaxAmt));
						cstmt.setString(12, rows[i].getString(colNm_Reason));
						cstmt.setString(13, rows[i].getString(colNm_Desc));
						...
						if (cstmt.getString(17).equals("Y") ) {
							if ( !cstmt.getString(18).equals("0000") ) {
								...
								row2.addColumnValue(cstmt.getString(19));
