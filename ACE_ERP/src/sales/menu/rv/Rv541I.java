...
public class Rv541I extends SuperServlet {
				...
				if(ds1!=null){
					switch(dsType){
						case 1:
							...
							StringUtil.printMsg("call","SALES.PR_RV541i_01",this);
//							cstmt = conn.prepareCall("{call SALES.PR_RV541i_01(0, '', '20060101', '20061231') }");
							cstmt = conn.prepareCall("{call SALES.PR_RV541I_01(?, ?, ?, ?, ?) }");
			...
			if(inputDs!=null){
				...
				for(int i=0; i<rows.length; i++){
					...
					case GauceDataRow.TB_JOB_UPDATE:
						StringUtil.printMsg("call","SALES.PR_RV541i_02-UPDATE",this);
						cstmt = conn.prepareCall("{call SALES.PR_RV541i_02(?, ?, ?, ?, ?, ?, ?, ?, ?) }");
