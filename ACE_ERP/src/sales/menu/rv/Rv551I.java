...
public class Rv551I extends SuperServlet {
					...
					switch(dsType){
						case 1:
							...
							if ( gauceName.equals("DS1")) {
								//StringUtil.printMsg("call","SALES.PR_RV551i_01",this);
//								cstmt = conn.prepareCall("{call SALES.PR_RV550i_01(0, '', '20060101', '20061231') }");
								cstmt = conn.prepareCall("{call SALES.PR_RV551I_01(?, ?, ?, ?, ?) }");
			...
			if(inputDs!=null){
				...
				for(int i=0; i<rows.length; i++){
					...
					case GauceDataRow.TB_JOB_UPDATE:
						//StringUtil.printMsg("call","SALES.PR_RV551i_02-UPDATE",this);
						cstmt = conn.prepareCall("{call SALES.PR_RV551i_02(?, ?, ?, ?, ?, ?, ?, ?, ?) }");
