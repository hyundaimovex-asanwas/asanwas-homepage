...
public class Rv560I extends SuperServlet {
				...
				if(ds1!=null){
					switch(dsType){
						case 1:
							...
							//StringUtil.printMsg("call","SALES.PR_RV560i_01",this);
							cstmt = conn.prepareCall("{call SALES.PR_RV560I_01(?, ?, ?, ?) }");
			...
			if(inputDs!=null){
				...
				for(int i=0; i<rows.length; i++){
					...
					case GauceDataRow.TB_JOB_INSERT:
						...
						cstmt = conn.prepareCall("{call SALES.PR_RV560i_02(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?) }");
						...
						cstmt.setInt(10, rows[i].getInt(colNm_ARoomTypeSid));
						cstmt.setInt(11, rows[i].getInt(colNm_RoomTypeSid));						
						cstmt.setInt(12, rows[i].getInt(colNm_AUpjangSid));
						cstmt.setInt(13, rows[i].getInt(colNm_UpjangSid));						
						cstmt.setString(14, rows[i].getString(colNm_Reason));
						cstmt.setString(15, rows[i].getString(colNm_Desc));
						...
						if (cstmt.getString(19).equals("Y") ) {
							if ( !cstmt.getString(20).equals("0000") ) {
								//StringUtil.printMsg("성공메시지",cstmt.getString(21),this);
								GauceDataRow row2 = msgDS.newDataRow();									
								row2.addColumnValue(cstmt.getString(21));
								msgDS.addDataRow(row2);
							}
						} else {
							//StringUtil.printMsg("실패메시지",cstmt.getString(23),this);								
							conn.rollback();
							GauceDataRow row2 = msgDS.newDataRow();								
							row2.addColumnValue(cstmt.getString(21));
							msgDS.addDataRow(row2);
					...
					case GauceDataRow.TB_JOB_UPDATE:
						...
						if (cstmt.getString(19).equals("Y") ) {
							...
							//StringUtil.printMsg("실패메시지",cstmt.getString(21),this);								
							conn.rollback();
							GauceDataRow row2 = msgDS.newDataRow();								
							row2.addColumnValue(cstmt.getString(21));
							msgDS.addDataRow(row2);
