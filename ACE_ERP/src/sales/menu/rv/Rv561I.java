...
public class Rv561I extends SuperServlet {
				...
				if(ds1!=null){
					switch(dsType){
						case 1:
							...
							//StringUtil.printMsg("call","SALES.PR_RV561i_01",this);
							cstmt = conn.prepareCall("{call SALES.PR_RV561I_01(?, ?, ?, ?, ?) }");
			...
			if(inputDs!=null){
				...
				for(int i=0; i<rows.length; i++){
					...
					case GauceDataRow.TB_JOB_UPDATE:
						//StringUtil.printMsg("확정등급",rows[i].getString(colNm_RoomTypeSid),this);
						
						
						
						//StringUtil.printMsg("call","SALES.PR_RV521i_02-UPDATE",this);
						cstmt = conn.prepareCall("{call SALES.PR_RV561I_02(?,?,?,?,?, ?,?,?,?) }");
