...
public class Rv613I extends SuperServlet {
    ...
    public void makeSql(){
		...
		//Main 조회
		arr_sql[0].append ("{call SALES.PR_RV613I_01(?,?,?) }" );
		
		//Sub 조회
		arr_sql[1].append ("{call SALES.PR_RV613I_02(?,?) }" );
		
		//Main 저장
		arr_sql[2].append ("{call SALES.PR_RV613I_03(?, ?, ?, ?, ?, ?, ?, ?) }");
		
		//Sub 저장
		arr_sql[3].append ("{call SALES.PR_RV613I_04(?, ?, ?, ?, ?, ?, ?, ?, ?, ?) }");
   }
