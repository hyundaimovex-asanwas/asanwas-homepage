...
public class Sy510B extends SuperServlet {
...
//					StringUtil.printMsg("엑셀 생성파일 경로:",fExcelFilePath, this);
				...
				// 영업장정보
				if (ds1 != null) {
					StringUtil.printMsg("첫번째시트 : ", "ds1", this);
					res.enableFirstRow(ds1);
...
//StringUtil.printMsg("arr["+row+"]["+column+"]", arr[row][column], this);
							switch(column){
								...
								case 9:
								case 11:
									gRow.addColumnValue(Double.parseDouble(arr[row][column]));	// 숫자열
									break;
								default:
									gRow.addColumnValue(arr[row][column]);						// 기본값=문자
									break;
							}
...
//StringUtil.printMsg("row value ["+ row + "]", row, this);
				...
				// 동정보
				if (ds2 != null) {
					StringUtil.printMsg("두번째시트 : ", "ds2", this);
					...
					StringUtil.printMsg("====ROW", sheet0.getRows()+"행",this);
					String arr[][] = new String[sheet0.getRows()][60];
					...
					//DataSet Header
					ds2.addDataColumn(new GauceDataColumn("DONG_SID", 	    GauceDataColumn.TB_DECIMAL,10)); // 0
					...
					ds2.addDataColumn(new GauceDataColumn("UPJANG_SID",     GauceDataColumn.TB_DECIMAL,10)); // 2
					
					ds2.addDataColumn(new GauceDataColumn("YN",        	GauceDataColumn.TB_STRING, 3));
					/*
					 * 고객정보 데이터가 있는 엑셀을 읽어서 각 필드의 오류 체크를 한다 
					 * 오류체크를 하기면서 오류가 있는경우에는 화면에 색으로 표시를 해준다 
					 */
					for (int row = 1; row < sheet0.getRows(); row++) {
						...
						//StringUtil.printMsg("컬럼수?=", sheet0.getColumns()+1, this);
						...
						gRow.addColumnValue(Double.parseDouble(arr[row][6]));	//number
...
//					StringUtil.printMsg("=========ROW", sheet0.getRows()+"",this);
					...
					ds7.addDataColumn(new GauceDataColumn("VEND_TYPE",      GauceDataColumn.TB_STRING, 1)); // 2
					ds7.addDataColumn(new GauceDataColumn("VEND_ID",        GauceDataColumn.TB_STRING, 15)); // 2
...
//								StringUtil.printMsg("INSERT",this);                                        
					...
					if ("EXCELDS7".equals(gauceName)) {                                                    
						...
						int colNm_UseYn  		= ds1.indexOfColumn("USE_YN");
						int colNm_VendType 		= ds1.indexOfColumn("VEND_TYPE");
						int colNm_VendId  		= ds1.indexOfColumn("VEND_ID");
...
//								StringUtil.printMsg("INSERT",this);                                        
								...
								if(!cstmt.getString(27).equals("Y"))
									throw new Exception (cstmt.getString(29));
								
								if(ds_result!=null){
									GauceDataRow row1 = ds_result.newDataRow();
									row1.addColumnValue(cstmt.getString(27));
									row1.addColumnValue(cstmt.getString(28));
									row1.addColumnValue(cstmt.getString(29));
