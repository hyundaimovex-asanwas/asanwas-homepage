...
public class Lc025 extends SuperServlet {
	...
	public void makeSql(){
		...
		//Main 조회
		arr_sql[0].append ("{call SECURITY.PR_LC025S_01(?,?,?,?) }" );
		
		
		// 저장
		arr_sql[2].append ("{call SECURITY.PR_LC025I_01(?,?,?,?,?,?) }" );
		...
		arr_sql[3].append ("	INSERT INTO EMMADB.DBO.EM_SMT_TRAN( 																	\n")
		            ...
		            .append ("	INSERT INTO EMMADB.DBO.EM_SMT_TRAN( 																	\n")
					.append ("		MT_REFKEY,                               	 	 													\n")
					.append ("		DATE_CLIENT_REQ,                                		  											\n")
					.append ("		CONTENT,                             																\n")
					.append ("		CALLBACK, 																							\n")
		...
		// 삭제
		arr_sql[4].append ("{call SECURITY.PR_LC025D_01(?,?,?,?,?) }" );		
	...
	public void apply(Connection conn, HttpGauceRequest greq, HttpGauceResponse gres)  throws ServletException, Exception {
		...
		try{
				...
				for(int i=0; i<rows.length; i++){
					...
					switch(rows[i].getJobType()){
						...
						case GauceDataRow.TB_JOB_INSERT:
							...
							cstmt.setString(6, rows[i].getString(colNm_STATUS));									
			...
			if (ds3 != null) {
			        ...
			        //StringUtil.printMsg("v_phone::",v_phone,this);							

					if(v_status.equals("Y")){
						Message = "[현대아산]" + v_drivedt + " " + v_from_tm +"시-" + v_to_tm +"시 " +v_car + "사용 승인 완료. 운행후 꼭 일지등록바랍니다.";
					}else if(v_status.equals("N")){
						Message = "[현대아산]" + v_drivedt + " " + v_from_tm +"시-" + v_to_tm +"시 " +v_car + "사용 반려되었습니다.";						
					}
			
						Message2 = "[현대아산]차량 운행시 흡연 절대금지. 적발시 배차제한. 외부주유시 리터(L)기준 주유";					
					...
					cstmt_ms.setString(3, Message2);
					cstmt_ms.setString(4, v_phone);					
