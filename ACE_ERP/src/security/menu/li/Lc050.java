...
public class Lc050 extends SuperServlet {
	...
	public void makeSql(){
		...
		//Main 조회
		arr_sql[0].append ("{call SECURITY.PR_LC050S_00(?,?) }" );			
		
		arr_sql[1].append ("{call SECURITY.PR_LC050S_01(?,?) }" );
		
		arr_sql[2].append ("{call SECURITY.PR_LC050S_02(?,?) }" );		
		...
		try {
          ...
          //Component 로부터 전송된 입력스트림의 Output GauceDataSet의 배열을 반환
            ...
            for(int i=0; i<dSets.length;i++){
            	...
            	if(dSets[i].getName().equals("ds_calendar")){
            		
					GauceDataSet ds1 = gis.read("DS_CALENDAR");
