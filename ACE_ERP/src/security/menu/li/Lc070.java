...
public class Lc070 extends SuperServlet {
	...
	public void makeSql(){
		...
		//Main 조회
		arr_sql[1].append ("{call SECURITY.PR_LC070S_01(?,?,?,?,?,?) }" );			

		arr_sql[2].append ("{call SECURITY.PR_LC070S_02(?,?) }" );		
	...
	public void select(Connection conn, HttpGauceRequest greq, HttpGauceResponse gres)  throws ServletException, Exception {
		...
		String v_enono    = HDUtil.nullCheckStr(greq.getParameter("v_enono")); 					// 사번
		String v_status   = HDUtil.nullCheckStr(greq.getParameter("v_status")); 				// 작성여부		
		String v_carmodel = HDUtil.nullCheckStr(greq.getParameter("v_carmodel")); 				// 자동차 종료
          ...
          //Component 로부터 전송된 입력스트림의 Output GauceDataSet의 배열을 반환
            ...
            for(int i=0; i<dSets.length;i++){
            	...
            	if(dSets[i].getName().equals("ds_default")){
					...
					cstmt.setString(3, v_enono);					
					cstmt.setString(4, v_status);					
					cstmt.setString(5, v_carmodel);	
