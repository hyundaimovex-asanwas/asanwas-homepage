    ...
    public static GauceDataSet getDataSet(ResultSet rs, GauceDataSet ds) throws ServletException, Exception{
		...
		for (int i = 0; i < columnName.length; i++) {
			...
			if(rsmd.getColumnTypeName(i+1).equals("NUMBER") ){
				...
				//StringUtil.printMsg(" 컬럼명"+i+"===", columnName[i] ,this);
				//System.out.println("columnName[i]:: "+columnName[i] );
				
				if(columnName[i].equals("ATT_FILE")){
					ds.addDataColumn(new GauceDataColumn(columnName[i], GauceDataColumn.TB_URL));
				}else{
			...
			for(int i=0; i<columnName.length; i++) {
				...
				if(rsmd.getColumnTypeName(i+1).equals("NUMBER") ){
					...
					if(columnName[i].equals("ATT_FILE")){
						...
						//System.out.println( "columnName[i]::"+columnName[i]+":getString ===" + buf.toString()+"rsmd.getColumnDisplaySize(i+1)"+rsmd.getColumnDisplaySize(i+1) );
						ds.put(columnName[i], HDUtil.nullCheckStr(buf.toString()),rsmd.getColumnDisplaySize(i+1) );
						
						
					}else{
						
						//System.out.println( "columnName[i]::"+columnName[i] +":getString ===" + rs.getString(columnName[i])+"rsmd.getColumnDisplaySize(i+1)"+rsmd.getColumnDisplaySize(i+1));
