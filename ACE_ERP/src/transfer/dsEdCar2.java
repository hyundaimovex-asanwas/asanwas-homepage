...
public class dsEdCar2 extends HttpServlet {
...
**********************************************************************************************/
        ...
        String str5 = GauceReq.getParameter("gstr5");   
				...
				String[] strArrCN = new String[]{ "FLAG","CAR_SEQ_NO", "USERNM", "LD_CARGONM", "OFF_CARGONM", "CARTYPENO","ORDER_NO","CAR_NAME",
					
				"CARGO_TYPE","CAR_CBM","CAR_TON","CTN_STDRD","IN_OUT","DIV_IO","EDT_DATE","USERNM"}; 

				int[] intArrCN = new int[] { 1, 10, 30, 150, 150, 10,10,50,10,13,13,50,1,1,8,30}; 
			
				int[] intArrCN2 = new int[]{-1, -1, -1, -1,  -1, -1,-1,-1,-1,2,2,-1,-1,-1,-1,-1}; 
				...
				if (isNotOnlyHeader) {					
					...
					sql.append(" SELECT DISTINCT '' AS FLAG, A.CAR_SEQ_NO,C.USERNM,  LD_CARGONM, OFF_CARGONM, CARTYPENO,IFNULL(X.ORDER_NO,'') AS ORDER_NO,CAR_NAME, \n")
						 ...
						 .append("				IFNULL(A.EDT_DATE ,'') AS EDT_DATE, C.USERNM \n")
						 .append("	 FROM TRANSFER.LTORDCAR A \n")				
						 .append("				LEFT JOIN TRANSFER.LTORDERMST M ON M.ORDER_NO = A.ORDER_NO \n")
						 .append("				LEFT JOIN TRANSFER.LTORDERDTL X ON X.ORDER_NO = A.ORDER_NO \n")
