...
public class tr00101_s9 extends HttpServlet {
	...
	String dirRealPath = request.getRealPath("/"); 
	//String filepath = "\\Root\\HdAsan\\upload_file\\";
	String filepath = dirRealPath+"\\upload_file\\";	
	String filename = "Packing List";
	
	//String dirPath = request.getContextPath(); 
	//String dirRealPath = request.getRealPath("/"); 
	
	//System.out.println("getRealPath::"+dirRealPath);
	...
	try {
		...
		// System.out.println("jys::step-2");
		conn = DriverManager.getConnection("jdbc:as400:203.242.32.198;user=jeong;password=hd12345$");
		**/
		
		HDConnJndi JndiConn = new HDConnJndi();
	    conn  = JndiConn.getHdConnection(request, response);
    ...
    if (client.indexOf("MSIE 5.5") != -1) {
		...
		if (stype.equals("A")) {
			...
			sheet01.setColumnWidth((short)6,(short)0);
		} else {
			sheet01.setColumnWidth((short)4,(short)0);
		...
		if (stype.equals("A")) workbook.setSheetName(0 , "개성 물품 리스트" ,HSSFWorkbook.ENCODING_UTF_16);
		else workbook.setSheetName(0 , "금강산 물품 리스트" ,HSSFWorkbook.ENCODING_UTF_16);
		...
		if (stype.equals("A")) cell00.setCellValue("개성 반출 물품 리스트");
		else cell00.setCellValue("금강산 반출 물품 리스트");
		...
		cell0100.setCellValue("( 반출일 : " + edate + " )");
		...
		cell0400 = row04.createCell((short)1);
		cell0400.setEncoding(HSSFCell.ENCODING_UTF_16); 
		cell0400.setCellStyle(style02);
		cell0400.setCellValue("주문번호");

		cell0400 = row04.createCell((short)2);
		cell0400.setEncoding(HSSFCell.ENCODING_UTF_16); 
		cell0400.setCellStyle(style02);
		cell0400.setCellValue("신청업체명");

		cell0400 = row04.createCell((short)3);
		cell0400.setEncoding(HSSFCell.ENCODING_UTF_16); 
		cell0400.setCellStyle(style02);
		cell0400.setCellValue("화주명");
		...
		for (int k=0; k<23; k++) {
		 ...
		 //System.out.println("jys::step-14");

		String sqlSheet = "";
		sqlSheet = " SELECT NVL(A.ENG_ARTCNM,'') ENG_ARTCNM, A.ARTC_NM, RTRIM(A.STN_SZNM) STN_SZNM, \n"
		         + " (SELECT MINORNM FROM TRANSFER.LTLCODE WHERE MAJORCD = '0008' AND  MINORCD = A.ARTC_UNIT) MINORNM, \n"
		         + "        A.ARTC_CNT, A.UNIT_PRICE, A.PRICE, \n"
						 + "        A.PUNIT_CNT, A.PUNIT_WGHT, A.PKG_LNGTH, A.PKG_HEIGHT, A.PKG_WIDTH, \n"
						 + "        NVL(ROUND(A.PKG_LNGTH*A.PKG_HEIGHT*A.PKG_WIDTH/1000000,3),0) CBM, \n"
						// + "        IFNULL(ROUND(A.ARTC_CNT/A.PUNIT_CNT,2),0) P_CNT, \n"
						 //+ "        IFNULL(ROUND(ROUND(A.ARTC_CNT/A.PUNIT_CNT,1)*PKG_CBM,1),0) TOT_CBM, \n"
						 //+ "        IFNULL(ROUND(A.ARTC_CNT/A.PUNIT_CNT*A.PUNIT_WGHT,0),0) TOT_WGHT, A.CANCLE_STS, \n"     
						 + "  CASE WHEN A.PUNIT_CNT = 0  THEN 0                                                 \n"
						 + "  		 ELSE ROUND(A.ARTC_CNT/A.PUNIT_CNT,2)END  P_CNT,  \n"
						 + "  CASE WHEN A.PUNIT_CNT = 0  THEN 0                                                 \n"
						 + "  		 ELSE ROUND(ROUND(A.ARTC_CNT/A.PUNIT_CNT,1)*PKG_CBM,1) END TOT_CBM,           \n"
						 + "  CASE WHEN A.PUNIT_CNT = 0  THEN 0                                                 \n"
						 + "  		 ELSE ROUND(A.ARTC_CNT/A.PUNIT_CNT*A.PUNIT_WGHT,0) END TOT_WGHT, A.CANCLE_STS,\n"
        						 + "        A.ORDER_NO, D.VEND_NM, E.VEND_NM \n"
						 + "   FROM TRANSFER.LTORDERDTL A, \n"
						 + "        TRANSFER.LTORDERMST C, \n"
						 + "        ACCOUNT.GCZM_VENDER D, \n"
						 + "        ACCOUNT.GCZM_VENDER E \n"
						 + "  WHERE A.ORDER_NO IN (" + gorder_no + ") \n"
						 + "    AND A.ORDER_NO = C.ORDER_NO \n"
						 + "    AND C.CUST_CD = D.VEND_CD \n"
						 + "    AND C.SHIPPER = E.VEND_CD \n"
						 + "  ORDER BY A.ORDER_NO, TO_NUMBER(ORDER_SEQ) ";
		...
		if (!gorder_no.equals("")) {
			...
			while(rs.next()) {
				...
				cell1000.setEncoding(HSSFCell.ENCODING_UTF_16); 
				cell1000.setCellStyle(style08);
				cell1000.setCellValue(rs.getString(18));

				cell1000 = row07.createCell((short)2);
				cell1000.setEncoding(HSSFCell.ENCODING_UTF_16); 
				cell1000.setCellStyle(style08);
				cell1000.setCellValue(rs.getString(19));

				cell1000 = row07.createCell((short)3);
				cell1000.setEncoding(HSSFCell.ENCODING_UTF_16); 
				cell1000.setCellStyle(style08);
				cell1000.setCellValue(rs.getString(20));
		...
		String realFileName = filepath+filename+".xls";
