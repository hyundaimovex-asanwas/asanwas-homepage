...
public class tr00111_excel_ci extends HttpServlet {
	...
	String filename = "Commcial_Invoice";
    ...
    if (client.indexOf("MSIE 5.5") != -1) {
		...
		workbook.setSheetName(0 , "Commercial Invoice" ,HSSFWorkbook.ENCODING_UTF_16);
    ...
    style02.setAlignment(HSSFCellStyle.ALIGN_LEFT);
		...
		cell00.setCellValue("COMMERCIAL INVOICE");
    ...
    style1000_c.setAlignment(HSSFCellStyle.ALIGN_CENTER);
		...
		//Region(int 시작row, short 시작col, int 종료row, short 종료col) 
		sheet.addMergedRegion(new Region(1,(short)2,0,(short)4)); 
		...
		//Region(int 시작row, short 시작col, int 종료row, short 종료col) 
		sheet.addMergedRegion(new Region(4,(short)2,22,(short)4)); 
		...
		cell23.setEncoding(HSSFCell.ENCODING_UTF_16); 
		cell23.setCellStyle(style0100);
		cell23.setCellValue("13.단가");

		cell23 = row23.createCell((short)4);
		cell23.setEncoding(HSSFCell.ENCODING_UTF_16); 
		cell23.setCellStyle(style0100);
		cell23.setCellValue("14.금액");
		...
		//Region(int 시작row, short 시작col, int 종료row, short 종료col) 
		sheet.addMergedRegion(new Region(28,(short)0,28,(short)4)); 
		...
		//Region(int 시작row, short 시작col, int 종료row, short 종료col) 
		sheet.addMergedRegion(new Region(30,(short)0,30,(short)4)); 
		...
		cell50.setCellValue("16.서명");

		//Region(int 시작row, short 시작col, int 종료row, short 종료col) 
		sheet.addMergedRegion(new Region(50,(short)3,50,(short)4)); 
		...
		**************************************************************/
		...
		cell200.setCellValue("COMMERCIAL INVOICE");
		...
		cell201.setEncoding(HSSFCell.ENCODING_UTF_16); 
		cell201.setCellStyle(style1100_c);
		cell201.setCellValue("단가(원)");

		cell201 = row201.createCell((short)7);
		cell201.setEncoding(HSSFCell.ENCODING_UTF_16); 
		cell201.setCellStyle(style1100_c);
		cell201.setCellValue("금액(원");
		...
		query = " SELECT A.CAR_NO, A.PACKING_CNT, A.GOODS_NM, A.STDRD_SZ, B.MINORNM, " +
						"				 A.GOODS_CNT, A.UNIT_PRICE, A.PRICE, A.REAMARK " +
		...
		while(rs.next()) {
			...
			for (int s=0; s<rst2.length; s++) {
				...
				cell202 = row202.createCell((short)9);
		...
		HSSFCell cell204 = null;

		for (int u=0; u<9; u++) {
			cell204 = row204.createCell((short)u);
