...
<!--
	// 이미지  창 띄우는 스크립트 
	 function viewGallery(idx) {
	  ...
	  window.open('cyber_gallery_pop.jsp?idx='+idx,'AsanGallery','left='+cx+',top='+cy+',width=620,height=600,fullscreen=0,toolbar=0,location=0,directories=0,status=0,menubar=0,scrollbars=0,resizable=0');
	 }
							...
							<% if( i < num && num -  i % num != 0 ) { %>
								<td>
									<table width="150" border="1" cellspacing="0" cellpadding="0">
										<tr>
											<td align="center" height="150"><a href="javascript:viewGallery('<%= hm.get("IDX") %>');"><img src="/asanway/common/download.jsp?filename=<%= Utility.ko((String)hm.get("IMGS"))%>&dir=gallery&sysfilename=<%= Utility.ko((String)hm.get("SYSIMGS"))%>" border="0" width="130" height="150"></a>
										<tr>
						...
						<Table border="0" cellspacing="0" cellpadding="0" align="center">
							<tr>
								...
								<%if(helper.getBeforegroup() > 0){%>
									<a href="/asanway/cyber/cyber_gallery_list.jsp?page=<%=helper.getBeforegroup()%>"><img src="../img/bt_05.gif" width="36" height="11" hspace="8"></a>
								<% } else { %>
							 ...
							 -->
								<td>
									<%if(helper.getAftergroup() > 0){%>
										<a href="/asanway/cyber/cyber_gallery_list.jsp.jsp?page=<%=helper.getAftergroup()%>"><img src="../img/bt_06.gif" width="36" height="11" hspace="8"></a>
									<% } else { %>
