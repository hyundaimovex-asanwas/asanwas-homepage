      ...
      <table width="850" height="100%" border="0" cellpadding="0" cellspacing="0">
		...
		%>
										...
										<tr align=center height=28 align=center height=28 style="cursor:hand" onmouseOver="this.style.backgroundColor='#E5E5E5';" onmouseout="this.style.backgroundColor='#FFFFFF';">
											   <td><%= num-- %></td>
											   <td align="left">
											   <a href="go4peace_qna_view.jsp?idx=<%= hm.get("IDX") %>&page=<%=currentP%>&category=go4qna&searchkey=<%=searchkey%>&searchvalue=<%=searchvalue%>" class="board_tx03"><%= (hm.get("TITLE") == null) ? "" : Utility.getStringcut( 28, Utility.ko((String)hm.get("TITLE")) ) %></a>
			...
			<Table border="0" cellspacing="0" cellpadding="0" align="center">
				<tr>
					...
					<%if(helper.getBeforegroup() > 0){%>
						<a href="go4peace_qna.jsp?category=go4qna&page=<%=helper.getBeforegroup()%>"><img src="/asanway/img/imjk_img/btn_prev.gif" hspace="7"></a>
					<% } else { %>
					...
					<td>
						...
						%>
							...
							<%  } else { %>
							<a href="go4peace_qna.jsp?category=go4qna&page=<%= i %>"><%= i %></a>&nbsp;
							<%  }
						  } %>
						<% if(totalP  == 0) { %>1<% } %>
					</td>
					<td>
						<%if(helper.getAftergroup() > 0){%>
							<a href="go4peace_qna.jsp?category=go4qna&page=<%=helper.getAftergroup()%>"><img src="/asanway/img/imjk_img/btn_next.gif" hspace="7"></a>
						<% } else { %>
							<img src="/asanway/img/imjk_img/btn_next.gif" hspace="7">
						<% } %>
					</td>
          				...
          				<tr>
          					<td align=right>
							<!--검색 START -->
    							<form name="search" method="get" action="go4peace_qna.jsp">
