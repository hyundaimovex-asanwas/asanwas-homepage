...
<%@ page import="kr.co.imjk.board.dao.KsBestQuestion"%>
...
//DAO
KsBestQuestion dao = new KsBestQuestion();
                ...
                <TD width=650 valign=top>
                	...
                	<tr>
                		<td>
							...
							<FORM METHOD=POST name="mainForm" ACTION="kstour_question_ok.jsp" >
							...
							<table width=100% border=0 cellspacing=0 cellpadding=0>
                				...
                				<tr>
                                 <td align=right>
                                  <%if(prev_board_seq>0){%> 
                                  <img src=<%=imageServer%>tour_img/btn_l.gif align=absmiddle><input type=button value='<<이전'  class=btn onclick="location.href='kstour_question_edit.jsp?board_seq=<%=prev_board_seq%><%=key_str%><%=page_str%>'" ><img src=<%=imageServer%>tour_img/btn_r.gif align=absmiddle><img src=<%=imageServer%>tour_img/btn_r.gif align=absmiddle style=margin-right:1;>
                                  <%}%>
								  <%if(next_board_seq>0){%>
								  <img src=<%=imageServer%>tour_img/btn_l.gif align=absmiddle><input type=button value='다음>>'  class=btn onclick="location.href='kstour_question_edit.jsp?board_seq=<%=next_board_seq%><%=key_str%><%=page_str%>'" ><img src=<%=imageServer%>tour_img/btn_r.gif align=absmiddle><img src=<%=imageServer%>tour_img/btn_r.gif align=absmiddle style=margin-right:1;>
								  <%}%>
								  </td>
                				</tr>
                			</table>
									...
									<table cellpadding="0" cellspacing="0" width="640">
										<tr>
											...
											<td background="<%=imageServer%>a_cellbg25.gif">
												<p>: <%if(prev_board_seq>0){%> <a href="kstour_question_edit.jsp?board_seq=<%=prev_board_seq%><%=key_str%><%=page_str%>">&nbsp;<%=Util.getStringLimit(prev_title.trim(),40)%></a> <%}else{%> 이전글이 없습니다. <% } %></p>
											</td>
										</tr>
										<tr>
											...
											<td>
												<p>: <%if(next_board_seq>0){%> <a href="kstour_question_edit.jsp?board_seq=<%=next_board_seq%><%=key_str%><%=page_str%>">&nbsp;<%=Util.getStringLimit(next_title.trim(),40)%></a> <%}else{%> 다음글이 없습니다. <% } %></p>
											</td>
										</tr>
									</table>
								</td>
                			...
                			<table width=100% border=0 cellspacing=0 cellpadding=0>
                				...
                				<tr>
                                 <td align=right>
								  ...
								  <%}%>
								  <img src=<%=imageServer%>tour_img/btn_l.gif align=absmiddle><input type=button value=목록 class=btn onclick="location.href='kstour_question.jsp?board_seq=<%=board_seq%><%=key_str%><%=page_str%>'"><img src=<%=imageServer%>tour_img/btn_r.gif align=absmiddle>         					
                                 </td>
                				</tr>
                			</table>
                            </FORM>
