           ...
           %>						
						<tr align="center">
							...
							<% }else{ %>
								...
								<%=re_img%>
								<a href="event_view.jsp?board_seq=<%=board_seq%>&requestedPage=<%=requestedPage%>&listnum=<%=listNum%><%=key_str%>" class="listtxt05"><%=Util.getStringLimit((title).trim(),30)%>(<%=comment_num%>)</a><%=new_img%>
								</td>
								<td>
								   <%=reg_date_string%>
								</td>
						...
						<tr>
							...
							<td width="40%" align="center">
								<%=HtmlUtil.doPageCut(requestedPage, pageCount , position , 
			                      "event_list.jsp", "500", key_str ,
			                      imageServer+"/imjk_img/", blockPerPage)%>
			                </td>
							<td width="30%" align="right">
							<% if(!session_id.equals("")){ %>
							   <a href="event_edit.jsp"><img src="<%=imageServer%>/tour_img/event/s_event_1_lovego_bt.gif" hspace="5"></a>
						    <% }else{ %>
