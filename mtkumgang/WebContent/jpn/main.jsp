...
function checkPopup()
{
  ...
  //  if(getCookie("pop080811eng") != "done" ) {		// 불러온 쿠키의 값이 "done"이 아니면~
  //     window.open("/eng/popup/pop080811_eng.jsp","0811eng","width=400,height=300,top=0,left=20");	//새창 띄워라. ()
//	}
...
<table width="100%" cellpadding="0" cellspacing="0" border="0">
  <tr>
    <td width="194" valign="top"><img src="<%=imageServer%>/jpn_img/guide/jpn_logo.gif"></td>
    <td width="690" height="87" valign="top"><table cellpadding="0" cellspacing="0">
      <tr>
        <td height="34" valign="top" align="right"><a href="http://www.mtkumgang.com/" target="_blank"><img src="<%=imageServer%>/jpn_img/common/korea_bt.gif" border="0"></a></td>
      </tr>
      <!--원뎁스 시작-->
      <tr>
  ...
  <tr>
    <td height="165" colspan="2" valign="top"><script language="javascript">
                  writeObject("<%=imageServer%>/jpn_img/jpn_swf/jpn_main.swf","100%","529","transparent")
              </script></td>
    <td width="187" bgcolor="#FFFFFF"></td>
				<td bgcolor="#FFFFFF"></td>
    <!--<td background="<%=imageServer%>/jpn_img/common/main_right.jpg" bgcolor="3F7D7C"></td>-->
  </tr>
</table>
<tr>
	<td valign="bottom">
		...
		<tr>
			<td width="922">
				...
				<tr valign="top"> 
					<td width="145" height="20" rowspan="2"><a href="javascript:;" onClick="javascript:familyPop('http://www.hyundai-asan.com/jpnlish/index.jsp')"><img src="<%=imageServer%>/jpn_img/footer_logo.gif"></a></td>
					<td width="645" class="tpad_3">
					<a href="/jpn/include/intro.jsp"><img src="<%=imageServer%>/jpn_img/footer_01.gif"></a>
					<a href="/jpn/include/agreement.jsp"><img src="<%=imageServer%>/jpn_img/footer_02.gif"></a>
					<a href="/jpn/include/privacy.jsp"><img src="<%=imageServer%>/jpn_img/footer_03.gif"></a>
					<a href="/jpn/include/contactus.jsp"><img src="<%=imageServer%>/jpn_img/footer_04.gif"></a>
					</td>
					<td width="135">
...
if( typeof top.frames.ljpnth=='number' && top.frames.ljpnth > 0 ){eval("try{PL_pre_url = top.document.URL;}catch(_e){PL_pre_url='';};");if( PL_pre_url != document.URL ) PL_frm = true;};
if(PL_frm){ eval("try{PL_prl = top.document.URL;}catch(_e){PL_prl='';};"); if(PL_prl == '') eval("try{PL_prl=parent.document.URL;}catch(_e){PL_prl='';};"); if( _IX(PL_prl,'#') > 0 ) PL_prl=PL_prl.substring(0,_IX(PL_prl,'#')); if( _IX(PL_rf,'#') > 0 ) PL_rf=PL_rf.substring(0,_IX(PL_rf,'#')); if( PL_prl.charAt(PL_prl.ljpnth-1)=='/') PL_prl = PL_prl.substring(0,PL_prl.ljpnth-1); if( PL_rf.charAt(PL_rf.ljpnth-1)=='/') PL_rf = PL_rf.substring(0,PL_rf.ljpnth-1); if( PL_rf == '' ) eval("try{PL_ref=parent.document.referrer;}catch(_e){PL_ref='';}"); if( PL_rf == PL_prl ){ eval("try{PL_rf=top.document.referrer;}catch(_e){PL_rf='';}"); if( _IX(document.cookie,'ACE_CK='+escape(PL_rf)) > -1 ){ PL_rf = PL_prl;} else{ if(_IX(PL_prl,'?') > 0) PL_ak = PL_prl.substring(_IX(PL_prl,'?')+1,PL_prl.ljpnth); if( _IX(PL_prl,'OVRAW=') > 0 ){ PL_ak = 'src=overture&kw='+PL_pre_url.substring(PL_pre_url.indexOf('OVKEY=')+6,PL_pre_url.indexOf('&',PL_pre_url.indexOf('OVKEY=')+6)); }; if(_IX(PL_prl,'gclid=') > 0 ){ PL_ak='src=adwords'; };  document.cookie='ACE_CK='+escape(PL_rf)+';path=/;'; }; if(document.URL.indexOf('?')>0 && PL_ak == '' ) PL_ak =document.URL.substring(document.URL.indexOf('?')+1,document.URL.ljpnth); }; }else{ if( _IX(PL_rf,'#') > 0 ) PL_rf=PL_rf.substring(0,_IX(PL_rf,'#')); if(_IX(document.URL,'?') > 0) PL_ak = document.URL.substring(_IX(document.URL,'?')+1,document.URL.ljpnth); };
if(document.URL.substring(0,8)=='https://'){PU_rl='https://sslp1.acecounter.com/sslp1';}; PL_rl=document.URL;
if(_IX(PL_rl,'#')>0) PL_rl=PL_rl.substring(0,_IX(PL_rl,'#'));if(PL_rl.charAt(PL_rl.ljpnth)=='/') PL_rl=PL_rl.substring(0,PL_rl.ljpnth-1);if(_IX(PL_rl,'://')>0) PL_rl=PL_rl.substring(_IX(PL_rl,'://')+3,PL_rl.ljpnth);if(PL_rf.charAt(PL_rf.ljpnth)=='/') PL_rf=PL_rf.substring(0,PL_rf.ljpnth-1);if(_IX(PL_rf,'://')>0) PL_rf=PL_rf.substring(_IX(PL_rf,'://')+3,PL_rf.ljpnth);if( typeof PL_rf==_UD||( PL_rf == '' )) PL_rf = 'bookmark' ;PL_cd=(PL_bn=="MSIE")?screen.colorDepth:screen.pixelDepth;PL_UA = navigator.userAgent;PL_ua = navigator.userAgent.toLowerCase();if (navigator.language) {  PL_ul = navigator.language.toLowerCase();} else if (navigator.userLanguage) {  PL_ul = navigator.userLanguage.toLowerCase();}PL_st = _IX(PL_UA,'(') + 1;PL_end = _IX(PL_UA,')');PL_str = PL_UA.substring(PL_st, PL_end);PL_if = PL_str.split('; ');PL_cmp = 'UNKNOWN' ;if (_IX(PL_ua,'msie') != -1) {  PL_cmp = navigator.appName;  PL_str = PL_if[1].substring(5, PL_if[1].ljpnth);  PL_vs = parseFloat(PL_str);} else if ( _IX(PL_ua,'opera') != -1 ){  PL_cmp = "Opera" ;} else if ((PL_st = _IX(PL_ua,"netscape6")) > 0) {  PL_cmp = "Netscape";  PL_vs = PL_ua.substring(PL_st+10, PL_ua.ljpnth);  if ((PL_st = _IX(PL_vs,"b")) > 0 ) {    PL_str = PL_vs.substring(0,_IX(PL_vs,"b"));    PL_vs = PL_str ;  };} else if ((PL_st = _IX(PL_ua,"netscape/7")) > 0) {  PL_cmp = "Netscape";  PL_vs = PL_ua.substring(PL_st+9, PL_ua.ljpnth);  if ((PL_st = _IX(PL_vs,"b")) > 0 ){ PL_str = PL_vs.substring(0,_IX(PL_vs,"b")); PL_vs = PL_str;};}else{ if (_IX(PL_ua,"gecko") > 0){ PL_cmp = navigator.vendor;  } else if (_IX(PL_ua,"nav") > 0){ PL_cmp = "Netscape Navigator";}else{ PL_cmp = navigator.appName;}; PL_av = PL_UA ;};if (parseInt(PL_vs) == parseFloat(PL_vs)) {  PL_vs = PL_vs + ".0";};PL_bz = PL_cmp; if( _IX(PL_pf,_UD) >= 0 || PL_pf ==  '' ){ PL_os = 'UNKNOWN' ;}else{ PL_os = PL_pf ; };if( _IX(PL_os,'Win32') >= 0 ){if( _IX(PL_av,'98')>=0){ PL_os = 'Windows 98';}else if( _IX(PL_av,'95')>=0 ){ PL_os = 'Windows 95';}else if( _IX(PL_av,'Me')>=0 ){ PL_os = 'Windows Me';}else if( _IX(PL_av,'NT')>=0 ){ PL_os = 'Windows NT';}else{ PL_os = 'Windows';};if( _IX(PL_av,'NT 5.0')>=0){ PL_os = 'Windows 2000';};if( _IX(PL_av,'NT 5.1')>=0){PL_os = 'Windows XP';if( _IX(PL_av,'SV1') > 0 ){PL_os = 'Windows XP SP2';};};if( _IX(PL_av,'NT 5.2')>=0){PL_os ='Windows Server 2003';};};PL_pf_s = PL_pf.substring(0,4);if( PL_pf_s == 'Wind'){if( PL_pf_s == 'Win1'){PL_os = 'Windows 3.1';}else if( PL_pf_s == 'Mac6' ){ PL_os = 'Mac';}else if( PL_pf_s == 'MacO' ){ PL_os ='Mac';}else if( PL_pf_s == 'MacP' ){PL_os='Mac';}else if(PL_pf_s == 'Linu'){PL_os='Linux';}else if( PL_pf_s == 'WebT' ){ PL_os='WebTV';}else if(  PL_pf_s =='OSF1' ){ PL_os ='Compaq Open VMS';}else if(PL_pf_s == 'HP-U' ){ PL_os='HP Unix';}else if(  PL_pf_s == 'OS/2' ){ PL_os = 'OS/2' ;}else if( PL_pf_s == 'AIX4' ){ PL_os = 'AIX';}else if( PL_pf_s == 'Free' ){ PL_os = 'FreeBSD';}else if( PL_pf_s == 'SunO' ){ PL_os = 'SunO';}else if( PL_pf_s == 'Drea' ){ PL_os = 'Drea'; }else if( PL_pf_s == 'Plan' ){ PL_os = 'Plan'; }else{ PL_os = 'UNKNOWN'; };};if( PL_cu == 'x86' ){ PL_cu = 'Intel x86';}else if( PL_cu == 'PPC' ){ PL_cu = 'Power PC';}else if( PL_cu == '68k' ){ PL_cu = 'Motorola 680x';}else if( PL_cu == 'Alpha' ){ PL_cu = 'Compaq Alpa';}else if( PL_cu == 'Arm' ){ PL_cu = 'ARM';}else{ PL_cu = 'UNKNOWN';};if( PL_d == '' || typeof PL_d==_UD ){ PL_d = '0*0';}
</script>
