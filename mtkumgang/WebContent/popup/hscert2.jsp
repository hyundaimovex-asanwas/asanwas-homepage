...
//	String hs_cert_svc_tx_seqno = "291200020091";				// 거래일련번호 ( 20자리의 문자열. 0-9,A-Z,a-z 사용.)
	...
	** 별도 생성하여 사용할 경우 동일 거래번호를 두번 사용할 수 없는 것에 주의하여 작성. **/
	...
	String mbphn_no = request.getParameter("aed");						// 아이디로 대체
...
//String return_url = "http://localhost:9080/newPwd3.jsp?SAed="+aed+"&ssID="+ssID;	// ** 본인인증 완료후 리턴될 URL (도메인 포함 full path)
String return_url = "http://erp.hdasan.com/services/newPwd3.jsp?SAed="+aed+"&ssID="+ssID;	// ** 본인인증 완료후 리턴될 URL (도메인 포함 full path)
...
<form name="form1">
  ...
  <!-- 필수 항목 //-->
  <!--// 선택 항목 -->
<input type="hidden" name="email" value="<%=aed%>">			  <%-- 이메일주소 (고객에게 본인인증 결과 메일을 송신할 경우) - 서비스 예정  --%>
