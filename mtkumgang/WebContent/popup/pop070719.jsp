...
if(!result.equals(ssID)){	//실패시
	...
	String clientIp = "203.242.32.195";												// 회원사 IP, InetAddress.getLocalHost().getHostAddress() 로 얻어올 수 있음.
	//String clientDomain = "www.mtkumgang.com";									// 회원사 도메인. (휴대폰인증번호 송신시 제휴사명에 노출)
	String clientDomain = "Hyundai Asan";									// 회원사 도메인. (휴대폰인증번호 송신시 제휴사명에 노출)
	...
	** 별도 생성하여 사용할 경우 동일 거래번호를 두번 사용할 수 없는 것에 주의하여 작성. **/
	...
	// # 리턴 URL 설정
	// ########################################################################
	String returnUrl = "http://ace.hdasan.com/newIndeX_act2.jsp?SAed="+aed+"&ssID="+ssID;	// ** 본인인증 완료후 리턴될 URL (도메인 포함 full path);
	//String returnUrl = "http://erp.hdasan.com/services/newIndeX_act2.jsp?SAed="+aed+"&ssID="+ssID;	// ** 본인인증 완료후 리턴될 URL (도메인 포함 full path);
	//String returnUrl = "http://www.hdasan.com/nember/safe_hs_cert3.jsp";// 본인인증 완료후 리턴될 URL (도메인 포함 full path)
	//okname 실행 정보
	// ########################################################################
