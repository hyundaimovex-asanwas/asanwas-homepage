...
<!--
  function PageMov(val) {
     if (val == "01"){
        document.form.action="res_srch3.jsp";
     }
  ...
  function paySum(val,vbl,vcl,vdl,vel,vfl) {
     ...
     if(pay_sum > 0){
         ...
         //eval(pay_button).innerHTML="<a href=\"javascript:;\" onclick=\"window.open ('popup_reservation_ticket.jsp?idx=<%=idx%>', 'movieview', 'width=518,height=600,scrollbars=yes,toolbar=no,location=no,directories=no,top=100,left=100,noresize');\"><img src='<%=imageServer%>/tour_img/btn/btn_ticketview.gif'></a>";
     }else{
         //eval(pay_button).innerHTML="<img src='<%=imageServer%>/tour_img/btn/btn_account.gif' class='btn' onclick=\"javascript:alert('결제하실 관광객을 선택해 주십시오.');return;\" onmouseover=\"this.style.cursor='hand'\" >";
         //eval(pay_button).innerHTML="<a href=\"javascript:;\" onclick=\"window.open ('popup_reservation_ticket.jsp?idx=<%=idx%>', 'movieview', 'width=518,height=600,scrollbars=yes,toolbar=no,location=no,directories=no,top=100,left=100,noresize');\"><img src='<%=imageServer%>/tour_img/btn/btn_ticketview.gif'></a>";
...
//alert(form2.u_pay_amt.value);
  ...
  function PayMov()
  {
	...
	function popConfirm(idx){
		var receiptURL ="res_print.jsp?rsv_sid=" + idx;
...
%>
            ...
            %>								
								...
								<tr align="center">
									...
									<td class="bg_white"><B>결제하기</B></td>
									<td class="bg_white">확인증</td>
								</tr>
	        <%
	            } //호텔변경시(1)...
	        %>								
								<tr align="center">
									...
									<%if( status_cd.equals("RA")  ||  status_cd.equals("DP")){%>
        									<%
        									  if (pay_yn.equals("Y")){	//완료인 경우 영수증 띄우기
        									%>
        									    <a href="javascript:popConfirm('<%=rsv_sid%>')"><img src="<%=imageServer%>/tour_img/btn/btn_s_view.gif" border=0></a>
        									<%
                                              }else{  
        									%>
        									    <img src="<%=imageServer%>/tour_img/btn/btn_s_view.gif" style="cursor:hand" onclick="alert('결제완료 후 확인증 출력이 가능합니다.');">
        									<%
                                              }                        
        									%>
      							    <%}%>
...
<form name="payForm" method="post">
   ...
   <input type="hidden" name="c_return_url" value="http://www.mtkumgang.com/preserv/cResultOnline2.jsp">  <!--리턴-->
