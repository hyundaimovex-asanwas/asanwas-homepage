...
<%
    try
    {
        ...
        cal.set(Integer.parseInt(today.substring(0,4)),Integer.parseInt(today.substring(4,6)) - 1,Integer.parseInt(today.substring(6)) + 12);
        year = String.valueOf(cal.get(Calendar.YEAR));       
        month = String.valueOf(cal.get(Calendar.MONTH) + 1); 
        ...
        for(int i = 0;i<list.length;i++)
        {
            ...
            if(i != list.length - 1) //마지막행이 아닐때
            {
                if(i == 0) //첫행일때
                {
                    ...
                    sb.append("          </room_type>\n"); // 한행 출력 완료
                    if(!tempDate.equals(list[i+1].getCurr_date()))//다음행과 날짜가 같지 않다면 ==>다음행부터 날짜가 바뀜,element를 닫아줌
                    {
                            sb.append("        </room_type_list>\n");
                            sb.append("      </upjang>\n");
                            sb.append("    </upjang_list>\n");
