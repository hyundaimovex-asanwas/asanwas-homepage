...
import com.hddutyfree.Df_item_bean;

/**
 *  *
 * 2015-05-18 수정 심동현
 */
public class Df_item_info  extends DAOHome{  
	...
	public int getCount(String part) throws  Exception{
		...
		query = new StringBuffer()
			...
			.append(" WHERE G_KIND='hddf' AND G_CORNER <> '02' AND IDX > 0              \n")
     ...
     * @author IMJK R&D  이소연 대리
     */
    public Df_item_bean[] getGoodsList(int requestedPage, int pageSize, String part) throws  Exception{
       
        Connection dbconn = getConnection();
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        Df_item_bean bean = null;
        ...
        sql.append(" SELECT RowNum as RNUM, X.* FROM ( \n");        
			sql.append(" SELECT IDX, G_CORNER, G_IMGFILE , G_NAME , G_STD , G_PRICE, G_MAKER, G_ETC, G_BIGO \n");
			sql.append(" FROM ASAN.NORTHGOODS WHERE G_KIND='hddf' AND G_CORNER <> '02' AND IDX > 0 \n");
			sql.append(  subSql);
			sql.append(" ORDER BY IDX ASC \n");
	        sql.append("      ) X \n");
	    sql.append(" )X1 \n");
        ...
        try {
            ...
            // 결과 담기
            rs = pstmt.executeQuery();
            list = Selecter.execute(rs, new Df_item_bean());
            rs.close();
            pstmt.close();
        } catch (SQLException e) {
            throw new DbException(e.getMessage());
        ...
        return (Df_item_bean [])list.toArray(new Df_item_bean[0]);        
    }
     ...
     * @author IMJK R&D  이소연 대리
     */
	public Df_item_bean [] getGoodsView(int idx) 
