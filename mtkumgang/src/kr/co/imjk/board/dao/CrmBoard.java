 ...
 * @modify 심동현
 *
 * 내가 정말 궁금한 것은 나름 멋지게 모듈을 만들어 놓고 
 * 왜 BOARD_TYPE 을 변수처리하지 않은 건지.. 왜 파라미터로 하지 않으셨을까요...
 * 이걸로 자바 게시판 하나 표준타입으로 수정해봐야것다.
 */

public class CrmBoard  extends DAOHome{
    ...
    public Tbrd_board_ma_bean[] getList(int pageSize, int requestedPage, String part, String keyword, String orderBy ) throws  Exception{
        ...
        if(chk_page > 0){
	        ...
	        query.append(" 		( SELECT BOARD_SEQ from asan.TBRD_BOARD_MA WHERE BOARD_TYPE = '14' AND USE_YN='1' \n");
	        query.append(          subSql              );
	        ...
	        query.append("       ) as tmp       ");
	        query.append(" ) AND \n");
	        query.append(" BOARD_TYPE = '14' AND USE_YN='1'  \n");
	        query.append(subSql);
	        query.append(orderbySql);
	        ...
	        query.append(" WHERE BOARD_TYPE = '14' AND USE_YN='1'  \n");
	        query.append(subSql);			
	        query.append(orderbySql);
        	query.append(" FETCH FIRST "+pageSize+" ROWS ONLY  \n");
        }
	...
	public int insert(Tbrd_board_ma_bean data) throws DbException , Exception {
		...
		try {     
			...
			if(data != null){	
				...
				pstmt.setString(11,data.getType_code());
     ...
     */
	 public int update(Tbrd_board_ma_bean data) throws DbException , Exception {
		...
		try { 
			...
			if(data != null){
				...
				pstmt.setInt(9,data.getBoard_seq());
	 ...
	 public Tbrd_board_ma_bean [] getPrevAndNextBoard(int board_seq) throws DbException , Exception {
		...
		sql.append(" WHERE BOARD_TYPE ='14' and USE_YN='1' and BOARD_SEQ = ?			");
