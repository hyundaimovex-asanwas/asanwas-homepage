 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class KsBestQuestion  extends DAOHome{
...
//		 페이징 처리
		...
		query.append(" SELECT * FROM ( SELECT RowNum AS RNUM, X.* FROM ( \n");
    		...
    		query.append(" WHERE BOARD_TYPE = '20' AND USE_YN='1' \n");
    		query.append(subSql);			
        ...
        if(chk_page > 0){
	        ...
	        query.append(" 		( SELECT BOARD_SEQ FROM asan.TBRD_BOARD_MA WHERE BOARD_TYPE = '20'  AND USE_YN='1' \n");
	        query.append(          subSql              );
	        query.append("        ORDER BY BOARD_SEQ DESC \n");
	        ...
	        query.append("       ) AS TMP       ");
	        query.append(" ) AND \n");
	        query.append(" BOARD_TYPE = '20' AND USE_YN='1' \n");
	        query.append(subSql);
	        query.append("   ORDER BY BOARD_SEQ DESC \n");
     ...
     */
	 ...
	 public Tbrd_board_ma_bean [] getPrevAndNextBoard(int board_seq) throws DbException , Exception {
		...
		sql.append(" WHERE BOARD_TYPE ='20' and USE_YN='1' and BOARD_SEQ = ?			");
