 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class KsTourNotice  extends DAOHome{  
    ...
    public Tbrd_board_ma_bean[] getList(int requestedPage, int pageSize, String part, String keyword ) throws  Exception{
        //System.out.println("getList () requestedPage:"+requestedPage);
        ...
        if(chk_page > 0){      
	        ...
	        .append(" 		( select board_seq from asan.TBRD_BOARD_MA where board_type='15' and USE_YN = '1' \n")
	        .append(subSql)			
	        ...
	        .append(" as tmp ) and \n")
	        .append(" board_type='15'  and USE_YN = '1' \n")
	        .append(subSql)	
	        .append(" order by REF_NO DESC,SORT_NO, LEVEL_NO , BOARD_SEQ DESC, BOARD_TOP_NO ")
	        .append(" fetch first "+pageSize+" rows only  \n").toString();*/
			//append(" ) X  WHERE X.Rnum > "+ (requestedPage-1)*pageSize +" and X.RNum <="+ requestedPage*pageSize );
        }else{         	
        	query = new StringBuffer()
        	.append(" SELECT BOARD_SEQ, REF_NO , LEVEL_NO , SORT_NO , TITLE, CONTENT, USER_ID , USER_NAME, USE_YN, \n") 
			.append(" BOARD_TOP_NO , READ_NUM , REG_DATE, BEST FROM ( \n")
        	.append(" SELECT RowNum as RNUM, BOARD_SEQ, REF_NO , LEVEL_NO , SORT_NO , TITLE, CONTENT, USER_ID , USER_NAME, USE_YN, \n") 
			.append(" BOARD_TOP_NO , READ_NUM , REG_DATE, BEST \n")
			.append(" FROM ASAN.TBRD_BOARD_MA  \n")
			.append(" WHERE BOARD_TYPE = '15' AND USE_YN = '1' \n")
        	.append(subSql) 
			.append(" ORDER BY REF_NO DESC,SORT_NO, LEVEL_NO , BOARD_SEQ DESC, BOARD_TOP_NO \n") 
			.append(" ) X WHERE X.RNUM <= 10 \n").toString() ;
    ...
    public Tbrd_board_ma_bean [] getPrevAndNextBoard(int board_seq) throws DbException , Exception {
		...
		sql.append(" WHERE BOARD_TYPE ='15' and BOARD_SEQ = ?							");
		...
		sql.append("     FROM asan.TBRD_BOARD_MA 										");
		sql.append("     WHERE BOARD_TYPE ='15' AND USE_YN='1' AND  BOARD_SEQ > ?		");
		sql.append(" )	UNION ALL														");		
